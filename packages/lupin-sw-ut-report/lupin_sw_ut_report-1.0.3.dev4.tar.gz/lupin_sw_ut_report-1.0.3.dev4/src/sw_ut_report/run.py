...
def main(
    ...
    simulate_exit_code: Optional[int] = typer.Option(None, "--simulate-exit-code", help="Simulate exit code for testing (0-9: typer.Exit, 10+: sys.exit)"),
):
    if ctx.invoked_subcommand is None:
        generate_report(input_folder, generate_markdown, create_jama_ut, module_name, dry_run, push_ut_test_results, simulate_exit_code)
...
def generate_report(
    ...
    # Test exit code simulation if requested
    if simulate_exit_code is not None:
        typer.echo(f"🧪 SIMULATION: Testing exit code {simulate_exit_code}")

        if 0 <= simulate_exit_code <= 9:
            typer.echo(f"Using typer.Exit(code={simulate_exit_code})")
            raise typer.Exit(code=simulate_exit_code)
        elif simulate_exit_code >= 10:
            typer.echo(f"Using sys.exit({simulate_exit_code})")
            sys.exit(simulate_exit_code)
        else:
            typer.echo("❌ Invalid exit code: must be >= 0", err=True)
