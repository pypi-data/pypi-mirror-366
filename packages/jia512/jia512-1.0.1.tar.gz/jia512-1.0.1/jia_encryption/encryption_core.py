...
class EncryptionCore:
    ...
    def verify_and_decrypt_payload(self, payload: Dict[str, Any], aes_key: bytes, hmac_key: bytes) -> bytes:
        ...
        try:
            ...
            if time_diff > self.config.max_timestamp_diff:
                # 尝试调整时间戳验证，允许更大的时间差
                if time_diff > self.config.max_timestamp_diff * 2:
                    raise ValueError(f"时间戳验证失败: 时间差 {time_diff} 秒超过限制 {self.config.max_timestamp_diff} 秒")
                else:
                    print(f"⚠️ 时间戳差异较大 ({time_diff} 秒)，但仍在可接受范围内")
            ...
            if not self.hmac_verify(hmac_key, payload_str.encode(), signature):
                # 尝试不同的签名验证方式
                print(f"⚠️ HMAC验证失败，尝试备用验证方式...")
                
                # 备用验证：移除可能的多余字段
                payload_clean = {k: v for k, v in payload_copy.items() 
                               if k in ["encrypted_data", "iv", "timestamp", "nonce", "tag"]}
                payload_str_clean = json.dumps(payload_clean, sort_keys=True)
                
                if not self.hmac_verify(hmac_key, payload_str_clean.encode(), signature):
