...
class Test_Scalar_cos(unittest.TestCase):
    ...
    def runTest(self):
        ...
        self.assertEqual(random.cos(), random.cos())        # unit should be OK
        ...
        self.assertEqual(angle.cos(), np.cos(angle.values)) # unit should be OK

        # Units should be removed
        values = np.random.randn(10)
        random = Scalar(values, unit=Unit.DEG)
        self.assertTrue(random.cos()._unit is None)
