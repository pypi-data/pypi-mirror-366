...
class Test_Scalar_tan(unittest.TestCase):
    ...
    def runTest(self):
        ...
        self.assertEqual(random.tan(), random.tan())        # unit should be OK
        ...
        self.assertEqual(angle.tan(), np.tan(angle.values)) # unit should be OK

        # Units should be removed
        values = np.random.randn(10)
        random = Scalar(values, unit=Unit.DEG)
        self.assertTrue(random.tan().unit_ is None)
