...
class GameClient:
    ...
    async def connect(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                logger.error(f"Error during connection session: {e}, attempting to reconnect in 5 seconds...")
                self.connected.clear()
                await asyncio.sleep(5)
    ...
    async def _run_connection_session(self) -> None:
        ...
        async with connect(
            ...
            # Anulăm task-urile rămase
            for t in pending:
                t.cancel()
            
            # Așteptăm ca toate task-urile anulate să se finalizeze (curățare)
            await asyncio.gather(*pending, return_exceptions=True)
            
            # Verificăm dacă există excepții în task-urile finalizate
            for t in done:
                if t.exception():
                    raise t.exception()
