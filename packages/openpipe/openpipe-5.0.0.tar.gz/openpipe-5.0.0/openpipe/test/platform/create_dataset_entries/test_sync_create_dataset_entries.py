...
def dataset_id():
    ...
    dataset = op_client.create_dataset(name="Test Dataset")
    yield dataset.id
    # Cleanup after tests
    op_client.delete_dataset(dataset_id=dataset.id)
...
class TestCreateDatasetEntries:
    def test_add_entries_to_dataset(self, reset_client, dataset_id):
        ...
        entry_creation = op_client.create_dataset_entries(dataset_id, entries=entries)

        assert entry_creation.entries_created == 10
        assert len(entry_creation.errors.data) == 0

    def test_add_entries_with_bad_metadata_fails(self, reset_client, dataset_id):
        ...
        try:
            op_client.create_dataset_entries(dataset_id, entries=entries)
