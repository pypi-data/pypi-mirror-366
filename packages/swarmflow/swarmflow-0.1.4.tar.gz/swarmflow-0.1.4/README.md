...
### Auto-Extracted LLM Metadata
```python
from openai import OpenAI

@swarm_task
def llm_task():
    # This will automatically extract metadata from LLM responses
    client = OpenAI()
...
#   ↳ Status: success
#   ↳ Duration: 1234 ms
#   ↳ Output: <OpenAI ChatCompletion object>
#   ↳ Metadata: {'agent': 'LLMProcessor', 'provider': 'OpenAI', 'model': 'gpt-4', 'tokens_used': 150, 'cost_usd': 0.0045}
```
...
- **Auto-extracted LLM metadata** (model, provider, token usage, cost estimation)
...
- **LLM metadata extraction** (OpenAI, Anthropic, and more)
