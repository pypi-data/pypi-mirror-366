                                                 ...
                                                 const KdTree<G>& tree )
{
    ...
    auto backwardMappingFactory = mesh.createBackwardMappingFactory( );
        ...
        for( auto isimplex : simplices )
        {
            ...
            if( auto localBounds = determineLocalBoundingBox<G>( mesh, icell, vertices, neighbors ) )
            {
                ...
                if( !polygon.empty( ) )
                {
                    ...
                    auto cellOffset = intersected.cells.size( );
                    ...
                    appendClippedSimplex<G, L>( polygon, intersected.cells, reference );
                    
                    celldata.offsets[icell + 1] += intersected.cells.size( ) - cellOffset;

                    for( size_t isubcell = cellOffset; isubcell < intersected.cells.size( ); ++isubcell )
                    {
                        intersected.cells[isubcell] = array::add( intersected.cells[isubcell], vertexOffset );
                        
                        // Add point normals
                        if constexpr( G == 1 && L == 0 )
                        {
                            intersected.normals.push_back( simplexMesh.normals[isimplex] );
                        }
                    }
                }
            }
                                                  ...
                                                  size_t niterations )
{
    ...
    [[maybe_unused]]
    auto chunksize = std::clamp( ncells / 1024, std::int64_t { 1 }, std::int64_t { 73 } );
