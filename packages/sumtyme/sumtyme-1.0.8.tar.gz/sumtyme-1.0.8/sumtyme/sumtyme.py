...
def read_dataframe(data: Union[str, pd.DataFrame], **kwargs) -> pd.DataFrame:
    ...
    **kwargs : dict, optional
    ...
    try:
        # First, convert to datetime objects, allowing flexible parsing
        datetime_series = pd.to_datetime(df['datetime'], format='mixed')

        # Then, format the datetime objects back into the desired string format
        df['datetime'] = datetime_series.dt.strftime('%Y-%m-%d %H:%M:%S')

    except ValueError as e:
        # Raise a more user-friendly error message if conversion fails
        raise ValueError(
            f"Could not convert 'datetime' column to a valid datetime format. "
            f"Ensure all entries in the column are correctly formatted. "
            f"Original error: {e}"
        ) from e
