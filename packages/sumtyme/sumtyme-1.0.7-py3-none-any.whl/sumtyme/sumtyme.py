...
def format_datetime_input(dt_input):
    """
    Parses a datetime string or formats a datetime object to a standard format.

    Args:
        dt_input (str or datetime.datetime): The datetime string or object to be processed.

    Returns:
        str: The formatted datetime string in '%Y-%m-%d %H:%M:%S' format,
    ...
    if isinstance(dt_input, datetime):
        ...
        try:
            # The dateutil.parser.parse() function is excellent for
            # automatically detecting and parsing a wide variety of datetime formats.
            dt_object = parser.parse(dt_input)
        except (ValueError, parser.ParserError) as e:
            # Catch exceptions if the string cannot be parsed
            # Return the original input for error handling in the DataFrame
            return f"Error: Could not parse '{dt_input}'."
    else:
        return f"Error: Invalid input type. Expected str or datetime.datetime, got {type(dt_input).__name__}."

    # If dt_object was successfully created, format it.
    if dt_object:
        # The strftime() method is then used to format the datetime object
        # into the specific string format requested.
        formatted_string = dt_object.strftime('%Y-%m-%d %H:%M:%S')
        return formatted_string
    
    return f"Error: Failed to process input '{dt_input}'."
...
client = EIPClient(subdomain='london', apikey_path="config.txt")

# Rolling analysis
entire_model = client.model_timeseries_environment(data_input='^DJI_max_1d.csv',interval=1,interval_unit='days',reasoning_mode='reactive')