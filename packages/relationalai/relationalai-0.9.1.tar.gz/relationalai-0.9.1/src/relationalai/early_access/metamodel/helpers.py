...
def is_type_box(node: ir.Lookup|ir.Relation):
    """ Whether this relation is a type box, by being marked with the type_box_info annotation. """
    if isinstance(node, ir.Lookup) and is_type_box(node.relation):
        return True
    return builtins.type_box_annotation in node.annotations
