...
class Replacer(visitor.Rewriter):
    ...
    def convert(self, arg:ir.Value, actual:ir.Type, expected:ir.Type, tasks:list[ir.Task]) -> ir.Value:
        actual_base = to_base_primitive(actual)
        expected_base = to_base_primitive(expected)
        if type_matches(actual, expected):
            return arg
        value = None
        ...
        new_arg = f.var(name, expected)
        conv = CONVERSIONS[(actual_base, expected_base)]
