...
def rate(padding_length, effective_integration_length) :
    return np.fft.fftshift(np.fft.fftfreq(zerofill(padding_length)[0], d=effective_integration_length))
...
def frinZparam(
    ...
    if freq_rate_2D_array is not None :
        flag = "frequency"
        integ_fft = freq_rate_2D_array.shape[0]
    elif lag_rate_2D_array is not None :
        flag = "time"
        integ_fft = lag_rate_2D_array.shape[0]
    else :
        raise ValueError
    ...
    rate_range  = rate(integ_fft, effective_integration_length) # rate range, the sampling frequency is 1 second if the outout value in xml-file is 1 Hz and the parameter if length is 1.
    ...
    if flag == "frequency" :
        ...
        SNR_freq_rate, noise_level_freq = noise(freq_rate_2D_array, fringe_freq_rate_00_amp, "frequency")
    ...
    if flag == "frequency" :
        return {"phase_list": fringe_freq_rate_00_phase1, "spectrum_list": fringe_freq_rate_00_amplitude1, "rate_list": fringe_freq_rate_00_rate, \
               "fringe_amp": fringe_freq_rate_00_amp, "fringe_phase": fringe_freq_rate_00_phase2, "fringe_spectrum": fringe_freq_rate_00_freq,\
               "snr": SNR_freq_rate, "noise": noise_level_freq}
    if flag == "time" :
        return {"delay_list":fringe_lag_rate_00_lag, "rate_list":fringe_lag_rate_00_rate, \
               "fringe_amp": fringe_lag_rate_00_amp, "fringe_phase": fringe_lag_rate_00_phase, \
