...
class Organic :
            ...
            while True :
                ...
                if Opcode in (1 , 2) :
                    try : Context.Signal.Request.Body = Json.loads(Body);
                        ...
                        if await Self.Idempotent(Context , Writer) is False :
                            if await Self.Cache(Context , Writer) is False : Asyncio.create_task(Self.Task(Copy.deepcopy(Context.Signal) , Context.Journal , Writer));
                else : break;
    ...
    @classmethod
    async def Idempotent(Self , Context:Simple , Writer:Asyncio.StreamWriter) -> bool :
        await Asyncio.sleep(0);
        if Context.Config.Idempotent.get('Status') is False : return False;
        Status , Response = Context.Config.Idempotent.get('Callable')(Simple(
            Network = Context.Signal.Network ,
        ...
        await Self.Response(Context.Signal , Context.Journal , Writer);
        return True;
    
    @classmethod
    async def Cache(Self , Context:Simple , Writer:Asyncio.StreamWriter) -> bool :
        await Asyncio.sleep(0);
        if Context.Config.Cache.get('Status') is False : return False;
        Status , Response = Context.Config.Cache.get('Callable')(Simple(
            Network = Context.Signal.Network ,
            Request = Context.Signal.Request ,
        ));
        if Status is False : return False;
