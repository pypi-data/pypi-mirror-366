...
class EnhancedSemanticAnalyzer(SemanticAnalyzer):
    ...
    def __init__(self, model_dir: Path, project_path: Optional[Path] = None):
        ...
        try:
            ...
            # SILENT EXCEPTION HANDLING FIX: Enhanced error reporting with context
            logger.warning(f"Failed to initialize codebase learning: {e}")
            logger.debug(f"Codebase learning initialization error details:", exc_info=True)
            
            # Analyze error type for better user guidance
            error_type = type(e).__name__
            if 'Import' in error_type or 'Module' in error_type:
                logger.info("Codebase learning requires enhanced ML modules - falling back to standard analysis")
                ...
                logger.warning(f"Insufficient memory for codebase learning initialization: {e}")
            else:
                logger.debug(f"Unexpected error during codebase learning setup: {error_type} - {e}")
    ...
    def analyze_code(self, code_text: str, file_path: str) -> List[Dict[str, Any]]:
        ...
        if self.learning_enabled and self.enhanced_ml_engine:
            try:
                ...
                logger.debug(f"Enhanced ML analysis error details for {file_path}:", exc_info=True)
                
                # Analyze error patterns for better diagnostics
                error_type = type(e).__name__
                if 'Memory' in error_type:
                    logger.warning(f"Memory pressure during enhanced analysis of {file_path} - consider reducing complexity")
                elif 'Timeout' in error_type:
                    logger.warning(f"Enhanced analysis timeout for {file_path} - file may be too large")
                elif 'AttributeError' in str(e) or 'NoneType' in str(e):
                    logger.debug(f"Enhanced ML engine state issue - may need reinitialization: {e}")
                else:
                    logger.debug(f"Unexpected enhanced analysis error: {error_type} - {e}")
                ...
                if self._analysis_failures > 10:
                    logger.warning(f"Enhanced analysis has failed {self._analysis_failures} times - consider investigating system issues")
    ...
    def _analyze_with_enhanced_ml(self, code_text: str, file_path: str) -> List[Dict[str, Any]]:
        ...
        for i, result in enumerate(enhanced_results):
            # MISSING NULL SAFETY FIX: Validate result object before processing
            if result is None:
                logger.warning(f"Skipping null result at index {i} from enhanced ML analysis")
                continue
            
            # Extract line number more accurately
            line_num = self._extract_line_number(code_text, result, i)
            
            # MISSING NULL SAFETY FIX: Safe attribute access with proper defaults
            ...
            finding = {
                ...
                # MISSING NULL SAFETY FIX: Simplified getattr calls (hasattr is redundant)
                ...
                "original_confidence": getattr(result, 'original_confidence', result_confidence),  # Use safe confidence
                ...
                "security_risk": getattr(result, 'security_risk', 'low'),  # MISSING NULL SAFETY FIX: Safe access with default
                "performance_impact": getattr(result, 'performance_impact', 'low')  # MISSING NULL SAFETY FIX: Safe access with default
            }
    ...
    def _get_file_analysis_metrics(self, code_text: str) -> dict:
        """Get file analysis metrics with cached line splitting."""
        # PERFORMANCE FIX: Split once and cache for multiple operations
        lines = code_text.split('\n')
        return {
