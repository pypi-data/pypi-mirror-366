...
class HealthMonitor:
    ...
    def _save_health_data(self) -> None:
        ...
        try:
            ...
            # ENHANCED FILE I/O ATOMICITY FIX: Robust directory creation with error handling
            try:
                self.health_file.parent.mkdir(parents=True, exist_ok=True)
                # Verify directory was created and is writable
                if not self.health_file.parent.exists():
                    raise OSError(f"Failed to create directory: {self.health_file.parent}")
                if not self.health_file.parent.is_dir():
                    raise OSError(f"Path exists but is not a directory: {self.health_file.parent}")
            except PermissionError as e:
                logger.error(f"Permission denied creating health data directory {self.health_file.parent}: {e}")
                return
            except OSError as e:
                logger.error(f"Failed to create health data directory {self.health_file.parent}: {e}")
                return
            
            # Write to temporary file first, then atomic rename
            temp_file = self.health_file.with_suffix('.tmp')
            
            # ADDITIONAL SAFETY: Ensure directory exists right before file creation
