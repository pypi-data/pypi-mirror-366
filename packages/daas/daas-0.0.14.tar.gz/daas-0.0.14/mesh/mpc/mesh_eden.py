...
from mesh.psi import Subscriber, Registry, Scheduler, Builtin
from mesh.tool import Tool
from mesh.types import Reference, Service, Event, Registration, URC, Topic, Versions
...
class MeshEden(Eden, RuntimeHook, Subscriber):
    ...
    def new_registration(self, metadata: URC, attachments: Dict[str, str]) -> Registration:
        registration = Registration()
        registration.instance_id = self.get_listened_address()
        registration.content = metadata
        registration.kind = "METADATA"
    ...
    async def start(self):
        ...
        self.task_id = await self.scheduler().period(timedelta(milliseconds=30 * 1000), self.topic)

    async def stop(self):
        await self.scheduler().cancel(self.task_id)
        await self.scheduler().shutdown(timedelta(milliseconds=3 * 1000))
        metadata = URC()
        metadata.services = []
        metadata.references = []
        try:
