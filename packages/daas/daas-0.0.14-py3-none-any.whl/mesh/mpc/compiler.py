...
class PythonCompiler(Compiler):
    ...
    def intype(self, method: Any) -> Type[Parameters]:
        ...
        for idx, (name, parameter) in enumerate(signature.parameters.items()):
            if idx == 0:
                variables[ATTACHMENTS] = Index(index=-1, name=ATTACHMENTS, kind=Dict)
                continue
            variables[name] = Index(index=idx, name=name, kind=parameter.annotation)
    ...
    def retype(self, method: Any) -> Type[Returns]:
        ...
        signature = inspect.signature(method)
        variables = {
            CODE: Index(index=0, name=CODE, kind=str),
            MESSAGE: Index(index=5, name=MESSAGE, kind=str),
            CAUSE: Index(index=10, name=CAUSE, kind=Cause),
            CONTENT: Index(index=15, name=CONTENT, kind=signature.return_annotation),
