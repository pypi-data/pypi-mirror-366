...
def dynamic_retry(experiment_config):
    """通用重试装饰器，不依赖openai异常"""
    return retry(
        stop=stop_after_attempt(experiment_config.max_retry_attempts),
        wait=wait_exponential(multiplier=1, min=2, max=10),  # 通用参数
        retry=retry_if_exception_type(Exception),  # 捕获通用异常
    )
