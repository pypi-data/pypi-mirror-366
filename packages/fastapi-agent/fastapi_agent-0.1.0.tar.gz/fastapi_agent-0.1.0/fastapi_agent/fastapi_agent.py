...
class APIResponse(BaseModel):
    ...
    status_code: int
    data: Any  # Union[Dict[str, Any], str, List[Dict[str, Any]]]
...
class FastAPIAgent(FastAPIDiscovery):
    def __init__(
        ...
        **kwargs,
        ...
        Kwargs:
            ...
            add_agent_routes (bool): add ai agent route to your FastAPI app.
        """
        if logger is None:
        ...
        if kwargs.get("add_agent_routes", False):
            self.add_agent_routes()
    ...
    def add_agent_routes(self):
        """Add all agent-related routes to FastAPI"""
        ...
        @self.app.post("/agent/query", response_model=AgentResponse, tags=["AI Agent"])
        async def query_ai_agent(request: AgentQuery):
            ...
            """
            try:
                response, history = await self.chat(request.query, request.history)
        ...
        @self.app.get("/agent/chat", response_class=HTMLResponse, tags=["AI Agent"])
