...
class RaschEvaluator:
    ...
    def export_to_excel(self, test_id: str, results: List[Dict],
        ...
        Ball va darajasiga qarab tartiblanadi (yuqoridan pastga)
        ...
        # Daraja tartibini belgilash (eng yuqoridan eng pastga)
        grade_order = {
            'A+': 7,
            'A': 6,
            'B+': 5,
        ...
        # Natijalarni nusxalash va tartibga solish
        sorted_results = sorted(
            results,
            key=lambda x: (
                grade_order.get(x['grade'], 0),  # Daraja bo'yicha
                x['rasch_score']  # Ball bo'yicha
            ),
            reverse=True  # Yuqoridan pastga
        )
        ...
        print(f"Jami {len(sorted_results)} ta natija ball va darajasiga qarab "
              f"tartiblanadi")
        return filepath
