...
def convert_coco(datadir, outdir, mapping = None):
    ...
    with open(os.path.join(outdir,'coco.json'), 'w+') as of:
        ...
        for f in sorted(annsfiles):
            ...
            # Find the corresponding metadata file by matching the filename field
            image_filename = anns.get('filename')
            if not image_filename:
                print(f"Warning: No 'filename' field found in {f}, skipping...")
                continue
                
            # Find metadata file with matching filename field
            metadata_file = None
            metadata = None
            for mf in os.listdir(metadir):
                if not mf.endswith('.json'):
                    continue
                try:
                    with open(os.path.join(metadir, mf), 'r') as meta_f:
                        temp_metadata = json.load(meta_f)
                        if temp_metadata.get('filename') == image_filename:
                            metadata_file = mf
            ...
            if metadata is None:
                print(f"Warning: No matching metadata file found for {image_filename}, skipping...")
                continue
