...
class Query(tp.NamedTuple):
    ...
    async def get_datasets(
        ...
        return [DatasetMetadataResponse(**d) for d in response.json()]

    async def create_dataset(self, request: Adapter) -> QueryLiveResponse:
        """
        ...
        response.raise_for_status()
        return QueryLiveResponse(**response.json())

    async def query_dataset(self, request: LiveQueryDatasetQuery) -> QueryLiveResponse:
        """
    ...
    async def delete_dataset(
        ...
        response.raise_for_status()
        return DeleteQueryDatasetResponse(**response.json())

    async def get_schema(self, request: LiveQueryDatasetMetadata) -> JsonSchemaModel:
        """
        ...
        response.raise_for_status()
        return JsonSchemaModel(**response.json())
