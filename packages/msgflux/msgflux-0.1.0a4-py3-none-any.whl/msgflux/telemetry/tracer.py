    ...
    def _build_sampler(self) -> None:
        ...
        if ratio is not None:
            try:
                ...
                logger.warning(f"Invalid sampling ratio '{ratio}', falling back to default sampler.")
        return None
    ...
    def configure_tracer(self):
        ...
        with self._lock:
            ...
            if exporter_type == "otlp":
                ...
                logger.debug(
                    f"Configured OTLP exporter with endpoint: {envs.telemetry_otlp_endpoint}"
                )
                ...
                logger.warning(
                    f"Unknown exporter type '{envs.telemetry_span_exporter_type}', defaulting to NoOp tracer"
                )
                no_op_provider = NoOpTracerProvider()
                trace.set_tracer_provider(no_op_provider)
                self._tracer = trace.get_tracer("msgflux.telemetry")
