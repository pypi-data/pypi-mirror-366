...
esim = ejectorSimu(params, fluid = "R1233zde")
recalc = False
if recalc :
    esim.calcPrimMassFlow()
else:
    esim.params['vin_crit']= 0.871875
    esim.params['mass_flow_crit'] = 0.58745
...
## plot entropy:
esim.primNozzleFlow["entropy"] = esim.primNozzleFlow.apply(lambda row: refProp.getTD(RP, row["h"], row["p"])["s"] , axis =1 )
