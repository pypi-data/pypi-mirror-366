...
class StdDevCls:
	...
	def get(self, measurement=repcap.Measurement.Default) -> float:
		"""SCPI: MEASurement<*>:RESult:STDDev \n
		Snippet: value: float = driver.measurement.result.stdDev.get(measurement = repcap.Measurement.Default) \n
		...
		response = self._core.io.query_str(f'MEASurement{measurement_cmd_val}:RESult:STDDev?')
