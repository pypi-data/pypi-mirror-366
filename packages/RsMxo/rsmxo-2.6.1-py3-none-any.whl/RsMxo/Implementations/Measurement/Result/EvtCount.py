...
class EvtCountCls:
	...
	def get(self, measurement=repcap.Measurement.Default) -> int:
		"""SCPI: MEASurement<*>:RESult:EVTCount \n
		Snippet: value: int = driver.measurement.result.evtCount.get(measurement = repcap.Measurement.Default) \n
		...
		response = self._core.io.query_str(f'MEASurement{measurement_cmd_val}:RESult:EVTCount?')
