...
class EventCls:
	...
	def get(self, measurement=repcap.Measurement.Default) -> bool:
		"""SCPI: MEASurement<*>:IMPRecise:STATus[:EVENt] \n
		Snippet: value: bool = driver.measurement.imprecise.status.event.get(measurement = repcap.Measurement.Default) \n
		...
		response = self._core.io.query_str(f'MEASurement{measurement_cmd_val}:IMPRecise:STATus:EVENt?')
