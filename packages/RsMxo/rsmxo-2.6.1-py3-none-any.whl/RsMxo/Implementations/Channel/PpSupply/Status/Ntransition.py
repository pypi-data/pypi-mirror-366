...
class NtransitionCls:
	...
	def set(self, value: bool, channel=repcap.Channel.Default) -> None:
		"""SCPI: CHANnel<*>:PPSupply:STATus:NTRansition \n
		Snippet: driver.channel.ppSupply.status.ntransition.set(value = False, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'CHANnel{channel_cmd_val}:PPSupply:STATus:NTRansition {param}')

	def get(self, channel=repcap.Channel.Default) -> bool:
		"""SCPI: CHANnel<*>:PPSupply:STATus:NTRansition \n
		Snippet: value: bool = driver.channel.ppSupply.status.ntransition.get(channel = repcap.Channel.Default) \n
		Sets the negative transition filter. If a bit is set, a transition from 1 to 0 in the condition part causes an entry to
		be made in the corresponding bit of the EVENt part of the register. \n
			:param channel: optional repeated capability selector. Default value: Ch1 (settable in the interface 'Channel')
			:return: value: No help available"""
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'CHANnel{channel_cmd_val}:PPSupply:STATus:NTRansition?')
