...
class ConditionCls:
	...
	def get(self, measurement=repcap.Measurement.Default) -> bool:
		"""SCPI: MEASurement<*>:LIMit:STATus:CONDition \n
		Snippet: value: bool = driver.measurement.limit.status.condition.get(measurement = repcap.Measurement.Default) \n
		...
		response = self._core.io.query_str(f'MEASurement{measurement_cmd_val}:LIMit:STATus:CONDition?')
