...
class UpperCls:
	...
	def get(self, measurement=repcap.Measurement.Default, refLevel=repcap.RefLevel.Default) -> float:
		"""SCPI: MEASurement<*>:REFLevel<*>:RESult:UPPer \n
		Snippet: value: float = driver.measurement.refLevel.result.upper.get(measurement = repcap.Measurement.Default, refLevel = repcap.RefLevel.Default) \n
		...
		response = self._core.io.query_str(f'MEASurement{measurement_cmd_val}:REFLevel{refLevel_cmd_val}:RESult:UPPer?')
