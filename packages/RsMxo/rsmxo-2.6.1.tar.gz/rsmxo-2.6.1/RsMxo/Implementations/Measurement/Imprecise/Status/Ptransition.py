...
class PtransitionCls:
	...
	def set(self, value: bool, measurement=repcap.Measurement.Default) -> None:
		"""SCPI: MEASurement<*>:IMPRecise:STATus:PTRansition \n
		Snippet: driver.measurement.imprecise.status.ptransition.set(value = False, measurement = repcap.Measurement.Default) \n
		...
		self._core.io.write(f'MEASurement{measurement_cmd_val}:IMPRecise:STATus:PTRansition {param}')

	def get(self, measurement=repcap.Measurement.Default) -> bool:
		"""SCPI: MEASurement<*>:IMPRecise:STATus:PTRansition \n
		Snippet: value: bool = driver.measurement.imprecise.status.ptransition.get(measurement = repcap.Measurement.Default) \n
		Sets the positive transition filter. If a bit is set, a transition from 0 to 1 in the condition part causes an entry to
		be made in the corresponding bit of the EVENt part of the register. \n
			:param measurement: optional repeated capability selector. Default value: Ix1 (settable in the interface 'Measurement')
			:return: value: No help available"""
		measurement_cmd_val = self._cmd_group.get_repcap_cmd_value(measurement, repcap.Measurement)
		response = self._core.io.query_str(f'MEASurement{measurement_cmd_val}:IMPRecise:STATus:PTRansition?')
