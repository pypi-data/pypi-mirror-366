...
class StopCls:
	...
	def get(self, measurement=repcap.Measurement.Default) -> float:
		"""SCPI: MEASurement<*>:RESult:EVENts:STOP \n
		Snippet: value: float = driver.measurement.result.events.stop.get(measurement = repcap.Measurement.Default) \n
		...
		response = self._core.io.query_str(f'MEASurement{measurement_cmd_val}:RESult:EVENts:STOP?')
