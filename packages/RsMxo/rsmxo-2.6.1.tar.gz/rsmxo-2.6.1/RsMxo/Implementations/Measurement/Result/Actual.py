...
class ActualCls:
	...
	def get(self, measurement=repcap.Measurement.Default) -> float:
		"""SCPI: MEASurement<*>:RESult[:ACTual] \n
		Snippet: value: float = driver.measurement.result.actual.get(measurement = repcap.Measurement.Default) \n
		...
		response = self._core.io.query_str(f'MEASurement{measurement_cmd_val}:RESult:ACTual?')
