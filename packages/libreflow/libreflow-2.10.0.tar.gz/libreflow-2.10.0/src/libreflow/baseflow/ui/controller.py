...
class FileData:
    ...
    def __init__(
        self, task_data, oid, task_name, goto_oid=None, activate_oid=None, ref_oid=None
    ):
        self.controller = task_data.controller
        self.session = task_data.session
        self.file_oid = oid
...
class TaskData:
    ...
    def create_file_data(self, oid, goto_oid=None, ref_oid=None):
        ...
        return FileData(
            self, oid, self.oid.split("/")[-1], goto_oid, activate_oid, ref_oid
        )
...
class Controller:
    ...
    def update_sites_order(self):
        ...
        self.current_site_short_name = self.get_short_names([self.current_site_name])
    ...
    def get_short_names(self, sites):
        ...
        current_site_short_name = self.session.cmds.Flow.get_value(
            "/"
            + self.oid.split("/")[1]
            + f"/admin/multisites/working_sites/{sites[0]}/short_name",
        )
        if current_site_short_name != "":
            return self.session.cmds.Flow.get_value(
                "/"
                + self.oid.split("/")[1]
                + f"/admin/multisites/working_sites/{sites[0]}/short_name",
