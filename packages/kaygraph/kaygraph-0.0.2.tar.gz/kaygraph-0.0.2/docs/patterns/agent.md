...
2. **Branching:** Use branching to connect each action node to an agent node. Use action to allow the agent to direct the [graph](../fundamentals/graph.md) between nodes—and potentially loop back for multi-step.
...
graph.run({"query": "Who won the Nobel Prize in Physics 2024?"})
```
