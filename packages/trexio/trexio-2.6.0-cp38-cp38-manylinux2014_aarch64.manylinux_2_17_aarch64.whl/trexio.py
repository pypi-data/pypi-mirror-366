...
def has_ao_1e_int_dipole_x(trexio_file) -> bool:
    """Check that ao_1e_int_dipole_x variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_ao_1e_int_dipole_x(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)

    return rc == TREXIO_SUCCESS

def has_ao_1e_int_dipole_y(trexio_file) -> bool:
    """Check that ao_1e_int_dipole_y variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_ao_1e_int_dipole_y(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)

    return rc == TREXIO_SUCCESS

def has_ao_1e_int_dipole_z(trexio_file) -> bool:
    """Check that ao_1e_int_dipole_z variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_ao_1e_int_dipole_z(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)
...
def has_ao_1e_int_dipole_x_im(trexio_file) -> bool:
    """Check that ao_1e_int_dipole_x_im variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_ao_1e_int_dipole_x_im(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)

    return rc == TREXIO_SUCCESS

def has_ao_1e_int_dipole_y_im(trexio_file) -> bool:
    """Check that ao_1e_int_dipole_y_im variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_ao_1e_int_dipole_y_im(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)

    return rc == TREXIO_SUCCESS

def has_ao_1e_int_dipole_z_im(trexio_file) -> bool:
    """Check that ao_1e_int_dipole_z_im variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_ao_1e_int_dipole_z_im(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)
...
def has_mo_1e_int_dipole_x(trexio_file) -> bool:
    """Check that mo_1e_int_dipole_x variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_mo_1e_int_dipole_x(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)

    return rc == TREXIO_SUCCESS

def has_mo_1e_int_dipole_y(trexio_file) -> bool:
    """Check that mo_1e_int_dipole_y variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_mo_1e_int_dipole_y(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)

    return rc == TREXIO_SUCCESS

def has_mo_1e_int_dipole_z(trexio_file) -> bool:
    """Check that mo_1e_int_dipole_z variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_mo_1e_int_dipole_z(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)
...
def has_mo_1e_int_dipole_x_im(trexio_file) -> bool:
    """Check that mo_1e_int_dipole_x_im variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_mo_1e_int_dipole_x_im(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)

    return rc == TREXIO_SUCCESS

def has_mo_1e_int_dipole_y_im(trexio_file) -> bool:
    """Check that mo_1e_int_dipole_y_im variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_mo_1e_int_dipole_y_im(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)

    return rc == TREXIO_SUCCESS

def has_mo_1e_int_dipole_z_im(trexio_file) -> bool:
    """Check that mo_1e_int_dipole_z_im variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_mo_1e_int_dipole_z_im(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)
...
def read_ao_1e_int_dipole_x(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the ao_1e_int_dipole_x array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of ao_1e_int_dipole_x will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to ao_1e_int_dipole_x values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_ao_1e_int_dipole_x_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_ao_1e_int_dipole_y(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the ao_1e_int_dipole_y array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of ao_1e_int_dipole_y will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to ao_1e_int_dipole_y values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_ao_1e_int_dipole_y_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_ao_1e_int_dipole_z(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the ao_1e_int_dipole_z array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of ao_1e_int_dipole_z will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to ao_1e_int_dipole_z values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_ao_1e_int_dipole_z_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_ao_1e_int_dipole_x_im(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the ao_1e_int_dipole_x_im array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of ao_1e_int_dipole_x_im will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to ao_1e_int_dipole_x_im values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_ao_1e_int_dipole_x_im_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_ao_1e_int_dipole_y_im(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the ao_1e_int_dipole_y_im array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of ao_1e_int_dipole_y_im will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to ao_1e_int_dipole_y_im values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_ao_1e_int_dipole_y_im_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_ao_1e_int_dipole_z_im(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the ao_1e_int_dipole_z_im array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of ao_1e_int_dipole_z_im will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to ao_1e_int_dipole_z_im values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_ao_1e_int_dipole_z_im_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_mo_1e_int_dipole_x(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the mo_1e_int_dipole_x array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of mo_1e_int_dipole_x will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to mo_1e_int_dipole_x values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_mo_1e_int_dipole_x_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_mo_1e_int_dipole_y(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the mo_1e_int_dipole_y array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of mo_1e_int_dipole_y will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to mo_1e_int_dipole_y values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_mo_1e_int_dipole_y_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_mo_1e_int_dipole_z(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the mo_1e_int_dipole_z array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of mo_1e_int_dipole_z will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to mo_1e_int_dipole_z values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_mo_1e_int_dipole_z_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_mo_1e_int_dipole_x_im(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the mo_1e_int_dipole_x_im array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of mo_1e_int_dipole_x_im will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to mo_1e_int_dipole_x_im values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_mo_1e_int_dipole_x_im_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_mo_1e_int_dipole_y_im(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the mo_1e_int_dipole_y_im array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of mo_1e_int_dipole_y_im will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to mo_1e_int_dipole_y_im values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_mo_1e_int_dipole_y_im_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_mo_1e_int_dipole_z_im(trexio_file, dim = None, doReshape = None, dtype = None):
    """Read the mo_1e_int_dipole_z_im array of numbers from the TREXIO file.
    ...
    dim (Optional): int
        Size of the block to be read from the file (i.e. how many items of mo_1e_int_dipole_z_im will be returned)
        If None, the function will read all necessary array dimensions from the file.
    ...
    Returns:
        ~dset_64~ if dtype is None or ~dset_converted~ otherwise: numpy.ndarray
        1D NumPy array with ~dim~ elements corresponding to mo_1e_int_dipole_z_im values read from the TREXIO file.
...
"""
    ...
    rc, dset_64 = pytr.trexio_read_safe_mo_1e_int_dipole_z_im_64(trexio_file.pytrexio_s, dim)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def write_ao_1e_int_dipole_x(trexio_file, dset_w) -> None:
    """Write the ao_1e_int_dipole_x array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of ao_1e_int_dipole_x values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_x_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_x_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_x_64(trexio_file.pytrexio_s, dset_w)

    if rc != TREXIO_SUCCESS:
        raise Error(rc)

def write_ao_1e_int_dipole_y(trexio_file, dset_w) -> None:
    """Write the ao_1e_int_dipole_y array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of ao_1e_int_dipole_y values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_y_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_y_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_y_64(trexio_file.pytrexio_s, dset_w)

    if rc != TREXIO_SUCCESS:
        raise Error(rc)

def write_ao_1e_int_dipole_z(trexio_file, dset_w) -> None:
    """Write the ao_1e_int_dipole_z array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of ao_1e_int_dipole_z values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_z_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_z_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_z_64(trexio_file.pytrexio_s, dset_w)
...
def write_ao_1e_int_dipole_x_im(trexio_file, dset_w) -> None:
    """Write the ao_1e_int_dipole_x_im array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of ao_1e_int_dipole_x_im values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_x_im_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_x_im_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_x_im_64(trexio_file.pytrexio_s, dset_w)

    if rc != TREXIO_SUCCESS:
        raise Error(rc)

def write_ao_1e_int_dipole_y_im(trexio_file, dset_w) -> None:
    """Write the ao_1e_int_dipole_y_im array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of ao_1e_int_dipole_y_im values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_y_im_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_y_im_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_y_im_64(trexio_file.pytrexio_s, dset_w)

    if rc != TREXIO_SUCCESS:
        raise Error(rc)

def write_ao_1e_int_dipole_z_im(trexio_file, dset_w) -> None:
    """Write the ao_1e_int_dipole_z_im array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of ao_1e_int_dipole_z_im values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_z_im_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_z_im_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_ao_1e_int_dipole_z_im_64(trexio_file.pytrexio_s, dset_w)
...
def write_mo_1e_int_dipole_x(trexio_file, dset_w) -> None:
    """Write the mo_1e_int_dipole_x array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of mo_1e_int_dipole_x values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_x_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_x_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_x_64(trexio_file.pytrexio_s, dset_w)

    if rc != TREXIO_SUCCESS:
        raise Error(rc)

def write_mo_1e_int_dipole_y(trexio_file, dset_w) -> None:
    """Write the mo_1e_int_dipole_y array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of mo_1e_int_dipole_y values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_y_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_y_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_y_64(trexio_file.pytrexio_s, dset_w)

    if rc != TREXIO_SUCCESS:
        raise Error(rc)

def write_mo_1e_int_dipole_z(trexio_file, dset_w) -> None:
    """Write the mo_1e_int_dipole_z array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of mo_1e_int_dipole_z values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_z_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_z_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_z_64(trexio_file.pytrexio_s, dset_w)
...
def write_mo_1e_int_dipole_x_im(trexio_file, dset_w) -> None:
    """Write the mo_1e_int_dipole_x_im array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of mo_1e_int_dipole_x_im values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_x_im_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_x_im_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_x_im_64(trexio_file.pytrexio_s, dset_w)

    if rc != TREXIO_SUCCESS:
        raise Error(rc)

def write_mo_1e_int_dipole_y_im(trexio_file, dset_w) -> None:
    """Write the mo_1e_int_dipole_y_im array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of mo_1e_int_dipole_y_im values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_y_im_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_y_im_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_y_im_64(trexio_file.pytrexio_s, dset_w)

    if rc != TREXIO_SUCCESS:
        raise Error(rc)

def write_mo_1e_int_dipole_z_im(trexio_file, dset_w) -> None:
    """Write the mo_1e_int_dipole_z_im array of numbers in the TREXIO file.
    ...
    dset_w: list OR numpy.ndarray
        Array of mo_1e_int_dipole_z_im values to be written. If array data type does not correspond to int64 or float64, the conversion is performed.
    ...
    if doConversion:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_z_im_64(trexio_file.pytrexio_s, dset_64)
    elif doFlatten:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_z_im_64(trexio_file.pytrexio_s, dset_flat)
    else:
        rc = pytr.trexio_write_safe_mo_1e_int_dipole_z_im_64(trexio_file.pytrexio_s, dset_w)
