...
# Three “corner” colours in display-sRGB (gamma-encoded, 0-1)
CORNERS_SRGB = np.array([
    [1.00, 0.00, 0.00],  # vivid red
    [0.00, 1.00, 0.00],  # vivid green
    [0.00, 0.00, 1.00]  # vivid blue
])  # shape (3, 3)  … columns are the corners

# parameters that control the lightness ramp
L_RANGE_LAB = (25, 90)  # (L_min, L_max) in CIELAB
L_RANGE_OKLAB = (0.35, 0.95)  # OKLab L is ~0..1
GAMMA = 0.50  # concave √m boost keeps tiny magnitudes visible
TINY_MAG = 0.01  # used for the “dark border” colour
...
CORNERS_OKLAB = np.transpose([srgb_to_oklab(c) for c in CORNERS_SRGB])
CORNERS_LAB = np.transpose([srgb_to_lab(c) for c in CORNERS_SRGB])


def triple_to_colour(triplet, space='oklab'):
    """
    Given 3 numbers in range 0..1, convert them to a color.
    Returns:
        MunchDict with keys:
            - color       : hex string
            - edge_color  : hex string (darker border)
            - magnitude   : float in [0, 1]
    """
    triplet = np.asarray(triplet, dtype=float)
    if triplet.min() < 0 or triplet.max() > 1:
        raise ValueError("Input triple must be in [0,1].")

    m_raw = float(triplet.sum())
    m = m_raw / 3.0  # normalize to [0,1]

    if m_raw == 0:
        return MunchDict({
            'color': mcolors.to_hex((0.90, 0.90, 0.90)),
            'edge_color': mcolors.to_hex((0.50, 0.50, 0.50)),
            'magnitude': 0.0
        })

    u = triplet / m_raw  # barycentric weights

    if space.lower() == 'oklab':
        corners = CORNERS_OKLAB
        L_min, L_max = L_RANGE_OKLAB
        to_rgb = oklab_to_srgb
    elif space.lower() in ('lab', 'cielab'):
        corners = CORNERS_LAB
        L_min, L_max = L_RANGE_LAB
        to_rgb = lab_to_srgb
    else:
        raise ValueError("space must be 'oklab' or 'lab' (CIELAB).")

    # (1) mix chroma
    base = corners @ u

    # (2) magnitude-based lightness
    L_new = L_min + (L_max - L_min) * m ** GAMMA
    colour = base.copy()
    colour[0] = L_new
    fill_rgb = to_rgb(colour)

    # (3) border color: same chroma, fixed tiny magnitude
    m_tiny = TINY_MAG / 3.0
    L_border = L_min + (L_max - L_min) * m_tiny ** GAMMA
    border = base.copy()
    border[0] = L_border
    edge_rgb = to_rgb(border)
