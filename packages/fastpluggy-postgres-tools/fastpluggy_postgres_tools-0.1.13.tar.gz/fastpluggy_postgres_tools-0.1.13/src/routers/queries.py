...
def get_query_performance_statistics(db_manager: Session, limit: int = 10, include_p95: bool = False):
    ...
    p95_column = ""
    if include_p95:
        p95_column = ", (SELECT percentile_cont(0.95) WITHIN GROUP (ORDER BY total_time/calls) FROM pg_stat_statements) AS p95_time"
    ...
    query = f"""
        SELECT
          ...
          total_time,
          mean_time,
          ROUND(total_time / GREATEST(calls, 1), 2) AS avg_time,
