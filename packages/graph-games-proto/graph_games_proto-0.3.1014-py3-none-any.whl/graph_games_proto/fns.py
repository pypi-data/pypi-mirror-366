...
def get_claim_path_legal_actions(game, player_idx):
    legal_actions = []
    
    for path in get_accessible_paths(game, player_idx):
        path_idx = path.num - 1
...
def get_accessible_paths(game, player_idx):
    return [
        path for path in game.game_config.fig.board_config.board_paths
        if is_path_empty(game, (path.num - 1)) and
        is_path_reachable_for_player(game, (path.num - 1), player_idx)
    ]


def is_path_empty(game, path_idx):
    return True


def is_path_reachable_for_player(game, path_idx, player_idx):
    return True
...
HOOK_NAMESPACE = {
    ...
    'get_claim_path_legal_actions': get_claim_path_legal_actions,
    'get_accessible_paths': get_accessible_paths,
    'is_path_empty': is_path_empty,
    'is_path_reachable_for_player': is_path_reachable_for_player,
...
DEFAULT_LEGAL_ACTIONS_HOOK = """def handler(game, player_idx):
    ...
    game = append_all_to_legal_actions(
        game,
        get_claim_path_legal_actions(game, player_idx),
    )
