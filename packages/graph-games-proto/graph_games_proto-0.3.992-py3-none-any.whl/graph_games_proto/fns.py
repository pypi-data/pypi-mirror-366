...
class ClaimablePath(PClass):
    path_idx = field(type=int)
    fulfillment = field(type=list)  # List[int]
    def __todict__(self):
        return {
            "path_idx": self.path_idx,
    ...
    state = State(
        ...
        decks=[
            Deck(
                ...
                units=[],
                facedown_stack=[
                    Card(idx=idx, is_wild=False, deck_idx=0, unit_uuid=str(uuid4())) for idx in range(gettotaldeckcards(fig))
                ],
                faceup_spots=[],
                discard=[],
            ),
            Deck(
                ...
                units=[],
                facedown_stack=[
                    Card(idx=idx, is_wild=False, deck_idx=1, unit_uuid=str(uuid4())) for idx in range(getnumroutecards(fig))
                ],
                faceup_spots=[],
                discard=[],
            )
...
def match_empty_tuple(game, fulfillment, cards, segments):
    ...
    tuples = get_tuples_sorted_small_first(cards, num_empty_segments)
    if len(tuples) == 0:
        return fulfillment, cards, segments
...
def get_tuples_sorted_small_first(cards, num_empty_segments):
    # TODO: implement this function to return a list of tuples sorted by their length, smallest first
    pass
...
def get_path_fulfillment(game, player_idx, path_idx):
    ...
    fulfillment, remaining_cards, remaining_segments = match_empty_tuple(game, fulfillment, remaining_cards, remaining_segments)
    if len(remaining_segments) == 0:
        return fulfillment
