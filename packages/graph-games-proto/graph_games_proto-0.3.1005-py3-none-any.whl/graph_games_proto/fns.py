...
class LegalActionClaimPath(PClass):
    path_idx = field(type=int)
    default_card_uuids = field(type=(list, type(None)), initial=None)  # List[str]
    def __todict__(self):
        return {
            "path_idx": self.path_idx,
            "default_card_uuids": self.default_card_uuids,
        }
    @staticmethod
    def __fromdict__(d):
        return LegalActionClaimPath(
            path_idx=d["path_idx"],
            default_card_uuids=d.get("default_card_uuids")  # Handle None case
        )
...
class ClaimablePath(PClass):
    path_idx = field(type=int)
    default_card_uuids = field(type=list)  # List[int]
    def __todict__(self):
        return {
            "path_idx": self.path_idx,
            "default_card_uuids": self.default_card_uuids,
    ...
    @staticmethod
    def __fromdict__(d):
        return ClaimablePath(
            path_idx=d["path_idx"],
            default_card_uuids=d["default_card_uuids"]
        )
...
def default_accept_action(game, action):
    print("****************************************** Inside default_accept_action")
    print("action.legal_action: ", action.legal_action)
    print("game.legal_actions_2: ", game.legal_actions_2)
    print("(action.legal_action in game.legal_actions_2): ", (action.legal_action in game.legal_actions_2))
...
def get_claimable_paths(game, player_idx):
    ...
    for path in game.game_config.fig.board_config.board_paths:
        ...
        if fulfillment:
            ...
            claimable_paths.append(
                ClaimablePath(
                    path_idx=path_idx,
                    default_card_uuids=[card.uuid for card in fulfillment],
                )
            )
        else:
            print(f"Path {path_idx} is not claimable by player {player_idx}")
    
    return claimable_paths
        ...
        game = append_to_legal_actions(
            ...
            LegalAction(
                ...
                config=LegalActionConfig(
                    ...
                    claim_path=LegalActionClaimPath(
                        path_idx=claimable_path.path_idx,
                        default_card_uuids=claimable_path.default_card_uuids,
                    )
                )
            )
        )
