    ...
    state = State(
        ...
        decks=[
            Deck(
                ...
                units=[],
                facedown_stack=[
                    Card(idx=idx, uuid=str(uuid4()), is_wild=False, deck_idx=0, unit_uuid=str(uuid4())) for idx in range(gettotaldeckcards(fig))
                ],
                faceup_spots=[],
                discard=[],
            ),
...
def get_uniform_sets(cards, length):
    ...
    uniform_sets_no_wilds = [
        cards for cards in unit_uuid_2_cards.values() if len(cards) >= length
    ]
    ...
    return [s for s in sorted if len(s) != length]
...
def get_path_fulfillment(game, player_idx, path_idx):
    ...
    print(f"Player cards: {[card.__todict__() for card in remaining_cards]}")
    print(f"Path segments: {[segment.__todict__() for segment in remaining_segments]}")
