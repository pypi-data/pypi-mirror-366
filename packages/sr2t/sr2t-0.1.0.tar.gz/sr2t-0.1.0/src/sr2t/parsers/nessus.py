        ...
        if row_count > 0:
            ws.add_table(
                ...
                6,
                {
                    "style": "Table Style Light 9",
                    "header_row": True,
                    "columns": [{"header_format": bold, "header": h} for h in header],
                },
            )
        ws.set_row(0, 30)
...
def nessus_parser(args, root, data_nessus, workbook):
    ...
    if args.output_csv:
        ...
        write_csv_file(base, "nessus", header, csv_array)
        write_csv_file(base, "nessus_portscan", [""] + ports, portscan_csv)

        for scan_key, scan in zip(scan_csv_arrays.keys(), scan_types):
            suffix = f"nessus_{scan['key']}"
            write_csv_file(
                base, suffix, scan_headers[scan_key], scan_csv_arrays[scan_key]
            )
