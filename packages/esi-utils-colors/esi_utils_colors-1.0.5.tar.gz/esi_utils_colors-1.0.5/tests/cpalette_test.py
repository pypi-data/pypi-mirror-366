...
def test_colormap():
    ...
    cmap = ColorPalette.fromColorMap(
        "viridis", np.arange(0, 10), np.arange(1, 11), viridis
    )
    zero_value = np.array(
    ...
    viridis = plt.get_cmap("viridis")
    cmap = ColorPalette.fromColorMap(
        "viridis", np.arange(-4, 5), np.arange(-3, 6), viridis, is_log=True
    )
...
def test_cpt():
    ...
    # meet colors
    ad = 50
    magma_cpt = plt.get_cmap("magma_r", 512)  # start with magma_r
    ...
    name = "test"
    cpt = ColorPalette.fromColorMap(name, z0, z1, slipmap, resolution=resolution)
    try:
        tdir = tempfile.mkdtemp()
        tfile = os.path.join(tdir, "test.cpt")
        cpt.write(tfile)
        ...
        np.testing.assert_almost_equal(
            cpt.getDataColor(150)[0], cpt2.getDataColor(150)[0]
        )
    except BaseException:
        pass
    finally:
