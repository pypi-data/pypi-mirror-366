...
def create_components_breakdown(annual_df, quarterly_df, ticker):
    ...
    if 'Q' in str(quarterly_sorted['Period'].iloc[0]):
        # This is actual quarterly data with Q1, Q2, etc.
        quarterly_sorted['period_sort'] = quarterly_sorted['Period'].str.replace('Q', '').astype(float)
    else:
...
def create_annual_waterfall_chart(ax, df, ticker):
    ...
    # Ensure data is sorted chronologically and get the last 5 years
    df_sorted = df.copy()
    ...
    recent_years = df_sorted.tail(5).copy()
    
    print(f"[DEBUG] Annual waterfall years selected: {recent_years['Period'].tolist()}")
...
def create_waterfall_chart(ax, df, ticker):
    ...
    if 'Q' in str(df_sorted['Period'].iloc[0]):
        # This is actual quarterly data with Q1, Q2, etc.
        df_sorted['period_sort'] = df_sorted['Period'].str.replace('Q', '').astype(float)
    else:
        # This is annual data treated as quarterly, sort by year
