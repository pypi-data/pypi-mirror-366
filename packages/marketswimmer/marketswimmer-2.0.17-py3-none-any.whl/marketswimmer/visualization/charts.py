...
def load_data():
    """Load both annual and quarterly CSV data with improved file discovery."""
    try:
        import glob
        import os
        ...
        # First, try to detect the ticker from the most recent XLSX download
        ticker_symbol = detect_ticker_symbol()
        print(f"[DEBUG] Detected ticker: {ticker_symbol}")
        
        # Search for ticker-specific files first (highest priority)
        ticker_patterns = []
        if ticker_symbol and ticker_symbol != "TICKER":
            clean_ticker = ticker_symbol.lower().replace('.', '')
            ticker_patterns = [
        ...
        # Then search for any owner earnings files (lower priority)
        general_patterns = [
        ...
        # Combine patterns with ticker-specific first
        search_patterns = ticker_patterns + general_patterns
        ...
        for pattern in search_patterns:
            ...
            if files:
                ...
                print(f"[DEBUG] Pattern '{pattern}' found: {files}")
        ...
        if not annual_files:
            ...
            print(f"[DEBUG] Searched patterns: {search_patterns}")
        ...
        # Find corresponding quarterly data file using same ticker
        quarterly_files = []
        if ticker_symbol and ticker_symbol != "TICKER":
            clean_ticker = ticker_symbol.lower().replace('.', '')
            quarterly_patterns = [
        ...
        # Add general quarterly patterns as fallback
        quarterly_patterns.extend([
        ...
        for pattern in quarterly_patterns:
            ...
            if files:
                ...
                print(f"[DEBUG] Quarterly pattern '{pattern}' found: {files}")
        ...
        if not quarterly_files:
            ...
            print(f"[DEBUG] Searched quarterly patterns: {quarterly_patterns}")
            return None, None
