...
def load_data():
    ...
    try:
        # Look for any owner earnings files in the data directory
        import glob
        
        # Find annual data files
        annual_files = glob.glob('data/owner_earnings_annual_*.csv')
        if not annual_files:
            # Fallback to old filename format
            annual_files = glob.glob('data/owner_earnings_financials_annual.csv')
        
        if not annual_files:
            print("[ERROR] No annual data files found in data/ directory")
            return None, None
            
        annual_path = annual_files[0]  # Use the first file found
        annual_df = pd.read_csv(annual_path)
        print(f"[OK] Loaded annual data: {len(annual_df)} years from {annual_path}")
        
        # Find quarterly data files
        quarterly_files = glob.glob('data/owner_earnings_quarterly_*.csv')
        if not quarterly_files:
            # Fallback to old filename format
            quarterly_files = glob.glob('data/owner_earnings_financials_quarterly.csv')
            
        if not quarterly_files:
            print("[ERROR] No quarterly data files found in data/ directory")
            return None, None
            
        quarterly_path = quarterly_files[0]  # Use the first file found
