...
class MessageLifecycleService:
    ...
    async def retry_message(self, message: Message, topic: str) -> None:
        ...
        try:
            ...
            # 更新过期时间（使用秒级时间戳）
            new_expire_time = (
                current_time * 1000 + retry_delay * 1000 + self.context.config.message_ttl
            )
            ...
            await self.context.lua_scripts["retry_message"](
                keys=[
                    ...
                    self.context.get_global_key(GlobalKeys.EXPIRE_MONITOR),
                    self.context.get_global_topic_key(topic, TopicKeys.PROCESSING),  # 新增：processing队列
                ],
                args=[
                    message.id,
                    message.model_dump_json(by_alias=True, exclude_none=True), # 新的 message 消息体
                    retry_delay,
                    topic,  # 新增：topic参数
                ],
            )
        ...
        try:
            ...
            await self.context.lua_scripts["move_to_dlq"](
                keys=[
                    ...
                    self.context.get_global_key(GlobalKeys.EXPIRE_MONITOR),
                    self.context.get_global_key(GlobalKeys.PAYLOAD_MAP),
                    self.context.get_global_topic_key(message.topic, TopicKeys.PROCESSING),  # 新增：processing队列
                ],
                args=[
                    message.id, 
