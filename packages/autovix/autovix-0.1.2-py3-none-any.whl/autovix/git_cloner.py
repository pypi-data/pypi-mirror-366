...
import subprocess
import os
from autovix.utils import UiDeployProcess, BackendDeployProcess
import time
...
class GitAutoCloner:
    ...
    def create_git_urls(self, json_data):
        ...
        for repo in repositories:
            run_command=repo.get("run_command",None)
    ...
    def deploy_all_projects(self, json_data):
        ...
        for entry in self.repo_entries:
            ...
            if entry["is_ui"]:
                ...
                backend_util.start_uvicorn_server(self.env_name, app_path,run_command )


json_data = {
    "env_name": "autovix",
    "git_data": {
        "username": "VenkeyReddy123",
        "token": "ghp_xxxxxx",  # Replace with a valid token
        "org_or_user": "VenkeyReddy123"
    },
    "base_path": "C:/deploy",
    "repositories": [
        # {
...
cloner = GitAutoCloner()
cloner.deploy_all_projects(json_data)