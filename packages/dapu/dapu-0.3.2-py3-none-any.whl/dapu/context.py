...
class DapuContext():
    ...
    def run(self, alias: str, sql: str | DataCapsule, do_return: bool = True, **kwargs: dict) -> DataCapsule:
        ...
        if isinstance(sql, str):
            ...
            capsule.set_flags(kwargs)
            capsule.set_flag("do_return", do_return)
        else:
            capsule = sql
        if not do_return:
    ...
    def send_outside(self, message: str = ""):
        if self.discord:
            self.discord.notify(message, f"Dapu server ({self.my_shift}) says") # TEST või PROD?
            #self.last_send = datetime.now() # kui saadame, siis regama aja, et saaks takistada pommitamist (discordi rahval saab ju halb olema)
    ...
    def check_pause(self) -> bool:
        """
        Prevents running if global pause is turned ON
        """
        stopper = self.context.find_registry_table_full_name('stopper')
