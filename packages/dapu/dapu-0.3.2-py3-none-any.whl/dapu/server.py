...
alternative_profiles = os.getenv("DAPU_PROFILES")
profiles_text: str = read_content_of_file(work_dir + "/conf/profiles.yaml") or read_content_of_file(alternative_profiles) or ""
...
def web_signal_info(request: Request):
    context: DapuContext = request.app.state.app_context
    global_state = "PAUSED" if context.check_pause() else "ENABLED"
    html = f"{global_state}"
    return HTMLResponse(html)
