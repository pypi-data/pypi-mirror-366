...
def get_entity_custom_fields(workspace_id: int, options: dict = None) -> str:
    ...
    Args:
        workspace_id: 项目ID（必填）
        options: 可选参数字典，支持以下字段：
            - entity_type: 类型，需求stories或任务tasks或迭代iterations或测试用例tcases（必填）
...
def get_iterations(workspace_id: int, options: dict = None) -> dict:
    ...
    Args:
        ...
        options:
            ...
            - custom_field_{number}: 自定义字段参数，具体字段名通过 get_entity_custom_fields 获取自定义字段配置，比如用户要获取XXX是 XXX 的迭代，如果不是上面的字段，则需要传入自定义字段名获取
            ...
    Returns: <str> 
    """
    if not workspace_id:
