            ...
            if img.get("data"):
                try:
                    ...
                    if img_base64:
                        ...
                        # 默认包含完整 Base64 数据以支持 MCP 协议
                        # 根据 MCP 官方文档，图片应该以 Base64 文本形式传输
...
# process_images 函数已移除
# 根据 MCP 官方文档，图片数据应该以 Base64 文本形式传输，
# 而不是直接返回 MCPImage 对象，以避免序列化错误
...
async def interactive_feedback(
    ...
    Returns:
        list: List containing TextContent objects representing user feedback (images are included as Base64 text)
    """
    # 环境检测
    ...
    try:
        ...
        # 图片数据已经在 create_feedback_text 中处理为 Base64 文本
        # 不再直接返回 MCPImage 对象以避免序列化错误
