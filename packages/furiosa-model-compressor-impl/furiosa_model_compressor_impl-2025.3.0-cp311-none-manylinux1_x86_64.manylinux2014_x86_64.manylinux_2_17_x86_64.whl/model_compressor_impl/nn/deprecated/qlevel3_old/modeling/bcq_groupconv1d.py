...
__all__ = ["_BCQ_GroupConv1d"]


class _BCQ_GroupConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, bias=True, out_dtype=None, **kwargs):
        super(_BCQ_GroupConv1d, self).__init__()
        ...
        factory_kwargs = {'device': device, 'dtype': torch.float32}  # this would be fixed
        factory_kwargs_bias = {'device': device, 'dtype': torch.float32}  # this would be fixed

        # set parameters
        self.basis = nn.Parameter(
            torch.empty((out_channels, in_channels // out_channels, 1), **factory_kwargs),
            requires_grad=False,
        )
        if bias:
    ...
    def forward(self, x):
        ...
        out = F.conv1d(
            out.t(), _basis, _bias, stride=1, groups=self.groups
        )  # t.() to make channels axis to first

        if not self._o_dtype.is_floating_point:
            # quantize
            out = away_from_zero_round(out)

        out = out.t().to(self._o_dtype)  # t.() reset order of axis to original
