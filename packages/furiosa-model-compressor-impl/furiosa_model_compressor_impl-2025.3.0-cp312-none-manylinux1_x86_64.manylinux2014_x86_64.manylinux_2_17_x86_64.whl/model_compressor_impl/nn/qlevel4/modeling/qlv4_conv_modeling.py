...
class QLV4_Conv2d_MOD(nn.Module):
    def __init__(
        self,
        org_target,
        weight_real_dtype,
        ...
        super(QLV4_Conv2d_MOD, self).__init__()
        self.stride = stride
        ...
        self.zero_point = zero_point_input
        self.emul_dtype = emul_dtype
        self.weight_real_dtype = weight_real_dtype
        self.org_target = org_target
        self.weight_shape = org_target.weight.shape
        self.node_name = node_name
        self.register_buffer(
            'weight_scale',
            weight_quantizer['merged_scale'] if weight_quantizer['_dequantize'] else None,
            persistent=False,
        )
        self.register_buffer(
            'weight_zero_point',
    ...
    def forward(self, input):
        ...
        output = wrap_func_with_type_emulation(
            [padded_input, _weight],
            [DATA_MATCHER[input_real_dtype], DATA_MATCHER[weight_real_dtype]],
            out_dtype,
            self.node_name + 'conv',
            self.emul_dtype,
            lambda padded_input, _weight: torch.ops.aten.conv2d(
                padded_input,
                _weight,
                None,
                self.stride,
    ...
    def decoding_weight(self, input, weight):
        ...
        if input.dtype == torch.bfloat16 and weight.dtype != torch.bfloat16:
            ...
            if self.weight_real_dtype == 'int4':
                weight = torch.ops.aten.view(
                    weight, [-1, self.group_size, self.kernel_size[0], self.kernel_size[1]]
                )
