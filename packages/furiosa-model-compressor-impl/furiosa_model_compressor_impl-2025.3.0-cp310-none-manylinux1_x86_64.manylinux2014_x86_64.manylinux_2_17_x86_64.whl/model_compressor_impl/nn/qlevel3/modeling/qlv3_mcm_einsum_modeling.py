...
__all__ = [
    "QLV3_ModelCompressorModuleEinsum_MOD",
]


# binary modeling은 항상 VE 에서 fp32 로 casting 이후 연산됨을 가정!
class QLV3_ModelCompressorModuleEinsum_MOD(nn.Module):
    ...
    def forward(self, equantion, input_0, input_1, *args, **kwargs):
        if not isinstance(input_0, torch.Tensor) and not isinstance(input_1, torch.Tensor):
            output = self._org_target(equantion, input_0, input_1, *args, **kwargs)
            return output
