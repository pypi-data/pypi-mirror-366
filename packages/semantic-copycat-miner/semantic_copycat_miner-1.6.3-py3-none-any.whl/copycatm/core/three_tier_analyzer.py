...
from ..analysis.copyright_extractor import CopyrightExtractor
from ..analysis.license_normalizer import LicenseNormalizer
from ..hashing.direct import DirectHasher
from ..hashing.fuzzy import FuzzyHasher
from ..hashing.semantic import SemanticHasher
from ..hashing.copyright_hasher import CopyrightHasher
...
class ThreeTierAnalyzer:
    ...
    def __init__(self, config: Optional[AnalysisConfig] = None):
        ...
        self.copyright_extractor = CopyrightExtractor()
        spdx_data_path = Path(__file__).parent.parent / 'data' / 'spdx_licenses.json'
        self.license_normalizer = LicenseNormalizer(str(spdx_data_path))
        self.copyright_hasher = CopyrightHasher()
        ...
        if SEMANTIC_ANALYZER_AVAILABLE:
            try:
                # Try to load reference implementation database
                ref_db_path = Path(__file__).parent.parent / 'data' / 'reference_implementations.json'
                if ref_db_path.exists():
                    self.semantic_analyzer = SemanticAnalyzer(str(ref_db_path))
                else:
    ...
    def _tier1_baseline_analysis(self, file_path: str, content: str, 
        ...
        winnowing_config = WinnowingConfig(
            k_gram_size=30,
            window_size=64,
            normalize_code=True,
            remove_comments=True
        )
        winnowing = Winnowing(winnowing_config)
        
        winnowing_signature = winnowing.generate_signature(content, metadata['language'])
        winnowing_hashes = {
            'fingerprint_count': winnowing_signature['fingerprint_count'],
            'fingerprint_hash': self.direct_hasher.sha256(
                json.dumps(sorted(winnowing_signature['hash_values']))
            ) if winnowing_signature['hash_values'] else None,
            'normalized_length': winnowing_signature['normalized_length'],
            'config': winnowing_signature['config']
        }
        ...
        # Extract copyright and authorship information
        copyright_info = self.copyright_extractor.extract(content)
        
        # Normalize licenses to SPDX
        if copyright_info.get('license_names'):
            for license_name in copyright_info['license_names']:
                spdx_id, confidence, _ = self.license_normalizer.normalize(license_name)
                if spdx_id and spdx_id not in copyright_info.get('license_spdx_ids', []):
                    copyright_info.setdefault('license_spdx_ids', []).append(spdx_id)
        
        # Generate copyright hashes
        copyright_hashes = self.copyright_hasher.generate_hashes(copyright_info)
        
        # Generate copyright signature
        copyright_signature = self.copyright_hasher.generate_copyright_signature(copyright_info)
        
        # Add copyright hashes to semantic hashes
        semantic_hashes.update({
            'copyright_hash': copyright_hashes.get('copyright_hash'),
            'license_hash': copyright_hashes.get('license_hash'),
            'authorship_hash': copyright_hashes.get('authorship_hash'),
            'normalized_copyright_hash': copyright_hashes.get('normalized_copyright_hash')
        })
        ...
        return {
            ...
            'hashes': {
                ...
                'copyright': copyright_hashes,
                'winnowing': winnowing_hashes
            },
            'code_metrics': {
            ...
            'copyright_signature': copyright_signature,
    ...
    def _tier2_traditional_analysis(self, content: str, language: str, 
        ...
        from ..analysis.winnowing_detector import WinnowingDetector
        detector = AlgorithmDetector(self.config)
        winnowing_detector = WinnowingDetector()
        ...
        try:
            ...
            # Enhance with winnowing-based detection
            winnowing_matches = winnowing_detector.detect_multiple_algorithms(content, language, threshold=0.3)
            
            # Merge winnowing results with pattern-based results
            for match in winnowing_matches:
                # Check if this algorithm was already detected by patterns
                already_detected = False
                for algo in algorithms:
                    if (algo.get('algorithm_type') == match.algorithm_type and
                        algo.get('algorithm_subtype') == match.algorithm_subtype):
                        # Enhance existing detection with winnowing confidence
                        algo['winnowing_confidence'] = match.confidence
                        algo['winnowing_similarity'] = match.similarity_score
                        algo['confidence'] = max(algo.get('confidence', 0), match.confidence)
                        already_detected = True
                        break
                
                if not already_detected:
                    # Add new algorithm detected only by winnowing
                    winnowing_algo = {
                        'algorithm_type': match.algorithm_type,
                        'algorithm_subtype': match.algorithm_subtype,
                        'confidence': match.confidence,
                        'detection_method': 'winnowing',
                        'winnowing_similarity': match.similarity_score,
                        'matching_regions': match.matching_regions,
                        'fingerprint_overlap': match.fingerprint_overlap,
                        'evidence': {
                            'pattern_matches': [],
                            'confidence_factors': {
                                'winnowing_similarity': match.similarity_score,
                                'fingerprint_coverage': match.fingerprint_overlap / match.total_fingerprints
                            }
                        }
                    }
                    algorithms.append(winnowing_algo)
            ...
            for algo in algorithms:
                ...
                if self.semantic_analyzer and algo.get('function_name'):
                    try:
                        # Get function code
                        func_code = evidence.get('code_snippet', evidence.get('normalized_representation', ''))
                        if func_code:
                            # Extract semantic fingerprint
                            semantic_fp = self.semantic_analyzer.extract_semantic_fingerprint(func_code, language)
                            
                            # Check against reference database
                            ref_matches = self.semantic_analyzer.find_similar_in_reference_db(semantic_fp, threshold=0.6)
                            
                            semantic_features = {
                                'winnowing_fine_count': len(semantic_fp.winnowing_fine),
                                'winnowing_medium_count': len(semantic_fp.winnowing_medium), 
                                'winnowing_coarse_count': len(semantic_fp.winnowing_coarse),
                                'cfg_hash': semantic_fp.cfg_hash,
                                'dfg_hash': semantic_fp.dfg_hash,
                                'transforms': semantic_fp.transforms,
                                'crypto_ops': semantic_fp.crypto_ops,
                                'reference_matches': ref_matches[:3]  # Top 3 matches
                            }
                            
                            # If we have reference matches, update algorithm classification
                            if ref_matches and ref_matches[0][1] > 0.7:
                                best_match = ref_matches[0]
                                logger.info(f"Found reference match: {best_match[0]} with similarity {best_match[1]:.3f}")
                                algo['reference_algorithm'] = best_match[0]
                                algo['reference_similarity'] = best_match[1]
                                # Update confidence if reference match is strong
                                algo['confidence'] = max(algo.get('confidence', 0), best_match[1])
                                
                    except Exception as e:
                        logger.debug(f"Semantic analysis failed for algorithm: {e}")
                ...
                if semantic_features:
                    signature['semantic_features'] = semantic_features
                
                # Add winnowing fields if present
                if 'winnowing_confidence' in algo:
                    signature['winnowing_confidence'] = algo['winnowing_confidence']
                if 'winnowing_similarity' in algo:
                    signature['winnowing_similarity'] = algo['winnowing_similarity']
                if algo.get('detection_method') == 'winnowing':
                    signature['detection_method'] = 'winnowing'
                    signature['fingerprint_overlap'] = algo.get('fingerprint_overlap', 0)
                    signature['matching_regions'] = algo.get('matching_regions', [])
    ...
    def _tier3_semantic_analysis(self, content: str, language: str,
        ...
        - Comprehensive semantic fingerprinting
        """
        ...
        if self.semantic_analyzer:
            try:
                # Extract comprehensive semantic fingerprint for the entire file
                file_semantic_fp = self.semantic_analyzer.extract_semantic_fingerprint(content, language)
                
                # Check for reference implementations
                ref_matches = self.semantic_analyzer.find_similar_in_reference_db(file_semantic_fp, threshold=0.5)
                if ref_matches:
                    logger.info(f"Found {len(ref_matches)} reference matches for file")
                    for match_name, match_score in ref_matches[:5]:
                        logger.info(f"  - {match_name}: {match_score:.3f}")
                
            except Exception as e:
                logger.warning(f"Semantic fingerprinting failed: {e}")
        ...
        try:
            ...
            for block in normalized_blocks:
                ...
                # Apply semantic analysis to the block if available
                block_semantic_features = None
                if self.semantic_analyzer and block.get('normalized_code'):
                    try:
                        # Extract semantic fingerprint for the block
                        block_fp = self.semantic_analyzer.extract_semantic_fingerprint(
                            block['normalized_code'], language
                        )
                        
                        # Check against reference database
                        block_ref_matches = self.semantic_analyzer.find_similar_in_reference_db(
                            block_fp, threshold=0.6
                        )
                        
                        block_semantic_features = {
                            'cfg_hash': block_fp.cfg_hash,
                            'dfg_hash': block_fp.dfg_hash,
                            'math_sequence': block_fp.math_sequence[:20],  # First 20 operations
                            'transforms': block_fp.transforms,
                            'quantization_ops': block_fp.quantization_ops,
                            'entropy_coding': block_fp.entropy_coding,
                            'crypto_ops': block_fp.crypto_ops,
                            'reference_matches': block_ref_matches[:3]
                        }
                        
                        # Update classification if we found a strong match
                        if block_ref_matches and block_ref_matches[0][1] > 0.75:
                            best_match = block_ref_matches[0]
                            block['classification'] = best_match[0]
                            block['confidence'] = max(block.get('confidence', 0), best_match[1])
                            logger.info(f"Block matched reference: {best_match[0]} ({best_match[1]:.3f})")
                            
                    except Exception as e:
                        logger.debug(f"Semantic analysis failed for block: {e}")
                ...
                # Add semantic features if available
                if block_semantic_features:
                    signature['semantic_features'] = block_semantic_features
    ...
    def _build_output(self, baseline_results: Dict[str, Any], 
        ...
        return {
            ...
            'copyright_info': baseline_results.get('copyright_info', {}),
            'copyright_signature': baseline_results.get('copyright_signature', {}),
