...
class AlgorithmDetector:
    ...
    def _check_external_patterns(self, func_text: str, func_name: str) -> Optional[Dict[str, Any]]:
        """Check function against external pattern definitions."""
        best_match = None
        best_score = 0.0
        ...
        for algo_type, type_patterns in self.external_patterns.items():
            for algo_name, pattern_config in type_patterns.items():
                ...
                keyword_matches = sum(1 for kw in keywords if kw in func_text or kw in func_name)
                if keyword_matches > 0:
                    score += min(keyword_matches * 0.2, 0.6)
                
                # Check required patterns
                required = pattern_config.get('required_patterns', [])
                required_matches = sum(1 for pattern in required if pattern in func_text)
    ...
    def _analyze_function_with_specificity(self, func: Dict[str, Any], ast_tree: Any, language: str) -> Optional[Dict[str, Any]]:
        ...
        # First check external patterns
        external_match = self._check_external_patterns(func_text, func_name)
        if external_match:
            best_match = external_match['type']
            best_confidence = external_match['confidence']
    ...
    def _calculate_specific_confidence(self, func_text: str, func_name: str, 
        ...
        if required_patterns:
            required_score = self._check_required_patterns(func_text, func_node, required_patterns, func_name)
    ...
    def _check_required_patterns(self, func_text: str, func_node: Any, required_patterns: List[str], func_name: str = "") -> float:
        ...
        pattern_checks = {
            # Sorting patterns
            'pivot_selection': lambda: 'pivot' in func_text.lower(),
            'partition_logic': lambda: (('left' in func_text.lower() and 'right' in func_text.lower()) or 
                                      'partition' in func_text.lower() or
