...
class TicketGroupChecker(Checker):
    model = Ticket
    verbose_name = _("Check whether author is member of the group")

    def get_checkdata_problems(self, ar, obj, fix=False):
        if not obj.group_id:
            yield False, _("Ticket has no group")
            # yield True, _("Ticket has no group")
            # if fix:
            #     obj.group = dd.plugins.groups.get_default_group()
            #     obj.full_clean()
            #     obj.save()
        if obj.group_id and obj.user_id:
            qs = obj.group.members.filter(user=obj.user)
            if not qs.exists():
                yield False, _("{user} is not member of {group}").format(
                    user=obj.user, group=self.obj)
