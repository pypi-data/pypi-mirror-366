...
class ColorHash:
    ...
    def _extend_hash(self, hash_data: bytes, target_length: int) -> bytes:
        """Extend hash to target length using iterative hashing"""
        extended = bytearray(hash_data)
        
        while len(extended) < target_length:
            # Hash current data to extend it
            new_hash = hashlib.sha256(bytes(extended)).digest()
            extended.extend(new_hash)
        
        return bytes(extended[:target_length])
    
    def hash_to_hex_string(self, data: bytes, salt: Optional[bytes] = None) -> str:
        """Hash data to hex string representation"""
