...
class ChromaCryptKEM:
    ...
    def __init__(self, security_level: int = 128, optimized: bool = False):
        ...
        # Parameters are pre-validated for PyPI packaging
    ...
    def _generate_public_colors(self, matrix: np.ndarray, secret: np.ndarray) -> list:
        ...
        for i in range(num_colors):
            ...
            error = self.lattice_engine.generate_error_vector(error_seed)
    ...
    def encaps(self, public_key: ChromaCryptPublicKey) -> Tuple[bytes, bytes]:
        """Encapsulate shared secret (alias for encapsulate)"""
    ...
    def encapsulate(self, public_key: ChromaCryptPublicKey) -> Tuple[bytes, bytes]:
        ...
        error_vector = self.lattice_engine.generate_error_vector(encap_seed[16:])
    ...
    def decaps(self, ciphertext: bytes, private_key: ChromaCryptPrivateKey) -> bytes:
        """Decapsulate shared secret (alias for decapsulate)"""
        return self.decapsulate(private_key, ciphertext)
