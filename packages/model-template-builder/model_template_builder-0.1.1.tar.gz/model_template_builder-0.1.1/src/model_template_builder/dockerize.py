...
def dockerize_hugging_face_model_vllm(model_name: str, image_name: Optional[str] = None) -> bool:
    if not image_name:
...
""")
        ...
        try:
            for line in client.api.build(path=str(tmp_path), tag=image_name, decode=True):
                if 'stream' in line:
        ...
        print("Usage: docker run -p 8000:8000 -e HUGGING_FACE_HUB_TOKEN='your-hugging-face-hub-token' --gpus all -t 'your-image-name'")
