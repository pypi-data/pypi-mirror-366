...
from google_workspace_sdk import GoogleWorkspaceSDK, DefaultAioHttpClient, AsyncGoogleWorkspaceSDK
from google_workspace_sdk._utils import is_dict
...
logging.getLogger("google_workspace_sdk").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[GoogleWorkspaceSDK]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with GoogleWorkspaceSDK(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncGoogleWorkspaceSDK]:
