...
class Page:
    ...
    async def act(
        ...
        try:
            ...
            # Execute the action using the web agent
            result = await self._web_agent.execute_interactive_step(prompt, context)
    ...
    async def inspect_element(self, x: int, y: int, num_ancestors: int = 3) -> str:
        ...
        return await inspect_element_recursive(
            self._stagehand_page, x, y, num_ancestors, mode="element"
        )
    ...
    async def serve_from_warc(
        ...
        async def warc_route_handler(route: Route):
            ...
            if self._warc_responses and request_url in self._warc_responses:
                ...
                # Return 404 for URLs not in WARC
                await route.fulfill(
                    status=404,
                    body=f"URL not found in WARC: {request_url}",
                )
