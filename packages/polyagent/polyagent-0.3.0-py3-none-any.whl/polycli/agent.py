...
from .utils.registerModel import *

class OpenSourceAgent(BaseAgent):
    def __init__(self, debug=False, openrouter_api_key=None, system_prompt=None, cwd=None) -> None:
        self.cwd = cwd
        self.messages = []
    ...
    def run(self, prompt: str, model="z-ai/glm-4.5", system_prompt=None):
        
        temp_agent = DefaultAgent(
            LitellmModel(model_name="openrouter/" + model),
        ...
        status, message = temp_agent.run(json_list_serializer(self.messages) + "\nuser (current task): " + prompt)
