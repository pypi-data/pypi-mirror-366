...
class ProcessorBase(ABC):
    ...
    async def process_file(self, file: QuivrFile) -> list[Document]:
        ...
        try:
            qvr_version = version("quivr-modified-milvus")
        ...
        for idx, doc in enumerate(docs, start=1):
            ...
            my_metadata = {}
            my_metadata["qfile_id"] = str(file.metadata["qfile_id"])
            my_metadata["qfile_path"] = str(file.metadata["qfile_path"])
            my_metadata["original_file_name"] = file.metadata["original_file_name"]
            my_metadata["file_sha1"] = file.metadata["file_sha1"]
            my_metadata["file_size"] = file.metadata["file_size"]
