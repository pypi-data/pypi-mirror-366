...
class ViewManager(QWidget):
    ...
    def __init__(self):
        ...
        self.__modifiedFiles = []
        # keep track of files reported by the file system watcher as modified

        # initialize the file system watcher
        watcher = EricFileSystemWatcher.instance()
        watcher.fileModified.connect(self.__watchedFileModified)
        watcher.fileClosed.connect(self.__watchedFileClosed)
    ...
    @pyqtSlot(str)
    def __watchedFileModified(self, filePath):
        """
        Private slot handling modification of a watched file.

        @param filePath path of the file
        @type str
        """
        if filePath not in self.__modifiedFiles:
            editorList = self.getOpenEditorList(filePath)
            if editorList:
                # there is at least one open editor for that file
                self.__modifiedFiles.append(filePath)

    @pyqtSlot(str)
    def __watchedFileClosed(self, filePath):
        """
        Private slot to handle closing a watched file.

        @param filePath path of the file
        @type str
        """
        if filePath in self.__modifiedFiles:
            editorList = self.getOpenEditorList(filePath)
            if editorList:
            ...
            with contextlib.suppress(ValueError):
                # don't care, if the file path is no longer in the list
                self.__modifiedFiles.remove(filePath)
