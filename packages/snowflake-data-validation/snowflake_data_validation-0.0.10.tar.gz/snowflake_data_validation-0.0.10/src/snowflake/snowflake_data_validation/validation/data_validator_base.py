...
from snowflake.snowflake_data_validation.utils.constants import (
    ...
    NOT_APPLICABLE_CRITERIA_DISPLAY,
...
class DataValidatorBase:
    ...
    def prepare_data_for_display(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        value_columns = [SOURCE_VALUE_KEY, SNOWFLAKE_VALUE_KEY]

        for column in value_columns:
            if column in display_df.columns:
                display_df[column] = display_df[column].astype(str)
                display_df[column] = display_df[column].replace(
                    str(NOT_APPLICABLE_CRITERIA_VALUE),
                    NOT_APPLICABLE_CRITERIA_DISPLAY,
                )
        ...
        if COMMENTS_KEY in display_df.columns:
            for idx, comment in display_df[COMMENTS_KEY].items():
                if str(NOT_APPLICABLE_CRITERIA_VALUE) in comment:
                    ...
                    if (
                        source_val == NOT_APPLICABLE_CRITERIA_DISPLAY
                        and target_val == NOT_APPLICABLE_CRITERIA_DISPLAY
                    ):
                        meaningful_comment = "Metric doesn't apply to this column in both source and target."
                    elif source_val == NOT_APPLICABLE_CRITERIA_DISPLAY:
                        meaningful_comment = (
                            "Metric doesn't apply to this column in source."
                        )
                    elif target_val == NOT_APPLICABLE_CRITERIA_DISPLAY:
                        meaningful_comment = (
                            "Metric doesn't apply to this column in target."
                        )
                    else:
                        meaningful_comment = "Metric doesn't apply to this column."
