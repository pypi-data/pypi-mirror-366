...
class TestMetadataExtractorTeradata(unittest.TestCase):
    ...
    def test_md5_methods_not_implemented(self):
        """Test that MD5-related methods raise NotImplementedError."""
        mock_table_context = self._create_mock_table_context()

        # Test all MD5-related methods
        ...
        with self.assertRaises(NotImplementedError):
            self.extractor.extract_chunks_md5(table_context=mock_table_context)

        with self.assertRaises(NotImplementedError):
            self.extractor.extract_md5_rows_chunk(
