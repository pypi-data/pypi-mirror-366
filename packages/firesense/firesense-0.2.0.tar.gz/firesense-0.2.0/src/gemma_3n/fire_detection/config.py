...
class Gemma3NE4BConfig(BaseModel):
    ...
    model_path: str = Field(default="google/gemma-2-2b-it", description="Model path (Hugging Face or local)")
    
    # Quantization settings (optional)
    use_quantization: bool = Field(default=False, description="Enable model quantization")
    quantization_type: str = Field(default="4bit", description="Quantization type (4bit, 8bit)")
    quantization_compute_dtype: str = Field(default="float16", description="Compute dtype for quantization")
    ...
    max_memory: Dict[int, str] = Field(default={0: "6GB"}, description="GPU memory allocation")
    
    # Inference settings
    max_new_tokens: int = Field(default=200, ge=50, le=1000, description="Max tokens to generate")
...
class VideoProcessingConfig(BaseModel):
    ...
    # Frame extraction
    frame_interval: float = Field(default=1.0, ge=0.1, le=60.0, description="Seconds between frames")
    max_frames: Optional[int] = Field(default=None, ge=1, description="Maximum frames to process")
    start_time: float = Field(default=0.0, ge=0.0, description="Start time in seconds")
    end_time: Optional[float] = Field(default=None, ge=0.0, description="End time in seconds")
    ...
    @validator("end_time")
    def validate_end_time(cls, v: Optional[float], values: dict) -> Optional[float]:
...
class DetectionConfig(BaseModel):
    ...
    # Output settings
    save_positive_frames: bool = Field(default=True, description="Save frames with fire detected")
    save_all_frames: bool = Field(default=False, description="Save all processed frames")
    frame_format: Literal["jpg", "png"] = Field(default="jpg", description="Frame image format")
    
    # Analysis settings
    enable_temporal_analysis: bool = Field(default=True, description="Enable temporal analysis")
    temporal_window: int = Field(default=5, ge=1, le=20, description="Frames for temporal analysis")
...
class OutputConfig(BaseModel):
    ...
    results_filename: str = Field(default="fire_detection_results", description="Results filename")
    
    # Output formats
    output_format: Literal["json", "csv", "both"] = Field(default="both", description="Output format")
    include_metadata: bool = Field(default=True, description="Include video metadata")
    include_timestamps: bool = Field(default=True, description="Include frame timestamps")
