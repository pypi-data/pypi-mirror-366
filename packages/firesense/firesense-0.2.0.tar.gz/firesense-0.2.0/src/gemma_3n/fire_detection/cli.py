...
def analyze(
    ...
    interval: float = typer.Option(1.0, "--interval", "-i", help="Frame extraction interval in seconds"),
    output_dir: Path = typer.Option(Path("."), "--output", "-o", help="Output directory for results"),
):
    """Download YouTube video, extract frames, and analyze for fire detection."""
    
    console.print(f"[bold green]🔥 Starting Fire Detection Analysis[/bold green]")
    console.print(f"[blue]Video ID: {video_id}[/blue]")
