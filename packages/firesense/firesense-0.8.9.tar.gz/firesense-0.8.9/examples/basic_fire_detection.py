...
async def analyze_video_frame_by_frame(use_quantization=False):
    ...
    print(f"📊 Processing frame by frame...")
    ...
    try:
        ...
        async for frame_data in video_processor.extract_frames(video_path):
            ...
            print(f"   🔍 Analyzing frame for fire detection...")
            ...
            print(f"   🎯 Fire Presence Probability: {detection_result.fire_presence_probability:.3f}")
            print(f"   ⚠️  Uncontrolled Fire Probability: {detection_result.uncontrolled_fire_probability:.3f}")
    ...
    if fire_detections:
        ...
        emergency_frames = [r for r in fire_detections 
        ...
        if fire_detections:
            ...
            for result in fire_detections:
                status = "🚨 EMERGENCY" if (result.fire_characteristics and 
                                          result.fire_characteristics.call_911_warranted) else "🔥"
                print(f"   {status} Frame {result.frame_number} at {result.timestamp:.1f}s "
    ...
    print(f"📈 Average Time per Frame: {total_processing_time/len(detection_results):.3f}s")
    
    print(f"\n🎯 Example completed successfully!")
    print(f"💡 Next step: Use this logic for webcam stream analysis")
...
def main():
    ...
    try:
        asyncio.run(analyze_video_frame_by_frame(use_quantization=args.use_quantization))
    except Exception as e:
        print(f"\n❌ Script failed: {e}")
        return 1
