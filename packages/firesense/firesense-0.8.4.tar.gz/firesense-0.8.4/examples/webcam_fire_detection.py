...
class WebcamFireDetector:
    ...
    def display_frame_with_results(self, frame_bgr, detection_result):
        ...
        if detection_result.fire_detected:
            ...
            status_text = f"🔥 FIRE DETECTED"
            confidence_text = f"Confidence: {detection_result.confidence:.1%}"
            
            # Add emergency warning if needed
            if (detection_result.fire_characteristics and 
                detection_result.fire_characteristics.call_911_warranted):
                cv2.rectangle(display_frame, (0, 0), (width, 80), (0, 0, 255), -1)
                cv2.putText(display_frame, "🚨 EMERGENCY - CALL 911", 
    ...
    def print_detection_result(self, detection_result):
        ...
        print(f"   ⚠️  Uncontrolled: {detection_result.uncontrolled_fire_probability:.1%}")
    ...
    async def run_detection_loop(self):
        """Main detection loop."""
        print(f"🔥 Starting real-time fire detection...")
        print(f"⏱️  Analysis interval: {self.analysis_interval}s")
        print(f"📷 Camera feed window will open")
        print(f"🛑 Press 'q' to quit or Ctrl+C in terminal")
        print("=" * 60)
        ...
        try:
            while self.running:
                ...
                if current_time - last_analysis_time >= self.analysis_interval:
                    ...
                    display_frame = self.display_frame_with_results(frame_bgr, detection_result)
                else:
                    ...
                    height, width = display_frame.shape[:2]
                    cv2.rectangle(display_frame, (0, 0), (width, 30), (100, 100, 100), -1)
                    cv2.putText(display_frame, "🎥 Monitoring - Fire Detection Active", 
    ...
    def print_summary(self):
        ...
        if fire_detections:
            avg_confidence = sum(r.confidence for r in fire_detections) / len(fire_detections)
            print(f"📊 Average Fire Confidence: {avg_confidence:.1%}")
            
            emergency_detections = [r for r in fire_detections 
        ...
        if self.start_time:
            ...
            if self.analysis_count > 0:
                print(f"📈 Analysis Rate: {self.analysis_count/total_time:.2f} analyses/second")
...
async def main(use_quantization=False):
    ...
    # Create detector
    detector = WebcamFireDetector(camera_index=0, analysis_interval=1.0, use_quantization=use_quantization)
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Webcam Fire Detection with Gemma 3N E4B")
