...
def clean_board_dict(board: dict, cloud_name: str | None = None) -> dict:
    ...
    # Preserve existing jira_gui_url or build new one if cloud_name is provided
    if "jira_gui_url" in board:
        # Preserve existing URL
        cleaned["jira_gui_url"] = board["jira_gui_url"]
    elif cloud_name:
        # Build new URL if one doesn't exist
        project_key = None
        ...
        cleaned["jira_gui_url"] = build_board_url(cloud_name, str(board["id"]), project_key)

    return cleaned


def clean_sprint_dict(sprint: dict, cloud_name: str | None = None) -> dict:
    """
...
def normalize_sprint_state(state: list[str] | None) -> list[str] | None:
    """
...
def create_sprint_result_dict(
    ...
    """
    result = {
        "board": clean_board_dict(board, cloud_name),
        "sprints": sprints,
        "is_last": response.get("isLast"),
        "total": response.get("total"),
        "current_offset": response.get("startAt"),
    ...
    if cloud_name:
        ...
        backlog_url = build_backlog_url(cloud_name, str(board["id"]), project_key)
        if backlog_url:
...
def build_issue_url(cloud_name: str | None, project_key: str, issue_key: str) -> str | None:
    ...
    base_url = _build_base_jira_url(cloud_name)
...
def build_user_url(cloud_name: str | None, user_id: str) -> str | None:
    ...
    return f"{base_url}/jira/people/{user_id}"
...
def _build_base_jira_url(cloud_name: str | None) -> str | None:
