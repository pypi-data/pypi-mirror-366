...
class SeqLinear(nn.Module):
    ...
    def __init__(self, d_in, d_out, n_heads,
        ...
        self.w_bias = nn.Parameter(torch.empty(n_heads, device=device))
        
        self._reset_parameters()
    
    def _reset_parameters(self):
        nn.init.xavier_uniform_(self.in_proj.weight)
        nn.init.uniform_(self.w_bias, -0.1, 0.1)
    ...
    def bidirectional(self, x):
        ...
        decay_f = F.silu(w * self.w_bias).transpose(1, 2).unsqueeze(-2).repeat(1, 1, src_len, 1)
    ...
    def forward(self, x, mode="causal"):
        ...
        if mode == "causal":
            decay = F.silu(w * self.w_bias).transpose(1, 2).unsqueeze(-2).repeat(1, 1, src_len, 1)
            mask = torch.tril(torch.ones(src_len, src_len, dtype=torch.bool, device=self.device), diagonal=-1)
            ...
            decay = F.softmax(-F.silu(w * self.w_bias), dim=1)
