...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        log_dt_f = torch.cumsum(-F.leaky_relu(dt + self.dt_bias), dim=1)
        ...
        log_dt_b = torch.cumsum(-F.leaky_relu(dt.flip() + self.dt_bias), dim=1)
    ...
    def forward(self, x, mode="causal"):
        ...
        log_dt = torch.cumsum(-F.leaky_relu(dt + self.dt_bias), dim=1)
