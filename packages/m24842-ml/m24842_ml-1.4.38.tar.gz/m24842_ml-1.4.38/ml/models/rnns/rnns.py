...
class SeqLinear(nn.Module):
    ...
    def __init__(self, d_model, n_heads, bias=True,
        ...
        self.dt_base = nn.Parameter(torch.empty(n_heads, device=device))
        self.dt_mod_bias = nn.Parameter(torch.empty(n_heads, device=device))
    ...
    def _reset_parameters(self):
        ...
        nn.init.uniform_(self.dt_mod_bias, -0.5, 0.5)
        nn.init.uniform_(self.dt_base, -0.1, 0.1)
    ...
    def bidirectional(self, x):
        ...
        log_dt_f = torch.cumsum(-F.silu(self.dt_base) * F.softplus(dt_mod + self.dt_mod_bias), dim=1)
        log_dt_f = log_dt_f - log_dt_f.topk(1, dim=1).values
        dt_f = torch.exp(log_dt_f)
        ...
        log_dt_b = torch.cumsum(-F.silu(self.dt_base) * F.softplus(dt_mod.flip(1) + self.dt_mod_bias), dim=1)
        log_dt_b = log_dt_b - log_dt_b.topk(1, dim=1).values
        dt_b = torch.exp(log_dt_b)
        ...
        W_f = torch.cumsum(torch.einsum('bsh, bshd, bshe -> bshde', dt_f, u_f, v_f), dim=1)
    ...
    def forward(self, x, mode="causal"):
        ...
        log_dt = torch.cumsum(-F.silu(self.dt_base) * F.softplus(dt_mod + self.dt_mod_bias), dim=1)
        log_dt = log_dt - log_dt.topk(1, dim=1).values
        dt = torch.exp(log_dt)
        ...
        out = torch.einsum('bshd, bshde -> bshe', z, W) / norm
