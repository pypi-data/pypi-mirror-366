...
class SeqLinear(nn.Module):
    ...
    def __init__(self, d_model, n_heads, bias=True,
                 expand=1, W_0=False, d_conv=4,
                 head_type="mh",
                 device="cpu"):
        super().__init__()
        self.d_model = d_model
        self.n_heads = n_heads
        assert head_type in ["mh", "mu", "mv"]
        ...
        self.u_proj = nn.Linear(d_model, d_model, bias=bias, device=device)
        self.v_proj = nn.Linear(d_model, d_model * expand, bias=bias, device=device)
    ...
    def forward(self, x, W_0=None):
        ...
        dt = self.dt_proj(x).unsqueeze(-1)
        if self.head_type == "mh":
            y = rearrange(x, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
            u = rearrange(self.u_proj(x), 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
            v = rearrange(self.v_proj(x), 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        elif self.head_type == "mu":
            y = rearrange(x, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
            u = rearrange(self.u_proj(x), 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
            v = self.v_proj(x).unsqueeze(-2)
        elif self.head_type == "mv":
            y = x.unsqueeze(-2)
            u = self.u_proj(x).unsqueeze(-2)
            v = rearrange(self.v_proj(x), 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        ...
        out = torch.matmul(y.unsqueeze(-2), W).squeeze(-2) / norm
        if self.head_type in ["mh", "mv"]:
...
class SeqMLP(nn.Module):
    def __init__(self, emb_dim, input_dim, output_dim,
                 ...
                 expand=1, W_0=False, head_type="mh",
                 use_embedding=True, weight_tying=False,
                 bidirectional=True,
