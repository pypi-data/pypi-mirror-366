...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        decay_f = F.silu(w * self.A).transpose(1, 2).unsqueeze(-2).repeat(1, 1, src_len, 1)
    ...
    def forward(self, x, mode="causal"):
        ...
        if mode == "causal":
            decay = F.silu(w * self.A).transpose(1, 2).unsqueeze(-2).repeat(1, 1, src_len, 1)
            mask = torch.tril(torch.ones(src_len, src_len, dtype=torch.bool, device=self.device), diagonal=-1)
            ...
            decay = F.softmax(-F.silu(w * self.A), dim=1)
