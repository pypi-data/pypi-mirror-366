...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        log_dt_f = torch.cumsum(-F.silu(self.dt_bias) * F.sigmoid(dt), dim=1)
        ...
        log_dt_b = torch.cumsum(-F.silu(self.dt_bias) * F.sigmoid(dt.flip(1)), dim=1)
    ...
    def forward(self, x, mode="causal"):
        ...
        log_dt = torch.cumsum(-F.silu(self.dt_bias) * F.sigmoid(dt), dim=1)
