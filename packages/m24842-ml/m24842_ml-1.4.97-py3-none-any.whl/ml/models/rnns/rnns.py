...
class SeqLinear(nn.Module):
    ...
    def __init__(self, d_in, d_out, n_heads,
        ...
        # d_in_proj = d_in + d_out + self.n_heads
        # self.in_proj = nn.Linear(d_in, d_in_proj, bias=False, device=device)
        self.I_scale = nn.Parameter(torch.empty(d_in, device=device))
        self.I_bias = nn.Parameter(torch.empty(d_in, device=device))
        self.O_scale = nn.Parameter(torch.empty(d_out, device=device))
        self.O_bias = nn.Parameter(torch.empty(d_out, device=device))
        self.w_proj = nn.Linear(d_in, n_heads, bias=False, device=device)
    ...
    def _reset_parameters(self):
        # nn.init.xavier_uniform_(self.in_proj.weight)
        nn.init.uniform_(self.I_scale, -1.0, 1.0)
        nn.init.constant_(self.I_bias, 0.)
        nn.init.uniform_(self.O_scale, -1.0, 1.0)
    ...
    def bidirectional(self, x):
        ...
        w = self.w_proj(x)
        I = self.I_scale * x + self.I_bias
        O = self.O_scale * x + self.O_bias
...
class SeqMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            x_norm = layer.norm(x)
            mixer_out = layer.mixer(x_norm, mode=self.mode)
            x = x + layer.dropout(mixer_out)
        x = self.norm_f(x)
        x = self.out_proj(x)
        return x