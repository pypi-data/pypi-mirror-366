...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        log_dt_f = torch.cumsum(-F.silu(dt + self.dt_bias).flip(1), dim=1).flip(1)
        ...
        log_dt_b = torch.cumsum(-F.silu(dt + self.dt_bias), dim=1).flip(1)
    ...
    def forward(self, x, mode="causal"):
        ...
        log_dt = torch.cumsum(-torch.exp(dt + self.dt_bias).flip(1), dim=1).flip(1)
