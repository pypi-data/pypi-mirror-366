...
class SeqLinear(nn.Module):
    ...
    def __init__(self, d_model, n_heads, bias=True,
                 expand=1, W_bias=True, b_bias=True,
                 W_0=False, b_0=False, d_conv=4,
                 device="cpu"):
        super().__init__()
        self.d_model = d_model
        self.n_heads = n_heads
        ...
        self.b_proj = nn.Linear(d_model, d_model * expand, bias=b_bias, device=device) if bias else None
        self.b_0 = nn.Parameter(torch.empty(n_heads, self.d_head*expand, device=device)) if bias and b_0 else None
    ...
    def _reset_parameters(self):
        ...
        if self.b_proj is not None: nn.init.xavier_uniform_(self.b_proj.weight)
        nn.init.xavier_uniform_(self.out_proj.weight)
        ...
        if self.b_0 is not None: nn.init.constant_(self.b_0, 0.)
    
    def forward(self, x, W_0=None, b_0=None):
        bsz, src_len, d_model = x.size()
        x = self.conv(x.transpose(1, 2)).transpose(1, 2)[:, :src_len]
        ...
        b = rearrange(self.b_proj(x), 'b s (h d) -> (b h) s d', h=self.n_heads).contiguous() if self.bias else 0.0
        ...
        b = torch.cumsum(b, dim=1) if self.bias else 0.0
        if b_0 is not None: b = b + rearrange(b_0, 'b h e -> (b h) e').unsqueeze(1)
        elif self.b_0 is not None: b = b + self.b_0.unsqueeze(1).repeat(bsz, 1, 1)
        
        norm = torch.cumsum(dt, dim=1)
        
        out = (torch.matmul(x_h.unsqueeze(-2), W).squeeze(-2) + b) / norm
