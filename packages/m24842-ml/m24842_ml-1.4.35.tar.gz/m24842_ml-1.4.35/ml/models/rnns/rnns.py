...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        log_dt_f = log_dt_f - log_dt_f.max(dim=1, keepdim=True).values
        ...
        log_dt_b = log_dt_b - log_dt_b.max(dim=1, keepdim=True).values
    ...
    def forward(self, x, mode="causal"):
        ...
        log_dt = log_dt - log_dt.max(dim=1, keepdim=True).values
        dt = torch.exp(log_dt)
