...
class SeqLinear(nn.Module):
    ...
    def __init__(self, d_model, n_heads,
                 W_0=False, d_conv=4, beta_eps=1e-1,
    ...
    def bidirectional(self, x):
        ...
        beta = torch.exp(self.beta) + self.beta_eps
        
        log_dt_f = torch.cumsum(-self.beta_silu(dt + self.dt_bias, beta), dim=1)
        ...
        log_dt_b = torch.cumsum(-self.beta_silu(dt.flip() + self.dt_bias, beta), dim=1)
    ...
    def forward(self, x, mode="causal"):
        ...
        beta = torch.exp(self.beta) + self.beta_eps
        log_dt = torch.cumsum(-self.beta_silu(dt + self.dt_bias, beta), dim=1)
