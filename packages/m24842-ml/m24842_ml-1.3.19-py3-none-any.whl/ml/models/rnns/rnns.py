...
class SeqLinear(nn.Module):
    ...
    def __init__(self, d_model, n_heads, bias=True,
        ...
        self.W_0 = nn.Parameter(torch.empty(n_heads, self.zu_dim, self.d_head, device=device)) if W_0 else None
        self.z_proj = nn.Linear(d_model, self.zu_dim, bias=bias, device=device)
        self.u_proj = nn.Linear(d_model, self.zu_dim, bias=bias, device=device)
        self.v_proj = nn.Linear(d_model, self.v_dim, bias=bias, device=device)
    ...
    def _reset_parameters(self):
        ...
        if self.z_proj.bias is not None:
