...
class SeqLinear(nn.Module):
    ...
    def forward(self, x, mode="causal"):
        ...
        if mode == "causal":
            dt = F.silu(dt * self.dt_bias).transpose(1, 2).unsqueeze(-1).repeat(1, 1, 1, src_len)
            mask = torch.tril(torch.ones(src_len, src_len, dtype=torch.bool, device=self.device), diagonal=-1)
            dt = dt.masked_fill(~mask, 0.0)
            dt = -torch.cumsum(dt, dim=-2)
            mask = torch.tril(torch.ones(src_len, src_len, dtype=torch.bool, device=self.device), diagonal=0)
            dt = dt.masked_fill(~mask, -torch.inf)
            dt = torch.exp(dt).flip(-1, -2)
            dt = dt / torch.sum(dt, dim=-2, keepdim=True)
            out = torch.einsum('bthd, bhst, bshd, bshe -> bthe', C, dt, B, x)
            ...
            # dt = torch.exp(torch.cumsum(-F.silu(dt + self.dt_bias).flip(1), dim=1).flip(1))
            # norm = torch.cumsum(dt, dim=1).unsqueeze(-1)
            # W = torch.cumsum(torch.einsum('bsh, bshd, bshe -> bshde', dt, B, x), dim=1)
            # out = torch.einsum('bshd, bshde -> bshe', C, W) / norm
        elif mode == "noncausal":
            dt = torch.exp(-F.silu(dt * self.dt_bias))
            norm = torch.sum(dt, dim=1, keepdim=True).unsqueeze(-1)
            out = torch.einsum('bthd, bsh, bshd, bshe -> bthe', C, dt, B, x) / norm
