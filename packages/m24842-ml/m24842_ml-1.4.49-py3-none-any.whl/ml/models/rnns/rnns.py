...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        log_dt_f = torch.cumsum(-torch.exp(dt + self.dt_bias), dim=1)
        ...
        log_dt_b = torch.cumsum(-torch.exp(dt.flip() + self.dt_bias), dim=1)
    ...
    def forward(self, x, mode="causal"):
        ...
        log_dt = torch.cumsum(-torch.exp(dt + self.dt_bias), dim=1)
