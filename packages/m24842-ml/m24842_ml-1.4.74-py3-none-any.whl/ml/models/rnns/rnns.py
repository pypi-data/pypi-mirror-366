...
class SeqLinear(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        nn.init.uniform_(self.beta, -1.0, 1.0)
    ...
    def forward(self, x, mode="causal"):
        ...
        beta = torch.log(1 + F.softplus(self.beta))
        
        if mode == "causal":
            dt = (beta * F.silu(dt * self.dt_bias)).transpose(1, 2).unsqueeze(-2).repeat(1, 1, src_len, 1)
            mask = torch.tril(torch.ones(src_len, src_len, dtype=torch.bool, device=self.device), diagonal=-1)
            dt = dt.masked_fill(~mask, 0.0)
            dt = -dt.flip(-1).cumsum(-1).flip(-1)
            mask = torch.tril(torch.ones(src_len, src_len, dtype=torch.bool, device=self.device), diagonal=0)
            dt = dt.masked_fill(~mask, float('-inf'))
            dt = F.softmax(dt, dim=-1)
            # import matplotlib.pyplot as plt
            # plt.clf()
            # plt.plot(dt[0, 0, -1].detach().cpu().numpy())
            # plt.pause(0.01)
            out = torch.einsum('bthd, bhts, bshd, bshe -> bthe', C, dt, B, x)
        elif mode == "noncausal":
            dt = F.softmax(-beta *F.silu(dt * self.dt_bias), dim=1)
            out = torch.einsum('bthd, bsh, bshd, bshe -> bthe', C, dt, B, x)
