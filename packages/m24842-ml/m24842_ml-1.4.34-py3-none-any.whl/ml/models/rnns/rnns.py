...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        log_dt_f = torch.cumsum((dt * self.dt_bias), dim=1)
        log_dt_f = log_dt_f - torch.max(log_dt_f, dim=1, keepdim=True).values
        ...
        log_dt_b = torch.cumsum((dt * self.dt_bias), dim=1)
        log_dt_b = log_dt_b - torch.max(log_dt_b, dim=1, keepdim=True).values
    ...
    def forward(self, x, mode="causal"):
        ...
        log_dt = torch.cumsum((dt * self.dt_bias), dim=1)
        log_dt = log_dt - torch.max(log_dt, dim=1, keepdim=True).values
        dt = torch.exp(log_dt)
