...
class SeqLinear(nn.Module):
    ...
    def forward(self, x, mode="causal"):
        ...
        if mode == "causal":
            ...
            W = torch.cumsum(torch.einsum('bsh, bshd, bshe -> bshde', dt, C, B), dim=1)
            out = torch.einsum('bshd, bshde -> bshe', x, W) / norm
        elif mode == "noncausal":
            dt = F.softplus(dt + self.dt_bias)
            norm = torch.sum(dt, dim=1, keepdim=True).unsqueeze(-1)
            out = torch.einsum('bshd, bth, bthd, bthe -> bshe', x, dt, C, B) / norm
