...
class SeqLinear(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        nn.init.constant_(self.dt_bias, 0.)
    ...
    def bidirectional(self, x):
        ...
        dt_f = torch.exp(torch.cumsum(dt + self.dt_bias, dim=1))
        ...
        dt_b = torch.exp(torch.cumsum(dt.flip(1) + self.dt_bias, dim=1))
    ...
    def forward(self, x, mode="causal"):
        ...
        dt = torch.exp(torch.cumsum(dt + self.dt_bias, dim=1))
