...
class SeqLinear(nn.Module):
    ...
    def __init__(self, d_model, n_heads, bias=True,
        ...
        self.dt_power = nn.Parameter(torch.empty(n_heads, device=device))
    ...
    def _reset_parameters(self):
        ...
        nn.init.uniform_(self.dt_bias, -0.5, 0.5)
        nn.init.uniform_(self.dt_power, -1.0, 1.0)
    ...
    def forward(self, x, W_0=None):
        ...
        t_pow = torch.pow(torch.linspace(1, 2, src_len, device=self.device).unsqueeze(-1), self.dt_power)
        dt = F.softplus(dt + self.dt_bias) * t_pow
