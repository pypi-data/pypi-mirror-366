...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        log_dt_f = torch.cumsum(-F.silu(dt + self.dt_bias), dim=1)
        ...
        log_dt_b = torch.cumsum(-F.silu(dt.flip(1) + self.dt_bias), dim=1)
    ...
    def forward(self, x, mode="causal"):
        ...
        log_dt = torch.cumsum(-F.silu(dt + self.dt_bias), dim=1)
