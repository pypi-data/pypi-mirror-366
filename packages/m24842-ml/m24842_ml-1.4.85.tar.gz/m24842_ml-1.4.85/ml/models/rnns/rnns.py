...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        decay_f = F.silu(dt * self.A).transpose(1, 2).unsqueeze(-2).repeat(1, 1, src_len, 1)
        ...
        out_f = torch.einsum('bthd, bhts, bshd, bshe -> bthe', C_f, decay_f, B_f, x_f)
        out_b = torch.einsum('bthd, bhts, bshd, bshe -> bthe', C_b, decay_b, B_b, x_b)
    ...
    def forward(self, x, mode="causal"):
        ...
        if mode == "causal":
            decay = F.silu(dt * self.A).transpose(1, 2).unsqueeze(-2).repeat(1, 1, src_len, 1)
            mask = torch.tril(torch.ones(src_len, src_len, dtype=torch.bool, device=self.device), diagonal=-1)
            ...
            out = torch.einsum('bthd, bhts, bshd, bshe -> bthe', C, decay, B, x)
        elif mode == "noncausal":
            decay = F.softmax(-F.silu(dt * self.A), dim=1)
            out = torch.einsum('bthd, bsh, bshd, bshe -> bthe', C, decay, B, x)
        ...
        self.layers = nn.ModuleList([
            nn.ModuleDict(
                dict(
                    ...
                    feedforward=SwiGLU(emb_dim, emb_dim, emb_dim, bias=False, device=device),
