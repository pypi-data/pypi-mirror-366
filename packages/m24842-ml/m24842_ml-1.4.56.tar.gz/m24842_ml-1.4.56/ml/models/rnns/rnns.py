...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        dt_f = F.softplus(dt + self.dt_bias)
        ...
        dt_b = F.softplus(dt.flip(1) + self.dt_bias)
        ...
        W_f = torch.cumsum(torch.einsum('bsh, bshd, bshe -> bshde', dt_f, B_f, x_f), dim=1)
        W_b = torch.cumsum(torch.einsum('bsh, bshd, bshe -> bshde', dt_b, B_b, x_b), dim=1)
        
        norm_f = torch.cumsum(dt_f, dim=1).unsqueeze(-1)
        norm_b = torch.cumsum(dt_b, dim=1).unsqueeze(-1)
        
        out_f = torch.einsum('bshd, bshde -> bshe', C_f, W_f) / norm_f
        out_b = torch.einsum('bshd, bshde -> bshe', C_b, W_b) / norm_b
