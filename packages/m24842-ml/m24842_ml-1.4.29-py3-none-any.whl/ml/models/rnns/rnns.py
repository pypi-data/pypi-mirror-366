...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        norm_f = torch.cumsum(dt, dim=1).unsqueeze(-1)
        norm_b = torch.cumsum(dt.flip(1), dim=1).unsqueeze(-1)
        out_f = torch.cumsum(torch.einsum('bshd, bsh, bshd, bshe -> bshe', z_f, dt_f, u_f, v_f), dim=1) / norm_f
        out_b = torch.cumsum(torch.einsum('bshd, bth, bthd, bthe -> bshe', z_b, dt_b, u_b, v_b), dim=1) / norm_b
    ...
    def forward(self, x, mode="causal"):
        ...
        if mode == "causal":
            norm = torch.cumsum(dt, dim=1).unsqueeze(-1)
            out = torch.cumsum(torch.einsum('bshd, bsh, bshd, bshe -> bshe', z, dt, u, v), dim=1) / norm
        elif mode == "noncausal":
            norm = torch.sum(dt, dim=1, keepdim=True).unsqueeze(-1)
            out = torch.einsum('bshd, bth, bthd, bthe -> bshe', z, dt, u, v) / norm
