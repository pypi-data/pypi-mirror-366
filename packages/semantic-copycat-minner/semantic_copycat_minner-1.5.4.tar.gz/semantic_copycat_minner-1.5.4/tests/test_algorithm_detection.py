...
class TestAlgorithmDetection:
    ...
    def test_batch_analysis_of_samples(self):
        ...
        if os.path.exists(self.samples_dir):
            directory_results = self.analyzer.analyze_directory(self.samples_dir)
            
            assert isinstance(directory_results, dict)
            assert "file_results" in directory_results
            
            results = directory_results["file_results"]
