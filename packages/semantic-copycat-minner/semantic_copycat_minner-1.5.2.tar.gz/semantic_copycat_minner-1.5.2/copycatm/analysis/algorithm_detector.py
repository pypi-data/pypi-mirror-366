...
class AlgorithmDetector:
        ...
        pattern_checks = {
            ...
            'n_minus_1': lambda: (
                (re.search(r'\b(n|num)\s*[-]\s*1\b', func_text) is not None and
                 'fibonacci' in func_text.lower()) or
    ...
    def _extract_functions_from_ast(self, ast_tree: Any) -> List[Dict[str, Any]]:
        ...
        # Special handling for JavaScript when using fallback AST
        # Check both direct language attribute and if it's using fallback AST
        is_javascript = (hasattr(ast_tree, 'language') and ast_tree.language == 'javascript') or \
                       (hasattr(ast_tree, '__class__') and 'FallbackAST' in str(ast_tree.__class__))
        
        # Also check if code looks like JavaScript
        if hasattr(ast_tree, 'code') and not is_javascript:
        ...
        if hasattr(ast_tree, 'code') and is_javascript:
            code = ast_tree.code
            ...
            for pattern, func_type in js_patterns:
