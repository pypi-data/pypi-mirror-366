...
def translate_word_from_package(payload: dict):
    try:
        p: TranslationPackage = TranslationPackage.from_dict(payload)
    except Exception as e:
        logger.exception(f"Failed to parse TranslationPackage from payload: {e}")
    ...
    try:
        ...
        logger.exception(f"Failed to translate Word document from package: {e}")
