...
class RoleBasedApprovalTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.director_role = MockRole.objects.create(name="Director", parent=self.manager_role)
        
        # Create test users
        self.user1 = User.objects.create_user(username='manager1', email='manager1@test.com', role=self.manager_role)
        self.user2 = User.objects.create_user(username='manager2', email='manager2@test.com', role=self.manager_role)
        self.user3 = User.objects.create_user(username='manager3', email='manager3@test.com', role=self.manager_role)
        self.user4 = User.objects.create_user(username='director1', email='director1@test.com', role=self.director_role)
        
        # Create mock document
        self.document = MockRequestModel.objects.create(title="Test Document", description="Test Description")
...
class TestConsensusStrategy(RoleBasedApprovalTestCase):
    ...
    @patch('approval_workflow.utils.get_users_for_role')
    def test_consensus_strategy_creates_multiple_current_instances(self, mock_get_users):
    ...
    def test_consensus_strategy_requires_all_approvals(self):
        ...
        # First user approves
        result = advance_flow(inst1, 'approved', self.user1, comment='Approved by user1')
        ...
        result = advance_flow(inst2, 'approved', self.user2, comment='Approved by user2')
...
class TestRoundRobinStrategy(RoleBasedApprovalTestCase):
    ...
    def test_round_robin_strategy_creates_single_instance(self, mock_least_assignments, mock_get_users):
        ...
        mock_least_assignments.assert_called_once_with([self.user1, self.user2, self.user3])
