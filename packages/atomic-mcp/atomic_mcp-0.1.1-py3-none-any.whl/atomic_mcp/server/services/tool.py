...
class ToolService:
    ...
    def _get_type_hint(self, field_info: Dict[str, Any]) -> str:
        ...
        if field_type == "integer":
            ...
            # For complex objects (like nested Pydantic models), use Dict[str, Any]
            # This allows the MCP protocol to pass through the raw object data
            # which will be properly validated by model_validate()
            return "Dict[str, Any]"
        else:
    ...
    def _serialize_response(self, response: Any) -> Any:
        """Serialize a response to return to the client.

        This handles the actual response serialization based on content types.
        Supports both ToolResponse objects and raw dictionaries/other data.

        Args:
            response: The response to serialize (ToolResponse or dict/other)

        Returns:
            The serialized response
        """
        # Handle raw dict or other non-ToolResponse objects
