...
def ORM(cls):
    ...
    try:
        ...
        type_hints = cls.__dict__.get('__annotations__', None)
        print(f'type_hints={type_hints}')
        type_hints = getattr(cls, "__annotations__", None)
    ...
    class Wrapper(cls):  # 继承原类
        ...
        @classmethod
        def create(cls, db):
            print('Wrapper create', type_hints, keys, indexes)
            try:
    ...
    ths = Wrapper.__dict__.get('__annotations__', None)
    print(f'ths={ths}')
    ths = getattr(Wrapper, "__annotations__", None)
    print(f'ths={ths}')
    return Wrapper
