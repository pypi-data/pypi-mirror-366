...
async def test_requestx_async():
    ...
    if hasattr(requestx, 'Session'):
        ...
        session_async_methods = [attr for attr in session_attrs if 'async' in attr.lower()]
    ...
    if HAS_AIOHTTP:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                print(f"  aiohttp Results: {len(successful)} successful, {len(failed)} failed")
                for result in results:
                    if result['success']:
                        print(f"    Request {result['request_id']}: Status {result['status']}, Time: {result['time']:.1f}ms")
                    else:
    ...
    if HAS_HTTPX:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                print(f"  httpx Results: {len(successful)} successful, {len(failed)} failed")
                for result in results:
                    if result['success']:
                        print(f"    Request {result['request_id']}: Status {result['status']}, Time: {result['time']:.1f}ms")
                    else:
