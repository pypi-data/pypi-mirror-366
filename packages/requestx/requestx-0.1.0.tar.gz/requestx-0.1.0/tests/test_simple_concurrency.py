...
def test_simple_performance():
    ...
    def test_library(http_func, library_name, concurrency, total_requests):
        ...
        print(f"  Results: {rps:.1f} RPS, {avg_time:.1f}ms avg, {success_rate:.1f}% success, {errors} errors")
    ...
    for concurrency, total_requests in test_configs:
        print(f"\n{'='*60}")
        print(f"Testing Concurrency Level: {concurrency} ({total_requests} total requests)")
        print(f"{'='*60}")
        
        # Test RequestX
        if HAS_REQUESTX:
            try:
                result = test_library(requestx.get, 'RequestX', concurrency, total_requests)
        ...
        if HAS_REQUESTS:
            try:
                result = test_library(requests.get, 'Requests', concurrency, total_requests)
                results.append(result)
            except Exception as e:
    ...
    print(f"{'Library':<10} {'Concurrency':<12} {'RPS':<8} {'Avg Time':<10} {'Success':<8} {'Errors':<8}")
    print("-" * 60)
    
    for result in results:
        print(f"{result['library']:<10} {result['concurrency']:<12} {result['rps']:<8.1f} {result['avg_time']:<10.1f} {result['success_rate']:<8.1f}% {result['errors']:<8}")
    ...
    if len(requestx_results) >= 2:
        ...
        for result in requestx_results[1:]:
            expected_rps = base_result['rps'] * (result['concurrency'] / base_result['concurrency'])
            actual_rps = result['rps']
            efficiency = (actual_rps / expected_rps) * 100 if expected_rps > 0 else 0
            
            print(f"Concurrency {result['concurrency']:2d}: {efficiency:5.1f}% scaling efficiency ({actual_rps:.1f} vs {expected_rps:.1f} expected RPS)")
