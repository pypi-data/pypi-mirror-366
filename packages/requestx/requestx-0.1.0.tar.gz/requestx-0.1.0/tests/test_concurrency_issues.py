    ...
    if HAS_REQUESTX:
        ...
        for i in range(10):
            try:
                ...
                end_time = time.time()
                print(f"  Request {i+1}: Status {response.status_code}, Time: {(end_time-start_time)*1000:.1f}ms")
...
def test_thread_safety():
    ...
    def worker_function(http_func, thread_id, results_list, errors_list):
        ...
        for i in range(requests_per_thread):
            try:
                ...
                print(f"    Thread {thread_id}, Request {i+1}: Status {response.status_code}, Time: {result['response_time']:.1f}ms")
    ...
    if HAS_REQUESTX:
        ...
        print(f"  RequestX Results: {len(requestx_results)} successful, {len(requestx_errors)} errors")
        if requestx_errors:
            print("  RequestX Errors:")
            for error in requestx_errors:
                print(f"    Thread {error['thread_id']}: {error['error_type']} - {error['error']}")
    ...
    if HAS_REQUESTS:
        ...
        print(f"  Requests Results: {len(requests_results)} successful, {len(requests_errors)} errors")
        if requests_errors:
...
def test_threadpool_executor_issues():
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in as_completed(futures, timeout=30):
                try:
                    ...
                    if result['success']:
                        print(f"  Request {result['request_id']}: Status {result['status_code']}, Time: {result['response_time']:.1f}ms")
                    else:
            ...
            print(f"  RequestX ThreadPool Results: {len(successful)} successful, {len(failed)} failed")
            
            if failed:
                print("  RequestX ThreadPool Errors:")
                for result in failed:
                    print(f"    Request {result['request_id']}: {result.get('error_type', 'Unknown')} - {result['error']}")
    ...
    if HAS_REQUESTS:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            print(f"  Requests ThreadPool Results: {len(successful)} successful, {len(failed)} failed")
