...
def export_measurement(m1, db1, filepath=None, silent=False):
    ...
    while True:
        ...
        CMD = f"influx -username '{u}' -password '{p}' {ssl} -host {m} -database {db1} -execute 'SELECT * FROM {m1} ORDER BY time ASC LIMIT {limit} OFFSET {offset}' -format csv -precision rfc3339"
