        ...
        if script_response is None or script_response.status_code != 200:
            ...
            if script_response.status_code == 404 and executing_user is None:
                return (
                    ...
                    f" Please provide the 'executing_user' parameter to change ownership temporarily or set the API token user as dashboard owner."
                )
        ...
        try:
            ...
            if response is None or response.status_code != 200:
                ...
                if response is None or response.status_code != 200:
                    ...
                    self.logger.error(
                        f"Failed to add/update shares for dashboard '{dashboard_id}' via fallback. Error: {error_message}"
                    )
