...
_USAGE_STRING = """Usage:
  python -m scene_rir.rir [command] [path] [rec_path] [ref_path]
  or
  python3 -m scene_rir.rir [command] [path] [rec_path] [ref_path]

  SCENE-RIR is a command to create swept-sine excitation signals and to extract room
    impulse responses from their recorded responses.
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--version":
            from scene_rir import __version__

            print(f"SCENE-RIR version {__version__}")
