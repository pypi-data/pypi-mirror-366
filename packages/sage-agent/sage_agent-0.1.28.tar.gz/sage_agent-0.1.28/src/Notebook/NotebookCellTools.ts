...
export class NotebookCellTools {
  ...
  private handleSmartScroll(
    ...
    const scrollContainer = cellNode.closest('.jp-WindowedPanel-outer') as HTMLElement;

    if (!scrollContainer) {
      console.warn('Could not find scroll container with class jp-WindowedPanel-outer');
      return;
    }

    // Get the cell input area (the actual cell content, not output)
    const cellInputArea = cellNode.querySelector('.jp-InputArea') as HTMLElement;
    if (!cellInputArea) {
    ...
    if (includeOutput) {
      const outputArea = cellNode.querySelector('.jp-OutputArea') as HTMLElement;
   ...
   */
  private setupScrollListener(scrollContainer: HTMLElement, currentCell: HTMLElement): void {
   ...
   */
  private checkUserScrolledAway(scrollContainer: HTMLElement, currentCell: HTMLElement): void {
    ...
    const lastCellInput = lastCellNode.querySelector('.jp-InputArea') as HTMLElement;
