...
def test_merge_mismatch_exception():
    # existing target object
    target = {
        "protocol_identifier": "TestTrial",
        "participants": [
            {
                "samples": [
                    {
                        "cimac_id": "CCGK012AB.01",
                        "collection_event_name": "event_name_OLD",
                    }
                ],
                "cohort_name": "Arm_OLD",
                "participant_id": "part_id_1",
                "cimac_participant_id": "CCGK012",
            }
        ],
    }
    # patch object that has differing cohort_name and participant_id values
    patch = {
        "protocol_identifier": "TestTrial",
        "participants": [
            {
                "samples": [
                    {
                        "cimac_id": "CCGK012AB.01",
                        "collection_event_name": "event_name_NEW",
                    }
                ],
                "cohort_name": "Arm_NEW",
                "participant_id": "part_id_NEW",
                "cimac_participant_id": "CCGK012",
            }
        ],
    }

    # expect 'Detected mismatch of' error for differing participant_id's
    with pytest.raises(
        prism_merger.MergeCollisionException,
        match="Detected mismatch of participant_id='part_id_1' and participant_id='part_id_NEW' in cimac_participant_id",
    ):
        prism_merger.merge_clinical_trial_metadata(patch, target)

    # Match the participant_id so the only mismatches are cohort_name and collection_event_name,
    # which are exception properties that should be accepted.
    patch["participants"][0]["participant_id"] = "part_id_1"
    merged_data, errors = prism_merger.merge_clinical_trial_metadata(patch, target)
    assert not errors
    participant = merged_data["participants"][0]
    # ensure the new cohort_name value was accepted and merged, even without allowed_cohort_names set
    assert participant["cohort_name"] == "Arm_NEW"
    # ensure the new collection_event_name value was accepted and merged, without allowed_collection_event_names set
    assert participant["samples"][0]["collection_event_name"] == "event_name_NEW"
