...
async def main():
    ...
    response = await order_manager.place_market_order("MGC", side=0, size=1)  # Buy 1 contract
    ...
    print(f"Bracket order placed: Entry={bracket.entry_order_id}, Stop={bracket.stop_order_id}, Target={bracket.target_order_id}")
...
class AsyncOrderManager:
    ...
    Example Usage:
        ...
        # Place simple orders
        response = await order_manager.place_market_order("MGC", side=0, size=1)  # Buy 1 contract
        response = await order_manager.place_limit_order("MGC", 1, 1, 2050.0)  # Sell 1 contract at 2050.0
        ...
        async def on_order_filled(order_data):
            print(f"Order {order_data.get('id')} filled at {order_data.get('filledPrice')}")
    ...
    async def get_tracked_order_status(
        ...
        Example:
                ...
                if status == 2:  # Filled
                    ...
                    if 'fills' in order_data:
                        for fill in order_data['fills']:
                            print(f"Partial fill: {fill.get('volume')} @ {fill.get('price')}")
            ...
            new_order_data = await order_manager.get_tracked_order_status("54321", wait_for_cache=True)
            ```
    ...
    async def place_bracket_order(
        ...
        Example:
            ...
            if bracket.success:
                ...
                entry_status = await order_manager.is_order_filled(bracket.entry_order_id)
                if entry_status:
    ...
    async def get_order_statistics(self) -> dict[str, Any]:
        ...
        Example:
            ...
            print(f"Session statistics: {orders_placed} orders placed, "
            ...
            print(f"Realtime tracking: {realtime_status}, {cached_orders} orders in cache")
            ...
            for contract_id, orders in relationships["position_summary"].items():
                print(f"  {contract_id}: {orders['entry']} entry, "
    ...
    def add_callback(
        ...
        Example:
            ...
            async def on_trade(trade_data):
                print(f"Trade executed: {trade_data.get('volume')} @ {trade_data.get('price')}")
