...
```python
import asyncio
from project_x_py import AsyncProjectX, AsyncProjectXRealtimeClient, AsyncRealtimeDataManager
...
async def main():
    ...
    while True:
        ...
        if loop_count % 100 == 0:
            ...
            print(f"Memory stats: {stats['total_bars']} bars, {stats['ticks_processed']} ticks")
    ...
    async def add_callback(
        ...
        Example:
            ...
            async def on_new_bar(data):
                tf = data["timeframe"]
                bar = data["data"]
                print(f"New {tf} bar: O={bar['open']}, H={bar['high']}, L={bar['low']}, C={bar['close']}")
