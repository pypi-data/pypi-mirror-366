...
class PagedAttentionGenerator(BaseGenerator):
    ...
    def prepare_decode_input_metadata(
        ...
        """
        new_key_location = []  # shape = (batch, sp_len)
        new_value_location = []  # shape = (batch, sp_len)
        valid_key_indices = []  # shape = (batch*(bucket_size - sp_len))
        valid_value_indices = []
            ...
            for i in range(max_length - prompt_len):
                if is_prefill:
                    ...
                    (
                        ...
                        valid_key_indices,
                        valid_value_indices,
                    ) = self.prepare_decode_input_metadata(sp_len=sp_len)

                    # for sp_len>1: make attention mask 3d
                    specdec_attention_mask = (
                        torch.clone(attention_mask).unsqueeze(1).repeat(1, sp_len, 1)
                    )

                    logit_target_locations = None  # for decode, not needed
                    input_ids = input_ids.repeat(1, sp_len)
                    position_ids = position_ids.repeat(1, sp_len)
                    forward_kwargs = {
                        "input_ids": input_ids,
                        "attention_mask": specdec_attention_mask,
                        "causal_mask": None,
...
def move_kv_cache_block_in_place(
    seq_idx: int, new_location: torch.Tensor, existing_block_indices: List[List[int]], sp_len: int
) -> None:
    ...
    for single_batch_block_indices, new_index in zip(existing_block_indices, new_location):
        # specdec specific
        single_batch_block_indices[seq_idx : seq_idx + sp_len] = new_index.tolist()
