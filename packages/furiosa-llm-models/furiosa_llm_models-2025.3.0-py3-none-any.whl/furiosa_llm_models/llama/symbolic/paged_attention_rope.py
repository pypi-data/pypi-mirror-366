...
from .paged_attention_utils import reshape_and_cache
...
class LlamaRotaryEmbedding(torch.nn.Module):
    ...
    def forward(self, x, seq_len=None):
        ...
        return self.sincos_cached[:seq_len].to(dtype=x.dtype)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        # KV-caching and updating using Paged-Attention method.
        past_key_states, past_value_states = past_key_value
        # Our method requires the key and value to have
        # dimensions of (batch, seq_len, num_heads, head_dim).
        key_states = key_states.permute(0, 2, 1, 3)
        value_states = value_states.permute(0, 2, 1, 3)

        # Reshape the data to store the current key and value in the cache.
        reshape_and_cache(
            target=key_states,
            total=past_key_states,
            new_target_location=input_metadata.new_key_location,
        )
        reshape_and_cache(
            target=value_states,
            total=past_value_states,
            new_target_location=input_metadata.new_value_location,
        )

        key_states = past_key_states
        value_states = past_value_states
        past_key_value = (key_states, value_states) if use_cache else None

        # Retrieve the key and value from the cache according to 'valid key/value indices'
        batch_size, _, _, head_size = query_states.shape
        ...
        key_states_gather = key_states[valid_key_indices].reshape(
            batch_size, bucket_size, self.num_key_value_groups, head_size
        )
        value_states_gather = value_states[valid_value_indices].reshape(
            batch_size, bucket_size, self.num_key_value_groups, head_size
        )

        # Revert to the original dimension (batch, num_groups, seq_len, head_dim).
        key_states = key_states_gather.permute(0, 2, 1, 3)
        value_states = value_states_gather.permute(0, 2, 1, 3)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # This should apply only during decoding, and we assume 'past_key_values' is always not None.  # noqa: E501
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    # Is a docstring required for LlamaForCausalLM.forward method?
    def forward(
        self,
        input_metadata: InputMetadata,
        input_ids: torch.LongTensor = None,
