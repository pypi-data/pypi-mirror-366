...
from ...gptj.kv_caching import kv_page_update_with_unified_kv_indices
from ...symbolic.helper import CausalLMSymbolicTrace
from ...utils.mask_utils import build_3d_mask
...
def make_causal_mask(
    ...
    """
    Process 3D causal masks for attention computation.
...
class NewLlamaAttention(FastLlamaAttention):
    def forward(
        ...
        new_kv_location: torch.IntTensor,
        ...
        sp_len: int = 3,
        kv_update_method: Callable = kv_page_update_with_unified_kv_indices,
    ) -> Tuple[torch.Tensor]:
        ...
        key_states, value_states = kv_update_method(
            ...
            new_kv_location=new_kv_location,
            past_valid_kv_indices=past_valid_kv_indices,
        ...
        # Apply attention masks with clean branching logic
        if is_prefill and causal_mask is not None:
            # Prefill phase uses causal_mask
            if causal_mask.dim() == 2:
                # 2D mask: use build_3d_mask with q@k reference shape
                triu_for_causal = True  # Always True
                causal_mask_3d = build_3d_mask(
                    mask=causal_mask,
                    reference_shape=(bsz, q_len, k_len),  # q@k shape without head
                    triu_for_causal=triu_for_causal,
                )
                # Use make_causal_mask instead of manual masked_fill
                causal_mask_3d = make_causal_mask(
                    causal_mask_3d, torch.bfloat16, attn_weights.device
                )
            else:
                # 3D mask: process using existing make_causal_mask logic
                raise NotImplementedError("3D causal mask is not supported for 2d model")
            attn_weights = get_prefill_attn_weigths(attn_weights, causal_mask_3d)

        elif not is_prefill and attention_mask is not None:
            # Decode phase uses attention_mask
            if attention_mask.dim() == 2:
                # 2D mask: use build_3d_mask with q@k reference shape
                triu_for_causal = True  # Always True
                attention_mask_3d = build_3d_mask(
                    mask=attention_mask,
                    reference_shape=(bsz, q_len, k_len),  # q@k shape without head
                    triu_for_causal=triu_for_causal,
                )
                # Use make_causal_mask instead of manual masked_fill
                attention_mask_3d = make_causal_mask(
                    attention_mask_3d, torch.bfloat16, attn_weights.device
                )
            else:
                # 3D mask: process using existing make_causal_mask logic
                raise NotImplementedError("3D attention mask is not supported for 2d model")
            attn_weights = get_decode_attn_weights(attn_weights, attention_mask_3d)
...
class NewLlamaDecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        new_kv_location: Optional[torch.IntTensor] = None,
        past_valid_kv_indices: Optional[torch.IntTensor] = None,
...
class NewLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        # Masks are passed as-is to attention layers
        # 2D masks will be expanded to 3D using utility functions
        # 3D masks will be processed using existing logic
        # This approach provides clean branching and maintains backward compatibility
