...
class GPTJAttention(nn.Module):
    ...
    def forward(
        ...
        # key and value are both, [batch, seq_len, head, head_dim]
        # layer_past will be none for the prefill phase
        # sicne output past_key_value can be represented as aliased address by runtime
        if layer_past is not None:
            # TODO(DONGHUN)
            # currently kv cache are stored in [batch, head, seq_length, head_dim]
            # thus, is permuted here when saving new kv cache
            ...
            past_key[:, new_index, :, :] = key
            past_value[:, new_index, :, :] = value
            key = past_key
            value = past_value
        ...
        # before this point,
        # key should be batch, seq_length, num_head, head_dim
...
class GPTJForCausalLM(GPTJPreTrainedModel):
    ...
    def forward(
        ...
        ] = None,  # number of layers(key, value)
        attention_mask: Optional[torch.FloatTensor] = None,
        token_type_ids: Optional[torch.LongTensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        head_mask: Optional[torch.FloatTensor] = None,
