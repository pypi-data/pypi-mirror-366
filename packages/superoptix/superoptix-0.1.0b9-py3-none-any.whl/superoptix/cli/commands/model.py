...
def _parse_backend(backend: Optional[str], model_name: str) -> Optional[SuperOptiXBackendType]:
    ...
    if model_name.startswith("mlx-community/") or model_name.startswith("mlx/"):
        ...
        # Try to auto-detect
        manager = SuperOptiXModelManager()
        return manager.detect_model_backend(model_name)
...
def install_model(
    ...
    console.print(f"🚀 Installing model [bold]{parsed_model_name}[/bold] with {backend_type.value} backend...")
    
    # Handle different backends
    if backend_type == SuperOptiXBackendType.HUGGINGFACE:
        success = _install_huggingface_model(parsed_model_name, backend_instance, start_time)
    else:
        # For other backends (MLX, Ollama, LMStudio), use the sync method
        success = backend_instance.install_model(parsed_model_name)
    
    if success:
        end_time = time.time()
        duration = end_time - start_time
        
        # Get model info for final summary
        try:
            # Calculate actual disk size and location
            model_dir = backend_instance.cache_dir / model_name.replace("/", "_")
            if not model_dir.exists():
                # Try the models-- format
                model_dir = backend_instance.cache_dir / f"models--{model_name.replace('/', '--')}"
            
            if model_dir.exists():
                # Calculate actual disk size (not symlink target size)
                total_size = 0
                ...
                console.print("="*60)
                console.print(f"📋 Model: {parsed_model_name}")
                console.print(f"🔧 Backend: {backend_type.value}")
                console.print(f"📁 Location: {model_dir}")
                console.print(f"📊 Disk size: {size_gb:.1f} GB")
                console.print(f"⏱️ Time taken: {duration:.1f} seconds")
                console.print("="*60)
            else:
                console.print("\n" + "="*60)
                console.print("🎉 [bold green]Installation Completed Successfully![/bold green]")
            ...
            console.print(f"✅ Installation completed in {duration:.1f} seconds")
    else:
        console.print("❌ Installation failed")
        raise typer.Exit(1)


def _install_huggingface_model(model_name: str, backend_instance, start_time: float) -> bool:
    """Install a HuggingFace model with detailed progress display."""
    import time
    import os
    from pathlib import Path
    ...
    try:
        ...
        console.print(f"🔍 Getting model information...")
        try:
            info = model_info(model_name)
            total_size = sum(sibling.size for sibling in info.siblings if sibling.size)
            size_gb = total_size / (1024**3)
            console.print(f"📊 Model size: {size_gb:.1f} GB")
            
            if size_gb > 5:
                console.print(f"⚠️ Large model detected - this may take a while")
        except Exception as e:
            console.print(f"⚠️ Could not get model size: {e}")
            size_gb = None
        
        # Step 2: Download with snapshot_download
        console.print(f"📥 Downloading model files...")
        ...
        # Step 3: Load with transformers (this will show detailed progress)
        console.print("🔄 Loading model to verify installation...")
        console.print("💡 This will show detailed file-by-file progress:")
        
        # Run transformers in a separate process to capture detailed progress
        import subprocess
        import sys
        import tempfile
        
        # Create a script that loads the model and captures progress
        load_script = f"""
import sys
import os
from transformers import AutoTokenizer, AutoModelForCausalLM
...
os.environ['HF_HUB_DISABLE_TQDM'] = '0'
...
"""
        ...
        try:
            ...
            # Display the captured output (which includes detailed progress)
            if result.stdout:
        ...
        if model_dir.exists():
            ...
            console.print(f"📊 Final size on disk: {size_mb:.1f} MB")
        ...
        if "No GPU or XPU found" in error_msg or "GPU" in error_msg or "XPU" in error_msg:
            console.print(f"❌ Hardware compatibility error: {error_msg}")
            console.print("💡 This model requires specific hardware (GPU/XPU) that your system doesn't have")
            console.print("💡 Try a different model that's compatible with your hardware")
            return False
        elif "out of memory" in error_msg.lower() or "memory" in error_msg.lower():
            console.print(f"❌ Memory error: {error_msg}")
            console.print("💡 This model is too large for your available memory")
            console.print("💡 Try a smaller model or increase your system memory")
            return False
        else:
            console.print(f"❌ Failed to install model: {error_msg}")
            return False
...
def remove_model(
    ...
    """��️ Remove SuperOptiX models with intelligent backend detection."""
    ...
    with suppress_warnings():
        ...
        if not backend_type:
            ...
            if backend_type:
                ...
                console.print(f"❌ Model {model_name} not found in any backend")
                console.print("💡 Use [bold]super model list[/bold] to see available models")
                raise typer.Exit(1)
                ...
                try:
                    ...
                    if is_available:
                        ...
                        if model_info:
                            ...
                            success = manager.uninstall_model(model_name, bt)
                            if success:
        ...
        success = manager.uninstall_model(model_name, backend_type)
            
        if success:
            console.print(f"🎉 Successfully removed {model_name} from {backend_type.value}")
            console.print("🔄 Run [bold]super model refresh[/bold] to update the cache")
        else:
...
def run_model(
    ...
    if not model_exists:
        console.print(f"📥 Model {model_name} not found in {backend}")
