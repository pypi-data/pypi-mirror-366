...
def install_model(
    ...
    console.print(
        f"🚀 SuperOptiX Model Intelligence - Installing [bold cyan]{model_name}[/bold cyan]"
    )
    ...
    if success:
        ...
        console.print(
            "  • Run [bold]super model backends[/bold] to check backend status"
        )
        console.print(
            "  • Run [bold]super model guide[/bold] for installation instructions"
        )
...
def remove_model(
    ...
    if all_backends:
        ...
        if removed_from:
            console.print(f"✅ Removed {model_name} from: {', '.join(removed_from)}")
        else:
            console.print(f"❌ Model {model_name} not found in any backend")
        return

    # Single backend removal - direct execution
    ...
    if success:
        ...
        console.print("💡 The model may not exist or may be in use")
...
def run_model(
    ...
    """🚀 Run a prompt against a SuperOptiX model (supports Ollama, MLX, HuggingFace)."""
    # Parse backend type
    backend_type = None
    if backend:
        try:
            backend_type = SuperOptiXBackendType(backend.lower())
            # Check if backend is supported for run command
            if backend_type == SuperOptiXBackendType.LMSTUDIO:
                console.print("❌ LM Studio is not supported for direct model execution")
                console.print("💡 LM Studio is designed for server mode and interactive chat")
                console.print("💡 Use 'lms chat' for interactive mode or 'lms server start' for API mode")
                raise typer.Exit(1)
        except ValueError:
            console.print(f"❌ Invalid backend: {backend}")
            console.print("Valid backends: ollama, mlx, huggingface")
            raise typer.Exit(1)
    ...
    if not backend_type:
        ...
        # Filter out LM Studio for run command
        if backend_type == SuperOptiXBackendType.LMSTUDIO:
            console.print("❌ LM Studio is not supported for direct model execution")
            console.print("💡 LM Studio is designed for server mode and interactive chat")
            console.print("💡 Use 'lms chat' for interactive mode or 'lms server start' for API mode")
        ...
        if backend_type:
            ...
            console.print(f"❌ Model {model_name} not found in any supported backend")
            console.print("💡 Supported backends for run command: ollama, mlx, huggingface")
            console.print("💡 Use [bold]super model list[/bold] to see available models")
            raise typer.Exit(1)

    # Check if model exists in the specified backend, if not, try to install it
    if backend_type:
        backend_instance = manager.backends[backend_type]
        ...
        # If model doesn't exist, try to install it
        if not model_exists:
            console.print(f"📥 Model {model_name} not found in {backend_type.value}")
            console.print(f"🔄 Attempting to install {model_name} in {backend_type.value}...")
            
            try:
                success = manager.install_model(model_name, backend_type)
                if success:
                    console.print(f"✅ Successfully installed {model_name} in {backend_type.value}")
                else:
                    console.print(f"❌ Failed to install {model_name} in {backend_type.value}")
                    console.print("💡 Please check the model name and try again")
                    raise typer.Exit(1)
            except Exception as e:
                console.print(f"❌ Error installing {model_name}: {e}")
                console.print("💡 Please check the model name and try again")
                raise typer.Exit(1)
    ...
    try:
        if interactive:
            console.print(f"🤖 Interactive mode with {model_name} ({backend_type.value})")
            console.print("💡 Type 'quit' or 'exit' to end the session")
            console.print("💡 Type 'clear' to clear the conversation")
            console.print("-" * 50)
            ...
            while True:
                try:
                    ...
                    # Create full prompt from conversation
                    full_prompt = "\n".join([msg["content"] for msg in conversation])
                    
                    # Run the model
                    result = manager.test_model(model_name, full_prompt, backend_type)
            ...
            # Single prompt mode
            console.print(f"🚀 Running {model_name} ({backend_type.value})...")
            result = manager.test_model(model_name, prompt, backend_type)
        ...
        console.print(f"❌ Failed to run model: {e}")
        raise typer.Exit(1)
