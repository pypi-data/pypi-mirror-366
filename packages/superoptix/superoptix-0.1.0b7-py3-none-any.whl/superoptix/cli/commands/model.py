...
def install_model(
    ...
    if "/" in model_name and not backend:
        backend_str, parsed_model_name = model_name.split("/", 1)
        backend = backend_str
    ...
    with suppress_warnings():
        ...
        if not backend_instance or not backend_instance.is_available():
            console.print(f"\n❌ [bold red]{backend_type.value} backend is not available![/bold red]")
        ...
        # Install the model using the manager
        success = manager.install_model(parsed_model_name, backend_type=backend_type)
...
def convert_model(
    ...
    try:
        ...
        console.print(f"  • Test: super model run {output_path} \"How are you\" --backend mlx")
        console.print(f"  • Info: super model info {output_path}")
