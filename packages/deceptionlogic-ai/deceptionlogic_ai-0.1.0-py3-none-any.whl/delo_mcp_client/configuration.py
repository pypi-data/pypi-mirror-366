...
def config(
    ...
    print("To change a value, type the new value and press enter. To keep the current value, just press enter.")
...
def load_config(config_file=os.path.expanduser("~/.config/deceptionlogic/mcp.conf")):
    ...
    if os.path.exists(config_file):
        ...
        config['Settings'] = {
            'USER COLOR': "ansiwhite",
            "ASSISTANT COLOR": "ansiblue"
        }
...
def validate_config(config):
    ...
    required_keys = {
        'ENV_VARS': ['DECEPTION LOGIC KEY ID', 'DECEPTION LOGIC SECRET KEY', 'OPENAI API KEY', 'GEMINI API KEY', 'ANTHROPIC API KEY'],
    ...
    for section, keys in required_keys.items():
        for key in keys:
            if section == "Settings" and key not in config[section]:
                ...
                config[section][key] = "" if key != "URL" else "https://deceptionlogic.ai/sse"
