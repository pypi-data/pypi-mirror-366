...
class McpToolAgent:
    ...
    async def _fetch_mcp_tool(self, task: str, context: str) ->Dict[str, Any]:
        ...
        mcp_tool_schemas = await self._get_tool_schema()
    ...
    def get_tool_schema(self) -> str:
        return asyncio.run(self._get_tool_schema());

    async def _get_tool_schema(self) -> str:
        mcp_tools = await self.mcp_client.get_tools()
        logging.info("get_tool_schema: has tools %s", [tool.name for tool in mcp_tools])
        ...
        for tool in mcp_tools:
            ...
            logging.info("mcp tool schema: %s", tool_schema)
    ...
    def call(self, task: str, context: str)-> Dict[str, Any]:
        return asyncio.run(self._call(task, context))

    async def _call(self, task: str, context: str) -> Dict[str, Any]:
        result = {}
        try:
