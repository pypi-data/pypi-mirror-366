"""
DataFlow Auto-Migration System - PostgreSQL Edition

Advanced PostgreSQL migration system that automatically detects schema changes,
provides visual confirmation, and supports rollback capabilities.

Features:
- Automatic PostgreSQL schema comparison and diff generation
- Visual confirmation before applying changes
- Rollback and versioning support
- PostgreSQL-optimized DDL generation
- Zero SQL knowledge required for users
- JSONB support and PostgreSQL-specific features

Alpha Release: PostgreSQL-only implementation.
"""
...
class PostgreSQLSchemaInspector:
    """PostgreSQL-optimized schema inspector for DataFlow alpha release."""
...
class PostgreSQLMigrationGenerator:
    """PostgreSQL-optimized migration generator for DataFlow alpha release."""
...
class AutoMigrationSystem:
    """
    PostgreSQL-optimized auto-migration system for DataFlow alpha release.

    Orchestrates PostgreSQL schema comparison, migration generation,
    and execution with visual confirmation and advanced features.
    """
    ...
    def __init__(
        ...
        self.connection = connection
        # Force PostgreSQL for alpha release
        if dialect.lower() != "postgresql" and dialect.lower() != "sqlite":
            logger.warning(
                f"DataFlow alpha only supports PostgreSQL. Ignoring dialect '{dialect}'"
            )
        self.dialect = "postgresql"
        self.migrations_dir = Path(migrations_dir)
        self.migrations_dir.mkdir(exist_ok=True)

        # Use PostgreSQL-optimized components
        self.inspector = PostgreSQLSchemaInspector(connection)
    ...
    async def _ensure_migration_table(self):
        """Ensure PostgreSQL migration tracking table exists with optimizations."""
        create_sql = """
        CREATE TABLE IF NOT EXISTS dataflow_migrations (
            ...
            -- PostgreSQL constraints and checks
        ...
        try:
            ...
            logger.error(f"Failed to create PostgreSQL migration table: {e}")
            raise
    ...
    async def _record_migration(self, migration: Migration):
        ...
        # PostgreSQL-optimized INSERT with JSONB
        insert_sql = """
        ...
        try:
            ...
            logger.error(f"Failed to record PostgreSQL migration: {e}")
            raise

    async def _update_migration_status(self, version: str, status: MigrationStatus):
        """Update migration status in PostgreSQL database."""
        update_sql = """
        UPDATE dataflow_migrations
        SET status = %s,
            applied_at = CASE WHEN %s = 'applied' THEN CURRENT_TIMESTAMP ELSE applied_at END
        WHERE version = %s
        """

        try:
            async with self.connection.cursor() as cursor:
                await cursor.execute(update_sql, (status.value, status.value, version))
        except Exception as e:
            logger.error(f"Failed to update PostgreSQL migration status: {e}")
            raise
