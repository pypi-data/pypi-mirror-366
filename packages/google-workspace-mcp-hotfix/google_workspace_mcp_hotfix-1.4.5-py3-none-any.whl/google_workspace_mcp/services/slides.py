    ...
    def create_multiple_slides_with_elements(
        ...
        try:
            ...
            for slide_index, slide_data in enumerate(slides_data):
                ...
                # Step 2: Add background, deferring private images to Apps Script
                use_regular_background = True
                if background_image_url and self._is_private_drive_url(
                    ...
                    if file_id:
                        logger.info(
                            f"Detected private background image {file_id} for slide {slide_id}, deferring to Apps Script."
                        )
                        apps_script_backgrounds.append(
                            {"slide_id": slide_id, "drive_file_id": file_id}
                        )
                        use_regular_background = (
                            False  # Skip adding it to the main batch request
                        )

                if use_regular_background and (
