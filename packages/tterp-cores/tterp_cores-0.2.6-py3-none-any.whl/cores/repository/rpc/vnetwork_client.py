...
class VNetworkClient(ClientBase):
    ...
    async def request(
        ...
        try:
            ...
            logger.error(
                f"HTTP Exception - {link}, method: {method}, "
                f"param: {params}, body: {body} - {err_message} - {exc}"
            )
        except httpx.NetworkError as exc:
            logger.error(
                f"NetworkError for {link}, method: {method}, "
                f"param: {params}, body: {body} - {err_message} - {exc}"
            )
        except httpx.TimeoutException as exc:
            logger.error(
                f"TimeoutException for {link}, method: {method}, "
                f"param: {params}, body: {body} - {err_message} - {exc}"
            )
        except httpx.ProtocolError as exc:
            logger.error(
                f"ProtocolError for {link}, method: {method}, "
                f"param: {params}, body: {body} - {err_message} - {exc}"
            )
        except httpx.DecodingError as exc:
            logger.error(
                f"DecodingError for {link}, method: {method}, "
                f"param: {params}, body: {body} - {err_message} - {exc}"
            )
        except httpx.TooManyRedirects as exc:
            logger.error(
                f"TooManyRedirects for {link}, method: {method}, "
                f"param: {params}, body: {body} - {err_message} - {exc}"
            )
        except httpx.StreamError as exc:
            logger.error(
                f"StreamError for {link}, method: {method}, "
                f"param: {params}, body: {body} - {err_message} - {exc}"
            )
        except Exception as exc:
            logger.error(
                f"Err for {link}, method: {method}, "
                f"param: {params}, body: {body} - {err_message} - {exc}"
            )
        finally:
            await client.aclose()
