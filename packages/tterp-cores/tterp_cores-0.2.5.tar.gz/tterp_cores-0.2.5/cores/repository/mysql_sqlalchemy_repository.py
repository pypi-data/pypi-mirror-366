...
class BaseSQLAlchemyRepository(Generic[Entity]):
    ...
    async def get(
        ...
        *,
        ...
        if columns:
            query = query = query.options(load_only(*columns))
    ...
    async def update(self, id: str | int, data_in: Schema | dict[str, Any]) -> bool:
        """Cập nhật nhiều trường của một bản ghi dựa trên ID
        (dùng `update().values()`)."""
    ...
    async def soft_update(
        ...
        LƯU Ý: Đây là logic nghiệp vụ đặc thù, không phải là một hàm CRUD
        thông thường.
        """
    ...
    async def bulk_update(
        ...
        Nếu chỉ truyền ids và updates là dict các trường chung,
        sẽ update tất cả ids với cùng giá trị.
        Nếu updates là dict với key là id, value là dict các trường,
        sẽ update từng bản ghi với giá trị riêng.
        Sử dụng câu lệnh `CASE` của SQL để hiệu năng cao.

        Args:
            updates (Dict): Dict với key là ID của bản ghi và value là một dict
            chứa các thay đổi, hoặc dict các trường chung cho tất cả ids.
