...
def main():
    parser = argparse.ArgumentParser(
        description="Unplug is a TUI for playing back recorded input events"
    )
    parser.add_argument(
        "directory",
        nargs="?",
        type=str,
        help="Directory containing .yml files produced by libinput record",
    )
    ...
    recordings_dir = Path(args.directory or "src/unplug/data")
    if not recordings_dir.exists():
        recordings_dir = Path(".")

    unplug = Unplug.load_from_directory(recordings_dir)
    if not unplug.devices and not args.directory:
        from importlib import resources

        try:
            datapath = resources.files("unplug.data")
            with resources.as_file(datapath) as path:
                unplug = Unplug.load_from_directory(path)
        except ModuleNotFoundError:
            # not installed
            pass

    if not unplug.devices:
        logger.error("No recordings found in this directory, exiting")
        sys.exit(1)
