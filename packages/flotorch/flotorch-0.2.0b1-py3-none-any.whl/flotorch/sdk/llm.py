...
class FlotorchLLM:
    ...
    def invoke(self, messages: List[Dict[str, str]], tools: Optional[List[Dict]] = None, extra_body: Optional[Dict] = None, **kwargs) -> LLMResponse:
        try:
            response = invoke(messages, self.model_id, self.api_key, self.base_url, tools=tools, extra_body=extra_body, **kwargs)
            return parse_llm_response(response)
        except Exception as e:
            log_error("FlotorchLLM.invoke", e)
            raise

    async def ainvoke(self, messages: List[Dict[str, str]], tools: Optional[List[Dict]] = None, extra_body: Optional[Dict] = None, **kwargs) -> LLMResponse:
        """
        ...
        try:
            ...
            response = await async_invoke(messages, self.model_id, self.api_key, self.base_url, tools=tools, extra_body=extra_body, **kwargs)
            return parse_llm_response(response)
        except Exception as e:
