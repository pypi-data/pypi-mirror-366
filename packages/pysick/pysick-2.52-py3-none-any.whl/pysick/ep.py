...
class epAdvancedCube:
    def __init__(self, x, y, z, size, color=(255, 255, 255), rotation=(0, 0, 0)):
        self.position = [x, y, z]
        self.size = size
        self.color = color
    ...
    def draw(self):
        epAdvancedDraw.cube(self)


class epAdvancedDraw:
    @staticmethod
    def cube(cube):
        x, y, z = cube.position
        s = cube.size / 2
        rx, ry, rz = cube.rotation
        color = pysick._color_to_hex(cube.color)
        canvas = pysick.ingine._get_canvas()
        ...
        def project(px, py, pz):
            fov = 500
            scale = fov / (pz + fov) if pz + fov != 0 else 1
            px = px * scale + pysick.ingine.width / 2
            py = py * scale + pysick.ingine.height / 2
            return px, py

        # Transform all points
        transformed = [project(*rotate(*p)) for p in points]
        ...
        # Draw lines
        for i1, i2 in edges:
            x1, y1 = transformed[i1]
            x2, y2 = transformed[i2]
