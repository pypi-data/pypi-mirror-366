...
def timestamp_duration(start: date | datetime | float,
                       ...
                       show_micros: bool = False) -> str | None:
    """
    Calculate and return the interval between *start* and *finish*.

    It is assumed that *start* and *finish* refer to the same timezone, and that the latter comes after the former.
    Elapsed hours, minutes, and seconds are returned. Specify *show_millis* to show milliseconds, or *show_micros*
    to show milliseconds and microseconds.

    :param start: beginning of the interval, given as *date*, *datetime*, or Unix *timestamp*
    :param finish: end of the interval, given as *date*, *datetime*, or Unix *timestamp*
    :param show_millis: show elapsed milliseconds
    :param show_micros: show elapsed milliseconds and microseconds
    ...
    interval: tuple[int, int, int, int, int] = timestamp_interval(start=start,
                                                                  finish=finish)
    result: str = f"{interval[0]}h{interval[1]:02d}m{interval[2]:02d}s"
    if show_millis or show_micros:
        result += f"{interval[3]:03d}ml"
        if show_micros:
            result += f"{interval[4]:06d}mc"
