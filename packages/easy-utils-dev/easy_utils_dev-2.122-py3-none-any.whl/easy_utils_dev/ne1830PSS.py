...
class PSS1830 :
    ...
    def createJhostInstance(self , neip , port) :
        if self.isjumpserver :
            ...
            dest_addr = ( neip , port )
            jump_channel = self.jump_channel = jump_transport.open_channel("direct-tcpip", dest_addr, src_addr, timeout=5)
            self.prevJhostMemLocation=self.jumpserver
            self.logger.debug(f'creating new transport for jhost completed')
            return jump_channel
        return None

    def _connect(self , mode='cli' , neip=None , port=22 , rootpw='QUx1MTIj') :
        ...
        try :
            ...
            if mode == 'direct_cli' :
                self.logger.debug(f"Force switch port from {port} to 22 | mode={mode}")
                port = 22 
    ...
    def connect(self ,
        ...
        for i in range(maxAttemps) :
            ...
            self.client , error , status = self._connect(mode  , neip , port , rootpw)
            if error == "AUTH_FAIL" :
