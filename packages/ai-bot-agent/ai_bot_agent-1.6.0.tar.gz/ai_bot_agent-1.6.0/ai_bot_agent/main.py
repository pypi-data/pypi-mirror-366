    ...
    def search_web(self, query: str) -> str:
        ...
        try:
            ...
            if data.get('Abstract'):
                ...
                return f"🌐 No direct answer found for '{query}'. Try rephrasing your question or use 'ai chat' for general AI assistance."
                
        except Exception as e:
            return f"🌐 Web search temporarily unavailable. Error: {e}\n\nTry using 'ai chat' for general AI assistance instead."
...
@app.command()
def edit(
    filename: str = typer.Argument(..., help="Name of the file to edit"),
    instruction: str = typer.Argument(..., help="Instruction for editing the file")
):
    """Edit an existing file with AI-powered modifications."""
    display_banner()
    ...
    console.print(f"[bold blue]✏️  Editing file: {filename}[/bold blue]")
    console.print(f"[blue]Instruction: {instruction}[/blue]\n")
    ...
    try:
        ...
        if file_extension in ['.py', '.js', '.ts', '.java', '.cpp', '.c', '.go', '.rs', '.php', '.rb']:
            prompt = f"Edit this {file_extension} code file according to the instruction: {instruction}\n\nCurrent code:\n{current_content}\n\nProvide only the modified code, no explanations."
        else:
            prompt = f"Edit this {file_extension} file according to the instruction: {instruction}\n\nCurrent content:\n{current_content}\n\nProvide only the modified content, no explanations."
        
        # Get AI response
        new_content = bot.chat(prompt)
        ...
        console.print(f"[green]✅ File edited: {filename}[/green]")
        console.print(f"[blue]Backup created: {backup_path}[/blue]")
        console.print(f"[blue]Size change: {len(current_content)} → {len(new_content)} characters[/blue]")
        
        # Show diff preview
        console.print(f"\n[bold blue]📄 Changes Preview:[/bold blue]")
        console.print("=" * 50)
        console.print(new_content[:500] + ("..." if len(new_content) > 500 else ""))
        console.print("=" * 50)
...
@app.command()
def refactor(
    filename: str = typer.Argument(..., help="Name of the file to refactor")
):
    ...
    console.print(f"[bold blue]🔧 Refactoring file: {filename}[/bold blue]\n")
    ...
    try:
        ...
        if file_extension not in ['.py', '.js', '.ts', '.java', '.cpp', '.c', '.go', '.rs', '.php', '.rb']:
            console.print(f"[yellow]⚠️  Refactoring is best for code files. File type: {file_extension}[/yellow]")
        
        # Generate refactor prompt
        prompt = f"Refactor this {file_extension} code to improve structure, readability, and maintainability. Follow best practices and coding standards.\n\nCurrent code:\n{current_content}\n\nProvide only the refactored code, no explanations."
        
        # Get AI response
        refactored_content = bot.chat(prompt)
        ...
        console.print(f"[green]✅ File refactored: {filename}[/green]")
        console.print(f"[blue]Original saved: {backup_path}[/blue]")
        console.print(f"[blue]Lines: {len(current_content.split())} → {len(refactored_content.split())}[/blue]")
        
        # Show preview
        console.print(f"\n[bold blue]📄 Refactored Preview:[/bold blue]")
        console.print("=" * 50)
        console.print(refactored_content[:500] + ("..." if len(refactored_content) > 500 else ""))
        console.print("=" * 50)
        
    except Exception as e:
        console.print(f"[red]❌ Error refactoring file: {e}[/red]")

@app.command()
def test(
    filename: str = typer.Argument(..., help="Name of the file to generate tests for")
):
    ...
    console.print(f"[bold blue]🧪 Generating tests for: {filename}[/bold blue]\n")
    ...
    try:
        ...
        if file_extension == '.py':
            ...
            test_filename = filename.replace(file_extension, f'.test{file_extension}')
        else:
            test_filename = f"test_{filename}"
        
        # Generate test prompt
        prompt = f"Generate comprehensive unit tests for this {file_extension} code file. Include tests for all functions, edge cases, and error conditions.\n\nCode:\n{current_content}\n\nProvide only the test code, no explanations."
        
        # Get AI response
        test_content = bot.chat(prompt)
        
        # Write test file
        with open(test_filename, 'w', encoding='utf-8') as f:
            f.write(test_content)
        
        console.print(f"[green]✅ Tests generated: {test_filename}[/green]")
        console.print(f"[blue]Test file size: {len(test_content)} characters[/blue]")
        
        # Show preview
        console.print(f"\n[bold blue]📄 Test Preview:[/bold blue]")
        console.print("=" * 50)
        console.print(test_content[:500] + ("..." if len(test_content) > 500 else ""))
        console.print("=" * 50)
...
@app.command()
def docs(
    filename: str = typer.Argument(..., help="Name of the file to generate documentation for")
):
    ...
    console.print(f"[bold blue]📚 Generating documentation for: {filename}[/bold blue]\n")
    ...
    try:
        ...
        # Determine documentation file name
        if file_extension == '.py':
            docs_filename = filename.replace('.py', '_docs.md')
        else:
            docs_filename = f"{filename}_docs.md"
        
        # Generate documentation prompt
        prompt = f"Generate comprehensive documentation for this {file_extension} code file. Include:\n- Overview and purpose\n- Function descriptions\n- Parameters and return values\n- Usage examples\n- Dependencies\n- Installation instructions\n\nCode:\n{current_content}\n\nProvide only the documentation in Markdown format, no explanations."
        
        # Get AI response
        docs_content = bot.chat(prompt)
        
        # Write documentation file
        with open(docs_filename, 'w', encoding='utf-8') as f:
            f.write(docs_content)
        
        console.print(f"[green]✅ Documentation generated: {docs_filename}[/green]")
        console.print(f"[blue]Documentation size: {len(docs_content)} characters[/blue]")
        
        # Show preview
        console.print(f"\n[bold blue]📄 Documentation Preview:[/bold blue]")
        console.print("=" * 50)
        console.print(docs_content[:500] + ("..." if len(docs_content) > 500 else ""))
        console.print("=" * 50)
...
@app.command()
def weather(
    location: str = typer.Argument(..., help="Location for weather information")
):
    ...
    console.print(f"[bold blue]🌤️  Weather for: {location}[/bold blue]\n")
    ...
    try:
        ...
        if 'current_condition' in data:
            ...
            console.print(f"[bold green]📍 Location:[/bold green] {location}")
            console.print(f"[bold green]🌡️  Temperature:[/bold green] {current['temp_C']}°C ({current['temp_F']}°F)")
            console.print(f"[bold green]💨 Wind:[/bold green] {current['windspeedKmph']} km/h")
            console.print(f"[bold green]💧 Humidity:[/bold green] {current['humidity']}%")
            console.print(f"[bold green]☁️  Condition:[/bold green] {current['weatherDesc'][0]['value']}")
            console.print(f"[bold green]👁️  Visibility:[/bold green] {current['visibility']} km")
        else:
            console.print(f"[red]❌ Weather information not available for {location}[/red]")
            
    except Exception as e:
        console.print(f"[red]❌ Error getting weather: {e}[/red]")
        console.print(f"[yellow]💡 Try: ai chat 'What's the weather like in {location}?'[/yellow]")

@app.command()
def news(
    topic: str = typer.Argument(..., help="Topic to search for news")
):
    ...
    console.print(f"[bold blue]📰 News about: {topic}[/bold blue]\n")
    
    try:
        import requests
        from urllib.parse import quote
        
        # Use NewsAPI (free tier - requires API key in production)
        # For now, use a simple web search approach
        search_query = f"latest news {topic} 2024"
        ...
        if data.get('Abstract'):
            console.print(f"[bold green]📰 Latest News:[/bold green]")
            console.print(f"{data['Abstract']}")
            if data.get('AbstractURL'):
                console.print(f"\n[blue]Source:[/blue] {data['AbstractURL']}")
        else:
            console.print(f"[yellow]💡 No recent news found for '{topic}'[/yellow]")
            console.print(f"[yellow]Try: ai chat 'Tell me about {topic}'[/yellow]")
            
    except Exception as e:
        console.print(f"[red]❌ Error getting news: {e}[/red]")
        console.print(f"[yellow]💡 Try: ai chat 'What are the latest developments in {topic}?'[/yellow]")

@app.command()
def translate(
    text: str = typer.Argument(..., help="Text to translate"),
    language: str = typer.Option("english", "--lang", "-l", help="Target language")
):
    ...
    console.print(f"[bold blue]🌐 Translation[/bold blue]")
    console.print(f"[blue]Text: {text}[/blue]")
    console.print(f"[blue]Target: {language}[/blue]\n")
    
    try:
        # Use AI for translation
        prompt = f"Translate the following text to {language}. Provide only the translation, no explanations:\n\n{text}"
        
        translation = bot.chat(prompt)
        
        console.print(f"[bold green]✅ Translation:[/bold green]")
        console.print(f"{translation}")
        
    except Exception as e:
        console.print(f"[red]❌ Error translating: {e}[/red]")

@app.command()
def debug(
    error: str = typer.Argument(..., help="Error message or code to debug")
):
    ...
    console.print(f"[bold blue]🐛 Debugging Help[/bold blue]")
    console.print(f"[blue]Error: {error}[/blue]\n")
    
    try:
        # Generate debugging prompt
        prompt = f"Help me debug this error or code issue. Provide step-by-step debugging guidance, potential solutions, and best practices to avoid this in the future.\n\nError/Issue: {error}\n\nProvide a comprehensive debugging guide."
        
        debugging_help = bot.chat(prompt)
        
        console.print(f"[bold green]🔧 Debugging Guide:[/bold green]")
        console.print("=" * 60)
        console.print(debugging_help)
        console.print("=" * 60)
        
    except Exception as e:
        console.print(f"[red]❌ Error getting debugging help: {e}[/red]")

@app.command()
def optimize(
    code: str = typer.Argument(..., help="Code to optimize")
):
    ...
    console.print(f"[bold blue]⚡ Code Optimization[/bold blue]")
    console.print(f"[blue]Code: {code}[/blue]\n")
    
    try:
        # Generate optimization prompt
        prompt = f"Analyze and optimize this code for better performance, readability, and maintainability. Provide specific suggestions and an optimized version.\n\nCode:\n{code}\n\nProvide optimization recommendations and an improved version."
        
        optimization_help = bot.chat(prompt)
        
        console.print(f"[bold green]🚀 Optimization Suggestions:[/bold green]")
        console.print("=" * 60)
        console.print(optimization_help)
        console.print("=" * 60)
        
    except Exception as e:
        console.print(f"[red]❌ Error optimizing code: {e}[/red]")

@app.command()
def review(
    filename: str = typer.Argument(..., help="File to review")
):
    ...
    console.print(f"[bold blue]👀 Code Review: {filename}[/bold blue]\n")
    ...
    try:
        ...
        prompt = f"Perform a comprehensive code review of this {file_extension} file. Include:\n- Code quality assessment\n- Potential bugs and issues\n- Security concerns\n- Performance improvements\n- Best practices recommendations\n- Specific suggestions for improvement\n\nCode:\n{content}\n\nProvide a detailed code review with actionable suggestions."
        
        review_result = bot.chat(prompt)
        
        console.print(f"[bold green]📋 Code Review Results:[/bold green]")
        console.print("=" * 60)
...
def init(
    ...
    try:
        ...
        prompt = f"Create a modern project structure for '{project_name}'. Include:\n- README.md with project description\n- requirements.txt or package.json\n- .gitignore\n- Basic project files\n- Setup instructions\n\nProvide the file structure and content for a well-organized project."
        
        project_structure = bot.chat(prompt)
        
        # Create basic files
        readme_content = f"# {project_name}\n\nA modern project created with AI assistance.\n\n## Setup\n\n```bash\npip install -r requirements.txt\n```\n\n## Usage\n\nDescribe your project usage here.\n"
        ...
        console.print(f"  • {project_name}/README.md")
        console.print(f"  • {project_name}/requirements.txt")
        console.print(f"  • {project_name}/.gitignore")
...
@app.command()
def setup_tool(
    tool_name: str = typer.Argument(..., help="Tool to set up")
):
    """Get AI-assisted setup instructions for development tools."""
    display_banner()
    
    bot = AIBotAgent()
    
    console.print(f"[bold blue]🔧 Setting up: {tool_name}[/bold blue]\n")
    
    try:
        # Generate setup prompt
        prompt = f"Provide step-by-step setup instructions for {tool_name}. Include:\n- Installation commands\n- Configuration steps\n- Basic usage examples\n- Troubleshooting tips\n- Best practices\n\nProvide comprehensive setup instructions."
        
        setup_instructions = bot.chat(prompt)
        
        console.print(f"[bold green]📋 Setup Instructions:[/bold green]")
        console.print("=" * 60)
        console.print(setup_instructions)
        console.print("=" * 60)
        
    except Exception as e:
        console.print(f"[red]❌ Error getting setup instructions: {e}[/red]")

@app.command()
def system(
    command: str = typer.Argument(..., help="System command to execute with AI assistance")
):
    """Execute system commands with AI assistance and safety checks."""
    display_banner()
    
    bot = AIBotAgent()
    
    console.print(f"[bold blue]💻 System Command: {command}[/bold blue]\n")
    
    # Safety check for dangerous commands
    dangerous_commands = ['rm -rf', 'sudo', 'chmod 777', 'format', 'dd', 'mkfs', 'fdisk']
    if any(dangerous in command.lower() for dangerous in dangerous_commands):
        console.print(f"[red]⚠️  Warning: This command could be dangerous![/red]")
        console.print(f"[yellow]Please review the command before executing: {command}[/yellow]")
        return
    ...
    try:
        ...
        analysis_prompt = f"Analyze this system command and provide:\n- What the command does\n- Potential risks\n- Safer alternatives if applicable\n- Best practices\n\nCommand: {command}\n\nProvide a safety analysis."
        
        analysis = bot.chat(analysis_prompt)
        
        console.print(f"[bold green]🔍 Command Analysis:[/bold green]")
        console.print("=" * 50)
        console.print(analysis)
        console.print("=" * 50)
        
        # Ask for confirmation
        console.print(f"\n[yellow]⚠️  Do you want to execute: {command}[/yellow]")
        console.print(f"[yellow]Type 'yes' to continue or anything else to cancel:[/yellow]")
        
        # For now, just show the analysis without executing
        console.print(f"[blue]Command analysis provided. Execute manually if needed.[/blue]")
        
    except Exception as e:
        console.print(f"[red]❌ Error analyzing command: {e}[/red]")

@app.command()
def docker(
    action: str = typer.Argument(..., help="Docker action (build, run, stop, etc.)")
):
    """Get AI-assisted Docker commands and help."""
    display_banner()
    
    bot = AIBotAgent()
    
    console.print(f"[bold blue]🐳 Docker: {action}[/bold blue]\n")
    
    try:
        # Generate Docker help prompt
        prompt = f"Provide Docker commands and guidance for: {action}\n\nInclude:\n- Exact commands to run\n- Explanation of what each command does\n- Common options and flags\n- Best practices\n- Troubleshooting tips\n\nProvide comprehensive Docker assistance."
        
        docker_help = bot.chat(prompt)
        
        console.print(f"[bold green]🐳 Docker Commands:[/bold green]")
        console.print("=" * 60)
        console.print(docker_help)
        console.print("=" * 60)
        
    except Exception as e:
        console.print(f"[red]❌ Error getting Docker help: {e}[/red]")

@app.command()
def git(
    action: str = typer.Argument(..., help="Git action (commit, push, pull, etc.)")
):
    """Get AI-assisted Git commands and help."""
    display_banner()
    
    bot = AIBotAgent()
    
    console.print(f"[bold blue]📚 Git: {action}[/bold blue]\n")
    
    try:
        # Generate Git help prompt
        prompt = f"Provide Git commands and guidance for: {action}\n\nInclude:\n- Exact commands to run\n- Explanation of what each command does\n- Common options and flags\n- Best practices\n- Troubleshooting tips\n- When to use this action\n\nProvide comprehensive Git assistance."
        
        git_help = bot.chat(prompt)
        
        console.print(f"[bold green]📚 Git Commands:[/bold green]")
        console.print("=" * 60)
        console.print(git_help)
        console.print("=" * 60)
        
    except Exception as e:
        console.print(f"[red]❌ Error getting Git help: {e}[/red]")

@app.command()
def ssh(
    server: str = typer.Argument(..., help="Server to connect to")
):
    """Get AI-assisted SSH connection help."""
    display_banner()
    
    bot = AIBotAgent()
    
    console.print(f"[bold blue]🔐 SSH: {server}[/bold blue]\n")
    
    try:
        # Generate SSH help prompt
        prompt = f"Provide SSH connection guidance for: {server}\n\nInclude:\n- SSH command syntax\n- Key-based authentication setup\n- Troubleshooting connection issues\n- Security best practices\n- Common SSH options\n- File transfer with SCP/SFTP\n\nProvide comprehensive SSH assistance."
        
        ssh_help = bot.chat(prompt)
        
        console.print(f"[bold green]🔐 SSH Commands:[/bold green]")
        console.print("=" * 60)
        console.print(ssh_help)
        console.print("=" * 60)
        
    except Exception as e:
        console.print(f"[red]❌ Error getting SSH help: {e}[/red]")
