    ...
    async def process(
        ...
        **kwargs
        ...
        try:
            ...
            # Debug: Log the entire conversation being sent to OpenAI
            logger.debug("=== FULL CONVERSATION SENT TO OPENAI ===")
            logger.debug(f"Number of messages: {len(messages)}")
            for i, msg in enumerate(messages):
                logger.debug(f"Message {i+1}:")
                logger.debug(f"  Role: {msg.get('role', 'unknown')}")
                if isinstance(msg.get('content'), str):
                    logger.debug(f"  Content: {msg.get('content', '')[:200]}{'...' if len(msg.get('content', '')) > 200 else ''}")
                elif isinstance(msg.get('content'), list):
                    logger.debug(f"  Content (multimodal): {len(msg.get('content', []))} parts")
                    for j, part in enumerate(msg.get('content', [])):
                        if part.get('type') == 'text':
                            logger.debug(f"    Part {j+1} (text): {part.get('text', '')[:100]}{'...' if len(part.get('text', '')) > 100 else ''}")
                        elif part.get('type') == 'image_url':
                            image_url = part.get('image_url', {}).get('url', '')
                            if image_url.startswith('data:'):
                                logger.debug(f"    Part {j+1} (image): base64 data ({len(image_url)} chars)")
                            else:
                                logger.debug(f"    Part {j+1} (image): {image_url}")
                        else:
                            logger.debug(f"    Part {j+1} ({part.get('type', 'unknown')}): {str(part)[:100]}...")
                if msg.get('tool_calls'):
                    logger.debug(f"  Tool calls: {len(msg.get('tool_calls', []))} calls")
            logger.debug("=== END OF CONVERSATION ===")
            
            # Debug: Log full message structure as JSON for easier analysis
            try:
                logger.debug("Full messages structure (JSON):")
                logger.debug(json.dumps(messages, indent=2, default=str))
            except Exception as e:
                logger.debug(f"Could not serialize messages to JSON: {e}")
