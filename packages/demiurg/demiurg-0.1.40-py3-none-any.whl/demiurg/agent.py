...
- Use available tools when appropriate"""
    ...
    async def process_message(
        ...
        try:
            ...
            if content is not None:
                ...
                logger.debug("Added current message with custom content")
                if isinstance(content, list):
                    logger.debug(f"Current message is multimodal with {len(content)} parts")
            else:
                messages.append({
                    "role": "user",
                    "content": message.content
                })
                logger.debug(f"Added current message with text content: {message.content[:100]}...")
    ...
    async def process_file_message(self, message: Message) -> str:
        ...
        try:
            ...
            if file_type == 'image':
                # For images, create multimodal content with the URL directly
                content = [
                ...
                # For audio files, pass the URL and let the agent decide whether to transcribe
                # Include metadata about the audio file
                audio_info = f"Audio file '{file_info['name']}' (URL: {file_info['url']})"
                content = f"{user_text}\n\n[{audio_info}]"
            
            else:
                # For text files, PDFs, and code, download and read the content
