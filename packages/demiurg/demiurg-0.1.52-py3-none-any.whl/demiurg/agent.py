...
from .utils.logging import configure_third_party_logging
...
class Agent:
    ...
    def __init__(
        ...
        # Configure third-party logging to reduce verbosity
...
- Use available tools when appropriate"""
    ...
    async def process_message(
        ...
        try:
            ...
            # Log conversation history if debug enabled
            if logger.isEnabledFor(logging.DEBUG) and history:
            ...
            if history:
                ...
                # No history found, adding current message
                messages.append({
                    "role": "user",
            ...
            if self.config.provider == "openai":
                ...
                if has_audio and hasattr(self.provider, 'is_audio_model') and not self.provider.is_audio_model(model_to_use):
                    logger.info(f"Audio detected: switching to gpt-4o-audio-preview")
            ...
            # If using audio model, modify the copy for compatibility
            if needs_audio_formatting:
                # 1. Convert any image_url messages to plain text in the copy
                for i, msg in enumerate(messages_for_request):
                    if isinstance(msg.get('content'), list):
                        ...
                        messages_for_request[i] = {
                            "role": msg.get('role', 'user'),
                            "content": ' '.join(text_parts) if text_parts else ""
                        }
                
                # 2. Modify system prompt to restrict transcribe tool usage
                for i, msg in enumerate(messages_for_request):
                    if msg.get('role') == 'system':
                        original_content = msg.get('content', '')
                        messages_for_request[i] = {
                            "role": "system",
                            "content": original_content + "\n\nIMPORTANT: Do not use the transcribe_audio tool unless explicitly requested by the user. You have native audio processing capabilities."
                        }
                        break
            ...
            if needs_audio_formatting and audio_url and len(messages_for_request) > 0:
                ...
                if last_user_msg_idx is not None:
                    try:
                        ...
                        logger.debug(f"Native audio format applied")
