...
# """
        ...
        parser.add_argument('--max-threads', '-mt', type=int, default=None, help='设置最大线程数 (默认: 4)')
        parser.add_argument('--no-multithreading', action='store_true', help='禁用多线程下载')
    ...
    def _init_service(self, args) -> bool:
        ...
        if self.service is None:
            ...
            config.update_from_args(
                ...
                enable_multithreading=not args.no_multithreading if hasattr(args, 'no_multithreading') else None
            )
    ...
    def handle_download_song(self, args):
        ...
        if len(music_ids) > 1:
            ...
            self.service.download_music_list_parallel(music_ids)
        else:
    ...
    def handle_download_playlist(self, args):
        ...
        if len(playlist_ids) > 1:
            ...
            self.service.download_playlist_parallel(playlist_ids[0])
