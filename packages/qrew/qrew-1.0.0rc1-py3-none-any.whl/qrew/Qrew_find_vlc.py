...
def _check_and_import_vlc():
    ...
    if not lib_dir:
        ...
        if not vlc_exe:
            ...
            _vlc_status["error_message"] = {
                "title": "VLC Not Found",
                "text": "Neither VLC libraries nor VLC application were found. "
    ...
    try:
        ...
        _vlc_status["error_message"] = {
            ...
            "text": f"Failed to import python-vlc module: {e}\n\nFalling back to VLC application.",
        }
        _vlc_status["backend"] = "subprocess"
    ...
    try:
        ...
        with open(dll_path, "rb") as f:
            ...
            if python_arch != vlc_arch:
                ...
                error_message += f"\n\nArchitecture mismatch: Python is {python_arch} but VLC is {vlc_arch}"
