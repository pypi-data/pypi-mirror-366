...
def setup():
    ...
    for pkg in packages_to_test:
        ...
        for bubble in dpncy.multiversion_base.glob(f"{pkg}-*"):
            if bubble.is_dir():
                print(f"   - Removing old bubble: {bubble.name}")
                shutil.rmtree(bubble)

    # Use dpncy to ensure the 'good' versions are installed
    print("   - Setting main environment to a known good state...")
    dpncy.smart_install(["numpy==1.26.4", "scipy==1.16.1"]) # Note: using a compatible scipy version
    print("✅ Environment is clean and ready for testing.")

def run_test():
    """The core of the DPNCY Nuclear Stress Test."""
    loader = DPNCYLoader()
...
def run():
    ...
    try:
        ...
        print_header("STEP 2: Creating Test Bubbles with `dpncy`")
        dpncy = Dpncy()
        packages_to_bubble = [
            "numpy==1.24.3",
            "scipy==1.12.0"
