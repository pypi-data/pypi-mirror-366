...
class BubbleIsolationManager:
    def __init__(self, config: Dict, parent_dpncy):
        self.config = config
        self.parent_dpncy = parent_dpncy
    ...
    def _try_pypi_api(self, package_name: str, version: str) -> Optional[List[str]]:
        try:
            ...
            headers = {
                'User-Agent': 'dpncy-package-manager/1.0',
    ...
    def _create_deduplicated_bubble(self, installed_tree: Dict, bubble_path: Path, temp_install_path: Path) -> bool:
        ...
        for pkg_name, pkg_info in installed_tree.items():
            ...
            # If the package contains any native code ('mixed' or 'native'),
            # we disable deduplication for it entirely to ensure stability.
            ...
            for file_path in pkg_info['files']:
                ...
                should_copy = True # Default to copying everything
                
                # Only attempt to deduplicate if the package is pure Python.
                if not is_native_pkg:
                    try:
                        file_hash = self._get_file_hash(file_path)
                        if file_hash in main_env_hashes:
                            should_copy = False # Safe to deduplicate this file
                    except (IOError, OSError):
                        should_copy = True # If we can't hash, copy to be safe.
                ...
                if should_copy:
                    try:
                        ...
                        # In case of any error, we can log it but continue
    ...
    def _get_or_build_main_env_hash_index(self) -> Set[str]:
        ...
        """
        if not self.parent_dpncy.redis_client: self.parent_dpncy.connect_redis()

        redis_key = f"{self.config['redis_key_prefix']}main_env:file_hashes"

        # --- The Fast Path ---
        if self.parent_dpncy.redis_client.exists(redis_key):
            print("    ⚡️ Loading main environment hash index from cache...")
            # Use SSCAN for memory efficiency on huge sets
            hashes = set(self.parent_dpncy.redis_client.sscan_iter(redis_key))
            print(f"    📈 Loaded {len(hashes)} file hashes from Redis.")
            return hashes

        # --- The Slow, One-Time Build Path ---
        ...
        with self.parent_dpncy.redis_client.pipeline() as pipe:
            for h in hash_set:
...
class Dpncy:
    ...
    def _update_hash_index_for_delta(self, before: Dict, after: Dict):
        ...
        with self.redis_client.pipeline() as pipe:
            ...
            # Add hashes from newly installed packages
    ...
    def _clean_and_format_dependencies(self, raw_deps_json: str) -> str:
	    ...
	    try:
	        ...
	        # Filter out extras, platform markers, etc., by ignoring anything with a semicolon
	        core_deps = [d.split(';')[0].strip() for d in deps if ';' not in d]
	        
	        # If the list is still huge, truncate it
	        if len(core_deps) > 5:
    ...
    def list_packages(self, pattern: str = None) -> int:
        ...
        for pkg, version in sorted(installed.items()):
            ...
            if info:
                ...
                print(f"  ⚠️  {pkg} v{version} - No metadata found. Run `dpncy rebuild-kb`.")
        return 0
