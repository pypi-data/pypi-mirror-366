...
class DpncyMetadataGatherer:
    ...
    def run(self, targeted_packages: List[str] = None):
        """
        The main execution loop. Now supports a "targeted" mode to only process
        a specific list of packages, avoiding a full-environment rescan.
        """
        ...
        # --- NEW: This is the core logic for the "Smart Snapshot & Diffing" strategy ---
        if targeted_packages:
            # If we received a list of packages, we are in TARGETED MODE.
            print(f"🎯 Running in targeted mode for {len(targeted_packages)} package(s).")
            
            # Convert the "pkg==ver" strings from the command line into the 
            # (name, version) tuple format that the rest of the script expects.
            for pkg_spec in targeted_packages:
                parts = pkg_spec.split('==')
                if len(parts) == 2:
                    # Use canonicalize_name to ensure consistency with how names are stored.
                    packages_to_process.append((canonicalize_name(parts[0]), parts[1]))
        else:
            # If no targets were provided, we are in FULL SCAN MODE.
            print("🔍 No specific targets provided. Discovering all installed packages...")
            packages_to_process = self.discover_all_packages()
        
        # The rest of the function proceeds as normal, but now with a much smaller
        # list of packages to process when in targeted mode.
        ...
        try:
            for dist in importlib.metadata.distributions():
                # Normalize the name for consistent matching with the security report
                active_packages[canonicalize_name(dist.metadata['Name'])] = dist.metadata['Version']
        except Exception as e:
        ...
        for package_name, version_str in package_iterator:
            # The 'package_name' from discover_all_packages is already canonicalized.
            if HAS_TQDM: 
                package_iterator.set_postfix_str(f"Current: {package_name} v{version_str}", refresh=True)
            try:
                # Use the already canonicalized name for Redis keys
