...
class TestMatrix(unittest.TestCase):
    ...
    def test_matrix_multiplication(self):
        m1 = Matrix((2, 3), init='uniform')
        m2 = Matrix((3, 4), init='uniform')
    ...
    def test_reshape_padding(self):
        ...
        self.assertTrue(np.allclose(reshaped.matrix[0], [1.0, 2.0]))

    def test_reshape_truncation(self):
        m = Matrix((2, 2))
        m.matrix = np.array([[1.0, 2.0], [3.0, 4.0]])
        reshaped = m.reshape((1, 2))
        self.assertEqual(reshaped.shape, (1, 2))
        self.assertTrue(np.allclose(reshaped.matrix, [[1.0, 2.0]]))
