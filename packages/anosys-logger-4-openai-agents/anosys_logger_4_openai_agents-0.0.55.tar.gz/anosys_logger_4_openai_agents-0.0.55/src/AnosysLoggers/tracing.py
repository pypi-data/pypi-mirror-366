...
def extract_span_info(span):
    ...
    def assign(variable, var_value):
        ...
        if isinstance(var_value, str):
            ...
            if var_value.startswith('{') or var_value.startswith('['):
                try:
                    variables[variable] = json.loads(var_value)
                    return
    ...
    assign('llm_tools', attributes.get('llm', {}).get('tools'))
    assign('llm_token_count', attributes.get('llm', {}).get('token_count'))
    assign('llm_output_messages', attributes.get('llm', {}).get('output_messages', {}).get('output_messages'))
    assign('llm_input_messages', attributes.get('llm', {}).get('input_messages', {}).get('input_messages'))
    assign('llm_model_name', attributes.get('llm', {}).get('model_name'))
    assign('llm_invocation_parameters', attributes.get('llm', {}).get('invocation_parameters'))

    assign('input', attributes.get('input', {}).get('value'))
    assign('output', attributes.get('output', {}).get('value'))
    assign('tool', attributes.get('tool', {}))
    assign('kind', attributes.get('fi', {}).get('span', {}).get('kind'))

    return reassign(clean_dict(variables))
