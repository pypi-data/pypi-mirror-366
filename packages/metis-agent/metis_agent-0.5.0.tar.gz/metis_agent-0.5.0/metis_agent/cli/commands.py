...
@click.option("--max-tokens", type=int, help="Maximum context tokens (default: 2000)")
def chat(query, session, max_tokens):
    """Start interactive chat or process a single query."""
    config = AgentConfig()
    
    # Configure enhanced memory (always enabled)
    memory_config = MemoryConfig()
    if max_tokens:
        memory_config.max_context_tokens = max_tokens
    
    # Initialize agent with enhanced memory management (always enabled)
    agent = SingleAgent(
...
def _start_interactive_mode(agent: SingleAgent, config: AgentConfig, session_id: str = None):
    ...
    click.echo("\nMemory commands:")
    click.echo("  - 'memory status' - Show memory statistics")
    click.echo("  - 'memory cleanup' - Clean old sessions")
    click.echo("  - 'memory config <setting> <value>' - Configure memory")
    ...
    while True:
        try:
            ...
            # Handle memory commands for enhanced agent
            elif user_input.lower().startswith('memory ') and agent.use_enhanced_memory:
                memory_cmd = user_input[7:].strip().lower()
                
                if memory_cmd == 'status':
                    agent.show_memory_status(current_session)
                    continue
                
                elif memory_cmd == 'cleanup':
                    result = agent.cleanup_memory_sessions()
                    if 'error' in result:
                        click.echo(f"Error: {result['error']}")
                    else:
                        click.echo(f"Cleaned {result['cleaned_sessions']} old sessions")
                    continue
                
                elif memory_cmd.startswith('config '):
                    config_parts = memory_cmd[7:].split()
                    if len(config_parts) >= 2:
                        setting = config_parts[0]
                        value = config_parts[1]
                        if agent.configure_memory(setting, value):
                            click.echo(f"Successfully updated {setting} to {value}")
                        else:
                            click.echo(f"Failed to update {setting}")
                    else:
                        click.echo("Usage: memory config <setting> <value>")
                        click.echo("Available settings: max_context_tokens, summarization_threshold, session_timeout_hours")
                    continue
                
                else:
                    click.echo("Available memory commands:")
                    click.echo("  memory status - Show memory statistics")
                    click.echo("  memory cleanup - Clean old sessions")
                    click.echo("  memory config <setting> <value> - Configure memory")
                    continue
