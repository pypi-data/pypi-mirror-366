...
- SIMPLE: Single concept questions (e.g., "Explain photosynthesis")
...
- SINGLE_TOOL: Use one tool (search, calculator, etc.)
...
{{
  ...
  "reasoning": "why you chose this classification"
    ...
    def _fallback_analysis(self, query: str) -> QueryAnalysis:
        ...
        elif any(word in query.lower() for word in action_words):
            if word_count > 15:
                complexity = QueryComplexity.COMPLEX
            else:
                complexity = QueryComplexity.MODERATE
        elif any(word in query.lower() for word in tool_words):
            complexity = QueryComplexity.MODERATE
        else:
        ...
        if any(word in query.lower() for word in ['code', 'program', 'script']):
