...
def view_executions(ctx, project_id=None, output_type='table', file_path=None, profile=None, override_config_path=None):
    ...
    try:
        commands.view_all_executions(ctx, project_id, output_type, file_path, profile, override_config_path)
    except Exception as e:
        out.vprint(e)
        raise click.Abort()
...
@click.option('--override-config-path', '-c', help='Path to config.json, default config stored in ~/.config.json')
@click.pass_context
def get_findings(ctx, execution_id=None, output_type='table', file_path=None, profile=None, override_config_path=None):
    """Get findings for a specific execution"""
    try:
        commands.get_findings(ctx, execution_id, output_type, file_path, profile, override_config_path)
    except Exception as e:
        out.vprint(e)
        raise click.Abort()
