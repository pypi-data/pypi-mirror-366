...
import RBFInterpolator as torchRBF
...
def test_correctness():
    ...
    for _ in range(1000):
        ...
        kernel = random.choice(list(torchRBF.SCALE_INVARIANT))
        rbf_torch = torchRBF.RBFInterpolator(y, d, neighbors=None, smoothing=smoothing, kernel=kernel, device=dev)
...
def test_device():
    ...
    rbf = torchRBF.RBFInterpolator(y, d, neighbors=None, smoothing=100, kernel='thin_plate_spline', device='cuda')
    out1 = rbf(x)
    rbf = rbf.to('cpu')
    out2 = rbf(x.to('cpu'))
...
if __name__ == '__main__':
    ...
    for params in b_forward:
        ...
        benchmark_forward(**params)
        print('-------------------')
    # benchmark_init(compare_scipy=True)
    # benchmark_forward(compare_scipy=True)