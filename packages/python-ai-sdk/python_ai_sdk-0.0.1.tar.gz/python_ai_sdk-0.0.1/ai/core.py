...
# Build provider classes dictionary dynamically
PROVIDER_CLASSES = {}
if HAS_OPENAI_PROVIDER:
    PROVIDER_CLASSES["openai"] = OpenAIProvider
if HAS_GOOGLE_PROVIDER:
    PROVIDER_CLASSES["google"] = GoogleProvider
...
def _get_provider_class(provider_name: str):
    ...
    if not ProviderClass:
        ...
        error_msg = f"Provider '{provider_name}' not available. Available providers: {available_providers}"
        
        if provider_name == "google" and not HAS_GOOGLE_PROVIDER:
            error_msg += "\nTo use Google provider, install: pip install google-genai"
        elif provider_name == "openai" and not HAS_OPENAI_PROVIDER:
