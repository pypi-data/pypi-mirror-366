...
from filtering_pipeline.utils.helpers import clean_protein_sequence
...
class Docking:
    ...
    def _run_vina(self, df_boltz):
        ...
        if df_vina['vina_dir'].isnull().any() == True: 
            ...
            df_missing_docked.rename(columns = {'output_dir':'vina_dir'}, inplace=True)
            
            vina_dir_mapping = df_missing_docked.set_index('Entry')['vina_dir'].to_dict()

            df_vina['vina_dir'] = df_vina.apply(
                lambda row: vina_dir_mapping.get(row['Entry'], row['vina_dir']) if pd.isnull(row['vina_dir']) else row['vina_dir'],
                axis=1
            )
            
        df_vina >> Save(Path(self.output_dir)/'vina.pkl')  
        return df_vina
