...
class ReferedDatasetPropertyManager:
    ...
    @property
    def dataset_dir_path(self)->Any:
        value = self.property_manager.dataset_dir_path
        if value is None and self.refer_manager is not None:
            return self.refer_manager.dataset_dir_path
        return value

    @property
    def annotation_file_path(self)->Any:
        value = self.property_manager.annotation_file_path
        if value is None and self.refer_manager is not None:
            return self.refer_manager.annotation_file_path
        return value
