...
class TaskRecord(ResourceRecord[TaskConfigManager]):
    ...
    def get_dataset_config(self, dataset_id:ID)->tuple[Path, Path]:
        ...
        property_manager = dataset_record.property_manager

        dataset_dir_path = property_manager.to_absolute_path(property_manager.dataset_dir_path)
        annotation_file_path = property_manager.to_absolute_path(property_manager.annotation_file_path)
