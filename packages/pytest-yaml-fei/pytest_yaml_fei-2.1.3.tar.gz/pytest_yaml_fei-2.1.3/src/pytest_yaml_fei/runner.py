...
class RunYaml(object):
    ...
    def run_testcase_step(self, args, step, context=None):
        ...
        log.info(f'执行用例文件-> {os.path.basename(testcase_file)}')

        # 处理直接传入dict类型的testcase_data
        if isinstance(testcase_data, dict) and 'config' not in testcase_data:
            # 如果testcase_data是dict但没有config，则将其视为单个用例
            testcase_data = {'config': {}, 'test_case': testcase_data}
