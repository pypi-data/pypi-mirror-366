...
class RunYaml(object):
            ...
            def execute_yaml_case(args):
                ...
                for step in case[call_function_name]:
                    ...
                    for item, value in step.items():
                        ...
                        if 'testcase' in step.keys():
                            self.run_testcase_step(args, step, context=step_context)
                            continue
                        elif 'api' in step.keys():
                            root_dir = args.get('request').config.rootdir  # 内置request 获取root_dir
                            api_path = Path(root_dir).joinpath(step['api'])
                        ...
                        elif 'request' in step.keys():
                            copy_value = copy.deepcopy(step['request'])  # 深拷贝一份新的value
    ...
    def run_testcase_step(self, args, step, context=None):
        ...
        if all_exports:
            export_result = {}
            for export_var in all_exports:
                if export_var in testcase_runner.module_variable:
                    export_result[export_var] = testcase_runner.module_variable[export_var]
                    self.g[export_var] = testcase_runner.module_variable[export_var]
            if export_result:
                log.info(f'export 全局导出变量-> {export_result}')
                if context:
        ...
        return testcase_runner.module_variable
                ...
                if ',' in key or '-' in key:
                    ...
                    try:
                        ...
                        if isinstance(value, (str, bytes, dict)):
                            raise TypeError(
                                f"Parameter value for '{key}' must be a list or iterable, got {type(value).__name__}")
                        new_parameters.append([[v] for v in value])
                    except TypeError as e:
                        # 如果value不可迭代，将其作为单个参数值处理
                        log.warning(f"Invalid parameter value for '{key}': {str(e)}")
                        new_parameters.append([[value]])
