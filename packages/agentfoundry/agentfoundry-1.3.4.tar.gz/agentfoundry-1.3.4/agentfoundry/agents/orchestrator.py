...
class Orchestrator(BaseAgent):
    ...
    def __init__(self, tool_registry: ToolRegistry, llm=None, llm_type=None):
        ...
        if not isinstance(agent_tool_map, dict):
            self.logger.warning(
                "Tool registry does not define a valid 'agent_tools' mapping. "
                "Falling back to single generic agent.")

            # Build a default mapping: one agent that gets every registered tool
            agent_tool_map = {
                "generic_agent": self.registry.as_langchain_tools(),
            }
