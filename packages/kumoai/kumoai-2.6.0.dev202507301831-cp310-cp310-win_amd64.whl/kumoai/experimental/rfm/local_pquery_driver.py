...
class LocalPQueryDriver:
    ...
    def _get_candidates(
        ...
        if anchor_time == 'entity':  # Sort by time:
            candidate_time = self._graph_store.time_dict[
                self._query.entity.pkey.table_name][candidate]
            kind = 'stable' if self._random_seed is not None else None
            perm = np.argsort(-candidate_time, kind=kind)  # type: ignore
