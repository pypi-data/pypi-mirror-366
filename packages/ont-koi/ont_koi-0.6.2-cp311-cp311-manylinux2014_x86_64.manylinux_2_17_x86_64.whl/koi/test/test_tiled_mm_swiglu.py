...
def test_tiled_mm_swiglu(dims):
    ...
    with torch.no_grad():
        ...
        weights_fp8 = (
            torch.index_select(weights, 2, weights_shuffle_index)
            .contiguous()
            .view((N // 16, K // 8, 16, 8))
        )
        ...
        def koi_impl_fp8():
            ...
            args = (
                ...
                koi_tensor(out_koi_fp8, ["M", "N", "m", "n"]),
                void_ptr(ctr_bfr[ctr]),
                1,
            )
            ctr += 1
        ...
        out_koi_fp8_2 = out_koi_fp8.permute((0, 2, 1, 3)).contiguous().view((M, N // 2))

        label = (
            f"{dims}, ref {t_ref:.3f}s, koi fp8 out {t_koi:.3f}s, {tflops:.3f} TFlops"
        )
        show_diff_result(
            label, out_ref, out_koi_fp8_2, 5e-2, 0.8, __name__ != "__main__"
        )
