...
from koi.utils import void_ptr, check_result, time_kernel, koi_tensor, quantize_tensor
...
def test_tiled_qkv_rotary(dims):
    ...
    with torch.no_grad():
        ...
        in_scale, in_i8 = quantize_tensor(in_bfr, 256, -1)
        in_scale.reciprocal_()
        in_quant = in_i8 * in_scale.view((N, T, 1))
        check_result("input quantisation", in_bfr, in_quant, True, 2e-3, 9e-3)
        in_bfr_tiled = (
            in_i8.view((N, T // 16, 16, C // 16, 16)).transpose(2, 3).contiguous()
        )

        weight_scale, weight_i8 = quantize_tensor(weight_bfr, 256, 0)
        weight_scale.reciprocal_()
        weight_quant = weight_i8 * weight_scale.view((1, 3, H, D))
        check_result("weight quantisation", weight_bfr, weight_quant, True, 2e-3, 9e-3)
        weight_bfr_tiled = (
            weight_i8.view((C // 16, 16, 3, H, D // 16, 16))
            .permute((2, 3, 4, 0, 5, 1))
            .contiguous()
        )
        ...
        def koi_impl():
            ...
            args = (
                stream_ptr,
                theta,
                koi_tensor(in_bfr_tiled, ["N", "T", "C", "t", "c"], (in_scale, "C")),
                koi_tensor(
                    weight_bfr_tiled,
                    [lib.KOI_DIM_MAT_Q_K_V, "H", "D", "C", "d", "c"],
                    (weight_scale, "C"),
                ),
