...
class tiktokService():
    ...
    def login(driver, data, options):
        ...
        if storage_data:
            print("🌐 使用缓存尝试自动登录")
            tiktokService.inject_storage(driver, storage_data)
        ...
        if need_login:
            # 执行登录流程
            login_res = tiktokService.account_login(driver, data, options)
            # 登录失败
    ...
    @staticmethod
    def account_login(driver, data, options):
        '''
        @Desc    : 账号登录
        @Author  : 钟水洲
        @Time    : 2024/05/31 15:42:22
        ...
        # 保存店铺缓存
        tiktokService.save_storage(driver, shop_global_id)

        return common.back(1, '登录成功')

    @staticmethod
    def inject_storage(driver, storage_data):
        '''
        @Desc    : 注入缓存
        @Author  : 钟水洲
        @Time    : 2024/05/31 15:42:22
    ...
    @staticmethod
    def save_storage(driver, shop_global_id):
        '''
        @Desc    : 保存店铺缓存
        @Author  : 钟水洲
        @Time    : 2024/05/31 15:42:22
