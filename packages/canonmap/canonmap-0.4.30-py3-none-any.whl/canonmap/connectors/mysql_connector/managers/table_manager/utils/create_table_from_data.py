    ...
    if data is not None:
        ...
        if data_list:
            ...
            if not boolean_columns and tinyint_columns:
                ...
                # Map cleaned column names back to original names using the same mapping as data insertion
                if isinstance(data, list) and len(data) > 0:
                    # Use the same col_name_map that will be used for data insertion
                    reverse_map = {v: k for k, v in col_name_map.items()}
                    logger.info(f"Column name mapping: {col_name_map}")
                    logger.info(f"Reverse mapping: {reverse_map}")
                    for tinyint_col in tinyint_columns:
                        logger.info(f"Looking for DDL column '{tinyint_col}' in reverse map...")
                        if tinyint_col in reverse_map:
                            original_col = reverse_map[tinyint_col]
                            boolean_columns.add(original_col)
                            logger.info(f"Mapped DDL column '{tinyint_col}' to original column '{original_col}'")
                        else:
                            logger.warning(f"DDL column '{tinyint_col}' not found in reverse map!")
                            logger.warning(f"Available keys in reverse map: {list(reverse_map.keys())}")
