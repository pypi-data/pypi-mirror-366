...
def create_mysql_ddl(table_name: str, data: Union[str, pd.DataFrame, list, Path], save_dir=None):
        ...
        if not non_null_vals:
            ...
            if val_set <= boolean_vals:
                ...
                if all_numeric:
                    ...
                    # Be very conservative to avoid row size issues - use TEXT for most strings
                    if max_len <= 50:  # Very conservative limit
    ...
    # Check if we need ROW_FORMAT=DYNAMIC for large tables
    column_count = len(optimized_column_defs)
    use_dynamic_format = column_count > 50  # Use dynamic format for tables with many columns
    ...
    if use_dynamic_format:
        ddl += "\n) ROW_FORMAT=DYNAMIC;"
        print(f"Info: Using ROW_FORMAT=DYNAMIC for table '{table}' with {column_count} columns")
    else:
        ddl += "\n);"
