...
def create_table_from_data_util(create_table_request: CreateTableRequest, connector: MySQLConnector, data: Union[str, pd.DataFrame, list, Path]) -> dict:
    ...
    if not skip_table_creation:
        ...
        try:
            ...
            error_msg = str(e)
            if "Row size too large" in error_msg or "1118" in error_msg:
                logger.warning(f"Row size too large, retrying with more conservative column types: {error_msg}")
                # Retry with all VARCHAR columns converted to TEXT
                ddl_fallback = ddl.replace("VARCHAR(", "TEXT -- was VARCHAR(")
                ddl_fallback = ddl_fallback.replace("VARCHAR(", "TEXT -- was VARCHAR(")
                # Remove the VARCHAR length specifications
                import re
                ddl_fallback = re.sub(r'VARCHAR\(\d+\)', 'TEXT', ddl_fallback)
                logger.info(f"Fallback DDL: {ddl_fallback}")
                
                try:
                    connector.run_query(ddl_fallback)
                    logger.info(f"Table {table_name} created successfully with fallback DDL")
                except Exception as e2:
                    error_msg2 = str(e2)
                    if "Row size too large" in error_msg2 or "1118" in error_msg2:
                        logger.warning(f"Still too large, trying with ROW_FORMAT=DYNAMIC: {error_msg2}")
                        # Add ROW_FORMAT=DYNAMIC to handle large tables
                        ddl_dynamic = ddl_fallback.replace(");", ") ROW_FORMAT=DYNAMIC;")
                        logger.info(f"Dynamic DDL: {ddl_dynamic}")
                        
                        try:
                            connector.run_query(ddl_dynamic)
                            logger.info(f"Table {table_name} created successfully with ROW_FORMAT=DYNAMIC")
                        except Exception as e3:
                            error_msg3 = str(e3)
                            if "Row size too large" in error_msg3 or "1118" in error_msg3:
                                logger.warning(f"Still too large with ROW_FORMAT=DYNAMIC, trying with all TEXT columns: {error_msg3}")
                                # Convert all VARCHAR to TEXT and try again
                                ddl_all_text = re.sub(r'VARCHAR\(\d+\)', 'TEXT', ddl_dynamic)
                                ddl_all_text = re.sub(r'VARCHAR\([^)]*\)', 'TEXT', ddl_all_text)
                                logger.info(f"All TEXT DDL: {ddl_all_text}")
                                
                                try:
                                    connector.run_query(ddl_all_text)
                                    logger.info(f"Table {table_name} created successfully with all TEXT columns")
                                except Exception as e4:
                                    error_msg4 = str(e4)
                                    if "Row size too large" in error_msg4 or "1118" in error_msg4:
                                        logger.error(f"Table {table_name} has too many columns ({len(ddl_fallback.split(','))} columns) even with all TEXT columns. This table is too large for MySQL's row size limits.")
                                        raise TableManagerError(f"Table '{table_name}' has too many columns for MySQL even with optimized settings. Consider using a different database engine or redesigning the data structure.")
                                    else:
                                        raise
                            else:
                                raise
