...
from canonmap.logger import get_logger
...
def create_api_command(name: Optional[str] = None) -> int:
    ...
    try:
        ...
        try:
            subprocess.run(
                [sys.executable, "-m", "pip", "install", "fastapi", "uvicorn", "python-dotenv"],
                check=True,
                capture_output=True,
                text=True
            )
            ...
            logger.warning(f"⚠️  Could not install dependencies automatically: {e}")
            logger.info("📋 Please install manually:")
            logger.info("   pip install fastapi uvicorn python-dotenv")
