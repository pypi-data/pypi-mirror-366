...
@mcp.tool()
async def get_product_list(ctx: Context = None) -> dict:
    """
    Get list of all product SKUs from the database
    
    Args:
        ctx: MCP context object
        
    Returns:
        Dictionary with list of all product codes (SKUs) and basic info
    """
    from .error_handling import (
        create_user_friendly_error,
        error_handler,
    ...
    try:
        ...
        # Формируем список SKU с базовой информацией
        product_list = []
        ...
        logger.info(f"Получен список из {len(product_list)} товаров")
        
        log_recovery_attempt(
            component="server",
            action=f"Успешное получение списка {len(product_list)} товаров",
            success=True
        )
