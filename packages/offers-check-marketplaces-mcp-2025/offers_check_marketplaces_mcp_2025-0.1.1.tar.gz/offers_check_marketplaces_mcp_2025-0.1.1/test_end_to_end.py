...
class EndToEndTester:
    ...
    async def test_search_products_tool(self) -> bool:
        """Test the search_products MCP tool"""
        logger.info("Testing search_products MCP tool...")
        
        try:
            # Import the MCP tool function
            from offers_check_marketplaces.server import search_products
            
            # Test with a sample product name
            test_product_name = "Полотно техническое БЯЗЬ ОТБЕЛЕННАЯ ГОСТ"
            
            # Call the MCP tool
            result = await search_products(test_product_name)
            
            if not result:
                logger.error("search_products returned no result")
                return False
            
            if result.get('status') == 'error':
                logger.error(f"search_products returned error: {result.get('message')}")
                return False
            
            logger.info(f"search_products tool executed successfully")
            logger.info(f"   Status: {result.get('status')}")
            logger.info(f"   Query: {result.get('query')}")
            logger.info(f"   Results count: {len(result.get('results', []))}")
            
            # Log some sample results
            for i, marketplace_result in enumerate(result.get('results', [])[:3]):
                logger.info(f"   Result {i+1}: {marketplace_result.get('marketplace')} - Found: {marketplace_result.get('product_found')}")
            
            self.test_results['search_products'] = {
                'status': 'success',
                'query': test_product_name,
            ...
            logger.error(f"search_products tool test failed: {e}")
            self.test_results['search_products'] = {
                'status': 'failed',
                'error': str(e)
            }
    ...
    async def run_full_test_suite(self) -> bool:
        ...
        tests = [
            ...
            ("Search Products Tool", self.test_search_products_tool),
