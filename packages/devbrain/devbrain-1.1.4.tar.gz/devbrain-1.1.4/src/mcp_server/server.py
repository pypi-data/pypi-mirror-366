...
mcp_server = FastMCP(
    name="DevBrain - MCP Server for Indie Developers and Founders (v1.1.4)",
...
def get_swiftui_view_from_3designs(code: str) -> str:
    ...
    Args:
        code: A 6 digits code that identifies exact SwiftUI view component to return.
    ...
    if not code.isdigit() or len(code) != 6:
        return "<code> must be a 6-digit string to retrieve the SwiftUI view."

    url = f"https://getsven.com/3designs/download/{code}/swiftUI"
    headers = {
        "authorization": f"Bearer {_token}",
    }
