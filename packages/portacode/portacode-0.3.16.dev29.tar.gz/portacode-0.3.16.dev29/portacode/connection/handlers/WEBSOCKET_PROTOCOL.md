...
*   `git_detailed_status` (object, optional): Detailed Git status with comprehensive file change information and content hashes. Contains:
    ...
    *   `staged_changes` (array, optional): Array of staged file changes. Each change contains:
        ...
        *   `change_type` (string): Type of change ('added', 'modified', 'deleted', 'renamed', 'untracked').
        *   `content_hash` (string, optional): SHA256 hash of current file content.
        *   `old_repo_path` (string, optional): For renamed files - old relative path from repository root.
        *   `old_name` (string, optional): For renamed files - old filename.
        *   `old_abs_path` (string, optional): For renamed files - old absolute path.
