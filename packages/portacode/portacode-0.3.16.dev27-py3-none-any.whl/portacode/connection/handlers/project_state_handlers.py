...
class GitManager:
    ...
    def get_detailed_status(self) -> GitDetailedStatus:
        ...
        try:
            ...
            logger.info("GIT STATUS DEBUG - Raw output:\n%s", status_output)
            ...
            for line in status_output.split('\n'):
                ...
                index_status = line[0]
                worktree_status = line[1]
                file_path = line[3:]  # Skip the XY status and space to get the file path
                
                # Debug logging for file path parsing
                logger.info("GIT STATUS DEBUG - Line: '%s' -> file_path: '%s'", line, file_path)
                ...
                if file_path.startswith('"') and file_path.endswith('"'):
                    ...
                    logger.info("GIT STATUS DEBUG - Unquoted file_path: '%s'", file_path)
                
                # Handle renamed files (format: "R  old_name -> new_name")
                if index_status == 'R' or worktree_status == 'R':
                    if ' -> ' in file_path:
                        old_path, new_path = file_path.split(' -> ', 1)
                        abs_new_path = os.path.join(self.project_path, new_path)
                        content_hash = self._compute_file_hash(abs_new_path)
                        ...
                        change = GitFileChange(
                            ...
                            old_path=old_path,
                            is_staged=(index_status == 'R')
                        )
                        
                        if index_status == 'R':
                            detailed_status.staged_changes.append(change)
                        if worktree_status == 'R':
                ...
                if index_status != ' ' and index_status != '?':
                    change_type = self._get_change_type(index_status)
                    content_hash = None
                    
                    if change_type in ['added', 'modified'] and os.path.exists(abs_file_path):
                        content_hash = self._compute_file_hash(abs_file_path)
                        logger.info("GIT STATUS DEBUG - Computed hash for %s: %s", abs_file_path, content_hash)
                    else:
                        logger.info("GIT STATUS DEBUG - Skipping hash for %s: change_type=%s, exists=%s", 
                                   abs_file_path, change_type, os.path.exists(abs_file_path))
                ...
                if worktree_status != ' ':
                    if worktree_status == '?':
                        # Untracked file
                        content_hash = self._compute_file_hash(abs_file_path) if os.path.exists(abs_file_path) else None
                        change = GitFileChange(
                            file_path=file_path,
                        ...
                        change_type = self._get_change_type(worktree_status)
                        content_hash = None
