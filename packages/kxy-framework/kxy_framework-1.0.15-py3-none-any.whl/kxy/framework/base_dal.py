...
class BaseDal(Generic[T]):
    ...
    UID_Field = 'UID'
    Status_Field = 'Status'
    IsDelete_Field = 'IsDelete'
    CreateUser_Field = 'CreateUser'
    CreateDate_Field = 'CreateDate'
    LastModifiedUser_Field = 'LastModifiedUser'
    LastModifiedDate_Field = 'LastModifiedDate'
    ...
    async def Insert(self, entity):
        setattr(entity,self.CreateUser_Field,self.UserId)
        setattr(entity,self.CreateDate_Field,datetime.now())
        setattr(entity,self.LastModifiedUser_Field,self.UserId)
        setattr(entity,self.LastModifiedDate_Field,datetime.now())
    ...
    async def Delete(self, id):
        ...
        setattr(entity,self.Status_Field,10)
        setattr(entity,self.IsDelete_Field,1)
        self.AddLogger('逻辑删除',f'删除id为{id}的数据')
        await self.Update(entity)
    async def DeleteBatch(self,ids,status=10):
        if len(ids):
            fil=[getattr(self.modal,self.Id_field).in_(status)]
            self.AddLogger('批量逻辑删除',f'删除id为{ids}的数据')
            await self.UpdateFields(fil,{self.Status_Field: status})

    async def TrueDel(self, id):
        self.AddLogger('物理删除',f'删除id为{id}的数据')
        if self.AutoCommit:
            fil = [getattr(self.modal,self.Id_field) == str(id)]
    ...
    async def TrueDelBatch(self,ids):
        if len(ids):
            ...
            if self.AutoCommit:
                fil = [getattr(self.modal,self.Id_field).in_(ids)]
                fil = self._add_extend_filter(fil)
    ...
    async def GetExistByUser(self, id)->T:
        '''获取id和uid为当前用户数据'''
        exist = await self.session.execute(select(self.modal).filter(getattr(self.modal,self.Id_field) == str(id),getattr(self.modal,self.UID_Field) == self.UserId).limit(1))
        result = exist.scalar()
        await self.IfCommit()
        if result is None:
            raise FriendlyException('不存在' + str(id))
    ...
    async def GetByUser(self):
        return await self.QueryOne([getattr(self.modal,self.UID_Field)==self.UserId])
