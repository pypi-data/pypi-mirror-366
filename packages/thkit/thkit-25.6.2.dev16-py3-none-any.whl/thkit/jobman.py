...
def submit_job_chunk(
    ...
    for chunk_index, task_list_current_chunk in enumerate(chunks):
        ...
        try:
            ...
            err_text = f"{time.strftime('%b%d_%H%M')} ERROR: Machine {machine_index} has error job: \n\t{e}"
            print(text_color(err_text, color=_COLOR_MAP[machine_index]))
        old_time = new_time
    return
...
def _remote_info(machine_dict) -> str:
    ...
    info_text = f"{' ' * 10}Remote host: {hostname}\n"
    info_text += f"{' ' * 10}Remote path: {remote_path}"
    return info_text
