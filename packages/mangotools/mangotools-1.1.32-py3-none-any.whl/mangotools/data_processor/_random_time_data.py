...
class RandomTimeData:
    ...
    @classmethod
    def time_now_ymdhms(cls, minute) -> str:
        """当前年月日时分秒,参数：minute"""
        if minute is None:
            minute = 0
    ...
    @classmethod
    def get_time_for_min(cls, minute) -> int:
    ...
    @classmethod
    def time_next_minute(cls, minute) -> str:
        """几分钟后的年月日时分秒 参数：分钟"""
        if minute is None:
            minute = 1
    ...
    @classmethod
    def time_day_reduce(cls, types) -> int:
        """获取今日日期的数字,参数：types"""
    ...
    @classmethod
    def time_day_plus(cls, types) -> int:
        """获取今日日期的数字,参数：types"""
        today = datetime.today()
        if types:
            return today.day + int(types)
