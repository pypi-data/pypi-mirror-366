...
from dens.models import (
    DenOwner,
    DiscordWebhook,
    MercenaryDen,
    MercenaryDenReinforcedNotification,
)
...
def create_reinforce_notification(notification_json: dict):
    ...
    if reinforce_notification.is_in_future():
        ...
        send_webhook_notification.delay(reinforce_notification.id)
...
def send_webhook_notification(notification_id: int):
    """Send reinforce notification to all active webhooks"""
    logger.info("Sending notification id %d to webhooks", notification_id)

    notification = MercenaryDenReinforcedNotification.objects.get(id=notification_id)

    content = (
        f"Mercenary den located in [{notification.den.location.name}]"
        f"({dotlan_solar_system_url(notification.den.location.name)}) is under attack"
    )

    fields = [
        ["Reinforced by", notification.reinforced_by.character_name],
        ["Exit reinforcement", notification.exit_reinforcement.isoformat()],
    ]
    for webhook in DiscordWebhook.objects.active_webhooks():
        webhook.send_alert(
            title="Mercenary Den Reinforced",
            content=content,
            fields=fields,
            author_name=notification.den.owner.character_name,
            author_icon_url=notification.den.owner.character_ownership.character.portrait_url_64,
        )
