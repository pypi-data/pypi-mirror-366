...
class InitCommands:
    ...
    def init_workspace(self, workspace_name: str | None = None) -> bool:
        ...
        try:
            ...
            # Step 8.5: Start Docker containers
            current_step += 1
            if not self._start_docker_containers(workspace_path, container_services, postgres_config):
                print("⚠️ Container creation failed but workspace configuration is complete")
                print("You can manually start containers later using:")
    ...
    def _select_container_services(self) -> list[str]:
        ...
        print("   • Genie Development Assistant (port 48886)")
        print()
...
"""
    ...
    def _start_docker_containers(
        ...
        """Start Docker containers for the selected services."""
        try:
            ...
            try:
                for service in container_services:
                    if service == "postgres" and postgres_config["type"] == "docker":
                        ...
                        # For agent service, we would typically start it via a different compose file
                        # For now, just acknowledge it's configured
                        print("✅ Agent environment configured (start with --agent-serve)")
                    
                    elif service == "genie":
                        print("🧞 Starting Genie Development Assistant...")
                        # For genie service, similar approach
                        print("✅ Genie assistant configured (start with --genie-serve)")
                
            finally:
                os.chdir(original_cwd)
            
            if success:
                print("🎉 All selected containers started successfully!")
            else:
                print("⚠️ Some containers failed to start")
