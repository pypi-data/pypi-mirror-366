...
class SmartVisionCNN:
    """
    SmartVisionCNN is a deep learning utility for image classification using EfficientNet.
    It supports flexible input types, grayscale or RGB images, training with callbacks,
    custom loss functions, and prediction from image paths, arrays, or tensors.
    """

    def __init__(self, input_shape=(224, 224, 3), num_classes=2, loss_function=None):
        """
        Initialize SmartVisionCNN model with EfficientNet backbone.

        Args:
            input_shape (tuple): Shape of input images.
            num_classes (int): Number of classification classes.
            loss_function (str or callable): Loss function name or function.
        """
        self.input_shape = input_shape
        self.num_classes = num_classes
        self.loss_function = loss_function or SparseCategoricalCrossentropy()
        self.model = self._BuildModel()
        self.class_names = []

    def _BuildModel(self):
        """
        Build and compile an EfficientNet-based model.
