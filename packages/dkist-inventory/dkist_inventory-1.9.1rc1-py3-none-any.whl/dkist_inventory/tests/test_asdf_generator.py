...
def test_old_vbi_mosaic_treatment(simulated_dataset):
    # This can be removed once all old VBI dataset are reprocessed
    ds = simulated_dataset("old-vbi-mosaic")
    files = list(ds.glob("*.fits"))
    files.sort()
    tree = asdf_tree_from_filenames(files)
    dataset = tree["dataset"]
    ...
    for nindex1, nindex2 in np.ndindex(dataset.shape):
        # The 3 - checks the flip and the tying of MINDEX1 to nindex2, etc. checks the striding
        assert dataset[nindex1, nindex2].headers["MINDEX1"] == 3 - nindex2
        assert dataset[nindex1, nindex2].headers["MINDEX2"] == nindex1 + 1
