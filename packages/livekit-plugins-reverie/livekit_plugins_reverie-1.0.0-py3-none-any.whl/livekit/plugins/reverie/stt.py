...
class STT(stt.STT):
    def __init__(
        ...
        *,
        ...
        reverie_api_key = (
            api_key if is_given(api_key) else os.environ.get("REVERIE_API_KEY")
        )
        ...
        reverie_app_id = (
            app_id if is_given(app_id) else os.environ.get("REVERIE_APP_ID")
        )
        ...
        if logging not in ("true", "no_audio", "no_transcript", "false"):
            raise ValueError(
                "Logging must be one of: 'true', 'no_audio', 'no_transcript', 'false'"
            )
            ...
            while True:
                try:
                    ...
                    raise APIConnectionError(
                        "failed to receive message from reverie"
                    ) from e
                ...
                if msg.type in (
                    ...
                    raise APIStatusError(
                        message="reverie connection closed unexpectedly"
                    )
