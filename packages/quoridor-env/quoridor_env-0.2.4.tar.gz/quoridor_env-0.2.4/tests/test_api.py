...
class TestFastAPIEndpoints(unittest.TestCase):
    ...
    def test_get_session_state_structure(self):
        """Test getting session state and validate observation structure."""
        # Create a session first
        create_response = self.client.post(
            "/api/v1/sessions",
            json={"players": 2}
        ...
        self.assertIn("players", observation)
        self.assertIn("walls", observation)
        
        # Validate players structure (this was the bug - accessing walls_left instead of walls_remaining)
        players = observation["players"]
        self.assertEqual(len(players), 2)  # 2-player game
        
        for i, player in enumerate(players):
            self.assertIn("id", player)
            self.assertIn("x", player)
            self.assertIn("y", player)
            self.assertIn("walls_remaining", player)  # This field was causing the crash
            self.assertEqual(player["id"], i)
            self.assertIsInstance(player["x"], int)
            self.assertIsInstance(player["y"], int)
            self.assertIsInstance(player["walls_remaining"], int)
            self.assertGreaterEqual(player["walls_remaining"], 0)
            self.assertLessEqual(player["walls_remaining"], 10)  # Max walls per player
        
        # Validate walls structure
        walls = observation["walls"]
        self.assertIn("horizontal", walls)
        self.assertIn("vertical", walls)
        self.assertIsInstance(walls["horizontal"], list)
        self.assertIsInstance(walls["vertical"], list)
