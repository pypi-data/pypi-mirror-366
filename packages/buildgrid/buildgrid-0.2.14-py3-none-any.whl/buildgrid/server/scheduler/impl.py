...
class Scheduler:
    ...
    def _match_job_to_bot(self, session: Session, job: JobEntry, failure_backoff: float) -> None:
        bot_statement = (
            select(BotEntry)
    ...
    def assign_job_by_priority(self, failure_backoff: float = 5.0) -> int:
        ...
        with self._sql.session() as session:
            ...
            if job is not None:
                self._match_job_to_bot(session, job, failure_backoff)
                session.flush()
                updated = True
        return 1 if updated else 0

    @timed(METRIC.SCHEDULER.ASSIGNMENT_DURATION)
    def assign_job_by_age(self, failure_backoff: float = 5.0) -> int:
