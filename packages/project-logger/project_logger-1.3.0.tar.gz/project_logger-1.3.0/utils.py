...
LOCAL_ACTIVATION_DIR = "~/.project_activation"
LOCAL_ACTIVATION_FILE_PREFIX = "activation_"
...
def generate_local_activation_filename(host_id, project_uuid, system_uuid_content):
    """生成本地激活文件名，确保项目和主机的唯一性"""
    # 使用主机ID、项目UUID和系统UUID内容生成唯一标识
    combined = f"{host_id}-{project_uuid}-{system_uuid_content}"
    hash_id = hashlib.sha256(combined.encode()).hexdigest()
    return f"{LOCAL_ACTIVATION_FILE_PREFIX}{hash_id[:16]}.dat"


def get_local_activation_file_path(host_id, project_uuid, system_uuid_content):
    """获取本地激活文件路径"""
    activation_dir = Path(LOCAL_ACTIVATION_DIR).expanduser()
    activation_dir.mkdir(exist_ok=True, parents=True)
    filename = generate_local_activation_filename(host_id, project_uuid, system_uuid_content)
    return activation_dir / filename


def create_local_activation_file(host_id, project_uuid, system_uuid_content,
                               project_path, is_activated, project_number_id=None):
    """创建或更新本地激活文件"""
    try:
        activation_file = get_local_activation_file_path(host_id, project_uuid, system_uuid_content)
        ...
        # 加密数据
        encrypted_data = encrypt_activation_data(activation_data)
        ...
        # 设置文件为隐藏
        set_file_hidden(str(activation_file))

        # logger.info(f"Local activation file created/updated: {activation_file}")
        return True

    except Exception as e:
        # logger.error(f"Failed to create local activation file: {e}")
        return False


def read_local_activation_file(host_id, project_uuid, system_uuid_content):
    """读取本地激活文件"""
    try:
        activation_file = get_local_activation_file_path(host_id, project_uuid, system_uuid_content)

        if not activation_file.exists():
            # logger.debug("Local activation file does not exist")
            return None
        ...
        # 解密数据
        activation_data = decrypt_activation_data(encrypted_data)

        if activation_data is None:
            # logger.warning("Failed to decrypt local activation file")
            return None

        # 验证数据完整性
        if not validate_activation_data(activation_data, host_id, project_uuid, system_uuid_content):
            # logger.warning("Local activation file validation failed")
            return None

        return activation_data

    except Exception as e:
        # logger.error(f"Failed to read local activation file: {e}")
        return None


def encrypt_activation_data(data):
    """加密激活数据"""
    try:
        import json
        import base64
        ...
        # logger.error(f"Failed to encrypt activation data: {e}")
        return None


def decrypt_activation_data(encrypted_data):
    """解密激活数据"""
    try:
        import json
        import base64
        ...
        for i, byte in enumerate(encrypted_bytes):
            decrypted_bytes.append(byte ^ ord(key[i % len(key)]))
        ...
        # logger.error(f"Failed to decrypt activation data: {e}")
        return None
...
def get_encryption_key():
    ...
    # 使用主机特征生成固定的加密密钥
    host_info = get_host_info()
    key_source = f"{host_info['hostname']}-{host_info['mac_address']}-{host_info['cpu_id']}"
    return hashlib.sha256(key_source.encode()).hexdigest()[:32]


def validate_activation_data(data, expected_host_id, expected_project_uuid, expected_system_uuid):
    """验证激活数据的完整性"""
    try:
        # 检查必要字段
        required_fields = ['host_id', 'project_uuid', 'system_uuid_content', 'is_activated']
        for field in required_fields:
            if field not in data:
                # logger.warning(f"Missing required field in activation data: {field}")
                return False

        # 验证主机和项目绑定
        if (data['host_id'] != expected_host_id or
            data['project_uuid'] != expected_project_uuid or
            data['system_uuid_content'] != expected_system_uuid):
            # logger.warning("Activation data does not match current host/project")
            return False

        return True

    except Exception as e:
        # logger.error(f"Failed to validate activation data: {e}")
        return False


def check_local_activation_status(host_id, project_uuid, system_uuid_content, project_path):
    """检查本地激活状态"""
    try:
        activation_data = read_local_activation_file(host_id, project_uuid, system_uuid_content)

        if activation_data is None:
            # logger.debug("No valid local activation data found")
            return False, None

        # 检查项目路径是否匹配（允许项目迁移，使用相对路径比较）
        stored_path = activation_data.get('project_path', '')
        current_path = project_path

        # 提取项目名称进行比较（处理项目迁移情况）
        stored_project_name = Path(stored_path).name if stored_path else ''
        current_project_name = Path(current_path).name if current_path else ''

        if stored_project_name and current_project_name:
            if stored_project_name != current_project_name:
                # logger.warning(f"Project name mismatch: stored={stored_project_name}, current={current_project_name}")
                logger.warning(f"")
                # 项目名称不匹配，但不直接返回False，而是记录警告

        is_activated = activation_data.get('is_activated', False)
        project_number_id = activation_data.get('project_number_id')

        # logger.info(f"Local activation status: {is_activated}, project_number_id: {project_number_id}")
        return is_activated, project_number_id

    except Exception as e:
        # logger.error(f"Failed to check local activation status: {e}")
        return False, None