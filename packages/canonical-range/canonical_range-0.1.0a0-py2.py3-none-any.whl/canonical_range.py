...
class CanonicalRange(Sequence[int]):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, int):
            ...
            offset_start, canonicalized_offset_stop, offset_step = slice_to_offset_range(self.length, index)

            if offset_step > 0:
                # `offset_start < canonicalized_offset_stop`
                left, intersection, right = split_interval((0, self.length), (offset_start, canonicalized_offset_stop),
                                                           lt)
                if intersection is not None:
                    intersection_offset_start, intersection_offset_stop = intersection
                elif left is not None:
                    intersection_offset_start = 0
                    intersection_offset_stop = 0
                else:
                    intersection_offset_start = self.length
                    intersection_offset_stop = self.length
            else:
                # `offset_start > canonicalized_offset_stop`
                reverse_left, reverse_intersection, reverse_right = split_interval(
                    (self.length - 1, -1),
                    (offset_start, canonicalized_offset_stop),
                    gt
                )
                if reverse_intersection is not None:
                    intersection_offset_start, intersection_offset_stop = reverse_intersection
                elif reverse_left is not None:
                    intersection_offset_start = self.length - 1
                    intersection_offset_stop = self.length - 1
                else:
                    intersection_offset_start = -1
                    intersection_offset_stop = -1

            new_start = self.start + self.step * intersection_offset_start
            new_stop = self.start + self.step * intersection_offset_stop
            new_step = self.step * offset_step
