...
from .core.shared import build_component_with_inserted_defaults
...
class UpdateProgressMessage(BaseModel):
...
def ensure_recursive_netlist(obj: Any) -> dict:
    ...
    if isinstance(obj, str):
        activate_pdk_by_name(get_settings().pdk.name)
        if "\n" in obj or obj.endswith(".pic.yml"):
            c = from_yaml(obj)
        else:
            c = build_component_with_inserted_defaults(obj)
