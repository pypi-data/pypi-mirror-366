...
class WaveletFractalAnalyzer:
    ...
    def _continuous_wavelet_transform(self, audio):
        """連続ウェーブレット変換"""
        try:
            # Morlet wavelet CWT using PyWavelets or fallback
            if pywt is not None:
                # PyWavelets implementation
                cwt_coeffs, frequencies = pywt.cwt(audio, self.wavelet_scales, 'morl', sampling_period=1.0/self.sample_rate)
            else:
                # Fallback implementation using scipy
                print("CWT解析エラー: module 'scipy.signal' has no attribute 'cwt'")
                # Simple approximation using spectrogram
                f, t, Sxx = signal.spectrogram(audio, self.sample_rate, nperseg=256)
                # Interpolate to match wavelet scales
                cwt_coeffs = np.zeros((len(self.wavelet_scales), len(audio)))
                for i, scale in enumerate(self.wavelet_scales):
                    freq_idx = int(scale * len(f) / max(self.wavelet_scales))
                    if freq_idx < len(f):
                        cwt_coeffs[i, :len(Sxx[freq_idx])] = Sxx[freq_idx]
