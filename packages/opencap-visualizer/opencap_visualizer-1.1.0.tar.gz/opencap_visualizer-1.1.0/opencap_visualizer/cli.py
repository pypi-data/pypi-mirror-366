        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                await self.page.evaluate(f"""
                    ...
                    // Use MP4 for direct recording (no conversion needed)
                    window.sessionComponent.recordingFormat = 'mp4';
                    
                    console.log('[Headless] Configured recording: {loop_count} loop(s), mp4 format');
                ...
                # Set all recording parameters exactly as they would be set manually
                await self.page.evaluate("""
                    // Set recording parameters exactly as they would be set manually
                    window.sessionComponent.recordingFormat = 'webm';  // Use WebM for best compatibility
                    window.sessionComponent.videoBitrate = 2000000;   // 2 Mbps for better performance
                    window.sessionComponent.loopCount = 1;            // 1 loop
                ...
                """)
                self._log("Started recording with manual-like parameter setup...")
                ...
                try:
                    ...
                    if actual_format == 'mp4':
                        ...
                        try:
                            ...
                            # Keep the WebM file for comparison
                            webm_path = output_video_path.replace('.mp4', '.webm')
                            Path(temp_webm_path).rename(webm_path)
                            self._log(f"Video saved as: {output_video_path} (converted from WebM to H.264 MP4)")
                            self._log(f"Original WebM saved as: {webm_path} (for comparison)")
                            
                        except FileNotFoundError:
                            # ffmpeg not installed - save as webm
                            Path(temp_webm_path).unlink()
                            webm_path = output_video_path.replace('.mp4', '.webm')
                            await download.save_as(webm_path)
                            self._log(f"Video saved as: {webm_path} (WebM format - install ffmpeg for MP4 conversion)")
