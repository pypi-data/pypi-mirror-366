...
class HexGrid(ABC):
    ...
    """
    def __init__(self, dimensions: tuple[int, int], square: bool = False):
        if not square:
            self.grid = [[Hex(q, s) for q in range(dimensions[0])] for s in range(dimensions[1])]
        else:
            self.grid = [[Hex(q, r) for q in range(dimensions[0]+dimensions[1]//2)]
                         for r in range(0-dimensions[0], dimensions[1], 1)]
...
class FoVGrid(HexGrid):
    def __init__(self, dimensions: tuple[int, int], blocking: list[tuple[int, int]]):
        super().__init__(dimensions=dimensions)
        self.blocking_hexes = [h for h in self.flat_grid if (h.q, h.r) in blocking]
        self.visible_grid = {h: False for h in self.flat_grid}

    def compute_fov(self, origin: Hex, fov: int):
        ...
        for radius in range(1, fov+1):
            ...
            for ring_hex in ring:
                if ring_hex in self.flat_grid:
                    ...
                    if all(h not in self.blocking_hexes for h in line_hexes) and ring_hex not in self.blocking_hexes:
