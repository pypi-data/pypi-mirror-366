...
def max_output_energy_sync(
    ...
    for phase_offset in range(sps):
        ...
        phase_energies.append(current_energy)
    ...
    # Find the optimal phase offset
    # optimal_phase2 = np.argmax(phase_energies)
    ...
    if plot:
        ...
        plt.plot(range(sps), phase_energies, marker="o", linestyle="-")
        plt.axvline(
            optimal_phase,
            color="r",
            linestyle="--",
...
def gen_rand_qam_symbols(N, M=4, seed=None):
    ...
    symbols = rng.choice(const, size=N, replace=True)
