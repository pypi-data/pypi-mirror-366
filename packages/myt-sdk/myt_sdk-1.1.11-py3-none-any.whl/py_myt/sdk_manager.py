...
class MYTSDKManager:
    ...
    def start_sdk(self, wait: bool = False, show_window: bool = False) -> subprocess.Popen:
        ...
        try:
            ...
            if sys.platform == "win32":
                if show_window:
                    # show_window=True时，静默运行，不显示窗口
                    creation_flags = subprocess.CREATE_NO_WINDOW | subprocess.CREATE_NEW_PROCESS_GROUP
                else:
                    # show_window=False时，正常运行，可以显示窗口
            ...
            if show_window:
                # show_window=True时，不弹出CMD也不重定向日志，静默运行
                ...
                # show_window=False时，正常运行重定向操作到日志文件
            ...
            if process.poll() is not None:
                ...
                if not show_window:
                    # show_window=False时才尝试读取日志文件（因为此时有重定向）
                    try:
