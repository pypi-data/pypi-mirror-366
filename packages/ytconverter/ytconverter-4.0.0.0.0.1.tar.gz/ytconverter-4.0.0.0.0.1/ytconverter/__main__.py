...
"""
version , version_type =llv()
def main():
    if "-U" in sys.argv or "--update" in sys.argv or "-u" in sys.argv:
        from ytconverter.utils.update import update_self
        update_self()
        sys.exit()

    elif "-v" in sys.argv or "--version" in sys.argv or "-V" in sys.argv:
        print(f"YTConverter version: {version}")
        sys.exit()
