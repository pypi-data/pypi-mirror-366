...
def print_test_results(results, mock_mode=False):
    ...
    if "mixed_precision" in results:
        ...
        # FP16 - depends on hardware
        if mp.get("fp16", {}).get("supported"):
            tflops = mp["fp16"].get("tflops", 0)
            speedup = mp.get("fp16_speedup", 0)
            indicator = " 🟡 HARDWARE-DEPENDENT" if mock_mode else ""
            print(f"  FP16: Supported ({speedup:.2f}x speedup, {tflops:.2f} TFLOPS){indicator}")
        else:
            indicator = " 🟡 HARDWARE-DEPENDENT" if mock_mode else ""
            print(f"  FP16: Not supported{indicator}")
        
        # BF16 - depends on hardware
        if mp.get("bf16", {}).get("supported"):
            tflops = mp["bf16"].get("tflops", 0)
            speedup = mp.get("bf16_speedup", 0)
            indicator = " 🟡 HARDWARE-DEPENDENT" if mock_mode else ""
            print(f"  BF16: Supported ({speedup:.2f}x speedup, {tflops:.2f} TFLOPS){indicator}")
        else:
            indicator = " 🟡 HARDWARE-DEPENDENT" if mock_mode else ""
            print(f"  BF16: Not supported{indicator}")
        
        # INT8 - depends on hardware
        if mp.get("int8", {}).get("supported"):
            tflops = mp["int8"].get("tflops", 0)
            speedup = mp.get("int8_speedup", 0)
            indicator = " 🟡 HARDWARE-DEPENDENT" if mock_mode else ""
            print(f"  INT8: Supported ({speedup:.2f}x speedup, {tflops:.2f} TFLOPS){indicator}")
        else:
            indicator = " 🟡 HARDWARE-DEPENDENT" if mock_mode else ""
            print(f"  INT8: Not supported{indicator}")
