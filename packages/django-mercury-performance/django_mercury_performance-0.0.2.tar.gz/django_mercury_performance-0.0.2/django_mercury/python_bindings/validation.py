...
from .constants import (
    MAX_VALUES, RESPONSE_TIME_THRESHOLDS, MEMORY_THRESHOLDS,
    QUERY_COUNT_THRESHOLDS, N_PLUS_ONE_THRESHOLDS
)
...
THRESHOLD_SCHEMA = {
    ...
    "properties": {
        ...
        "memory_overhead_mb": {
            "type": "number",
            "minimum": 0,
            "maximum": MAX_VALUES['MEMORY_MB']
        },
        "query_count_max": {
            "type": "integer",
            "minimum": 0,
            "maximum": MAX_VALUES['QUERY_COUNT']
        },
        "cache_hit_ratio_min": {
            "type": "number",
            "minimum": 0,
...
def validate_thresholds(thresholds: Dict[str, Union[int, float]]) -> Tuple[bool, Optional[List[str]]]:
    """
...
def validate_operation_name(operation_name: str) -> Tuple[bool, Optional[str]]:
    ...
    if len(operation_name) > MAX_VALUES['OPERATION_NAME_LENGTH']:
        return False, f"Operation name exceeds maximum length of {MAX_VALUES['OPERATION_NAME_LENGTH']}"
...
def validate_metrics_values(
    ...
    if response_time < 0:
        ...
        errors.append(f"Response time {response_time}ms exceeds maximum {MAX_VALUES['RESPONSE_TIME_MS']}ms")
...
def load_and_validate_config(config_path: Union[str, Path]) -> Tuple[Optional[Dict[str, Any]], Optional[List[str]]]:
    """
