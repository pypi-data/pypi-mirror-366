...
try:
    ...
    C_EXTENSIONS_AVAILABLE = False  # Disable conflicting metrics_engine to prevent struct incompatibility
except ImportError:
...
# Load the enhanced C library with fallback
...
try:
    ...
    logger.info(f"Successfully loaded performance library from {lib_path}")
except Exception as e:
    logger.error(f"Failed to load C performance library: {e}")
...
class EnhancedPerformanceMetrics_Python:
    ...
    def __init__(self, c_metrics: ctypes.POINTER(EnhancedPerformanceMetrics), operation_name: str, query_tracker=None) -> None:
        ...
        if lib:
            self.query_count = lib.get_query_count(c_metrics) if hasattr(lib, 'get_query_count') else 0
            self.cache_hits = lib.get_cache_hit_count(c_metrics) if hasattr(lib, 'get_cache_hit_count') else 0
            self.cache_misses = lib.get_cache_miss_count(c_metrics) if hasattr(lib, 'get_cache_miss_count') else 0
            self.cache_hit_ratio = lib.get_cache_hit_ratio(c_metrics) if hasattr(lib, 'get_cache_hit_ratio') else 0.0
            
            # Fallback to Python query tracker if C returns 0 and tracker exists
            if self.query_count == 0 and hasattr(self, '_query_tracker_ref') and self._query_tracker_ref:
    ...
    def _analyze_django_issues(self, c_metrics: ctypes.POINTER(EnhancedPerformanceMetrics)) -> DjangoPerformanceIssues:
        ...
        estimated_cause = lib.estimate_n_plus_one_cause(c_metrics) if lib and hasattr(lib, 'estimate_n_plus_one_cause') else 0
        fix_suggestion_bytes = lib.get_n_plus_one_fix_suggestion(c_metrics) if lib and hasattr(lib, 'get_n_plus_one_fix_suggestion') else None
        fix_suggestion = fix_suggestion_bytes.decode('utf-8') if fix_suggestion_bytes else "Use Django's select_related() or prefetch_related() to optimize queries"
        ...
        has_severe = bool(lib.detect_n_plus_one_severe(c_metrics)) if lib and hasattr(lib, 'detect_n_plus_one_severe') else False
        has_moderate = bool(lib.detect_n_plus_one_moderate(c_metrics)) if lib and hasattr(lib, 'detect_n_plus_one_moderate') else False
        has_pattern = bool(lib.detect_n_plus_one_pattern_by_count(c_metrics)) if lib and hasattr(lib, 'detect_n_plus_one_pattern_by_count') else False
        severity_level = lib.calculate_n_plus_one_severity(c_metrics) if lib and hasattr(lib, 'calculate_n_plus_one_severity') else 0
...
class EnhancedPerformanceMonitor:
    ...
    def enable_educational_guidance(self, operation_context: Optional[Dict[str, Any]] = None) -> Self:
    ...
    def _generate_educational_guidance(self, violations: List[str]) -> str:
        ...
        if has_query_violation:
            ...
            if self._metrics and self._metrics.query_count > 10:
                guidance_lines.append("💡 Possible N+1 query issue detected - consider using select_related() and prefetch_related()")
                if self.operation_type == "detail_view":
                    guidance_lines.append("   → queryset = Model.objects.select_related('related_field').prefetch_related('many_to_many_field')")
                elif self.operation_type == "list_view":
                    guidance_lines.append("   → Implement pagination and optimize QuerySet with select_related()/prefetch_related()")
            else:
                guidance_lines.append("💡 Query count exceeded - review database access patterns")
                guidance_lines.append("   → Consider adjusting query_count_max threshold if current count is reasonable")
        
        if has_response_violation:
            guidance_lines.append("⏱️  Response time exceeded - optimize performance")
            guidance_lines.append("   → Check database indexes, reduce query complexity, or adjust response_time_ms threshold")
        
        if has_memory_violation:
            guidance_lines.append("🧠 Memory usage exceeded - optimize memory consumption")
            guidance_lines.append("   → Reduce data loading, implement pagination, or adjust memory_overhead_mb threshold")
        ...
        if guidance_lines:
            ...
            if has_memory_violation and self._metrics:
                ...
                if memory_value is None:
                    memory_value = getattr(self._metrics, 'memory_usage', 100)  # Default to 100MB if not found
            ...
            if threshold_suggestions:
                guidance_lines.append(f"   cls.set_performance_thresholds({{{', '.join(threshold_suggestions)}}})")
    ...
    def __enter__(self) -> Self:
        ...
        try:
            from .c_bindings import c_extensions
            if c_extensions.metrics_engine and hasattr(c_extensions.metrics_engine, 'reset_global_counters'):
        ...
        # Start C-level monitoring with thread-safe libperformance.so
        if lib:
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        ...
        if self.handle is not None and lib:
            # Stop C-level monitoring with thread-safe libperformance.so
            ...
            if c_metrics:
                ...
                self._metrics = EnhancedPerformanceMetrics_Python(c_metrics, self.operation_name, self._query_tracker)
            ...
            if self._test_file and self._test_line and self._test_method:
                ...
                try:
                    ...
                    while frame and frame_count < 50:  # Look at more frames
                        ...
                        if ('performance_testing' in filename or
                            '/unittest/' in filename or
                            'unittest/case.py' in filename or
                        ...
                        try:
                            ...
                            try:
                                ...
                                if relative_path:
                                    ...
                                    # Final fallback: just show filename
                                    error_msg += f" [📁 {Path(test_file).name}:{test_line} in {test_name}()]"
                            except (IndexError, ValueError):
                                # Final fallback: just show filename
                                error_msg += f" [📁 {Path(test_file).name}:{test_line} in {test_name}()]"
