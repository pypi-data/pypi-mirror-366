...
class DjangoMercuryAPITestCase(DjangoPerformanceAPITestCase):
            ...
            try:
                ...
                if test_executed and metrics:
                    ...
                    if not self._verbose_reporting:
                        ...
                        logger.info(f"✓ {original_method.__name__} (Grade {metrics.performance_score.grade} - {metrics.response_time:.1f}ms, {metrics.query_count}Q)")
    ...
    def _provide_technical_diagnostics(self, test_name: str, error_msg: str, operation_type: str, context: Dict[str, Any]):
        ...
        try:
            while frame:
                ...
                if ('/tests/' in filename and 
                    (function.startswith('test_') or 'test_' in function)):
                    test_file = filename
        ...
        print(f"{colors.colorize('📁 File', EduLiteColorScheme.INFO)}: {colors.colorize(f'{test_file}:{test_line}', EduLiteColorScheme.ACCENT)}")
        
        if response_time and expected_time:
            over_time = response_time - expected_time
            print(f"{colors.colorize('⏱️  Response', EduLiteColorScheme.WARNING)}: {colors.colorize(f'{response_time:.0f}ms', EduLiteColorScheme.CRITICAL)} (expected <{expected_time}ms) {colors.colorize(f'+{over_time:.0f}ms over', EduLiteColorScheme.CRITICAL)}")
        
        if query_count and expected_queries:
            extra_queries = query_count - expected_queries
            print(f"{colors.colorize('🗃️  Queries', EduLiteColorScheme.WARNING)}: {colors.colorize(str(query_count), EduLiteColorScheme.CRITICAL)} (expected <{expected_queries}) {colors.colorize(f'+{extra_queries} extra queries', EduLiteColorScheme.CRITICAL)}")
        
        print(f"{colors.colorize('🎯 Score', EduLiteColorScheme.INFO)}: {colors.colorize(f'{score} (failing)', EduLiteColorScheme.CRITICAL)}")
        ...
        if operation_type == 'detail_view' and query_count and query_count > 3:
            print(colors.colorize("Add select_related() to UserRetrieveView queryset", EduLiteColorScheme.OPTIMIZATION))
            print(f"  {colors.colorize('→', EduLiteColorScheme.FADE)} queryset = User.objects.select_related('userprofile').prefetch_related('groups')")
        elif operation_type == 'list_view' and query_count and query_count > 5:
            print(colors.colorize("Implement select_related/prefetch_related for list queries", EduLiteColorScheme.OPTIMIZATION))
        elif response_time and response_time > 200:
            print(colors.colorize("Check database indexes and optimize queries", EduLiteColorScheme.OPTIMIZATION))
        else:
            print(colors.colorize(f"Adjust thresholds: response_time_ms: {int(response_time * 1.5) if response_time else 200}, query_count_max: {query_count + 2 if query_count else 10}", EduLiteColorScheme.OPTIMIZATION))
    
    def _provide_educational_guidance(self, test_name: str, error_msg: str, operation_type: str, context: Dict[str, Any]):
        ...
        if "Response time" in error_msg:
            ...
            if time_match:
                ...
                print(f"⏱️  {colors.colorize('Response Time:', EduLiteColorScheme.WARNING)} {colors.colorize(f'{actual_time:.0f}ms', EduLiteColorScheme.CRITICAL)} (limit: {expected_time}ms)")
                print(f"   {colors.colorize(f'→ {over_time:.0f}ms over limit ({percent_over:.0f}% exceeded)', EduLiteColorScheme.CRITICAL)}")
        ...
        if "Query count" in error_msg:
            ...
            if query_match:
                ...
                print(f"🗃️  {colors.colorize('Query Count:', EduLiteColorScheme.WARNING)} {colors.colorize(str(actual_queries), EduLiteColorScheme.CRITICAL)} (limit: {expected_queries})")
                print(f"   {colors.colorize(f'→ {extra_queries} extra queries ({percent_over:.0f}% exceeded)', EduLiteColorScheme.CRITICAL)}")
        ...
        if "Memory usage" in error_msg:
            ...
            if mem_match:
                ...
                print(f"🧠 {colors.colorize('Memory Usage:', EduLiteColorScheme.WARNING)} {colors.colorize(f'{actual_mem:.1f}MB', EduLiteColorScheme.CRITICAL)} (limit: {expected_mem}MB)")
                print(f"   {colors.colorize(f'→ {over_mem:.1f}MB over limit ({percent_over:.0f}% exceeded)', EduLiteColorScheme.CRITICAL)}")
        ...
        if "Query count" in error_msg:
            ...
            if operation_type == 'delete_view':
                ...
                print(f"   Consider optimizing queries with select_related() and prefetch_related()")
        ...
        if "Memory usage" in error_msg:
            ...
            if mem_match:
                ...
                recommended_thresholds['memory_overhead_mb'] = int(actual_mem - 80) + 5  # Subtract Django baseline
        ...
        if recommended_thresholds:
            ...
            print(f"   cls.set_performance_thresholds({{'response_time_ms': 200, 'query_count_max': 10}})")
