...
@dataclass
class MemoryEntry:
    """Memory entry for long-term memory system."""
...
# Type aliases for memory system
MemoryContent: TypeAlias = Union[str, Dict[str, Any], List[Any]]
MemoryFilter: TypeAlias = Dict[str, Any]
Timestamp: TypeAlias = datetime
Score: TypeAlias = float  # Confidence/relevance scores between 0.0 and 1.0
...
class MemoryStats:
    ...
    memories_by_type: Dict[MemoryType, int] = field(default_factory=dict)
    total_size_bytes: int = 0
    average_relevance_score: Score = 0.0
    oldest_memory: Optional[Timestamp] = None
    newest_memory: Optional[Timestamp] = None
    user_count: int = 0
    active_sessions: int = 0
    cache_hit_rate: Percentage = 0.0


class PrivacyMode(str, Enum):
    """Privacy modes for memory handling."""
    STRICT = "strict"
    BALANCED = "balanced"
...
class CacheStrategy(str, Enum):
    """Cache strategies for memory operations."""
    LRU = "lru"
    LFU = "lfu"
    TTL = "ttl"
    NONE = "none"
