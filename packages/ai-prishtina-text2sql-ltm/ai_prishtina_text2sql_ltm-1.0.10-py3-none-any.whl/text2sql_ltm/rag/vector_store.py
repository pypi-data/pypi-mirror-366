...
class VectorStoreConfig(BaseModel):
    ...
    # Storage settings (for local vector stores like FAISS)
...
def create_vector_store(config: VectorStoreConfig) -> BaseVectorStore:
    ...
    # Import vector store implementations dynamically to avoid dependency issues
    store_map = {
        VectorStoreType.CHROMA: ChromaVectorStore,
    }

    # Try to import each vector store implementation
    try:
        from .pinecone_store import PineconeVectorStore
        store_map[VectorStoreType.PINECONE] = PineconeVectorStore
    except ImportError:
        pass

    try:
        from .faiss_store import FAISSVectorStore
        store_map[VectorStoreType.FAISS] = FAISSVectorStore
    except ImportError:
        pass

    try:
        from .qdrant_store import QdrantVectorStore
        store_map[VectorStoreType.QDRANT] = QdrantVectorStore
    except ImportError:
        pass

    try:
        from .weaviate_store import WeaviateVectorStore
        store_map[VectorStoreType.WEAVIATE] = WeaviateVectorStore
    except ImportError:
        pass

    try:
        from .redis_store import RedisVectorStore
        store_map[VectorStoreType.REDIS] = RedisVectorStore
    except ImportError:
        pass

    try:
        from .pgvector_store import PgVectorStore
        store_map[VectorStoreType.PGVECTOR] = PgVectorStore
    except ImportError:
        pass
