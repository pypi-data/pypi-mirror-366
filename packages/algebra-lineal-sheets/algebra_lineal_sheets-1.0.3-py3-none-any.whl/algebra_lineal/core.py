"""
📚 ALGEBRA LINEAL - Módulo Core (CORREGIDO)
==============================
...
def _obtener_dimensiones_y_tipo(valor):
    ...
    CORREGIDO: Maneja escalares correctamente
    """
...
def workspace(sheet_name=None):
    ...
    CORREGIDO: Detecta escalares 1x1 correctamente
    ...
    try:
        ...
        for i, worksheet in enumerate(spreadsheet.worksheets(), 1):
            ...
            try:
                ...
                # DIMENSIONES REALES en Google Sheets
                filas_sheets = len(datos_filtrados)
                cols_sheets = max(len(fila) for fila in datos_filtrados)

                # CORREGIDO: Determinar tipo basado en layout original
    ...
    for nombre_solicitado in nombres_matrices:
        ...
        if pestaña_encontrada:
            try:
                ...
                if not datos_filtrados:
                    print(
                        f"⚠️  '{nombre_solicitado}' está vacía después del filtrado")
                    continue
                ...
                # CORREGIDO: Determinar tipo y valor preservando formato
                if array_np.size == 1:
                    valor = array_np.item()  # Convertir a escalar Python
                    tipo = "escalar"
                ...
                # CORREGIDO: Mostrar dimensiones de forma segura
...
def _cargar_todas_matrices(spreadsheet):
    ...
    for worksheet in spreadsheet.worksheets():
        ...
        try:
            ...
            # CORREGIDO: Preservar formato original
            if array_np.size == 1:
                valor = array_np.item()  # Convertir a escalar Python
                tipo = "escalar"
            elif array_np.shape[0] == 1 and array_np.shape[1] > 1:
...
def exportar(*nombres_variables, sheet_name=None, sobrescribir=True):
    ...
    CORREGIDO: Maneja escalares sin .shape
...
def _preparar_datos_para_sheets(valor):
    ...
    CORREGIDO: Preserva formato de vectores fila/columna y maneja escalares
    """
    if isinstance(valor, (int, float, np.number)):
        return [[float(valor)]]
    elif isinstance(valor, np.ndarray):
...
def listar_variables_exportables():
    ...
    if variables_numpy:
        ...
        for nombre, valor in variables_numpy.items():
            # CORREGIDO: Usar función segura para obtener dimensiones
...
def version():
    ...
    try:
        ...
        print("👨‍🏫 Sistema de álgebra lineal con Google Sheets")
