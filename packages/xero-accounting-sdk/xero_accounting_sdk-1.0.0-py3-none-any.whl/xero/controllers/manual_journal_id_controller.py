...
class ManualJournalIDController(BaseController):

    """A Controller to access Endpoints in the xero API."""
    def __init__(self, config):
        super(ManualJournalIDController, self).__init__(config)

    def allows_you_to_retrieve_a_specified_manual_journals(self,
                                                           manual_journal_id,
                                                           xero_tenant_id):
        """Does a GET request to /ManualJournals/{ManualJournalID}.
    ...
    def allows_you_to_update_a_specified_manual_journal(self,
                                                        manual_journal_id,
                                                        xero_tenant_id,
                                                        body):
        """Does a POST request to /ManualJournals/{ManualJournalID}.

        Args:
            manual_journal_id (str): (Required) Unique identifier for a
                ManualJournal
            xero_tenant_id (str): (Required) Xero identifier for Tenant
            body (AllowsYouToUpdateASpecifiedManualJournalRequest): The
