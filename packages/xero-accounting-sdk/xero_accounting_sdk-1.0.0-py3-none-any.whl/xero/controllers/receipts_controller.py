...
class ReceiptsController(BaseController):
    ...
    def allows_you_to_retrieve_draft_expense_claim_receipts_for_any_user(self,
                                                                         where,
                                                                         order,
                                                                         unitdp,
                                                                         if_modified_since,
                                                                         xero_tenant_id):
        """Does a GET request to /Receipts.
    ...
    def allows_you_to_create_draft_expense_claim_receipts_for_any_user(self,
                                                                       unitdp,
                                                                       xero_tenant_id,
                                                                       body):
        """Does a PUT request to /Receipts.

        Args:
            unitdp (str): e.g. unitdp=4 – (Unit Decimal Places) You can opt in
                to use four decimal places for unit amounts
            xero_tenant_id (str): (Required) Xero identifier for Tenant
            body (AllowsYouToCreateDraftExpenseClaimReceiptsForAnyUserRequest):
