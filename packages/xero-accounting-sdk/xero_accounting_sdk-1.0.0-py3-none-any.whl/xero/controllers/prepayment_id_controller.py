...
class PrepaymentIDController(BaseController):

    """A Controller to access Endpoints in the xero API."""
    def __init__(self, config):
        super(PrepaymentIDController, self).__init__(config)

    def allows_you_to_retrieve_a_specified_prepayments(self,
                                                       prepayment_id,
                                                       xero_tenant_id):
        """Does a GET request to /Prepayments/{PrepaymentID}.
    ...
    def allows_you_to_create_an_allocation_for_prepayments(self,
                                                           summarize_errors,
                                                           prepayment_id,
                                                           xero_tenant_id,
                                                           body):
        """Does a PUT request to /Prepayments/{PrepaymentID}/Allocations.

        Args:
            summarize_errors (bool): If false return 200 OK and mix of
                successfully created objects and any with validation errors
            prepayment_id (str): (Required) Unique identifier for Prepayment
            xero_tenant_id (str): (Required) Xero identifier for Tenant
            body (AllowsYouToCreateAnAllocationForPrepaymentsRequest): The
