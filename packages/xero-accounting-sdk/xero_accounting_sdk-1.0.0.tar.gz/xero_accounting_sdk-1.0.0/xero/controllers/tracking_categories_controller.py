...
class TrackingCategoriesController(BaseController):

    """A Controller to access Endpoints in the xero API."""
    def __init__(self, config):
        super(TrackingCategoriesController, self).__init__(config)

    def allows_you_to_retrieve_tracking_categories_and_options(self,
                                                               where,
                                                               order,
                                                               include_archived,
                                                               xero_tenant_id):
        """Does a GET request to /TrackingCategories.

        Args:
            where (str): Filter by an any element
            order (str): Order by an any element
            include_archived (str): e.g. includeArchived=true - Categories and
                options with a status of ARCHIVED will be included in the
                response
    ...
    def allows_you_to_create_tracking_categories(self,
                                                 xero_tenant_id,
                                                 body):
        """Does a PUT request to /TrackingCategories.

        Args:
            xero_tenant_id (str): (Required) Xero identifier for Tenant
            body (AllowsYouToCreateTrackingCategoriesRequest): The request
                body parameter.
