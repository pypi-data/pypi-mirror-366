...
class BotClient:
    def __init__(self, token: str, state_file: str = "bot_state.json", rate_limit: float = 0.5, use_webhook: bool = False):
        self.token = token
    ...
    def _load_state(self):
        try:
            with open(self.state_file, "r") as f:
                ...
                self.processed_messages = deque(state.get("processed_messages", []), maxlen=10000)
                logger.info(f"Loaded state: next_offset_id={self.next_offset_id}")
    ...
    async def send_message(
        ...
        try:
            ...
            if response.get("status") == "OK" and "data" in response and "message_id" in response["data"]:
                self.processed_messages.append(str(response["data"]["message_id"]))
                if not self.use_webhook:
                    self._save_state()
                logger.info(f"Message sent successfully to chat_id={chat_id}, message_id={response['data']['message_id']}")
            else:
        ...
        if buttons:
            inline_keypad = Keypad(
                rows=[
                    KeypadRow(
                        buttons=[
                            Button(id=btn["id"], type=ButtonTypeEnum.SIMPLE, button_text=btn["text"])
                            for btn in row
                        ]
                    )
                    for row in buttons
    ...
    async def send_poll(self, chat_id: str, question: str, options: List[str]) -> Dict:
        data = {
            "chat_id": chat_id,
            "question": question,
            "options": options
    ...
    async def get_updates(self, limit: int = 100, offset_id: str = "") -> List[Union[Update, InlineMessage]]:
        ...
        try:
            ...
            self.next_offset_id = response.get("data", {}).get("next_offset_id")
            if not self.use_webhook:
                self._save_state()
        except Exception as e:
    ...
    async def forward_message(
        ...
        if response.get("status") == "OK" and "data" in response and "new_message_id" in response["data"]:
            self.processed_messages.append(str(response["data"]["new_message_id"]))
            if not self.use_webhook:
                self._save_state()
        return response
