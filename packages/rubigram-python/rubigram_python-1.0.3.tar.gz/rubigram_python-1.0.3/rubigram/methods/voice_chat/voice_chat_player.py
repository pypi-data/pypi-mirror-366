...
class VoiceChatPlayer:
    async def heartbeat(self: "rubigram.Client", chat_guid: str, voice_chat_id: str) -> None:
        while True:
            try:
    ...
    async def speaking(self: "rubigram.Client", chat_guid: str, voice_chat_id: str) -> None:
        while True:
            try:
    ...
    async def voice_chat_player(
        ...
        if voice_chat_id is None:
            if chat_guid.startswith("g0"):
                ...
                voice_chat_id = voice_chat.find_keys("voice_chat_id") or getattr(voice_chat.group_voice_chat_update, "voice_chat_id", None)
            else:
                voice_chat = await self.create_channel_voice_chat(chat_guid)
                voice_chat_id = voice_chat.find_keys("voice_chat_id") or getattr(voice_chat.channel_voice_chat_update, "voice_chat_id", None)
