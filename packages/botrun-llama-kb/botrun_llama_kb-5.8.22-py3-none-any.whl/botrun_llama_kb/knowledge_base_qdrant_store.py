...
class KnowledgeBaseQdrantStore(KnowledgeBaseStore):
    ...
    def _setup_llm(self) -> None:
        ...
        self.agent_llm = GeminiMultiModal(
    ...
    def create_hybrid_agent(self, query_engine: RetrieverQueryEngine) -> ReActAgent:
        ...
        query_tool = QueryEngineTool.from_defaults(
            ...
            description=(
                "查詢知識庫中的相關資訊和文件內容。"
                "使用混合搜尋技術，結合語義理解和關鍵字匹配，"
                "提供更準確和全面的搜尋結果。"
                "支援繁體中文、台灣用語和中英混合查詢。"
            ),
        )

        # Enhanced default system prompt for Taiwan Chinese
        agent_prompt = """
        - 你只能根據 tool 的結果回答問題，如果沒有資訊，就回知識庫裡面沒有資訊，你千萬不要自己回答。
        - 你在結尾都會說一個笑話，並加上 emoji
        """
    ...
    async def query_knowledge_base(self, query: str, top_k: int = 3) -> str:
        """查詢知識庫
        ...
        try:
            ...
            print(response)
            # Create hybrid agent
            agent = self.create_hybrid_agent(self.query_engine)
