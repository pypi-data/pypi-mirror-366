...
async def delete_all_sandboxes(
    ...
    """Delete all sandboxes from the cloud and clear local cache

    This tool will delete all async and sync sandboxes associated with your API key
    and clear the local sandbox cache. Use with caution as this operation cannot be undone.

    Returns:
        Dictionary containing deletion results and statistics
    """
    try:
        logger.info("[MCP工具调用 delete_all_sandboxes] 请求删除所有沙箱")
        ...
        # 删除异步沙箱
        try:
            logger.info("正在删除异步沙箱...")
            async_sandboxes = await AsyncSandbox.list(api_key=api_key, domain=domain)
            
            for sandbox in async_sandboxes:
                try:
                    await AsyncSandbox.kill(sandbox.sandbox_id, api_key=api_key, domain=domain)
                    async_deleted += 1
                    logger.debug(f"删除异步沙箱成功: {sandbox.sandbox_id}")
                except Exception as e:
                    logger.warning(f"删除异步沙箱失败 {sandbox.sandbox_id}: {str(e)}")
                    
        except Exception as e:
            logger.error(f"获取异步沙箱列表失败: {str(e)}")

        # 删除同步沙箱
        try:
            logger.info("正在删除同步沙箱...")
            sync_sandboxes = Sandbox.list(api_key=api_key, domain=domain)
            
            for sandbox in sync_sandboxes:
                try:
                    Sandbox.kill(sandbox.sandbox_id, api_key=api_key, domain=domain)
                    sync_deleted += 1
                    logger.debug(f"删除同步沙箱成功: {sandbox.sandbox_id}")
                except Exception as e:
                    logger.warning(f"删除同步沙箱失败 {sandbox.sandbox_id}: {str(e)}")
                    
        except Exception as e:
            logger.error(f"获取同步沙箱列表失败: {str(e)}")

        # 清理本地缓存
        sandbox_manager._sandbox = None
        sandbox_manager._sandbox_info = None
        logger.info("本地沙箱缓存已清理")

        total_deleted = async_deleted + sync_deleted
        logger.info(f"删除操作完成 - 总计删除 {total_deleted} 个沙箱")

        return {
            "success": "true",
            "message": f"Successfully deleted {total_deleted} sandboxes",
            "total_deleted": total_deleted,
            "async_deleted": async_deleted,
            "sync_deleted": sync_deleted
        }

    except Exception as e:
        logger.error(f"[MCP工具错误] 删除沙箱失败: {str(e)}", exc_info=True)
        return {
            "success": "false",
            "error": {
                "name": type(e).__name__,
