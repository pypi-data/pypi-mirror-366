...
# Try importing importlib.resources (Python 3.7+), fallback to pkg_resources
try:
...
def load_temp_domains():
    try:
        # Try to load from package data first (when installed via pip)
        if HAS_IMPORTLIB_RESOURCES is False:
            try:
                # Use pkg_resources fallback
                data = pkg_resources.resource_string('bloodrecon', 'modules/list-imp/temp_domains.txt')
                content = data.decode('utf-8')
                return set(line.strip() for line in content.split('\n') if line.strip() and not line.startswith("#"))
            except Exception:
                pass
        
        # Try relative path approach (works for both development and installed packages)
        script_dir = os.path.dirname(os.path.abspath(__file__))
        file_path = os.path.join(script_dir, 'list-imp', 'temp_domains.txt')
        
        if os.path.exists(file_path):
            with open(file_path, 'r') as f:
                return set(line.strip() for line in f if line.strip() and not line.startswith("#"))
        
        # If the above fails, try from the package root
        import bloodrecon
        package_dir = os.path.dirname(bloodrecon.__file__)
        file_path = os.path.join(package_dir, 'modules', 'list-imp', 'temp_domains.txt')
        
        if os.path.exists(file_path):
            with open(file_path, 'r') as f:
                return set(line.strip() for line in f if line.strip() and not line.startswith("#"))
                
        raise FileNotFoundError("temp_domains.txt not found in any expected location")
            
    except Exception as e:
        print(f"[!] Temp email domain list not found: {str(e)}")
        return set()
