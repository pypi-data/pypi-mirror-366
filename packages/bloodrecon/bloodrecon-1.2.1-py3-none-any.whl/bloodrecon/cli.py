...
def get_wordlist_path(filename):
    """Get the proper path to a wordlist file"""
    try:
        # Try relative path approach (works for both development and installed packages)
        script_dir = os.path.dirname(os.path.abspath(__file__))
        file_path = os.path.join(script_dir, 'modules', 'list-imp', filename)
        ...
        file_path = os.path.join(package_dir, 'modules', 'list-imp', filename)
        
        if os.path.exists(file_path):
            return file_path
            
        # Return None if file not found (module will use default wordlist)
        return None
...
def handle_menu_choice(choice):
    ...
    modules = {
        ...
        '27': lambda: directory_bruteforce.bruteforce_directories(target, get_wordlist_path('common.txt')),
...
Version: {VERSION}
    ...
    if args.about:
        ...
        directory_bruteforce.bruteforce_directories(args.dir_brute, get_wordlist_path('common.txt'))
