...
def deserialize_research_environments(
    ...
    for workbench in workbenches:
        if workbench.get("type") == "Workbench":
            ...
            environments.append(deserialize_entity_scaffolding(workbench))
...
def deserialize_service_errors(service_errors_data: list) -> Iterable[ServiceError]:
    service_errors = []
    for error in service_errors_data:
        ...
        # Log all service errors for monitoring and debugging
        logger.error(
            f"Service Error Detected: {error_obj.error_type} in {error_obj.service_name}",
            extra={
                "error_type": error_obj.error_type,
                "resource_id": error_obj.resource_id,
                "service_name": error_obj.service_name,
                "message": error_obj.message,
                "details": error_obj.details,
                "can_retry": error_obj.can_retry,
                "service_error_context": "workspace_api_response"
            }
        )
