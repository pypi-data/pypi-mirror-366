...
async def search(query: str) -> str:
    ...
    IMPORTANT: Search for the TOOL or ACTION you want to perform, not the outcome you
    want. Think "what tool do I need?" not "what do I want to achieve?"

    Examples:
    - GOOD: "search the web" → finds web search tools
    - BAD: "find scone recipes" → too specific, search for the search tool instead
    - GOOD: "read files" → finds filesystem tools
    - BAD: "check what's in config.json" → search for file reading tools instead
    - GOOD: "execute code" → finds code execution tools
    - BAD: "calculate fibonacci" → search for code/calculation tools instead

    This semantic search understands tool names, descriptions, and capabilities. Use it
