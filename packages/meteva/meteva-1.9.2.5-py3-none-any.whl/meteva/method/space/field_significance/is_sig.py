...
def MCdof(x, ntrials=5000, field_sig=0.05, zfun="rnorm", zfun_args='NULL',
    ...
    for i in range(1, ntrials):
        ...
        if which_test == "cor.test":
            ...
            '''
        tmp_small = tmp[field_sig>tmp]
        if tmp_small.size == 0:
            B_dof_test[i] = 1
        else:
            B_dof_test[i] = np.mean(tmp_small)  # 输出结果
...
def is_sig(X, blockboot_result_df, n=3000, fld_sig=0.05, show=False):
    output = {}
    '''
    X = errfield
