...
class CrystalStructureGenerator:
    ...
    def __init__(self, config: dict, out_dir: Path):
        ...
        self.reps = (rx, ry, rz)
        # Los valores de réplica provistos en la configuración se ignoran, ya
        # que la estructura generada tomará las dimensiones del archivo de
        # defecto para asegurar coincidencia total.
    ...
    def _read_defect_box(self):
        ...
        self.box_center = np.array([(xlo + xhi)/2,
                                    (ylo + yhi)/2,
                                    (zlo + zhi)/2])
        # Dimensiones de la caja del defecto
    ...
    def generate(self) -> Path:
        ...
        Genera una estructura cristalina con las mismas dimensiones y
        centro que el archivo de defecto, escribe relax_structure.dump y
        devuelve su Path.
        """
        coords, dims = self._build_replica(self.reps)
        coords_centered = coords - dims/2
        coords = self._build_replica(self.box_dims)
        coords_centered = coords - self.box_dims/2
        ...
        self._write_dump(coords_aligned, self.box_limits, out_file)
        return out_file

    def _build_replica(self, reps: Tuple[int, int, int]) -> Tuple[np.ndarray, np.ndarray]:
    def _build_replica(self, dims: np.ndarray) -> np.ndarray:
        """
        Construye una red cristalina que cubra completamente las dimensiones
        indicadas en ``dims``. Se generan las repeticiones necesarias y luego
        se recortan los átomos que queden fuera de la caja objetivo.
        """
        base = None
        ...
        reps = np.ceil(dims / self.lattice).astype(int)
        coords = []
        ...
        coords = coords[mask]
        return np.unique(np.round(coords,6), axis=0)
