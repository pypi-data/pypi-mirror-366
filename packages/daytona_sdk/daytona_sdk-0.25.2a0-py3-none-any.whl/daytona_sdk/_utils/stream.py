...
async def process_wss_streaming_response(
    url: str,
    headers: dict,
    ...
    async with websockets.connect(url, additional_headers=headers) as ws:
        while True:
            try:
                # Implement timeout around recv()
                chunk = await asyncio.wait_for(ws.recv(), timeout=chunk_timeout)
                ...
                if should_end:
                    exit_check_streak += 1
                    if not require_consecutive_termination or exit_check_streak > 1:
                        print("Terminating WebSocket stream")
                        await ws.close()
                        break
                else:
                    exit_check_streak = 0
            ...
            if first == 0x01:
                ...
                # optional: unknown first byte
                print(f"Unknown stream type: {first}")
