...
from .._utils.stream import process_streaming_response, process_wss_streaming_response
...
class Process:
    ...
    def get_session_command_logs(self, session_id: str, command_id: str) -> SessionCommandLogsResponse:
        ...
        Returns:
            SessionCommandLogsResponse: Command logs containing: stdout and stderr
        ...
        Example:
            ...
            print(f"Command output: {logs.stdout}")
            print(f"Command error: {logs.stderr}")
            ```
        """
    ...
    async def get_session_command_logs_async(
        ...
        preview_url = self._get_preview_link(2280)
        print(f"Preview URL: {preview_url.url}")
        print(f"OG URL: {url}")
        url = preview_url.url.replace("http", "ws") + url[url.index("/process") :]

        print(f"URL: {url}")
        print(f"Token: {preview_url.token}")
        ...
        #         "x-daytona-preview-token": preview_url.token,
        #     },
        #     on_chunk=on_stdout,
        #     should_terminate=should_terminate,
        # )

        await process_wss_streaming_response(
            url=url,
            headers={
                **self._toolbox_api.api_client.default_headers,
                "x-daytona-preview-token": preview_url.token,
