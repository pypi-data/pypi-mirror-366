# selectivly copied and edited from vllm/transformers_utils/config.py
# https://github.com/vllm-project/vllm/blob/v0.6.6/vllm/transformers_utils/config.py
...
from furiosa_models.transformers_utils.utils import check_gguf_file

MISTRAL_CONFIG_NAME = "params.json"
HF_TOKEN = os.getenv("HF_TOKEN", None)

# TODO: Add custom model classes to the registry when needed
    ...
    **kwargs,
    ...
    if config_format == ConfigFormat.AUTO:
        if is_gguf or file_or_path_exists(model, HF_CONFIG_NAME, revision=revision):
            ...
            file_exists(model, HF_CONFIG_NAME, revision=revision, token=HF_TOKEN)
...
def get_hf_file_to_dict(file_name: str, model: Union[str, Path], revision: Optional[str] = "main"):
    ...
    if file_or_path_exists(model=model, config_name=file_name, revision=revision):
...
def load_params_config(
    ...
    config_dict["max_position_embeddings"] = config_dict.get("max_position_embeddings", 128_000)
