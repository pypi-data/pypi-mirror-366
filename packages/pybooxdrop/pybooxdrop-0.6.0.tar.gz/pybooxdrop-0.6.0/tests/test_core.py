...
def test_boox_base_url_inferred_from_client(mock_client: "Mock", url: BooxUrl):
    mock_client.base_url = url
    boox = Boox(client=mock_client)
    assert boox.base_url is url


def test_explicit_base_url_takes_precedence(mock_client: "Mock"):
    url_taking_precedence = BooxUrl.EUR
    boox = Boox(client=mock_client, base_url=url_taking_precedence)
    assert boox.base_url is url_taking_precedence


def test_client_base_url_overrides_constructor_value(mock_client: "Mock"):
    url_taking_precedence = BooxUrl.EUR
    url_not_taking_precedence = BooxUrl.PUSH
    mock_client.base_url = url_taking_precedence
    boox = Boox(client=mock_client, base_url=url_not_taking_precedence)
    assert boox.base_url is url_taking_precedence


def test_boox_raises_validation_error_for_invalid_url(mock_client: "Mock"):
    mock_client.base_url = "http://random.url"
    with pytest.raises(ValidationError, match="Input should be", check=_shows_all_members):
        Boox(client=mock_client)


@pytest.mark.parametrize("url", list(BooxUrl))
def test_boox_base_url_can_be_set(mock_client: "Mock", url: BooxUrl):
    boox = Boox(client=mock_client)
    boox.base_url = url
    assert boox.base_url == url.value


def test_boox_base_url_set_raises_on_invalid_url(mock_client: "Mock"):
    boox = Boox(client=mock_client)
    with pytest.raises(ValidationError, match="Input should be", check=_shows_all_members):
        boox.base_url = "http://random.url"  # pyright: ignore[reportAttributeAccessIssue]


def test_boox_is_not_closed_after_init(mock_client: "Mock"):
    boox = Boox(client=mock_client)
    assert boox.is_closed is False


def test_boox_is_closed_after_context_exit(mock_client: "Mock"):
    with Boox(client=mock_client) as boox:
        pass
    assert boox.is_closed


def test_boox_client_exposed_in_context(mock_client: "Mock"):
    with Boox(client=mock_client) as boox:
        assert boox.client is mock_client
...
def test_boox_close_calls_internal_method(mocker: "MockerFixture", mock_client: "Mock"):
    boox = Boox(client=mock_client)
...
def test_boox_raises_on_closed_client(mock_client: "Mock"):
...
def test_close_calls_client_and_sets_flag(mocker: "MockerFixture", mock_client: "Mock"):
    boox = Boox(client=mock_client)
    spy = mocker.spy(mock_client, "close")
...
def test_close_skips_if_already_closed(mocker: "MockerFixture", mock_client: "Mock"):
    boox = Boox(client=mock_client)
    spy = mocker.spy(mock_client, "close")
...
def test_close_skips_if_client_missing(mocker: "MockerFixture", mock_client: "Mock"):
    boox = Boox(client=mock_client)
    spy = mocker.spy(mock_client, "close")
    del boox.client
...
def test_boox_client_is_assigned_properly(mock_client: "Mock"):
    boox = Boox(client=mock_client)
    assert boox.client is mock_client


def test_boox_users_api_is_initialized(mock_client: "Mock"):
    boox = Boox(client=mock_client)
    assert isinstance(boox.users, UsersApi)


def test_boox_config_users_api_is_initiated(mock_client: "Mock"):
    boox = Boox(client=mock_client)
    assert isinstance(boox.config_users, ConfigUsersApi)


def test_boox_sets_default_headers(mock_client: "Mock"):
    boox = Boox(client=mock_client)
...
def test_boox_token_is_extracted_from_client_headers(faker: "Faker", mock_client: "Mock"):
    token = faker.uuid4()
    mock_client.headers.update({"Authorization": f"Bearer {token}"})
    boox = Boox(client=mock_client)
    assert boox.token == token
    assert boox.client.headers.get("Authorization") == f"Bearer {token}"


def test_client_token_takes_precedence_over_inline_token(faker: "Faker", mock_client: "Mock"):
    client_token = faker.uuid4()
    inline_token = faker.uuid4()
    mock_client.headers.update({"Authorization": f"Bearer {client_token}"})
    boox = Boox(client=mock_client, token=inline_token)
