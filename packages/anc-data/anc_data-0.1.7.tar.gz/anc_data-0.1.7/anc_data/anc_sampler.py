...
class AncSampler(DistributedSampler):
        ...
        if ratio_float > 0:
            if isinstance(indices, CustomizedRange):
                ...
                start = (
                    0
                    if not self.shuffle
                    else self._random.randint(0, len(tmp_indices) - length)
                )
    ...
    def _get_indices(self, sub_lengths, offset=0, return_sub_indices=False, ratio=[1]):
        ...
        if self.shuffle:
            self._random.shuffle(indices_list)
    ...
    def _create_chunk_indices_from_bz_and_worker(self, indices):
        ...
        while True:
            ...
            if self.global_shuffle:
                ...
                if self.mem_save_mode:
                    shift_offset = self._random.randint(0, len(indices.list_of_ranges))
                    indices.random_reset(shift_offset)
                else:
                    shift_offset = self._random.randint(0, len(indices))
    ...
    def _iter_init(self):
        ...
        if self.epoch == self.last_iter_epoch:
            ...
            if isinstance(self.sub_lengths[0], list):
                ...
                if self.shuffle:
                    self._random.shuffle(sub_indices_list)
                if not self.mem_save_mode:
        ...
        return self._create_chunk_indices_from_bz_and_worker(
            self.indices
        )
