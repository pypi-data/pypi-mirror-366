...
@click.option("-n", "--namespace", required=True, help="Namespace where Astronomer is installed")
...
@click.option("-c", "--cluster", help="Specify the cluster name")
    ...
    if interactive:
        ...
        if result.success:
            ...
            if output_file:
                ...
                click.echo(f"After they apply it, retrieve data with: orbis scanner retrieve -n {namespace}")
        else:
...
def scanner_clean(namespace, verbose, kubeconfig, dry_run):
    ...
    if dry_run:
        ...
        click.echo(f"orbis scanner clean -n {namespace}")
        return
