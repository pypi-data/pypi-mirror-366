...
class JobManager:
    ...
    def create_job(self, config: ScannerConfig) -> str:
        ...
        try:
            ...
            for obj in created_objects:
                if obj.kind == "Job" and obj.metadata and obj.metadata.name:
                    self.logger.info("Created job: %s", obj.metadata.name)
    ...
    def _log_job_timeout(
        ...
        if init_container_running:
            ...
            self.logger.warning("To retrieve when ready: orbis scanner retrieve -a %s", namespace)
