...
# 直接从环境变量和命令行参数获取配置值
WIKI_API_BASE_URL = args.api_url or os.getenv("MC_WIKI_API_BASE_URL", "http://mcwiki.rice-awa.top")
DEFAULT_TIMEOUT = args.timeout or int(os.getenv("MC_WIKI_API_TIMEOUT", "30"))
MAX_RETRIES = args.max_retries or int(os.getenv("MC_WIKI_API_MAX_RETRIES", "3"))
...
# 日志配置
LOG_LEVEL = args.log_level or os.getenv("MC_WIKI_LOG_LEVEL", "INFO")
...
logging.getLogger().handlers.clear()  # 清除之前的handler
logging.basicConfig(
    level=log_level,
    format=LOG_FORMAT,
    handlers=[StderrHandler()],
