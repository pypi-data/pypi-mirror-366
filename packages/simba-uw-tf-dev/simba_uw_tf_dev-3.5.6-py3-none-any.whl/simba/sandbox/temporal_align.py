...
@jit(nopython=True)
def map_time_stamps(start_time: int, end_time: int, time_stamps: np.ndarray):


    results = np.full(shape=(time_stamps.shape[1], time_stamps.shape[0]+1), fill_value=-1, dtype=np.int64)
    #frm_id = np.arange(start_time, end_time + 1).reshape(-1, 1)

    for i in prange(start_time, end_time, 1):
        for j in range(time_stamps.shape[0]):
...
def temporal_align(data_dir: Union[str, os.PathLike], keys: Optional[List[str]] = None):
    meta_paths = recursive_file_search(directory=data_dir, extensions=['npz'])[0:3]
    ...
    x = map_time_stamps(start_time=min_start_time, end_time=max_end_time, time_stamps=time_stamps)
    print(x)
