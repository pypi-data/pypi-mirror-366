...
class ToolParser:
    ...
    @staticmethod
    def buildJsonTools(funcList, schemaType="chat_completions"):
        """
        Build a list of JSON schemas for the given functions.
        schemaType can be 'completions' or 'responses'.
        Returns a list of schema dicts.
        """
        return [ToolParser.parseJsonSchema(f, schemaType) for f in funcList]

    @staticmethod
    def buildTypedTools(funcList):
        """
        Build a single types.Tool object containing FunctionDeclaration for each function.
        Returns [types.Tool].
        """
        declarations = [ToolParser.parseTypedSchema(f) for f in funcList]
