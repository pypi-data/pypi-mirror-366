...
class SecurityManager:
    ...
    def __init__(self, workspace_path: str = "."):
        ...
        # Use user's home directory for AI Helper Agent data
        from pathlib import Path
        user_home = Path.home()
        self.ai_helper_user_dir = user_home / ".ai_helper_agent"
        self.ai_helper_user_dir.mkdir(exist_ok=True)
        
        # Create subdirectories in user directory
        self.generated_dir = self.ai_helper_user_dir / "generated"
        self.generated_dir.mkdir(exist_ok=True)
        
        self.output_dir = self.ai_helper_user_dir / "output"
        self.output_dir.mkdir(exist_ok=True)
        
        self.temp_dir = self.ai_helper_user_dir / "temp"
        self.temp_dir.mkdir(exist_ok=True)
        
        # Safe write directories - include both workspace and user directories
        self.safe_write_dirs = {
            str(self.workspace_path),
            str(self.workspace_path / "output"),
            str(self.workspace_path / "temp"),
            str(self.ai_helper_user_dir),
    ...
    def create_safe_file(self, filename: str, content: str, subdir: str = "generated") -> bool:
        """Safely create a file in user's AI Helper Agent directory"""
        try:
            # Use user directory instead of workspace
            if subdir == "generated":
                safe_dir = self.generated_dir
            elif subdir == "output":
                safe_dir = self.output_dir
            elif subdir == "temp":
                safe_dir = self.temp_dir
            else:
                # Custom subdirectory in user directory
                safe_dir = self.ai_helper_user_dir / subdir
            ...
            logger.info("File created successfully in user directory", file=str(file_path))
            return True
