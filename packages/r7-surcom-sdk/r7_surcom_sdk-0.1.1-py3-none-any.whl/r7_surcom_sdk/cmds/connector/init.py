...
write it to your Connector development workspace
    ...
    def __init__(self, connectors_parser):
        ...
        cmd_docstr = self.__doc__.format(
            ...
            FILENAME=constants.TEMPLATE_CONFIG_YAML
        )
    ...
    def run(self, args):
        ...
        conn_spec = sdk_helpers.render_jinja_template(
            template=constants.TEMPLATE_CONFIG_YAML,
            templates_path=constants.TEMPLATE_PATH_CONNECTORS,
            data=template_data
        )

        sdk_helpers.write_file(
            path=os.path.join(path_connector_dir, constants.TEMPLATE_CONFIG_YAML),
            contents=conn_spec.rendered_template
        )

        sdk_helpers.print_log_msg(f"Generated the '{constants.TEMPLATE_CONFIG_YAML}' file with default settings for "
                                  f"the Connector '{c_id}' in '{path_connector_dir}'")

        sdk_helpers.print_log_msg(f"Now edit the '{constants.TEMPLATE_CONFIG_YAML}' file, change into the "
