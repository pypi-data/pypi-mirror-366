...
`invoke` the Connector.
    ...
    def _render_types_py(
        ...
        if not config_types:
            sdk_helpers.print_log_msg(
                f"No 'types' were found in the '{constants.TEMPLATE_CONFIG_YAML}' file for this Connector",
    ...
    def _render_types_yaml(
        ...
        if not config_types:
            raise SurcomSDKException(
                f"No types were found in the '{constants.TEMPLATE_CONFIG_YAML}' file. A Connector"
                "must have at least one type",
                solution=f"Add a Type to the 'types' section of the '{constants.TEMPLATE_CONFIG_YAML}' file"
            )
    ...
    def _render_settings_py(
        ...
        if not settings:
            sdk_helpers.print_log_msg(
                f"No settings were found in the '{constants.TEMPLATE_CONFIG_YAML}' file for this Connector",
                log_level=logging.WARNING
            )
            return None
    ...
    def _render_functions_py(
        ...
        if not config_fns:
            raise SurcomSDKException(
                f"No functions were found in the '{constants.TEMPLATE_CONFIG_YAML}' file. A Connector"
                "must have at least one function",
                solution=f"Add a Function to the 'functions' section of the '{constants.TEMPLATE_CONFIG_YAML}' file"
            )
    ...
    def run(self, args):
        ...
        path_conn_spec = os.path.join(path_connector, constants.TEMPLATE_CONFIG_YAML)
