...
class StreamAdvancedReasoningModelTask(BaseTask):
    ...
    def execute_streaming(self, **kwargs) -> Generator[dict, None, None]:
        ...
        yield {
            'type': 'intermediate',
            'iteration': 0,
            'overlay_image': self._image_to_base64(BaseTask.overlay_grid_on_image(original_image, grid_size[0], grid_size[1])[0]),
        ...
        while not is_terminal_state and len(overlay_samples) < max_crops:
            ...
            yield {
                ...
                'overlay_image': self._image_to_base64(BaseTask.overlay_grid_on_image(image, _grid_size[0], _grid_size[1])[0]),
                'is_terminal': is_terminal_state,
