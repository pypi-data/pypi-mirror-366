...
def read_pdf_text(file_path: str, start_page: int = 1, end_page: Optional[int] = None) -> str:
    ...
    for page_num in range(start_page - 1, end_page):
        ...
        markdown += f"---------- page {page_num + 1} of {file_name} ----------\n"
        markdown += page_text + "\n"
    markdown += f"---------- total {end_page - start_page + 1} pages in {file_name} ----------\n"
...
def read_by_ocr(file_path: str, start_page: int = 1, end_page: Optional[int] = None,
    ...
    for page_num in range(start_page - 1, end_page):
        ...
        text_content += f"---------- page {page_num + 1} of {file_name} ----------\n"
        text_content += page_text + "\n"
    
    text_content += f"---------- total {end_page - start_page + 1} pages in {file_name} ----------\n"
...
def read_pdf_to_file(file_paths: List[str], use_ocr: Literal["yes", "no"] = "no",
    ...
    for file_path in file_paths:
        ...
        for page_num in range(total_pages):
            ...
            text_content += f"---------- page {page_num + 1} of {display_file_name} ----------\n"
            text_content += page_text + "\n"
        
        text_content += f"---------- total {total_pages} pages in {display_file_name} ----------\n"
