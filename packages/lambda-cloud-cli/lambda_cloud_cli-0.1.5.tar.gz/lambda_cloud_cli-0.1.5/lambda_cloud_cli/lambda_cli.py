import os
import typer
from lambda_api_client import LambdaAPIClient
from lambda_cloud_cli.config import load_api_key, save_api_key
...
def login():
    ...
    typer.echo("✅ API key saved to ~/.lambda-cli/config.json")

@app.command(name="list-instances")
def list_instances():
    instances = client.list_instances().get("data", [])
    for inst in instances:
        typer.echo(f"{inst['id']}: {inst['name']} ({inst['status']})")

@app.command(name="terminate")
def terminate(instance_id: str):
    result = client.terminate_instances([instance_id])
    typer.echo(result)
...
def update_instance_name(instance_id: str, new_name: str):
    result = client.update_instance_name(instance_id, new_name)
    typer.echo(result)
...
def get_firewall_rules():
    rules = client.get_firewall_rules().get("data", [])
    for rule in rules:
        typer.echo(rule)

@app.command(name="get-firewall-rulesets")
def get_firewall_rulesets():
    rulesets = client.get_firewall_rulesets().get("data", [])
    for rs in rulesets:
...
def delete_firewall_ruleset(ruleset_id: str):
    result = client.delete_firewall_ruleset(ruleset_id)
    typer.echo(result)
