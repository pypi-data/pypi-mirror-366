...
def solve(path, **kwargs):
    ...
    # cmd = ["lumi", path]
    # try:
    c = run(["Luminescent", path, " --julia-args -t4"])
    if c != 0:
        print("failed")
        exit(1)
...
def load_sparams(sparams):
    ...
    return {
        characteristic_wavelength: {k: (v["re"] + 1j * v["im"]) for k, v in d.items()}
        for characteristic_wavelength, d in sparams.items()
    }
...
def load_component(path):
    c = gf.import_gds(os.path.join(path, "design1.gds"))
    d = json.load(open(os.path.join(path, "config.json")))
    for p in d["ports"]:
