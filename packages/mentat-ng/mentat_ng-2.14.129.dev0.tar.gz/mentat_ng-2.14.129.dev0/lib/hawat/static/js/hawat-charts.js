...
function inlineStyles(elem) {
    ...
    const origElems = elem.querySelectorAll('*');
    const clonedElems = clonedElem.querySelectorAll('*');

    for (let i = 0; i < origElems.length; i++) {
        const origElem = origElems[i];
        const clonedElem = clonedElems[i];
        const styles = window.getComputedStyle(origElem);
        clonedElem.style = styles;

        let cssText = '';
        for (const prop of styles) {
            cssText += `${prop}: ${styles.getPropertyValue(prop)}; `;
        }
        clonedElem.style.cssText = cssText;
    }
...
function addHTMLDownloadLink(chartId, chartElem) {
    document.getElementById(`${chartId}_export_html`)?.addEventListener('click', () => {
        const inlinedElem = inlineStyles(chartElem);
        const blob = new Blob([inlinedElem.outerHTML], {type: 'text/html'});
