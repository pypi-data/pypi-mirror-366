...
def plot_contact_map(cons, L=1, minsep=24, cutoffs=[None], ss=[5], cc=["gray"], f=None, ax=None, vmin=None, vmax=None):
    max_L = 0
    if ax is None:
...
def eval_hetero_oligomer(
    ...
    for i, (con, cutoff, s, c) in enumerate(zip(cons, cutoffs, ss, cc)):
        ...
        for chain_idx in range(2):
            ...
            if cutoff is None:
                top = sub_con.shape[0] * L
                # Use the filtered indices directly
                if len(vals) > top:  # Make sure we have enough values
                    use_cutoff = np.sort(vals)[::-1][top-1]
                else:
