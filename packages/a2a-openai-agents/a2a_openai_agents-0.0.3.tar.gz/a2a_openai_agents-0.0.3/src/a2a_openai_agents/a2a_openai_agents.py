...
class A2AWrapper:
    ...
    def _create_request_handler(self):
        ...
        class AgentRequestHandler(RequestHandler):
            ...
            async def on_cancel_task(
                ...
                if task:
                    task.status = TaskStatus(state="canceled", message=None)
                return task
    ...
    def a2a_skills(self, skills: list[A2ASkillConfig]):
        ...
        self._agent_card.skills = self._convert_skills_to_agent_skills()
    ...
    async def handle_a2a_task_update(
        ...
        if task_id in self._tasks:
            ...
            task.status = TaskStatus(state=status, message=None)
