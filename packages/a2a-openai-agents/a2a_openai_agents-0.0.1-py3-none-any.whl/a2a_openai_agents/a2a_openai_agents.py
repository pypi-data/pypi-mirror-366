...
class A2AWrapper:
            ...
            async def on_cancel_task(
                ...
                if task:
                    task.status = TaskStatus(
                        state="CANCELLED", message="Task cancelled by user request"
                    )
                return task
    ...
    async def _process_message(self, params) -> Task:
        ...
        if not text_content.strip():
            ...
            error_task = Task(
                id=task_id,
                status=TaskStatus(state="FAILED", message="No text content found in message"),
                messages=messages,
            )
        ...
        try:
            ...
            response_task = Task(
                id=task_id,
                status=TaskStatus(state="COMPLETED", message="Processed successfully"),
                messages=messages + [Message(role="agent", parts=[Part(type="text", text=result)])],
            )
            ...
            error_task = Task(
                id=task_id,
                status=TaskStatus(state="FAILED", message=f"Error processing message: {str(e)}"),
                messages=messages,
            )
            self._tasks[task_id] = error_task
            return error_task
    ...
    async def handle_a2a_task_update(
        ...
        Args:
            ...
            status: The new status (RUNNING, COMPLETED, FAILED, etc.)
            message: Status message
            result: Optional result data for completed tasks
        """
        if task_id in self._tasks:
            task = self._tasks[task_id]
            task.status = TaskStatus(state=status, message=message)
            if result:
                task.result = result
...
def run_a2a_wrapper_server(
    ...
    """
    # Get the FastAPI app from the wrapper
    app = wrapper_instance.get_app()
