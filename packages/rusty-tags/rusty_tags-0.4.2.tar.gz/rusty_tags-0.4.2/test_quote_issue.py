...
def test_quote_formatting():
    ...
    div1 = rt.Div(ds_signals=signals(count=0, name="test"))
    result1 = str(div1)
    ...
    div2 = rt.Div(cls=reactive_class(active="$isActive", disabled="$count === 0"))
    result2 = str(div2)
    ...
    # div3 = rt.Div(ds_signals={"count": 0, "user": {"name": "test"}})
    div3 = rt.Div(ds_signals=signals(count=0, user=dict(name="test")))
    result3 = str(div3)
    ...
    div3b = rt.Div(ds_store=dict(users=[dict(id=1, name="test")], active=True))
    result3b = str(div3b)
    print(f"complex JSON result: {result3b}")
    assert 'data-store="' in result3b, "Should use double quotes for HTML attribute"
