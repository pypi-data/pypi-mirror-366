...
🔗 MCP RUNNER HELP:
  ...
  MCP Client Configuration (Claude Desktop):
    ...
    def run(self, app_instance, **kwargs):
        ...
        logger.info(f"Available functions: {list(self.yaapp._registry.keys())}")
    ...
    async def _execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        if '.' in tool_name:
            ...
            if len(parts) >= 2:
                ...
                if class_or_namespace in self.yaapp._registry:
                    cls, exposer = self.yaapp._registry[class_or_namespace]
                    if inspect.isclass(cls):
                        # Execute class method
                        return await self._execute_class_method(cls, exposer, method_name, arguments)
                
                # Try full path in registry
                if tool_name in self.yaapp._registry:
                    func, exposer = self.yaapp._registry[tool_name]
