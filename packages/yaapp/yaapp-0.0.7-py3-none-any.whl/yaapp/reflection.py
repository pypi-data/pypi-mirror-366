...
class CLIBuilder:
    ...
    def build_cli(self) -> Optional['click.Group']:
        """Build the main CLI group with root-level options instead of commands."""
        if not HAS_CLICK:
            print("Click not available. Install with: pip install click")
            return None

        @click.group(invoke_without_command=True)
        @click.option('--server', is_flag=True, help='Start web server instead of running commands')
        ...
        @click.option('--reload', is_flag=True, help='Enable auto-reload [used with --server]')
        @click.option('--tui', type=click.Choice(['prompt', 'typer', 'rich', 'click']), help='Start TUI instead of running commands: prompt|rich|click|typer')
        @click.option('--list', 'list_functions', is_flag=True, help='List all exposed functions instead of running commands')
        @click.pass_context
        def cli(ctx, server, host, port, reload, tui, list_functions):
            ...
            if server:
                ...
                ctx.exit()
            # If no options and no command, show help
