...
class UnifiedCLIBuilder:
    ...
    def _create_main_cli_group(self, available_runners: Dict, available_commands: List[str]):
        ...
        @click.pass_context
        def cli(ctx, verbose, config, **runner_kwargs):
    ...
    def _create_method_command(self, method, method_name: str, plugin_name: str):
        ...
        def method_command(ctx, **kwargs):
            ...
            result = self.app.execute_function(plugin_name, **kwargs)
            if hasattr(result, 'is_ok') and result.is_ok():
                plugin_instance = result.unwrap()
                
                # Call the method on the plugin instance
                try:
                    method_result = getattr(plugin_instance, method_name)(**kwargs)
                    print(f"Result: {method_result}")
                except Exception as e:
                    print(f"Error: {e}")
            else:
                print(f"Error: Could not get {plugin_name} instance")
    ...
    def _show_main_help(self, available_runners: Dict, available_commands: List[str]):
        ...
        print("Usage: app.py [OPTIONS] COMMAND [ARGS]...")
        print("")
        print("  Storage plugin example")
        print("")
        print("Options:")
