...
class Yaapp(YaappCore):
    ...
    def _discover_runner_plugins(self):
        ...
        try:
            ...
            # Import the global yaapp instance to get registered runners
            from . import yaapp as global_yaapp
            
            # Get runner plugins from global registry and copy to local registry
            for runner_name in discovered_runners.keys():
                # Check global registry first
                if runner_name in global_yaapp._registry:
                    obj, exposer = global_yaapp._registry[runner_name]
                    # Check if it's a class that needs instantiation
                    if inspect.isclass(obj) and hasattr(obj, 'help') and hasattr(obj, 'run'):
                        # Instantiate the runner class
                        runner_instance = obj()
                        # Copy to local registry if not already present
                        if runner_name not in self._registry:
                            self._registry[runner_name] = (obj, exposer)
                        self._runner_plugins[runner_name] = runner_instance
                        print(f"✅ Discovered runner plugin: {runner_name}")
                    elif hasattr(obj, 'help') and hasattr(obj, 'run'):
                        ...
                        print(f"⚠️ Object '{runner_name}' in global registry is not a valid runner")
                # Check local registry as fallback
                elif runner_name in self._registry:
                    ...
                    if inspect.isclass(obj) and hasattr(obj, 'help') and hasattr(obj, 'run'):
                        ...
                        runner_instance = obj()
                        self._runner_plugins[runner_name] = runner_instance
                        print(f"✅ Discovered runner plugin: {runner_name} (local)")
                    elif hasattr(obj, 'help') and hasattr(obj, 'run'):
                        # It's already an instance
                        self._runner_plugins[runner_name] = obj
                        print(f"✅ Discovered runner plugin: {runner_name} (local)")
                    else:
                        print(f"⚠️ Object '{runner_name}' in local registry is not a valid runner")
                else:
                    print(f"⚠️ Runner '{runner_name}' not found in any registry")
