...
class YaappConfig:
    ...
    def _discover_and_configure_plugins(self, config_data: Dict[str, Any]) -> None:
        ...
        for section_name, plugin_class in discovered_plugins.items():
            if section_name in config_data:
                try:
                    ...
                    # Store plugin class and config for later registration
                    # (avoid circular import during singleton creation)
                    ...
                    print(f"✅ Loaded plugin: {section_name} ({plugin_class.__name__})")
    ...
    def register_discovered_plugins(self, app_instance):
        ...
        for section_name, plugin_info in self.plugins.items():
            if isinstance(plugin_info, dict) and plugin_info.get('instance') is None:
                try:
                    # Instantiate plugin with its configuration
                    plugin_class = plugin_info['class']
                    plugin_config = plugin_info['config']
                    plugin_instance = plugin_class(plugin_config)
                    
                    # Set yaapp instance for plugins that need it
                    if hasattr(plugin_instance, 'yaapp'):
                        plugin_instance.yaapp = app_instance
                    
                    # Expose plugin methods individually as root-level commands
                    self._expose_plugin_methods_individually(app_instance, plugin_instance, section_name)
                    
                    # Update stored reference
                    plugin_info['instance'] = plugin_instance
                    
                    print(f"🔗 Registered plugin: {section_name}")
                    
                except Exception as e:
                    print(f"Warning: Failed to register plugin '{section_name}': {e}")
    
    def _expose_plugin_methods_individually(self, app_instance, plugin_instance, plugin_name):
        """Expose each plugin method as an individual root-level command."""
        import inspect
        
        # Get all public methods of the plugin
        for method_name in dir(plugin_instance):
            if not method_name.startswith('_') and method_name not in ['yaapp', 'config']:
                method = getattr(plugin_instance, method_name)
                if callable(method) and not isinstance(method, type):
                    # Create a wrapper function that calls the plugin method
                    def create_wrapper(plugin_method, original_name):
                        # Get the method signature for proper argument handling
                        sig = inspect.signature(plugin_method)
                        
                        def wrapper(*args, **kwargs):
                            """Wrapper function for plugin method."""
                            return plugin_method(*args, **kwargs)
                        
                        # Copy the signature and docstring
                        wrapper.__signature__ = sig
                        wrapper.__doc__ = plugin_method.__doc__ or f"{original_name} from {plugin_name} plugin"
                        wrapper.__name__ = original_name
                        
                        return wrapper
                    
                    # Create wrapper and expose it
                    wrapper_func = create_wrapper(method, method_name)
                    result = app_instance.expose(wrapper_func, name=method_name)
                    
                    if not result.is_ok():
                        print(f"Warning: Failed to expose method '{method_name}' from plugin '{plugin_name}': {result.as_error}")
