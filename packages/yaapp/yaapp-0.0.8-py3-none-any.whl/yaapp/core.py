...
class YaappCore:
    ...
    def expose(
        ...
        if obj is None:
            # Check if this is a direct call with None (has explicit name)
            if name is not None:
                return Result.error("Cannot expose None object")
            # This is decorator usage with parameters - return decorator function
            def decorator(func):
                result = self.expose(func, name=name, custom=custom, execution=execution)
                if result.is_ok():
