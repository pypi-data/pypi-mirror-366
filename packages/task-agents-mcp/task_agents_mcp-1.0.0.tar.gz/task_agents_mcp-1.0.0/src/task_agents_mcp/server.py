...
# 2. Look for agents in package directory
# 3. Fall back to current working directory (for Claude Code CLI)
config_dir = os.environ.get('TASK_AGENTS_PATH')
if not config_dir:
    # Check if agents directory exists in package
    package_agents = os.path.join(os.path.dirname(__file__), "agents")
    if os.path.exists(package_agents):
        config_dir = package_agents
    else:
