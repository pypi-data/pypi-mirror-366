...
def pytest_generate_tests(metafunc):
    ...
    if not case_data:
        ...
        metafunc.parametrize("data", [case_data, ], ids=['#1'], scope="function")
        return
    ...
    if not url:
        ...
        if not case_common.get("url"):
            logger.warning(f"测试数据request中缺少必填字段: url", case_data)
    ...
    if not method:
        ...
        if not case_common.get("method"):
            logger.warning(f"测试数据request中缺少必填字段: method", case_data)
            # pytest.exit(ExitCode.YAML_MISSING_FIELDS)
...
def pytest_collection_finish(session):
    ...
    session.items = [item for item in session.items if item.name not in ["test_setup", "test_setup"]]
