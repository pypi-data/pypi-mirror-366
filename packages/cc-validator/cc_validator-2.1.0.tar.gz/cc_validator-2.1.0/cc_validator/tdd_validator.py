        ...
        if self.detect_test_files(file_path, content):
            ...
            if test_count > 1:
                return {
                    ...
                    "suggestions": [],  # LLM will generate language-specific suggestions
...
- **suggestions**: Provide specific, actionable TDD guidance for {self.detect_language(file_path)}
  - For Terraform: Suggest native test framework (tests/*.tftest.hcl)
  - For application code: Suggest appropriate unit test approach
  - Include concrete examples and commands specific to the language
