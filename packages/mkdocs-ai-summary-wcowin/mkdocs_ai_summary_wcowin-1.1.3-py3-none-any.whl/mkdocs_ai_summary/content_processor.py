...
class ContentProcessor:
    ...
    def get_page_language(self, page) -> str:
        ...
        if self.debug:
            if page_meta:
                print(f"🔍 Page Meta解析成功: {list(page_meta.keys())}")
                if 'ai_summary_lang' in page_meta:
                    print(f"🌐 发现页面语言设置: ai_summary_lang = '{page_meta['ai_summary_lang']}'")
                else:
                    print(f"⚠️ 未找到ai_summary_lang配置，可用键: {list(page_meta.keys())}")
            else:
                print(f"❌ Page Meta为空或不存在")
        ...
        if page_meta and 'ai_summary_lang' in page_meta:
            ...
            if page_lang in ['zh', 'en', 'both']:
                if self.debug:
                    print(f"✅ 使用页面语言设置: '{page_lang}' (覆盖全局设置 '{self.summary_language}')")
                return page_lang
            else:
                if self.debug:
                    print(f"⚠️ 无效语言设置 '{page_lang}'，使用全局设置 '{self.summary_language}'")
        else:
            if self.debug:
                print(f"📝 使用全局语言设置: '{self.summary_language}'")
    ...
    def get_content_hash(self, file_path: str, language: Optional[str] = None) -> str:
        """生成内容哈希
        ...
        path_with_lang = f"{file_path}_{lang}"
        return hashlib.md5(path_with_lang.encode('utf-8')).hexdigest()
    ...
    def format_summary(self, summary: str, service: str, language: str) -> str:
        ...
        """
        if self.debug:
            print(f"🎨 格式化摘要: 接收语言参数='{language}', 服务={service}")
        ...
        if self.debug:
            print(f"🏷️ 生成标题: '{title}' (基于语言='{language}')")
