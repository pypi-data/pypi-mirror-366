...
class AISummaryPlugin(BasePlugin):
    ...
    def on_page_markdown(self, markdown: str, page: Page, config: MkDocsConfig, files) -> str:
        ...
        try:
            ...
            # 调试日志：显示最终使用的语言设置
            if self.config['debug']:
                print(f"🎯 最终语言设置: '{page_language}' (全局: '{self.config['summary_language']}')")            
            
            # 清理内容并生成哈希
            cleaned_content = self.content_processor.clean_content_for_ai(markdown)
            content_hash = self.content_processor.get_content_hash(file_path, page_language)
            ...
            if cached_summary:
                ...
                if not self.config_manager.should_generate_new_summary():
                    ...
                    # 调用AI服务生成摘要
                    if self.config['debug']:
                        print(f"🤖 生成中... (语言: {page_language}, 标题: {page.title})")
                    ...
                    # 调试日志：显示传递给AI服务的参数
                    if self.config['debug']:
                        print(f"📤 AI服务参数: 语言={page_language}, 内容长度={len(truncated_content)}, 标题='{page.title}'")
                    ...
                    if summary_result and self.content_processor.validate_summary_content(summary_result['summary']):
                        ...
                        if self.config_manager.get_fallback_enabled():
                            ...
                            service_used = 'fallback'
                            if self.config['debug']:
                                print(f"⚠️ 生成失败，使用备用 ({page_language})")
                        else:
                            if self.config['debug']:
                                print(f"❌ 生成失败，跳过")
                            return markdown
            ...
            # 格式化并插入摘要
            if self.config['debug']:
                print(f"🎨 格式化摘要: 语言={page_language}, 服务={service_used}, 摘要长度={len(summary_text)}")
