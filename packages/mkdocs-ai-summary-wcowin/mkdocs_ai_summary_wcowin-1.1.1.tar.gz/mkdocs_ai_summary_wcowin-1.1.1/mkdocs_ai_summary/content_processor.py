...
class ContentProcessor:
    ...
    def get_page_language(self, markdown: str) -> str:
        ...
        if self.debug:
            if front_matter:
                print(f"🔍 Front Matter解析成功: {list(front_matter.keys())}")
                if 'ai_summary_lang' in front_matter:
                    print(f"🌐 发现页面语言设置: ai_summary_lang = '{front_matter['ai_summary_lang']}'")
                else:
                    print(f"⚠️ 未找到ai_summary_lang配置，可用键: {list(front_matter.keys())}")
            else:
                print(f"❌ Front Matter解析失败或不存在")
                # 显示markdown前几行用于调试
                lines = markdown.split('\n')[:10]
                print(f"📄 Markdown前10行: {lines}")
