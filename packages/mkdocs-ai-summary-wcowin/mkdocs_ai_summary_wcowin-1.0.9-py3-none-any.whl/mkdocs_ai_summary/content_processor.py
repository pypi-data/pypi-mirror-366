...
class ContentProcessor:
            ...
            if front_matter_match:
                try:
                    ...
                    if hasattr(self, 'debug') and self.debug:
                        print(f"[DEBUG] Front matter parsed successfully: {front_matter}")
                        if 'ai_summary_lang' in front_matter:
                            print(f"[DEBUG] Found ai_summary_lang: {front_matter['ai_summary_lang']}")
                    ...
                    if hasattr(self, 'debug') and self.debug:
                        ...
                        print(f"[DEBUG] YAML content: {front_matter_yaml}")
                    continue
        
        # 调试日志
        if hasattr(self, 'debug') and self.debug:
            print(f"[DEBUG] No front matter found in markdown")
            print(f"[DEBUG] Markdown start: {markdown[:100]}...")
    ...
    def get_page_language(self, markdown: str) -> str:
        ...
        if self.debug:
            print(f"[DEBUG] get_page_language - front_matter: {front_matter}")
            print(f"[DEBUG] get_page_language - global language: {self.summary_language}")
        
        if front_matter and 'ai_summary_lang' in front_matter:
            page_lang = front_matter['ai_summary_lang']
            if self.debug:
                print(f"[DEBUG] get_page_language - found ai_summary_lang: {page_lang}")
            
            # 验证语言设置是否有效
            if page_lang in ['zh', 'en', 'both']:
                if self.debug:
                    print(f"[DEBUG] get_page_language - using page language: {page_lang}")
                return page_lang
            else:
                if self.debug:
                    print(f"[DEBUG] get_page_language - invalid language '{page_lang}', using global: {self.summary_language}")
        else:
            if self.debug:
                print(f"[DEBUG] get_page_language - no ai_summary_lang found, using global: {self.summary_language}")
