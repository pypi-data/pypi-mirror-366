...
class GridLonLat(Grid):
    ...
    def is_compatible(self, other: Grid) -> bool:
        ...
        ## Lowest possible value of coordinates for this grid type
        #lowest_coord = {'lon': -180, 'lat': -90}

        #for axis in ['lon', 'lat']:
        #    if self.is_reduced(axis) != other.is_reduced(axix):
        #        return False
        #    
        #    if self.dx[axis] != other.dx[axis]:
        #        return False

        #    n_self = round(self.xmin[axis] - lowest_coord[axis])/self.dx[axis]
        #    first_coord_self = self.xmin[axis] - n_self*self.dx[axis]

        #    n_other = round(self.xmin[axis] - lowest_coord[axis])/self.dx[axis]
        #    first_coord_other = other.xmin[axis] - n_other*other.dx[axis]

        #    if np.isclose(first_coord_self, first_coord_other, rtol=1.e-7):
        #        return False
