...
class PostgreSQLUtils:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # 连接建立时初始化本连接周期的已重置表集合
            self._sequence_reset_tables = set()
            return True
    ...
    def disconnect(self):
        ...
        if self.connection and not self.connection.closed:
            ...
            self.connection = None
        # 断开连接时清空已重置表集合
        if hasattr(self, '_sequence_reset_tables'):
            self._sequence_reset_tables.clear()
    ...
    def add(self, table: str, data: Dict[str, Any], **kwargs) -> bool:
        ...
        # 仅在本连接周期内未重置过该表时重置序列
        if not hasattr(self, '_sequence_reset_tables'):
            self._sequence_reset_tables = set()
        if table not in self._sequence_reset_tables:
            self._reset_sequence_for_table(table)
            self._sequence_reset_tables.add(table)
