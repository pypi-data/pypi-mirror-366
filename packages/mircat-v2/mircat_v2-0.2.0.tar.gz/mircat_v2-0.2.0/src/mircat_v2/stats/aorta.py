...
class Aorta(Vessel):
    ...
    def aggregate_stats(self):
        ...
        if (
            all(region.get('entire_region', 1) for region in aggregated.values()) and 
            all_regions == set(aggregated.keys())
        ):
    ...
    def _extract_diameters(
        ...
        diameters = {}
        # Filter out any cross section missing diameters or too small to be sure about
        diam_cross_sections = []
        for cs in region:
            if cs.mean_diameter is not None:
                if cs.mean_diameter > 5:
