...
class Run(BaseModel):
    ...
    async def _create_from_pull_result(run_id: str, client: Client) -> "Run":
        ...
        return Run(
            ...
            model=model,
            pass_percentage=(
                result.run.pass_percentage * 100 if result.run.pass_percentage is not None else None
            ),
            pass_rate=result.run.pass_rate.value if result.run.pass_rate else None,
            pass_rate_error=(result.run.pass_rate.error if result.run.pass_rate else None),
            success_rate=(result.run.success_rate.value if result.run.success_rate else None),
            success_rate_error=(result.run.success_rate.error if result.run.success_rate else None),
    ...
    async def list_runs(
        ...
        return [RunMetadata.from_graphql(run) for run in result.runs_with_count.run_results]
    ...
    async def resume_run(
        ...
        remaining_tests = {
            ...
            if test._id not in completed_qa_pairs and test._id not in completed_test_results
        }
    ...
    async def get_run_dataframe(cls, run_id: str) -> pd.DataFrame:
        ...
        for row in rows:
            ...
            row_dict.update({f"modifiers.{k}": v for k, v in modifiers.items()})  # Flatten
