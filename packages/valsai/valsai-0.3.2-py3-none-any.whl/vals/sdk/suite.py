...
class Suite(BaseModel):
    ...
    async def from_id(
        ...
        if suite_data.global_checks is not None:
            global_checks = [Check.from_graphql(check.dict()) for check in suite_data.global_checks]
    ...
    async def from_dict(cls, data: dict[str, Any]) -> "Suite":
        ...
        global_checks = [
            Check.from_graphql(check_dict) for check_dict in data.get("global_checks", [])
        ]
        tests = [
            Test(
                input_under_test=test["input_under_test"],
                checks=[Check.from_graphql(check_dict) for check_dict in test["checks"]],
    ...
    async def from_inspect_json_file(
        ...
        for test in data:
            inspect_context = {
                ...
                "task_state": {k: v for k, v in test.items() if k != "target" and k != "input"},
            }
    ...
    async def create(self, force_creation: bool = False, max_upload_concurrency: int = 10) -> None:
        """
    ...
    async def delete(self) -> None:
        ...
        """
        if self.id is None:
            raise Exception("This suite has not been created yet, so there's nothing to delete")
    ...
    async def update(
        ...
        """
        if self.id is None:
            raise Exception("This suite has not been created yet, so there's nothing to update")
    ...
    async def _safe_local_execution(self, coro, run_id: str | None = None):
        ...
        try:
            ...
            if run_id:
                await self._client.update_run_status(run_id=run_id, status=RunStatus.ERROR)
            raise
        except Exception:
            if run_id:
                await self._client.update_run_status(run_id=run_id, status=RunStatus.ERROR)
    ...
    async def run(
        ...
        if isinstance(model, Callable) or isinstance(model, InspectWrapper):
            ...
            if run_id is None and qa_set_id is None:
                ...
                run_id is None and qa_set_id is not None or run_id is not None and qa_set_id is None
            ):
    ...
    async def _process_file(
        ...
        if file_id in [f.file_id for f in all_files_in_test if f != file and f.file_id is not None]:
    ...
    async def _upload_files(
        ...
        for test in tqdm(self.tests, desc="Checking each test for files to upload"):
            files_to_upload = await self._process_files_under_test(test, upload_files_path)
        ...
        with asyncio_tqdm(total=len(all_files_to_upload), desc="Uploading files") as pbar:
            ...
            await asyncio.gather(
                *[upload_file_task(file, file_path) for file, file_path in all_files_to_upload]
            )
    ...
    async def _process_single_test(
        ...
        if is_simple_model_function:
            ...
            casted_model_function = cast(ModelFunctionWithFilesAndContextType, model_function)
            if inspect.iscoroutinefunction(casted_model_function):
                # Already async, just await it
                output = await casted_model_function(test.input_under_test, files, test.context)
            else:
                # Run synchronous function in a thread pool to avoid blocking
                output = await loop.run_in_executor(
                    None,
                    lambda: casted_model_function(test.input_under_test, files, test.context),
                )
    ...
    async def _process_single_local_eval(
        ...
        if inspect.iscoroutinefunction(custom_operator):
            ...
            # Run synchronous function in a thread pool to avoid blocking
            result = await loop.run_in_executor(None, lambda: custom_operator(operator_input))
    ...
    async def _generate_qa_pairs_from_function(
            ...
            async with semaphore:
                ...
                async with upload_lock:  # Lock while updating shared state
                    ...
                    while len(qa_pairs) >= last_uploaded_idx + upload_concurrency:
                        ...
                        task = asyncio.create_task(
                            self._client.batch_add_question_answer_pairs(qa_set_id, batch_to_upload)
                        )
                        upload_tasks.append(task)
                        last_uploaded_idx += upload_concurrency
        ...
        with tqdm(
            total=(len(self.tests) if remaining_tests is None else len(remaining_tests)),
            desc="Processing tests",
        ) as pbar:
            await asyncio.gather(
                *(
                    process_and_upload_test(test)
                    for test in (self.tests if remaining_tests is None else remaining_tests)
                )
            )
