...
class ContinuousConversationHelper:
    ...
    def update_continuous_conversation(chat: Chat, cc_id: str, messages: List[MessageDraft], execution_context: ExecutionContext) -> ContinuousConversation:
        ...
        new_cc = ContinuousConversation(messages=messages, creator_id=execution_context.executor.id, forced_send=False, created_at=datetime.now(ZoneInfo("UTC")), updated_at=datetime.now(ZoneInfo("UTC")))
