...
class ContinuousConversationHelper:
    ...
    def build_filled_template_data(chat: Chat, cc: ContinuousConversation, template_name: str) -> FilledTemplateData:
        ...
        filled_recipient_parameters = [FilledRecipientParameter(id="preview_message", text=preview_message)] #type: ignore
    ...
    @staticmethod
    def create_continuous_conversation(chat: Chat, messages:SendMessagesFromAgentToChat, execution_context: ExecutionContext) -> ContinuousConversation:
        """
        ...
        cc = ContinuousConversation(messages=messages.messages, creator_id=execution_context.executor.id, forced_send=messages.forced_send, created_at=datetime.now(ZoneInfo("UTC")), updated_at=datetime.now(ZoneInfo("UTC")))
        chat.continuous_conversations.append(cc)
        return cc

    @staticmethod
    def update_continuous_conversation(chat: Chat, cc_id: str, messages: SendMessagesFromAgentToChat, execution_context: ExecutionContext) -> ContinuousConversation:
        """
        ...
        new_cc = ContinuousConversation(messages=messages.messages, creator_id=execution_context.executor.id, forced_send=messages.forced_send)  #type: ignore
