...
class Bottle(FastBottle):
    ...
    def get(
        ...
        if not isinstance(prefix, CIDR):
            prefix = CIDR(prefix)
        node = super().get(prefix, exact, covering)
        return node
    ...
    def insert(
        ...
        if not isinstance(prefix, CIDR):
            prefix = CIDR(prefix)
        node = self.set(prefix, value=value, aggregate=aggregate)
        node._cls = self._cls
    ...
    def set(
        ...
        if not isinstance(prefix, CIDR):
            prefix = CIDR(prefix)
        return super().set(prefix, value, aggregate)
