...
from typing import Type, TypeVar, List, get_origin, Any  # noqa
    ...
    def __init_subclass__(cls: Type, **kwargs):
        ...
        for name, type_hint in cls.__annotations__.items():
            ...
            if not is_has:
                ...
                typeo = type(origin_value)
                type_hint_o = get_origin(type_hint)
                if type_hint_o is not typeo:
                    continue

            # 检查这个原始类型是否在我们想要自动处理的映射中
            if typeo in MUTABLE_TYPE_FACTORIES:
                setattr(cls, name, field(default_factory=lambda: origin_value))
