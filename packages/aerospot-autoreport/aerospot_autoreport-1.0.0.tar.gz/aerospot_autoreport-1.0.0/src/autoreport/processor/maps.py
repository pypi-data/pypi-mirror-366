        ...
        try:
            ...
            for idx, row in uav_data.iterrows():
                ...
                x, y, is_inside = geo_to_image_coords(
                    lat, lon, img_width, img_height, self.satellite_geo_bounds
                )
                if is_inside:
                    # 获取该点的RGB值
            ...
            # 将预测结果重塑回原始图像形状
            self.water_mask = predictions.reshape(img_height, img_width).astype(
                np.uint8
            )
            ...
            # 保存图像
            mask_vis_path = self.path_manager.get_file_path(
                "maps", "water_mask_visualization.png"
            )
