...
class Dataset:
    ...
    def observers(self):
        ...
        Notes
        -----
        This mapping includes all point-observer files, followed by all
        stream-observer files. The reason for this ordering is based on the
        assumtion that users who iterate over all observers (e.g., in a
        visualization script) will want to analyze point observers first, since
        they are most readily comparable to satellite observations. Furthermore,
        most EPREM simulation runs will include far fewer point observers than
        stream observers, which means that this ordering should not be
        inconvenient to users who are more interesting in analyzing stream
        observers.

        See Also
        --------
        `~points`: all available point-observer files
        `~streams`: all available stream-observer files
        """
        if self._observers is None:
            self._observers = {**self.points, **self.streams}
