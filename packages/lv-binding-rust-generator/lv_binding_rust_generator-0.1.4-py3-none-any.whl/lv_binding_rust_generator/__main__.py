...
def convert_enum_block(code: str) -> tuple[str, List[tuple[str, int]]]:
    ...
    def eval_enum_expr(expr, last_value):
        try:
            ...
            raise ValueError(f"Unable to parse expression '{expr}': {e}")
    ...
    for entry in entries:
        ...
        value = eval_enum_expr(expr.strip(), last_value)
...
def generate_rs_file(file: Path) -> str:
    ...
    #     T.map(lambda l: T.pipe(
    #         l,
    #         T.map(lambda e: f"pub const {e[0]}: c_int = {e[1]};"),
    #         '\n'.join,
    #         lambda s: f"#[allow(non_camel_case_types)]\npub enum {l[0]} {{\n{s}\n}}",
    #     )),
    #     '\n\n'.join,
    # )
