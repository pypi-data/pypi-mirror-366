...
class CrateDataPipeline:
    ...
    async def fetch_metadata_batch(
        ...
        logging.info(
            f"Fetched metadata for {
                len(results)} out of " f"{
                len(crate_names)} requested crates.")
        return results
    ...
    async def _enhance_with_scraping(self, crate: CrateMetadata) -> None:
        ...
        try:
            ...
            if scraping_results:
                ...
                logging.info(
                    f"Enhanced scraping for {
                        crate.name}: " f"{
                        len(scraping_results)} sources")
        except Exception as e:
    ...
    async def run(self) -> Union["tuple[List[EnrichedCrate], Dict[str, Any]]", None]:
        ...
        crate_batches = [self.crates[i: i + batch_size]
                         for i in range(0, len(self.crates), batch_size)]

        for i, batch_names in enumerate(crate_batches):
            logging.info(
                f"Processing batch {i + 1}/{len(crate_batches)} " f"({len(batch_names)} crates)"
            )
