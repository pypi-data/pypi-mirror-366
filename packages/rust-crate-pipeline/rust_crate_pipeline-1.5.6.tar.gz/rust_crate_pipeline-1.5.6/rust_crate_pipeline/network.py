    ...
    def _fetch_metadata(self, crate_name: str) -> "dict[str, Any] | None":
        ...
        try:
            ...
            if r.ok:
                ...
                deps: list[dict[str, Any]] = (deps_response.json().get(
                    "dependencies", []) if deps_response.ok else [])
                ...
                if "github.com" in repo and self.config.github_token:
                    ...
                    if match:
                        ...
                        if self.config.github_token:
                            gh_headers["Authorization"] = f"token {
        ...
        try:
            ...
            if r.ok:
                ...
                keywords = ([k.text.strip()
                             for k in keywords_elem.find_all("a")] if keywords_elem else [])
        ...
        try:
            ...
            search_url = (
                f"https://api.github.com/search/repositories?" f"q={crate_name}+language:rust")
