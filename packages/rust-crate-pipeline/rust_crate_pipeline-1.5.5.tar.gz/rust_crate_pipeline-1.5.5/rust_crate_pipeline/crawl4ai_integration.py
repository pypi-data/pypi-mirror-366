...
class CrawlAIManager:
    ...
    def __init__(self,
        ...
        self._crate_schemas = {
            ...
            "github": {
                "name": "repository",
                "fields": [
                    {"name": "title", "selector": ".repository-content h1", "type": "text"},
                    {"name": "description", "selector": ".repository-content .f4", "type": "text"},
                    {"name": "stars", "selector": "#repo-stars-counter-star", "type": "text"},
                    {"name": "language", "selector": ".BorderGrid-cell .text-bold", "type": "text"},
    ...
    def _process_crawl_result(self, raw_result: Any, url: str) -> CrawlResult:
        ...
        try:
            ...
            if hasattr(
                    raw_result,
                    "extracted_content") and raw_result.extracted_content:
                try:
            ...
            return CrawlResult(
                url=getattr(
                    raw_result, "url", url), success=getattr(
                    raw_result, "success", False), structured_data=structured_data, markdown=(
                    raw_result.markdown.fit_markdown if hasattr(
                        raw_result, "markdown") else None), html=getattr(
                        raw_result, "html", None), metadata=metadata, status_code=getattr(
                            raw_result, "status_code", None), timestamp=getattr(
                                raw_result, "timestamp", None), )
                        ...
                        try:
                            ...
                            if len(path_parts) >= 2:
                                github_path = f"{
                                    path_parts[0]}/{
                                    path_parts[1]}"
                                crate_results["github"] = await crawler.crawl_github_repo(
                                    github_path
                                )
                        except Exception as e:
