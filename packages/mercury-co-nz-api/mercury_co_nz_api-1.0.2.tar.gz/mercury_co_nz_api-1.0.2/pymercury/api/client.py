...
class ElectricityUsage:
    ...
    def __init__(self, data: Dict[str, Any]):
        ...
        # Mercury.co.nz returns usage in arrays with different labels (actual, forecast, etc.)
        ...
        # Note: Temperature data is only available for daily intervals, None for hourly/monthly
...
class MercuryAPIClient:
    ...
    def get_electricity_usage(self, customer_id: str, account_id: str, service_id: str,
        ...
        self._log(f"⚡ Getting electricity usage for service {service_id} from {start_date} to {end_date}")

        url = self.endpoints.electricity_usage(customer_id, account_id, service_id, interval, start_date, end_date)
        response = self._make_request('GET', url)

        if response.status_code == 200:
            data = response.json()
            self._log("✅ Electricity usage data retrieved successfully")

            return ElectricityUsage(data)
        else:
            self._log(f"⚠️ Electricity usage request returned {response.status_code} for service {service_id}")
            return None
