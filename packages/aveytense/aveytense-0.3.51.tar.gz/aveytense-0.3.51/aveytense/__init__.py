...
def _get_all_item_types(i: __.Iterable[_T], /): # 0.3.51
    
    assert isinstance(i, __.Iterable)
    
    _protocol_meta = getattr(_typing, "_ProtocolMeta", type(__.Protocol))
    
    if _sys.version_info >= (3, 9):
        return _util.uniquetuple([(__.cast(type[_T], e) if type(e) in (_protocol_meta,) else type(e)) for e in i])
    
    else:
        return _util.uniquetuple([(__.cast(_subb.type[_T], e) if type(e) is (_protocol_meta,) else type(e)) for e in i])
...
def _is_bool_callback(v: _Any, /) -> __.TypeIs[__.Callable[[_Any], bool]]:
    ...
    if _version.VERSION_INFO >= (0, 3, 47) or True:
        ...
        return callable(v) and v.__name__ == "<lambda>" and v.__code__.co_argcount == 1 and v.__defaults__ is None
...
class Tense( # 0.3.24
    ...
    @classmethod
    def getAllItemsTypes(self, i: __.Iterable[_T], /):
        """
        @lifetime >= 0.3.51 \\
        https://aveyzan.xyz/aveytense#aveytense.Tense.getAllItemsTypes
        
        Returns all items' types in a set-like tuple
        """
    ...
    if versionInfo < (0, 3, 46) and False:
        ...
        @classmethod
        @__.overload
        def pick(self, i: __.Mapping[_KT, _VT], /) -> _VT: ...
    
    @classmethod # before 0.3.46 this signature had no annotations
    def pick(self, i: __.Union[__.Sequence[_T], __.Uniqual[_T], __.Mapping[_Any, _T]], /):
        """
        @lifetime >= 0.3.8 \\
        @standard-lifetime >= 0.3.24 \\
