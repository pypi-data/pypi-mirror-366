...
This GTK4 port modernizes the tray widgets while maintaining compatibility
with Sugar's tray-based interface patterns.
...
class _TrayViewport(Gtk.ScrolledWindow):
    ...
    def __init__(self, orientation):
        ...
        # Set up scrolled window for GTK4
        if self.orientation == Gtk.Orientation.HORIZONTAL:
            self.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.NEVER)
        else:
            self.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)

        # Create the toolbar container - using Box instead of Toolbar for GTK4
        ...
        # Set up size allocation monitoring
    ...
    def do_get_preferred_width(self):
        """GTK4 preferred width calculation."""
        if self.orientation == Gtk.Orientation.HORIZONTAL:
    ...
    def do_get_preferred_height(self):
        """GTK4 preferred height calculation."""
    ...
    def _size_changed_cb(self, widget, pspec):
        """Handle size changes to update scrollability."""
        self._update_scrollable_state()

    def _update_scrollable_state(self):
        """Update scrollable state based on content size."""
        allocation = self.get_allocation()
        if allocation.width <= 1 and allocation.height <= 1:
            return

        # Get the natural size of the traybar content
    ...
    def _adjustment_changed_cb(self, widget, pspec):
        ...
        # Check if we can scroll previous
...
class _TrayScrollButton(ToolButton):
    ...
    def __init__(self, icon_name, scroll_direction):
        ...
        # GTK4: Use set_child instead of set_icon_widget
...
class HTray(Gtk.Widget):
    ...
    GTK4 port using Widget as base class with custom layout management.
    """
    ...
    def __init__(self, **kwargs):
        ...
        # Connect scroll buttons to viewport
        scroll_left.viewport = self._viewport
        scroll_right.viewport = self._viewport

        # Add alignment spacer if needed
...
class _IconWidget(Gtk.Widget):
    ...
    def __init__(self, icon_name=None, xo_color=None):
        ...
        # Set up gesture controllers for GTK4
...
class TrayIcon(Gtk.Button):
    ...
    def get_icon_name(self):
        """Get the icon name or file path."""
        return self._icon_widget.get_icon_name()

    def set_icon_name(self, icon_name):
        """Set the icon name or file path."""
...
# Add CSS styling for drag active state
def _apply_tray_css():
...
# Apply styling when module is loaded
try:
    _apply_tray_css()
except Exception:
...
# Set CSS names for GTK4 if available
