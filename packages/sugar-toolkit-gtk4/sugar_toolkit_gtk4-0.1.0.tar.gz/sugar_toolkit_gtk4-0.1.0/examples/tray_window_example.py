...
class TrayAndWindowExampleActivity(SimpleActivity):
    ...
    def _on_add_vtray_item(self, button):
        ...
        child_count = len(self.vtray.get_children())
        new_icon = TrayIcon(icon_name="document-new", xo_color=XoColor.get_random_color())
        new_icon.connect("clicked", self._on_tray_item_clicked, f"New VIcon {child_count}")
        self.vtray.add_item(new_icon)
