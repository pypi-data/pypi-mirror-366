...
def build_quadtree_iterative(gdf: gpd.GeoDataFrame, max_size: int = 1000, max_degree: float = 1.0):
    queue = deque()
    queue.append((gdf, 0))
    leaves = []
    ...
    while queue:
        ...
        if len(left) == len(subset) or len(right) == len(subset):
            leaves.append(subset.index.to_numpy())
            continue
...
def build_quadtree(gdf: gpd.GeoDataFrame, max_size: int = 1000, depth: int = 0, max_degree: float = 1.0):
    n = len(gdf)
    if n == 0:
        return []
    ...
    if len(left) == len(gdf) or len(right) == len(gdf):
        return [gdf.index.to_numpy()]

    return build_quadtree(left, max_size, depth + 1, max_degree) + build_quadtree(
        right, max_size, depth + 1, max_degree
    )
