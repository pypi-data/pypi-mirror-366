...
def initLogging(rootname='mlz', rootlevel='info', logdir='/tmp/log',
                colorize=colorize, tracebacks_on_console=False):
    global log
    ...
    log.addHandler(ColoredConsoleHandler(colorize=colorize,
                                         show_traceback=tracebacks_on_console))
...
class ConsoleFormatter(Formatter):
    ...
    def __init__(self, fmt=None, datefmt=None, colorize=None, show_traceback=False):
    ...
    def format(self, record):
        ...
        if record.exc_info:
            ...
            s += self.colorize('red', self.formatException(record.exc_info))
        else:
            s += '\n'
        return s
...
def formatExtendedFrame(frame):
    ret = []
    secret_affixes = ('credential', 'passwd', 'password', 'secret')
    for key, value in frame.f_locals.items():
        if key.startswith(secret_affixes) or key.endswith(secret_affixes):
            valstr = '<hidden>'
        else:
            try:
...
class ColoredConsoleHandler(StreamHandler):
    ...
    def __init__(self, colorize=colorize, *, show_traceback=False):
