...
class DefaultCMM(CMM.VerifiableInterface, AwsCryptographyMaterialProvidersTypes.ICryptographicMaterialsManager):
    ...
    def GetEncryptionMaterials_k(self, input):
        ...
        def iife0_(_is_21):
            return isinstance(_is_21, MultiKeyring.MultiKeyring)
        if not((MultiKeyring.default__.Verified_q((self).keyring)) or (iife0_((self).keyring))):
    ...
    def DecryptMaterials_k(self, input):
        ...
        def iife0_(_is_22):
            return isinstance(_is_22, MultiKeyring.MultiKeyring)
        if not((MultiKeyring.default__.Verified_q((self).keyring)) or (iife0_((self).keyring))):
