...
class default__:
    ...
    def SerializeFixedInfo(ecdhKeyDerivationUtf8, curveSpecUtf8, senderPublicKey, recipientPublicKey, canonicalizedEC, keyringVersion):
        return (((((((((((ecdhKeyDerivationUtf8) + (Constants.default__.ECDH__KDF__DELIMITER)) + (curveSpecUtf8)) + (Constants.default__.ECDH__KDF__DELIMITER)) + (Constants.default__.ECDH__KDF__PRF__NAME)) + (Constants.default__.ECDH__KDF__DELIMITER)) + (senderPublicKey)) + (recipientPublicKey)) + (Constants.default__.ECDH__KDF__DELIMITER)) + (keyringVersion)) + (Constants.default__.ECDH__KDF__DELIMITER)) + (canonicalizedEC)
...
class EcdhUnwrap(MaterialWrapping.UnwrapMaterial, Actions.ActionWithResult, Actions.Action):
    ...
    def Invoke(self, input):
        ...
        d_3_valueOrError0_ = Wrappers.default__.Need((len(d_1_wrappedMaterial_)) > (Constants.default__.CIPHERTEXT__WRAPPED__MATERIAL__INDEX), default__.E(_dafny.Seq("Received ciphertext is shorter than expected.")))
        ...
        d_5_valueOrError1_ = Wrappers.default__.Need((len(d_1_wrappedMaterial_)) > ((Constants.default__.ECDH__WRAPPED__KEY__MATERIAL__INDEX) + (d_4_KeyLength_)), AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Received EDK Ciphertext of incorrect length3.")))
        if (d_5_valueOrError1_).IsFailure():
            res = (d_5_valueOrError1_).PropagateFailure()
            return res
        d_6_kdfNonce_: _dafny.Seq
        d_6_kdfNonce_ = _dafny.Seq((d_1_wrappedMaterial_)[:Constants.default__.ECDH__COMMITMENT__KEY__INDEX:])
        d_7_iv_: _dafny.Seq
        ...
        d_19_calculatedCommitmentKey_ = _dafny.Seq((d_18_derivedKeyingMaterial_)[:32:])
...
class EcdhWrapKeyMaterial(MaterialWrapping.WrapMaterial, Actions.ActionWithResult, Actions.Action):
    ...
    def Invoke(self, input):
        ...
        d_9_commitmentKey_: _dafny.Seq
        d_9_commitmentKey_ = _dafny.Seq((d_8_derivedKeyingMaterial_)[:32:])
