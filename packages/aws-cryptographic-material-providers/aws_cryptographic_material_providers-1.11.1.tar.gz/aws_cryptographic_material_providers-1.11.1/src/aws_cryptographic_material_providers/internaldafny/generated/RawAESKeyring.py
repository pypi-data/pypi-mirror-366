...
class RawAESKeyring(Keyring.VerifiableInterface, AwsCryptographyMaterialProvidersTypes.IKeyring):
    ...
    def ValidProviderInfo(self, info):
        d_0_keyname__size_ = len((self).keyName)
        return ((((((len(info)) == (StandardLibrary_MemoryMath.default__.Add4(d_0_keyname__size_, default__.AUTH__TAG__LEN__LEN, default__.IV__LEN__LEN, ((self).wrappingAlgorithm).ivLength))) and ((_dafny.Seq((info)[:d_0_keyname__size_:])) == ((self).keyName))) and ((StandardLibrary_UInt.default__.SeqToUInt32(_dafny.Seq((info)[d_0_keyname__size_:(d_0_keyname__size_) + (default__.AUTH__TAG__LEN__LEN):]))) == (128))) and ((128) == ((((self).wrappingAlgorithm).tagLength) * (8)))) and ((StandardLibrary_UInt.default__.SeqToUInt32(_dafny.Seq((info)[(d_0_keyname__size_) + (default__.AUTH__TAG__LEN__LEN):((d_0_keyname__size_) + (default__.AUTH__TAG__LEN__LEN)) + (default__.IV__LEN__LEN):]))) == (((self).wrappingAlgorithm).ivLength))) and ((StandardLibrary_UInt.default__.SeqToUInt32(_dafny.Seq((info)[(d_0_keyname__size_) + (default__.AUTH__TAG__LEN__LEN):((d_0_keyname__size_) + (default__.AUTH__TAG__LEN__LEN)) + (default__.IV__LEN__LEN):]))) == (12))
...
class AesGenerateAndWrapKeyMaterial(MaterialWrapping.GenerateAndWrapMaterial, Actions.ActionWithResult, Actions.Action):
    ...
    def Invoke(self, input):
        ...
        out1_: Wrappers.Result
        out1_ = ((self).wrap).Invoke(MaterialWrapping.WrapInput_WrapInput(d_3_plaintextMaterial_, (input).algorithmSuite, (input).encryptionContext, (input).serializedEC))
