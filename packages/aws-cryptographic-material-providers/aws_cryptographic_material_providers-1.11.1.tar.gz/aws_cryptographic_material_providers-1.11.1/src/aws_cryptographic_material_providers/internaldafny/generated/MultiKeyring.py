...
class default__:
    ...
    @staticmethod
    def Verified_q(keyring):
        def iife0_(_is_8):
            return isinstance(_is_8, RawAESKeyring.RawAESKeyring)
        def iife1_(_is_9):
            return isinstance(_is_9, RawECDHKeyring.RawEcdhKeyring)
        def iife2_(_is_10):
            return isinstance(_is_10, RawRSAKeyring.RawRSAKeyring)
        def iife3_(_is_11):
            return isinstance(_is_11, AwsKmsDiscoveryKeyring.AwsKmsDiscoveryKeyring)
        def iife4_(_is_12):
            return isinstance(_is_12, AwsKmsEcdhKeyring.AwsKmsEcdhKeyring)
        def iife5_(_is_13):
            return isinstance(_is_13, AwsKmsHierarchicalKeyring.AwsKmsHierarchicalKeyring)
        def iife6_(_is_14):
            return isinstance(_is_14, AwsKmsKeyring.AwsKmsKeyring)
        def iife7_(_is_15):
            return isinstance(_is_15, AwsKmsMrkDiscoveryKeyring.AwsKmsMrkDiscoveryKeyring)
        def iife8_(_is_16):
            return isinstance(_is_16, AwsKmsMrkKeyring.AwsKmsMrkKeyring)
        def iife9_(_is_17):
            return isinstance(_is_17, AwsKmsRsaKeyring.AwsKmsRsaKeyring)
        return (((((((((iife0_(keyring)) or (iife1_(keyring))) or (iife2_(keyring))) or (iife3_(keyring))) or (iife4_(keyring))) or (iife5_(keyring))) or (iife6_(keyring))) or (iife7_(keyring))) or (iife8_(keyring))) or (iife9_(keyring))
    ...
    def AttemptDecryptDataKey(keyring, input):
        ...
        def iife0_(_is_18):
            return isinstance(_is_18, MultiKeyring)
        if not((default__.Verified_q(keyring)) or (iife0_(keyring))):
    ...
    def OnEncrypt_k(self, input):
        ...
        if ((self).generatorKeyring).is_Some:
            ...
            if (d_4_valueOrError1_).IsFailure():
                ...
                return res
            def iife0_(_is_19):
                return isinstance(_is_19, MultiKeyring)
            if not((default__.Verified_q(((self).generatorKeyring).value)) or (iife0_(((self).generatorKeyring).value))):
                d_5_valueOrError2_: Wrappers.Outcome = Wrappers.Outcome.default()()
                d_5_valueOrError2_ = Wrappers.default__.Need(Materials.default__.EncryptionMaterialsHasPlaintextDataKey(((d_3_onEncryptOutput_).value).materials), AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Could not retrieve materials required for encryption")))
        ...
        for d_7_i_ in range(0, hi0_):
            d_8_onEncryptInput_: AwsCryptographyMaterialProvidersTypes.OnEncryptInput
            d_8_onEncryptInput_ = AwsCryptographyMaterialProvidersTypes.OnEncryptInput_OnEncryptInput(d_1_returnMaterials_)
            d_9_child_: AwsCryptographyMaterialProvidersTypes.IKeyring
            d_9_child_ = ((self).childKeyrings)[d_7_i_]
            d_10_onEncryptOutput_: Wrappers.Result
            out1_: Wrappers.Result
            out1_ = (d_9_child_).OnEncrypt(d_8_onEncryptInput_)
            d_10_onEncryptOutput_ = out1_
            d_11_valueOrError4_: Wrappers.Outcome = Wrappers.Outcome.default()()
            d_11_valueOrError4_ = Wrappers.default__.Need((d_10_onEncryptOutput_).is_Success, AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Child keyring failed to encrypt plaintext data key")))
            if (d_11_valueOrError4_).IsFailure():
                res = (d_11_valueOrError4_).PropagateFailure()
                return res
            def iife1_(_is_20):
                return isinstance(_is_20, MultiKeyring)
            if not((default__.Verified_q(d_9_child_)) or (iife1_(d_9_child_))):
                d_12_valueOrError5_: Wrappers.Outcome = Wrappers.Outcome.default()()
                d_12_valueOrError5_ = Wrappers.default__.Need(Materials.default__.EncryptionMaterialsHasPlaintextDataKey(((d_10_onEncryptOutput_).value).materials), AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Could not retrieve materials required for encryption")))
                if (d_12_valueOrError5_).IsFailure():
                    res = (d_12_valueOrError5_).PropagateFailure()
                    return res
                d_13_valueOrError6_: Wrappers.Outcome = Wrappers.Outcome.default()()
                d_13_valueOrError6_ = Wrappers.default__.Need(Materials.default__.ValidEncryptionMaterialsTransition(d_1_returnMaterials_, ((d_10_onEncryptOutput_).value).materials), AwsCryptographyMaterialProvidersTypes.Error_AwsCryptographicMaterialProvidersException(_dafny.Seq("Child keyring performed invalid transition on encryption materials")))
                if (d_13_valueOrError6_).IsFailure():
                    res = (d_13_valueOrError6_).PropagateFailure()
                    return res
            d_1_returnMaterials_ = ((d_10_onEncryptOutput_).value).materials
