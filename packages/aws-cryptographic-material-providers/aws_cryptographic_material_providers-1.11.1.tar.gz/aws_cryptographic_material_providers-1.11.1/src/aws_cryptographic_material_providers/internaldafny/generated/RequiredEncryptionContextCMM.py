...
class RequiredEncryptionContextCMM(CMM.VerifiableInterface, AwsCryptographyMaterialProvidersTypes.ICryptographicMaterialsManager):
    ...
    def GetEncryptionMaterials_k(self, input):
        ...
        def iife0_(_pat_let23_0):
            def iife1_(d_3_dt__update__tmp_h0_):
                def iife2_(_pat_let24_0):
        ...
        def iife4_(_is_25):
            return isinstance(_is_25, DefaultCMM.DefaultCMM)
        def iife5_(_is_26):
            return isinstance(_is_26, RequiredEncryptionContextCMM)
        if not((iife4_((self).underlyingCMM)) or (iife5_((self).underlyingCMM))):
    ...
    def DecryptMaterials_k(self, input):
        ...
        def iife0_(_is_27):
            return isinstance(_is_27, DefaultCMM.DefaultCMM)
        def iife1_(_is_28):
            return isinstance(_is_28, RequiredEncryptionContextCMM)
        if not((iife0_((self).underlyingCMM)) or (iife1_((self).underlyingCMM))):
