    ...
    def ExtractCustomEncryptionContext(encryptionContext):
        def iife0_():
            ...
            for compr_0_ in ((encryptionContext).keys).Elements:
                ...
                if ((d_1_k_) in ((encryptionContext).keys)) and ((default__.ENCRYPTION__CONTEXT__PREFIX) <= (d_1_k_)):
                    coll0_ = coll0_.union(_dafny.Set([d_1_k_]))
            return _dafny.Set(coll0_)
        d_0_prefixKeys_ = iife0_()

        def iife1_():
            coll1_ = _dafny.Set()
            compr_1_: _dafny.Seq
            for compr_1_ in (d_0_prefixKeys_).Elements:
                d_3_k_: _dafny.Seq = compr_1_
                if (d_3_k_) in (d_0_prefixKeys_):
                    coll1_ = coll1_.union(_dafny.Set([(UTF8.default__.Encode(_dafny.Seq((d_3_k_)[len(default__.ENCRYPTION__CONTEXT__PREFIX)::])), UTF8.default__.Encode((encryptionContext)[d_3_k_]))]))
            return _dafny.Set(coll1_)
        d_2_encodedEncryptionContext_ = iife1_()

        def lambda0_(forall_var_0_):
            d_5_i_: tuple = forall_var_0_
            return not ((d_5_i_) in (d_2_encodedEncryptionContext_)) or ((((d_5_i_)[0]).is_Success) and (((d_5_i_)[1]).is_Success))

        d_4_valueOrError0_ = Wrappers.default__.Need(_dafny.quantifier((d_2_encodedEncryptionContext_).Elements, True, lambda0_), AwsCryptographyKeyStoreTypes.Error_KeyStoreException(_dafny.Seq("Unable to encode string")))
        if (d_4_valueOrError0_).IsFailure():
            ...
            def iife2_():
                coll2_ = _dafny.Map()
                compr_2_: tuple
                for compr_2_ in (d_2_encodedEncryptionContext_).Elements:
                    d_6_i_: tuple = compr_2_
                    if (d_6_i_) in (d_2_encodedEncryptionContext_):
                        coll2_[((d_6_i_)[0]).value] = ((d_6_i_)[1]).value
