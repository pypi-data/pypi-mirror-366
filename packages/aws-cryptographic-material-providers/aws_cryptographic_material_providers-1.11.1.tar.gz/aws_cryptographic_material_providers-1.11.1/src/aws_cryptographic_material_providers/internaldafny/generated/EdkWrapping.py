...
class default__:
    ...
    def WrapEdkMaterial(encryptionMaterials, wrap, generateAndWrap):
        ...
        if (((encryptionMaterials).plaintextDataKey).is_Some) and ((((encryptionMaterials).algorithmSuite).edkWrapping).is_DIRECT__KEY__WRAPPING):
            ...
            out0_ = (wrap).Invoke(MaterialWrapping.WrapInput_WrapInput(((encryptionMaterials).plaintextDataKey).value, (encryptionMaterials).algorithmSuite, (encryptionMaterials).encryptionContext, CanonicalEncryptionContext.default__.EncryptionContextToAAD((encryptionMaterials).encryptionContext)))
            ...
            out2_ = (generateAndWrap).Invoke(MaterialWrapping.GenerateAndWrapInput_GenerateAndWrapInput((encryptionMaterials).algorithmSuite, (encryptionMaterials).encryptionContext, CanonicalEncryptionContext.default__.EncryptionContextToAAD((encryptionMaterials).encryptionContext)))
    ...
    def UnwrapEdkMaterial(wrappedMaterial, decryptionMaterials, unwrap):
        ...
        if (((decryptionMaterials).algorithmSuite).edkWrapping).is_DIRECT__KEY__WRAPPING:
            d_1_valueOrError1_: Wrappers.Result = None
            out0_: Wrappers.Result
            out0_ = (unwrap).Invoke(MaterialWrapping.UnwrapInput_UnwrapInput(wrappedMaterial, (decryptionMaterials).algorithmSuite, (decryptionMaterials).encryptionContext, CanonicalEncryptionContext.default__.EncryptionContextToAAD((decryptionMaterials).encryptionContext)))
            d_1_valueOrError1_ = out0_
            if (d_1_valueOrError1_).IsFailure():
                ret = (d_1_valueOrError1_).PropagateFailure()
                return ret
