...
class Heads(DatabaseEntity):
    ...
    def get_all(self) -> Iterator[ea_t]:
        ...
        """
        assert self.m_database is not None  # The database is checked by the decorator
        return self.get_between(self.m_database.minimum_ea, self.m_database.maximum_ea)
    ...
    def get_between(self, start_ea: ea_t, end_ea: ea_t) -> Iterator[ea_t]:
        ...
        """
        assert self.m_database is not None  # The database is checked by the decorator
        if start_ea < self.m_database.minimum_ea or start_ea > self.m_database.maximum_ea:
            raise InvalidEAError(start_ea)
        if end_ea < self.m_database.minimum_ea or end_ea > self.m_database.maximum_ea:
    ...
    def get_next(self, ea: ea_t) -> ea_t | None:
        ...
        """
        assert self.m_database is not None  # The database is checked by the decorator
        if ea < self.m_database.minimum_ea or ea > self.m_database.maximum_ea:
