    ...
    Example:
        ...
        ConfigurationManager.init(
            config=MQConfig(
                ...
                aggregator_url="redis://localhost:6379/0"
            )
        )
        ...
        @start_consumer(
            ...
            send_queues=["output_queue"],
            config=MQConfig(
                connection_url="amqp://other-host/",
                prefetch_count=20
            )
        )
        async def another_consumer(messages: list[dict]) -> Any:
            # 处理消息
            return result
    """
    def decorator(main: Callable[[list[Any]], Any | list[Any]]):
        return ConsumerManager(main, listen_queues, send_queues, config)
