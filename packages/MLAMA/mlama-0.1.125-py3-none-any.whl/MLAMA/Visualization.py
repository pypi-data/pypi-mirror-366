...
def plot_weekly_difference(
    ...
    """
    Plots prediction differences (Prediction - Observed) as points and lines for a specific prediction week
    and prediction length. X-axis is the future target Week. Shift is inferred from data.
    ...
    # Extract predictions for the given length
    df_all = extract_predictions_from_dict(weekly_prediction_future, target_length=prediction_length, models=models)

    # Filter to the relevant prediction week
    df_filtered = df_all[
        (df_all['CurrentWeek'] == prediction_week) &
        (df_all['Length'] == prediction_length)
    ]
    ...
    # Convert and sort by target week
    df_filtered['Week'] = pd.to_datetime(df_filtered['Week'])
    df_filtered = df_filtered.sort_values('Week')
    x = df_filtered['Week']

    # Get shift value for plot title
    shift_val = df_filtered['Shift'].unique()
    shift_val = shift_val[0] if len(shift_val) == 1 else str(shift_val)
    ...
    required_models = ['ARIMA', 'MLAMA']
    optional_models = list(models.keys())
    all_models = list(dict.fromkeys(required_models + optional_models))  # preserve order

    for model in all_models:
        if model in df_filtered.columns:
            observed = df_filtered['Observed'].values
            predicted = df_filtered[model].values
            differences = predicted - observed
            color = model_colors.get(model) if model_colors and model in model_colors else None

            # Plot line and points
            plt.plot(x, differences, linestyle='-', linewidth=1.5, color=color, alpha=0.7, zorder=1)
            if model.upper() == 'MLAMA':
                plt.scatter(x, differences, s=100, c=color, marker='*', label=model, zorder=5, edgecolors='black', linewidths=1.5)
            else:
                plt.scatter(x, differences, s=60, c=color, marker='o', label=model)

    plt.xlabel("Prediction Target Date (Week)")
    plt.ylabel("Difference (Prediction - Observed)")
    plt.title(f"Differences from Week = {prediction_week}, Length = {prediction_length}, Shift ({shift_type}) = {shift_val}")
    plt.xticks(rotation=45)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
