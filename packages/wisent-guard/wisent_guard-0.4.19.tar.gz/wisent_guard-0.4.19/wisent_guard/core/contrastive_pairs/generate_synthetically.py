...
class SyntheticContrastivePairGenerator:
    ...
    def _generate_new_questions_parallel(self, target_count: int) -> list[str]:
        ...
        # If we got very few questions due to timeouts, try simpler approaches
        if len(all_questions) < target_count // 2:
            print(f"   ⚠️ Only got {len(all_questions)} questions from parallel generation, trying sequential fallback...")
            
            # Try multiple simple prompts sequentially
            simple_prompts = [
                f"List {target_count} simple questions about everyday life:\n1.",
                f"Write {target_count} short questions people ask daily:\n1.",
                f"Generate {target_count} basic questions:\n1.",
            ]
            
            for prompt in simple_prompts:
                try:
                    response = self._cached_generate_response(prompt, QUESTION_GEN.CONFIG)
                    fallback_questions = self._parse_questions_from_response(response)
                    if fallback_questions:
                        all_questions.extend(fallback_questions)
                        print(f"   ✅ Fallback generated {len(fallback_questions)} additional questions")
                        if len(all_questions) >= target_count // 2:
                            break
                except Exception as e:
        ...
        # Emergency fallback: use pre-defined questions if still too few
        if len(all_questions) < 5:
            print(f"   ⚠️ Emergency fallback: adding pre-defined questions")
            emergency_questions = [
            ...
            all_questions.extend(emergency_questions[:max(5, target_count - len(all_questions))])
