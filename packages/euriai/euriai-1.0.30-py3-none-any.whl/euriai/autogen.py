    ...
    def create(self, params: Dict[str, Any]) -> SimpleNamespace:
        ...
        for _ in range(n):
            try:
                ...
                # Create error response - don't print debug in production
                choice = SimpleNamespace()
                choice.message = SimpleNamespace()
...
class EuriaiAutoGen:
    ...
    def create_assistant_agent(
        ...
        **kwargs
        ...
        self._debug_print(f"Debug: Creating AssistantAgent with name={name}, model={model or self.default_model}")
        
        # First, let's try to determine the correct constructor signature
        import inspect
        try:
            sig = inspect.signature(AssistantAgent.__init__)
            self._debug_print(f"Debug: AssistantAgent constructor signature: {sig}")
        except Exception as e:
            self._debug_print(f"Debug: Could not inspect AssistantAgent signature: {e}")
        ...
        try:
            self._debug_print("Debug: Trying model_client=EuriaiModelClient")
            ...
            try:
                self._debug_print("Debug: Trying minimal constructor")
                agent = AssistantAgent(
                    name=name,
                    system_message=system_message,
                    **kwargs
                ...
                try:
                    self._debug_print("Debug: Trying with just name")
                    agent = AssistantAgent(
                        name=name,
                        **kwargs
                    )
                    self._debug_print("Debug: Success with just name")
                except TypeError as e:
                    last_error = e
                    self._debug_print(f"Debug: Failed with just name: {e}")
                    raise last_error
        ...
        self._debug_print(f"Debug: AssistantAgent '{name}' methods: {[m for m in dir(agent) if not m.startswith('_')]}")
        self._debug_print(f"Debug: AssistantAgent '{name}' has initiate_chat: {hasattr(agent, 'initiate_chat')}")
    ...
    def create_user_proxy_agent(
        ...
        **kwargs
        ...
        try:
            self._debug_print(f"Debug: Creating UserProxyAgent '{name}' with full parameters")
            ...
            self._debug_print(f"Debug: UserProxyAgent '{name}' created successfully")
        except TypeError as e:
            last_error = e
            self._debug_print(f"Debug: UserProxyAgent creation failed with full parameters: {e}")
            
            # Pattern 2: Minimal parameters
            try:
                self._debug_print(f"Debug: Creating UserProxyAgent '{name}' with minimal parameters")
                agent = UserProxyAgent(
                    name=name,
                    **kwargs
                )
                self._debug_print(f"Debug: UserProxyAgent '{name}' created successfully with minimal parameters")
            except TypeError as e:
                last_error = e
                self._debug_print(f"Debug: UserProxyAgent creation failed with minimal parameters: {e}")
                raise last_error
        ...
        # Debug: Check what methods the agent has
        self._debug_print(f"Debug: UserProxyAgent '{name}' methods: {[m for m in dir(agent) if not m.startswith('_')]}")
        self._debug_print(f"Debug: UserProxyAgent '{name}' has initiate_chat: {hasattr(agent, 'initiate_chat')}")
