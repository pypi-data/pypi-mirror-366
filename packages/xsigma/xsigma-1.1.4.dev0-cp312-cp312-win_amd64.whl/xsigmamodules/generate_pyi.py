...
Options are as follows:
 ...
 -b BINDIR     The extras directory to create [default is none]
 -h HELP
...
def fix_annotations(signature):
    ...
    """
    # Handle Python keywords by temporarily replacing them
    if signature.startswith("def lambda("):
        signature = signature.replace("def lambda(", "def lambda_func(")
        restore_lambda = True
    else:
        restore_lambda = False
    ...
    # Restore Python keywords
    if restore_lambda:
        signature = signature.replace("def lambda_func(", "def lambda(")
...
def main(argv=sys.argv):
    ...
    parser.add_argument(
        "-b", "--bindir", type=str, help="Extras directory for binaries [none]."
    )
    parser.add_argument(
        "--test",
        action="count",
    ...
    for modname in modules:
        ...
        if args.test:
            ...
            try:
                ...
                sys.stderr.write(
                    f"Warning: Could not import {packagename}.{modname}: {e}\n"
                )
                sys.stderr.write(f"Creating empty .pyi file for {modname}\n")
                with open(pyifile, "w") as f:
                    f.write(f"# Placeholder for {modname} module\n")
                    f.write(
                        "# This file was auto-generated because the module could not be imported\n"
                    )
    ...
    # Create extras folder if specified
    if bindir:
