...
class CodeGenerator(BaseCodeGenerator):
    ...
    def _generate_function_code_with_retry(self, spec: FunctionSpec, retry_count: int) -> str:
        ...
        try:
            ...
            response = self.ai_client.generate_with_retry(prompt, max_retries=2)  # Reduced to avoid double retry
...
"""
    ...
    def _order_functions_by_dependencies(self, functions: List[FunctionSpec]) -> List[FunctionSpec]:
        ...
        """
        # For now, implement a simple ordering by module name and function name
    ...
    def _extract_required_imports(self, code: str, func_spec: FunctionSpec, module_info, specs: SpecificationSet) -> List[str]:
        """Extract required imports from the generated code."""
        imports = []
        
        # Standard library imports commonly needed
        import_patterns = {
            r'\bmath\.(cos|sin|tan|log|sqrt|pi)': 'import math',
            ...
            r'\bStatisticsError': 'from statistics import StatisticsError',
            r'\bre\.(match|search|findall)': 'import re',
            r'\bos\.': 'import os',
            r'\bsys\.': 'import sys',
            r'\bjson\.(loads|dumps)': 'import json',
            r'\brequests\.(get|post)': 'import requests',
            r'\bBeautifulSoup': 'from bs4 import BeautifulSoup',
            r'\bpandas\b': 'import pandas as pd',
        ...
        # Add cross-module imports if this module has dependencies
        if module_info and module_info.dependencies:
            for dep in module_info.dependencies:
                # Convert module name to import statement
                if '.' in dep:
                    # For nested modules: 'parsers.html_parser' -> 'from parsers import html_parser'
                    parts = dep.split('.')
                    if len(parts) == 2:
