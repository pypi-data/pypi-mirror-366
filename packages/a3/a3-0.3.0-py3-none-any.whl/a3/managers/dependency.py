...
class DependencyAnalyzer(BaseDependencyAnalyzer):
    ...
    def _extract_from_docstring(self, docstring: str, from_function: str, 
        ...
        # Look for function names mentioned in docstring
        patterns = [
            r'uses?\s+(\w+)',
            r'calls?\s+(\w+)',
            r'depends?\s+on\s+(\w+)',
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                for module in all_modules:
                    for function in module.functions:
                        if function.name == match:
                            to_function = f"{module.name}.{function.name}"
                            dependencies.append(FunctionDependency(
                                from_function=from_function,
                                to_function=to_function,
                                dependency_type=DependencyType.DATA_DEPENDENCY,
                                confidence=0.6,  # Lower confidence for docstring analysis
                                context=f"Mentioned in docstring: '{docstring[:50]}...'"
                            ))
                            break
    ...
    def _extract_from_type_hint(self, type_hint: str, from_function: str,
        ...
        # Skip basic types and None
        if not type_hint or type_hint in ['None', 'str', 'int', 'float', 'bool', 'Dict', 'List', 'Any']:
            return dependencies
        
        # Look for custom types that might be defined in other modules
        custom_type_pattern = r'([A-Z]\w+)'
        matches = re.findall(custom_type_pattern, type_hint)
        
        for match in matches:
            if match not in ['List', 'Dict', 'Optional', 'Union', 'Tuple', 'Any', 'Callable', 'None']:
                # Try to find this type in existing modules first
                found_in_module = False
                for module in all_modules:
                    # Check if any function returns this type (simple heuristic)
                    for func in module.functions:
                        if hasattr(func, 'return_type') and match in func.return_type:
                            dependencies.append(FunctionDependency(
                                from_function=from_function,
                                to_function=f"{module.name}.{func.name}",
                                dependency_type=DependencyType.TYPE_DEPENDENCY,
                                confidence=0.4,
                                context=f"Type hint: {type_hint}"
                            ))
                ...
                # Only create unknown dependency if we couldn't find it in existing modules
                if not found_in_module:
                    dependencies.append(FunctionDependency(
                        from_function=from_function,
                        to_function=f"unknown.{match}",
                        dependency_type=DependencyType.TYPE_DEPENDENCY,
                        confidence=0.2,
                        context=f"Type hint: {type_hint}"
                    ))
