...
async def view_image(name: str, arguments: dict[str, Any]) -> list[types.TextContent | types.ImageContent | types.EmbeddedResource]:
    """
    查看本地图片并将其发送给LLM进行分析
    ...
    try:
        # 读取并编码图片
        encoded_image, mime_type = read_and_encode_image(image_path)
...
async def setup_server(server: Server = None) -> Server:
    ...
    async def list_tools() -> list[types.Tool]:
        ...
        return [
            types.Tool(
                name="view_image",
                description="查看本地图片并将其发送给LLM进行分析，包含详细的图片信息",
                inputSchema={
                    "type": "object",
                    "required": ["image_path"],
                    "properties": {
                        "image_path": {
                            "type": "string",
                            "description": "本地图片文件的完整路径"
                        },
                        "max_file_size": {
                            "type": "integer",
                            "description": f"可选，图片文件的最大大小（字节），默认{DEFAULT_MAX_FILE_SIZE}字节"
                        }
                    },
                },
            )
