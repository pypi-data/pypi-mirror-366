...
from mcp_imgutils.server import get_server, view_image
...
@pytest.mark.asyncio
@patch("mcp_imgutils.server.validate_image_path")
@patch("mcp_imgutils.server.read_and_encode_image")
@patch("mcp_imgutils.server.get_image_details")
@patch("os.path.getsize")
async def test_view_image_success(mock_getsize, mock_get_details, mock_read_encode, mock_validate):
    """测试成功查看图片"""
    # 模拟文件大小和成功读取编码
...
@patch("mcp_imgutils.server.validate_image_path")
@patch("os.path.getsize")
async def test_view_image_too_large(mock_getsize, mock_validate):
    """测试图片文件过大"""
    # 模拟文件大小超过限制
    ...
    result = await view_image("view_image", {
...
@patch("mcp_imgutils.server.read_and_encode_image")
@patch("os.path.getsize")
async def test_view_image_processing_error(mock_getsize, mock_read_encode, mock_validate):
    """测试图片处理错误"""
    # 模拟文件大小和处理错误
    mock_getsize.return_value = 1024  # 1KB
