...
class CachedResourceManager(ResourceManager):
    ...
    def __init__(
        ...
        memory_cache_ttl: Optional[float] = 3600.0,  # 1小时
        disk_cache_ttl: Optional[float] = 86400.0 * 7,  # 7天
    ):
        """
        初始化多级缓存资源管理器
    ...
    async def get_resource(self, resource_id: str) -> Optional[Tuple[bytes, ResourceMetadata]]:
        """获取资源"""
        # 首先尝试从内存缓存获取
    ...
    async def get_resource_path(self, resource_id: str) -> Optional[str]:
        """获取资源本地路径"""
        # 检查资源是否存在（这会更新访问时间）
    ...
    async def get_metadata(self, resource_id: str) -> Optional[ResourceMetadata]:
        """获取资源元数据"""
    ...
    async def cleanup_by_size(self, max_disk_size: int, max_memory_size: Optional[int] = None) -> int:
        """
        按大小清理资源
