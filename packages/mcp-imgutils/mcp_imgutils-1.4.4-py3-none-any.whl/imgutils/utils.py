...
def validate_image_path(image_path: str) -> None:
    ...
    if path.suffix.lower() not in ALLOWED_IMAGE_FORMATS:
        raise ValueError(f"不支持的图片格式：{path.suffix}。支持的格式：{', '.join(ALLOWED_IMAGE_FORMATS)}")
...
def process_exif_value(value) -> str:
    ...
    if isinstance(value, bytes):
        ...
        # 处理分数值（如曝光时间）
        if len(value) == 2 and all(isinstance(x, int) for x in value) and value[1] != 0:
...
def read_and_encode_image(image_path: str, max_quality: int = 85) -> tuple[str, str]:
    """
    读取图片并转换为base64编码

    Args:
        image_path: 图片文件路径
        max_quality: JPEG压缩质量，范围1-100（当前未使用，保留以兼容现有代码）
