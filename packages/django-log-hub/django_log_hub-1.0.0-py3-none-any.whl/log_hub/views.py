...
def log_view(request):
    ...
    try:
        ...
        limit = int(request.GET.get('limit', 50))  # Gösterilecek log sayısı (default: 50)
        ...
        if log_type and log_type in log_files:
            ...
            return HttpResponseNotFound("Seçilen log dosyası bulunamadı.")
        ...
        try:
            ...
            return HttpResponseBadRequest("Invalid date format. Please use 'YYYY-MM-DDTHH:MM' format.")

        logs = []
        hata = None
        try:
            with open(log_file_path, 'r') as file:
                exclude_keywords = [kelime.strip().lower() for kelime in exclude.split(",")]
                s = 0
                for line in file:
                    ...
                    if s >= int(limit):
                        break
                logs.reverse() # tersten listele
        except Exception as e:
            hata = f"Log dosyası oluşurken bir hata oluştu: {str(e)}"
        ...
        return HttpResponseNotFound(f"Belirsiz Parametre, {e}")
...
def clear_log(request, log_file_name):
    ...
    if not os.path.exists(log_file_path):
        raise Http404("Belirtilen log dosyası bulunamadı.")
    
    try:
        with open(log_file_path, 'w'):
            pass  # Dosya içeriğini temizle

        return JsonResponse({"status": "success", "message": f"'{log_file_name}' başarıyla temizlendi."})
    except Exception as e:
        return JsonResponse({"status": "error", "message": f"Dosya temizlenirken bir hata oluştu: {str(e)}"}, status=500)
...
def download_log(request, log_file_name):
    ...
    if not os.path.exists(log_file_path):
        raise Http404("Seçilen log dosyası bulunamadı.")
