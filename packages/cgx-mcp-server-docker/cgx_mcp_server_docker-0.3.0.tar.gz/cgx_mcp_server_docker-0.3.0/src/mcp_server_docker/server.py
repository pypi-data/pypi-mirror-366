...
async def call_tool(
    ...
    try:
        ...
        error_msg = f"Failed to validate input provided by LLM: {str(e)}"
        logger.error(error_msg)
        try:
            await app.request_context.session.send_log_message("error", error_msg)
        ...
        error_msg = f"Docker error in {name}: {str(e)}"
        logger.error(error_msg)
        try:
            await app.request_context.session.send_log_message("error", error_msg)
        except Exception as log_error:
        ...
        return [
            types.TextContent(
                type="text", text=f"ERROR: Docker operation failed: {e}"
            )
        ]
        ...
        return [
            types.TextContent(
                type="text", text=f"ERROR: Unexpected error: {e}"
            )
        ]
