...
import pytest
from mcp_feet_master.server import create_server
...
class TestGetFootNum:
    ...
    def test_basic_calculation(self):
        ...
        assert "3 rabbits x 4 feet + 5 chickens x 2 feet = 22 feet" in result["calculation"]
        assert result["formula"] == "3 x 4 + 5 x 2 = 22"
    ...
    def test_negative_rabbits(self):
        ...
        with pytest.raises(ValueError, match="Number of rabbits cannot be negative"):
            self.get_foot_num(-1, 5)

    def test_negative_chickens(self):
        """Test that negative chicken count raises ValueError."""
        with pytest.raises(ValueError, match="Number of chickens cannot be negative"):
...
class TestCalculateAnimalsFromFeet:
    ...
    def test_rabbits_only_invalid(self):
        ...
        assert "cannot be made with only rabbits" in result["explanation"]
    ...
    def test_chickens_only_invalid(self):
        ...
        assert result["possible"] is False
        assert "cannot be made with only chickens" in result["explanation"]
    ...
    def test_negative_feet(self):
        ...
        with pytest.raises(ValueError, match="Total feet cannot be negative"):
