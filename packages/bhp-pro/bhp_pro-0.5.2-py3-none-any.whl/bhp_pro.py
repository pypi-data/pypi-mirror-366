...
def teamerror():
    ...
    def script006():
        ...
        def save_individual_config(proxy, output_dir, index):
            """Save single proxy config with numbered filename - PROPERLY SANITIZED"""
            try:
                ...
                # === ACTUALLY USE sanitize_filename() THIS TIME ===
                raw_filename = f"{index}_{proxy['type']}.yaml"
                filename = sanitize_filename(raw_filename)  # Now properly sanitized
                
                output_path = os.path.join(output_dir, filename)
                
                # Write normally (no extra steps needed)
