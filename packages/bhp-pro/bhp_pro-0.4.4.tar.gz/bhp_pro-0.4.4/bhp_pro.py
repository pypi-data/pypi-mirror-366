...
def teamerror():
    ...
    def script005():
        ...
        def a4():
            ...
            file123 = input("Enter the name of your vmess or vless file: ")

            try:
                # Open with UTF-8 encoding and error handling
                with open(file123, 'r', encoding='utf-8', errors='ignore') as file:
                    urls = [line.strip() for line in file if line.strip()]
                ...
                print(FAIL + f"File '{file123}' not found. Please provide a valid file name." + ENDC)
            except Exception as e:
    ...
    def script006():
        ...
        def bs4():
            ...
            # Get input filename (just the name, we'll look in current directory)
            input_filename = input("Enter filename with links (must be in current directory): ").strip()
            
            # First try current directory
            if not os.path.exists(input_filename):
                # If not found, try joining with current directory (just in case)
                input_file = os.path.join(os.getcwd(), input_filename)
                if not os.path.exists(input_file):
                    print(f"❌ File not found in current directory: {input_filename}")
                    return
            else:
                input_file = input_filename
            
            # Create output directory in current directory
            output_dir = "proxy_configs"
