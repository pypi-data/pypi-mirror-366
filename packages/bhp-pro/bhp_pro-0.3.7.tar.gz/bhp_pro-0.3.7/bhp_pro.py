...
def dork_scanner():
    ...
    # ====== CONFIG ======
    STEALTH_DELAY = (1, 3)  # Random delay between requests (seconds)
    USER_AGENTS = [
    ...
    def normalize_url(url):
        """Clean URL while preserving subdomains and paths"""
        url = re.sub(r'^(https?://|www\.)', '', url, flags=re.I)  # Remove http/https/www
        url = re.sub(r'/[?#].*$', '', url)  # Remove params/fragments
        url = url.strip('/')
        return url.lower()

    def filter_junk(urls):
        """Remove social metrics while keeping full subdomains/paths"""
        junk_patterns = [
            r'\d+\+?\s*(reactions|views|comments)',
            r'\d+\s*years?\s*ago',
            r'^[^a-zA-Z0-9./-]+$'  # No valid URL chars
        ]
        cleaned = []
        for url in urls:
            if not any(re.search(p, url, re.I) for p in junk_patterns):
                normalized = normalize_url(url)
                if '.' in normalized and len(normalized) <= 200:  # Basic validity check
                    cleaned.append(normalized)
        return cleaned

    def stealth_delay():
        time.sleep(random.uniform(*STEALTH_DELAY))

    # ====== SEARCH ENGINES ======
    def google_search(query, page):
        base_url = 'https://www.google.com/search'
        params = {'q': query, 'start': page * 10}
        resp = requests.get(base_url, params=params, headers=random_headers(), timeout=15)
        soup = bsoup(resp.text, 'html.parser')
        return [cite.text for cite in soup.findAll('cite')]
    ...
    def duckduckgo_search(query, page):
        ...
        resp = requests.post(base_url, data=params, headers=random_headers(), timeout=15)
        soup = bsoup(resp.text, 'html.parser')
        return [a['href'] for a in soup.select('a.result__url') if 'duckduckgo.com' not in a['href']]

    def startpage_search(query, page):
        base_url = 'https://www.startpage.com/do/search'
        params = {'query': query, 'page': page + 1}
        resp = requests.get(base_url, params=params, headers=random_headers(), timeout=15)
        soup = bsoup(resp.text, 'html.parser')
        return [a['href'] for a in soup.select('a.w-gl__result-url')]

    def brave_search(query, page):
        base_url = 'https://search.brave.com/search'
        params = {'q': query, 'offset': page * 10}
        resp = requests.get(base_url, params=params, headers=random_headers(), timeout=15)
        soup = bsoup(resp.text, 'html.parser')
        return [a['href'] for a in soup.select('a.result-header')]

    def qwant_search(query, page):
        base_url = 'https://www.qwant.com/'
        params = {'q': query, 'page': page + 1}
        resp = requests.get(base_url, params=params, headers=random_headers(), timeout=15)
        soup = bsoup(resp.text, 'html.parser')
        return [a['href'] for a in soup.select('a.result__url')]
    ...
    def yandex_search(query, page):
        ...
        resp = requests.get(base_url, params=params, headers=random_headers(), timeout=15)
        soup = bsoup(resp.text, 'html.parser')
        return [a['href'] for a in soup.select('a[accesskey]') if 'yandex' not in a['href']]

    def mojeek_search(query, page):
        base_url = 'https://www.mojeek.com/search'
        params = {'q': query, 's': page * 10}
        resp = requests.get(base_url, params=params, headers=random_headers(), timeout=15)
        soup = bsoup(resp.text, 'html.parser')
        return [a['href'] for a in soup.select('a[rel="noopener"]')]

    # ====== CORE FUNCTIONALITY ======
    def search_engine_wrapper(func, query, pages):
        results = []
        for page_num in range(pages):
            try:
                stealth_delay()
                page_results = func(query, page_num)
                results.extend(page_results)
            except Exception as e:
                print(f"[!] Error in {func.__name__}")
                continue
        return filter_junk(results) 

    def save_results(filename, results):
        unique_results = sorted(list(set(results)))  # Final deduplication
        with open(filename, 'w') as f:
            f.write('\n'.join(unique_results))
        print(f"[+] Saved {len(unique_results)} URLs to {filename}")
    ...
    def dork_scanner():
        ...
        query = input("[?] Enter search query (e.g., 'site:example.com'): ").strip()
        pages = input("[?] Pages per engine (Default: 1): ").strip()
        pages = int(pages) if pages.isdigit() else 1
        output_file = input("[?] Output filename (e.g., results.txt): ").strip()
        ...
        all_results = []
        for engine in tqdm(engines, desc="Searching Engines"):
            all_results.extend(search_engine_wrapper(engine, query, pages))

        print(f"\n[+] Found {len(all_results)} URLs before deduplication")
        if output_file:
