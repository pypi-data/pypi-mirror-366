...
def teamerror():
    ...
    def script006():
        ...
        def save_individual_config(proxy, output_dir):
            ...
            # Only allow alphanumeric characters in filename (no _, -, or special chars)           
            filename = f"{proxy['type']}{proxy['name'].replace(' ', '_')[:50]}.yaml"
            safe_name = ''.join(c for c in f"{proxy['type']}{proxy['name'][:50]}" if c.isalnum())
        ...
        def bs4():
            ...
            # Get input file
            input_file = input("Enter path to links file: ").strip()
            if not os.path.exists(input_file):
                print(f"❌ File not found: {input_file}")
                return
            
            output_dir = os.path.join(os.path.dirname(input_file), "proxy_configs")
