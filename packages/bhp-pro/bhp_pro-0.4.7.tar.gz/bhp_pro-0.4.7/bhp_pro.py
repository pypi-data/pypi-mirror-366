...
def teamerror():
    ...
    def script003():
        ...
        def show_menu():
            ...
            print(f"\n{CYAN}Select an operation:{ENDC}")
            print(f"{YELLOW}1.{ENDC} Replace host, sni or addr in vmess file")
            print(f"{YELLOW}2.{ENDC} Update IP addresses in ss/vless/hyst file")
            print(f"{YELLOW}3.{ENDC} SNI/Host Replacement for Vless/Trojan etc...")
            print(f"{YELLOW}4.{ENDC} Decode Vless configs")
            print(f"{YELLOW}5.{ENDC} Go back to main menu")

        def get_valid_file(prompt):
            """Get valid file path from user with retry"""
            while True:
                file_path = input(prompt).strip()
                if os.path.exists(file_path):
                    return file_path
                print(f"{FAIL}File '{file_path}' not found. Please try again.{ENDC}")
                print(f"Current directory: {os.getcwd()}")

        def replace_fields_in_json(input_file, output_file, replace_host=None, replace_sni=None, replace_host_in_json=None):
            """
            Replace specified fields in a JSON config file
            Args:
                input_file: Path to input JSON file
                output_file: Path to output JSON file
                replace_host: New address value (None to skip)
                replace_sni: New SNI value (None to skip)
                replace_host_in_json: New host value (None to skip)
            """
            try:
                ...
                for entry in data:
                    if replace_host and 'add' in entry:
                        entry['add'] = replace_host
                    if replace_sni and 'sni' in entry:
                        entry['sni'] = replace_sni
                    if replace_host_in_json and 'host' in entry:
                        entry['host'] = replace_host_in_json

                with open(output_file, 'w') as f:
                    json.dump(data, f, indent=4)

                print(f"{GREEN}Successfully updated fields in {output_file}{ENDC}")
                return True
                
            except json.JSONDecodeError:
                print(f"{FAIL}Error: Invalid JSON format in {input_file}{ENDC}")
            except Exception as e:
                print(f"{FAIL}An error occurred: {e}{ENDC}")
            return False

        def update_ip_addresses_in_file(file_name, new_ip):
            """Update all IP addresses in a file"""
            try:
                ...
                for line in tqdm(lines, desc="Updating IPs", unit="line"):
                ...
                if modified:
                    ...
                    print(f"{GREEN}Successfully updated IPs in {file_name}{ENDC}")
                else:
                    print(f"{YELLOW}No IP addresses found to update{ENDC}")
                    
            except Exception as e:
                print(f"{FAIL}Error updating IPs: {e}{ENDC}")

        def update_addresses_in_file(file_name, new_sni=None, new_host=None):
            """Update SNI and/or host addresses in a file"""
            try:
                with open(file_name, 'r', encoding='utf-8') as file:
                    lines = file.readlines()
                ...
                for line in tqdm(lines, desc="Updating", unit="line"):
                    original_line = line
                    
                    if new_sni:
                        line = re.sub(r'(sni=)([\w\.-]+)', rf'\1{new_sni}', line)
                    if new_host:
                        line = re.sub(r'(ws&host=)([\w\.-]+)', rf'\1{new_host}', line)
                ...
                if modified:
                    with open(file_name, 'w', encoding='utf-8') as file:
                        file.writelines(modified_lines)
                    print(f"{GREEN}Successfully updated addresses in {file_name}{ENDC}")
                else:
                    print(f"{YELLOW}No matching fields found to update{ENDC}")
                    
            except Exception as e:
                print(f"{FAIL}Error updating addresses: {e}{ENDC}")

        def decode_vless_config(encoded_str):
            """Decode a single VLESS config string"""
            try:
        ...
        def process_vless_file(input_file):
            """Process and split decoded VLESS configs"""
            try:
                output_dir = f"{input_file}_decoded"
                os.makedirs(output_dir, exist_ok=True)
                
                # Decode all configs
                decoded_configs = []
                with open(input_file, 'r', encoding='utf-8') as f:
                    for line in tqdm(f, desc="Decoding"):
                        if match := re.search(r'([A-Za-z0-9+/=]+)', line):
                            if decoded := decode_vless_config(match.group(1)):
                                decoded_configs.append(decoded)
                
                # Split into chunks of 500
                chunk_size = 500
                for i in range(0, len(decoded_configs), chunk_size):
                    chunk = decoded_configs[i:i + chunk_size]
                    output_file = os.path.join(output_dir, f"chunk_{i//chunk_size + 1:04d}.txt")
                    with open(output_file, 'w', encoding='utf-8') as f:
                        f.write('\n'.join(chunk))
                
                print(f"{GREEN}Successfully decoded and split:{ENDC}")
                print(f"  Input: {input_file}")
                print(f"  Output: {output_dir} (contains {len(decoded_configs)} configs)")
                return True
                
            except Exception as e:
                print(f"{FAIL}Error processing file: {e}{ENDC}")
                return False
        ...
        def main_operation():
            ...
            while True:
                clear_screen()
                show_menu()
                choice = input(f"{CYAN}Enter your choice (1-5): {ENDC}").strip()
                
                if choice == '1':
                    clear_screen()
                    print(f"{CYAN}Replace host/sni/addr in vmess file{ENDC}")
                    input_file = get_valid_file("Input JSON file: ")
                    output_file = input("Output JSON file: ")
                    replace_host = input("New address (leave blank to skip): ") or None
                    replace_sni = input("New SNI (leave blank to skip): ") or None
                    replace_host_in_json = input("New host (leave blank to skip): ") or None
                    
                    if replace_fields_in_json(input_file, output_file, replace_host, replace_sni, replace_host_in_json):
                        input(f"{GREEN}Press Enter to continue...{ENDC}")
                    
                elif choice == '2':
                    clear_screen()
                    print(f"{CYAN}Update IP addresses{ENDC}")
                    file_name = get_valid_file("Input file: ")
                    ...
                    clear_screen()
                    print(f"{CYAN}SNI/Host Replacement{ENDC}")
                    file_name = get_valid_file("Input file: ")
                    new_sni = input("New SNI (leave blank to skip): ") or None
                    new_host = input("New Host (leave blank to skip): ") or None
                    update_addresses_in_file(file_name, new_sni, new_host)
                    input(f"{GREEN}Press Enter to continue...{ENDC}")
                    
                elif choice == '4':
                    clear_screen()
                    print(f"{CYAN}Decode Vless configs{ENDC}")
                    input_file = get_valid_file("Input file: ")
                    process_vless_file(input_file)
                    input(f"{GREEN}Press Enter to continue...{ENDC}")
                    
                elif choice == '5':
                    clear_screen()
                    print(f"{GREEN}Returning to main menu...{ENDC}")
                    time.sleep(1)
                    break
        ...
        try:
            ...
            print(f"\n{YELLOW}Operation cancelled by user.{ENDC}")
        except Exception as e:
            print(f"{FAIL}Unexpected error: {e}{ENDC}")
    ...
    def script005():
        ...
        def test_connection(server_address, server_port):
            """Test TCP connection to server with timeout"""
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.settimeout(5)
                    ...
                    return b"HTTP/1.1" in response or b"cloudflare" in response.lower()
            except Exception:
                return False

        def test_vmess_url(vmess_url):
            """Test if VMess URL is working"""
            try:
                ...
                return vmess_url, 1 if test_connection(server_address, server_port) else 0
                
            except Exception:
                return vmess_url, 0

        def test_vless_url(vless_url):
            """Test if VLESS URL is working"""
            try:
                ...
                return vless_url, 1 if test_connection(server_address, server_port) else 0
        ...
        def get_file_path(prompt):
            ...
            while True:
                ...
                if os.path.exists(file_path):
                    return file_path
                print(FAIL + f"File '{file_path}' not found. Please try again." + ENDC)
                print(f"Current directory: {os.getcwd()}")

        def save_results(connected_urls):
            """Handle saving results to file"""
            if not connected_urls:
                return
                
            save_file = input("Do you want to save working URLs to a file? (yes/no): ").lower()
            if save_file in ('y', 'yes'):
                output_file = get_file_path("Enter output file name (or full path): ")
                try:
                    with open(output_file, 'w', encoding='utf-8') as f:
                        f.write("\n".join(connected_urls))
                    print(CYAN + f"Saved {len(connected_urls)} working URLs to '{output_file}'" + ENDC)
                except Exception as e:
                    print(FAIL + f"Failed to save file: {e}" + ENDC)

        def test_proxy_urls():
            """Main function to test proxy URLs"""
            print("\n" + "="*50)
            print(f"{CYAN}Proxy Connection Tester{ENDC}".center(50))
            print("="*50)
            
            file_path = get_file_path("\nEnter path to your proxy URLs file: ")
            ...
            try:
                ...
                print(f"\nFound {len(vmess_urls)} VMess URLs and {len(vless_urls)} VLESS URLs")
                ...
                if vmess_urls:
                    with ThreadPoolExecutor(max_workers=10) as executor:
                        results = list(tqdm(
                            executor.map(test_vmess_url, vmess_urls),
                            total=len(vmess_urls),
                            desc="Testing VMess URLs",
                            unit="URL"
                        ))
                        connected_urls.extend(url for url, status in results if status)

                # Test VLESS URLs
                if vless_urls:
                    with ThreadPoolExecutor(max_workers=10) as executor:
                        results = list(tqdm(
                            executor.map(test_vless_url, vless_urls),
                ...
                save_results(connected_urls)

            except Exception as e:
                print(FAIL + f"\nAn error occurred: {e}" + ENDC)
            finally:
    ...
    def script006():
        ...
        def parse_vless(link):
            ...
            try:
                parts = re.match(r'vless://([^@]+)@([^:]+):(\d+)\?(.*)#(.+)', link)
                if not parts: return None
                
                uuid, server, port, params, remark = parts.groups()
                query = parse_qs(params)
                
                config = {
                    "name": f"VLESS-{remark.strip() or f'{server}:{port}'}",
                    "type": "vless",
                    "server": server,
                    "port": int(port),
                    "uuid": uuid,
                ...
                if 'host' in query:
                    if config['network'] == 'ws':
                        config['ws-opts'] = {"headers": {"Host": query['host'][0]}}
                    elif config['network'] == 'tcp':
                        config['servername'] = query['host'][0]
                
                if 'path' in query:
                    path = urllib.parse.unquote(query['path'][0])
                    if config['network'] == 'ws' and 'ws-opts' in config:
                ...
                print(f"\n⚠️ Error parsing VLESS: {str(e)}", file=sys.stderr)
                return None
        ...
        def parse_vmess(link):
            ...
            try:
                ...
                print(f"\n⚠️ Error parsing VMess: {str(e)}", file=sys.stderr)
                return None

        def save_individual_config(proxy, output_dir):
            """Save single proxy config with standard rules"""
            config = {
                "proxies": [proxy],
                "proxy-groups": [{
                    "name": "PROXY",
            ...
            safe_name = re.sub(r'[^\w\s-]', '', proxy['name']).strip().replace(' ', '_')
            safe_name = re.sub(r'[-\s]+', '-', safe_name)
            filename = f"{proxy['type']}_{safe_name[:50]}.yaml"
        ...
        def bs4():
            ...
            print("----------------------------------")
            print("Creates one Clash config file per proxy")
            print("----------------------------------")
            
            # Get input file path
            while True:
                input_filename = input("\nEnter filename with links (or full path): ").strip()
                ...
                # If not found, try joining with current directory
            ...
            default_output = os.path.join(os.path.dirname(input_file), "proxy_configs")
            output_dir = input(f"Enter output directory (leave blank for {default_output}): ").strip()
            output_dir = output_dir if output_dir else default_output
            
            try:
                if not os.path.exists(output_dir):
                    os.makedirs(output_dir)
            except Exception as e:
                print(f"❌ Failed to create output directory: {str(e)}", file=sys.stderr)
                return
            ...
            successful = 0
            print("\n🔄 Processing links...")
            for link in tqdm(links, desc="Converting", unit="link"):
                try:
                    ...
                    if config:
                        save_individual_config(config, output_dir)
                        successful += 1
                except Exception as e:
                    print(f"\n⚠️ Failed to process link: {str(e)}", file=sys.stderr)
