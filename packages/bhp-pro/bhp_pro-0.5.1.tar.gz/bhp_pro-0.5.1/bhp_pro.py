...
def teamerror():
    ...
    def script006():
        ...
        def save_individual_config(proxy, output_dir, index):
            """Save single proxy config with numbered filename - ONLY FIXES FILENAME ISSUE"""
            try:
                ...
                # === ONLY CHANGE: STRICT FILENAME SANITIZATION ===
                filename = f"{index}_{proxy['type']}.yaml"
                # Remove hidden Unicode/control characters
                filename = "".join(c for c in filename if ord(c) < 128 and c.isprintable())
                # Ensure no leading/trailing junk
                filename = filename.strip()
                
                output_path = os.path.join(output_dir, filename)
                
                # Write normally (no temp file if you didn't need it before)
