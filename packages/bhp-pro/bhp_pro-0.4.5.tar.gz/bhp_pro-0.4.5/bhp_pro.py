...
def teamerror():
    ...
    def script003():
        ...
        def split_decoded_file(decoded_file, chunk_size=500):
            """Split an already-decoded file into chunks"""
            output_dir = f"{decoded_file}"
            ...
            with open(decoded_file, 'r', encoding='utf-8') as f:
                ...
                for line in tqdm(f, desc="Splitting"):
                    ...
                    if len(chunk) >= chunk_size:
                        output_file = os.path.join(output_dir, f"chunk_{file_num:04d}.txt")
                        with open(output_file, 'w', encoding='utf-8') as out:
                            out.write('\n'.join(chunk))
                        file_num += 1
                        chunk = []
                ...
                if chunk:
                    output_file = os.path.join(output_dir, f"chunk{file_num:04d}.txt")
                    with open(output_file, 'w', encoding='utf-8') as out:
                        out.write('\n'.join(chunk))

        def split_and_decode_vless(input_file):
            # Step 1: Decode all links into one file
            decoded_file = f"{input_file}"
            
            with open(input_file, 'r', encoding='utf-8') as infile, \
                open(decoded_file, 'w', encoding='utf-8') as outfile:
                
                for line in tqdm(infile, desc="Decoding"):
                    match = re.search(r'([A-Za-z0-9+/=]+)', line)
                    if match:
                        try:
                            decoded = base64.b64decode(match.group(1)).decode('utf-8')
                            outfile.write(decoded.strip() + '\n')
                        except:
                            continue
            
            # Step 2: Split the decoded file
            split_decoded_file(decoded_file)
            
            print(f"Decoded file: {decoded_file}")
            print(f"Split files saved to: {decoded_file}_split/")
    ...
    def script005():
        ...
        def a4():
            ...
            file_path = input("Enter the name of your vmess or vless file: ")
