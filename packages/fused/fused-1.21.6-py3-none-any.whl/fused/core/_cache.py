...
from fused._options import StorageStr, _cache_directory
...
def _hash_func_closures(func) -> str:
    ...
    field_check_priority = ["fused:gitLastModified", "fused:id"]
    for k, v in func_closures:
        ...
        for field in field_check_priority:
            if udf.metadata:
                if field in udf.metadata:
                    id_str += _hashify(udf.metadata[field])
                    break
        else:
            id_str += _hashify(udf.code)
    return id_str
...
def _get_cache_base_path(sub_path, storage: StorageStr = "auto") -> Path:
    ...
    base_path = (
        OPTIONS.cache_directory if storage == "auto" else _cache_directory(storage)
    )
