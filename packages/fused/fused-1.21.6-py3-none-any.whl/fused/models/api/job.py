...
class JobStepConfig(FusedBaseModel):
    ...
    _hidden_method_alias_map = {
        "run_remote": "run_batch",
    }

    def __getattr__(self, item: str):
        # Main purpose is to still provide access to hidden methods as some autocompletes are static analyzers and
        # don't use  __dir__, so we rename the methods and provide attribute access.
        #
        ...
        if item in self.__private_attributes__.keys():
            return super().__getattr__(item)

        # This access takes advantage of the above private attributes check or else we'd be in an infinite loop.
        if item in self._hidden_method_alias_map:
            return super().__getattr__(self._hidden_method_alias_map[item])
