...
BatchType = Literal["thread_pool", "async_loop"]
    ...
    **kwargs,
    ...
    Args:
        ...
        collect: If True, waits for all jobs to complete and returns the collected DataFrame
            ...
            Default is True.
        batch_type: The type of batching to use. Either "thread_pool" (default) for
    ...
    Returns:
        JobPool, AsyncJobPool, or DataFrame depending on batch_type and collect parameters
    ...
    """
    if batch_type not in ("thread_pool", "async_loop"):
        raise ValueError(
            f"batch_type must be 'thread_pool' or 'async_loop', got {batch_type}"
        )
...
"""
    ...
    if batch_type == "async_loop":
        if HAS_NEST_ASYNCIO:
            nest_asyncio.apply()
        else:
            raise ImportError("async_loop batch_type requires 'nest_asyncio'")
