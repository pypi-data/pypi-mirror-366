...
def _cache_directory(storage: StorageStr) -> Path:
...
def _get_data_dir(storage: StorageStr) -> Path:
    base_path = _get_writable_dir(storage)
...
def _get_writable_dir(storage: StorageStr) -> Path:
...
class Options(OptionsBaseModel):
    ...
    def _set_default_directories(self):
        if self.cache_directory is None:
            self.cache_directory = _cache_directory(self.cache_storage)
