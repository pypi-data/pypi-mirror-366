...
def test_common_join_count_query():
    ...
    results = client.ontology.objects.join([agg, aggt,aggr], on_fields_list=[("ad_id", "ad_id"),("ad_id", "ad_id")], join_types=["right","left"]).where().limit(2).offset(0).count_all()
    #results = client.ontology.objects.eq_vessel.where(eq_vessel.id<10).group_by(eq_vessel.ship_type).count("ship_type").sum("ship_type").having("count >2 and sum > 17").compute()
    print(results)
...
def test_join_compute_query():
    ...
    client = TGClient(api_name, ontology_api_name)
    agg,aggt,aggr = client.models
    results = client.ontology.objects.join([agg, aggt], on_fields_list=[("ad_id", "ad_id")], join_types=["right"]).where().group_by(aggt.clicks).count(agg.ad_id).having("clicks > 0").order_by(aggt.clicks.desc()).compute()
    print(results)
