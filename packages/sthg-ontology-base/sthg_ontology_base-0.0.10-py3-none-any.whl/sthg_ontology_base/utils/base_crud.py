...
class JoinedQuerySet:
    ...
    def __init__(self,  models, on_fields_list=None, on_list=None, join_types=None):
        ...
        if on_fields_list:
            ...
            self.on_conditions = []
            left_model = models[0]  # 初始的 left_model 是第一个模型
            prev_table = left_model.__table__  # 初始主表是第一个模型的表

            for idx, right_model in enumerate(models[1:]):  # 遍历模型的后续部分
                ...
                # 动态构建连接条件
                if self.join_types[idx] == "left":  # LEFT JOIN，左表为主表
                    on = prev_table.c[field_a] == table_b.c[field_b]
                    # LEFT JOIN 后，我们保持左表作为主表，不更新它
                elif self.join_types[idx] == "right":  # RIGHT JOIN，右表为主表
                    on = table_b.c[field_b] == prev_table.c[field_a]  # 使用前一个表的字段
                    prev_table = table_b  # 右连接后，继续使用左表作为下次连接的左表
                else:  # 默认情况下使用内连接，使用左表字段
                    on = prev_table.c[field_a] == table_b.c[field_b]
                    prev_table = table_a  # 内连接时，继续使用右表作为下次连接的左表
    ...
    def _build_agg_query(self, db):
        current_from = self.models[0].__table__
