...
class Client:
    ...
    def _is_insert_values_query(self, query: str) -> bool:
        clean_query = re.sub(r'--.*?$|/\*.*?\*/', '', query, flags=re.MULTILINE | re.DOTALL)
        clean_query = ' '.join(clean_query.split()).upper()
        return (
            clean_query.startswith('INSERT') and 
            'VALUES' in clean_query and
            not clean_query.startswith(('INSERT INTO', 'INSERT FROM'))  # Это особый случай
        )
    ...
    def execute(
        ...
        # Обработка INSERT VALUES запросов
        if self._is_insert_values_query(query):
            return self._execute_insert_values_query(query, params, external_tables)
    ...
    def _execute_insert_values_query(
        self,
        query: str,
        params: Optional[Union[Dict[str, Any], List[Any]]] = None,
        external_tables: Optional[List[Dict[str, Any]]] = None
    ) -> List[Tuple]:
        """Специальная обработка INSERT запросов с VALUES"""
        try:
            # Извлекаем имя таблицы и колонки
            table_match = re.search(r'INSERT\s+INTO\s+([^\s(]+)\s*(\([^)]+\))?', query, re.IGNORECASE)
            if not table_match:
                raise ValueError("Не удалось определить имя таблицы в INSERT запросе")
            
            table_name = table_match.group(1)
            columns = table_match.group(2) or ""
            
            # Подготовка данных
            if params is None:
                raise ValueError("Для INSERT VALUES запроса необходимо передать параметры")
                
            if isinstance(params, dict):
                params = [params]
                
            if not isinstance(params, (list, tuple)):
                raise ValueError("Параметры должны быть списком, кортежем или словарём")
                
            # Форматирование данных
            if all(isinstance(row, (tuple, list)) for row in params):
                # Данные в виде кортежей
                data = "\n".join(
                    "\t".join(self._format_value(value) for value in row)
                    for row in params
                )
                format_clause = "FORMAT TabSeparated"
            elif all(isinstance(row, dict) for row in params):
                # Данные в виде словарей
                data = "\n".join(json.dumps(row) for row in params)
                format_clause = "FORMAT JSONEachRow"
            else:
                raise ValueError("Неподдерживаемый формат данных для INSERT")
            
            # Формируем полный запрос
            full_query = f"INSERT INTO {table_name}{columns} {format_clause}\n{data}"
            
            response = self.session.post(
                f"{self.base_url}?query={quote(full_query)}",
                verify=self.verify_ssl,
                timeout=self.timeout
            )
            ...
            raise ConnectionError(f"Failed to execute INSERT VALUES query: {str(e)}") from e
    ...
    def _process_response(
        ...
        try:
            ...
            # Для INSERT запросов может прийти пустой ответ
            if not content:
                return []
