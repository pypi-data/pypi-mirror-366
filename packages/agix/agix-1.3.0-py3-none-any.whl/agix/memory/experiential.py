...
class GestorDeMemoria:
    ...
    def registrar(
        ...
        exp = Experiencia(entrada, decision, resultado, exito, juicio_etico, ts)
    ...
    def guardar(self, ruta: Optional[str] = None) -> None:
        ...
        if self.backend == "json":
            data = {
                "experiencias": [asdict(e) for e in self.experiencias],
    ...
    def cargar(self, ruta: Optional[str] = None) -> None:
        ...
        if self.backend == "json":
            ...
            cur.execute(
                "SELECT entrada, decision, resultado, exito, juicio_etico, timestamp FROM experiencias"
            )
            rows = cur.fetchall()
            self.experiencias = [
                Experiencia(
                ...
                for entrada, decision, resultado, exito, juicio, timestamp in rows
            ]
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS dialogos (
