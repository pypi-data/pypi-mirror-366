...
def test_load_existing_db(db_file):
    db = DB(path=db_file)
    db.db = {"provider": {"item": "value"}}
    db._save()
    loaded_db = DB(path=db_file)
    assert loaded_db.db == {"provider": {"item": "value"}}
...
def test_put_hierarchical_key():
    ...
    assert db.db == {"provider": {"item": "value"}}


def test_put_legacy_key():
    db = DB()
    db.put("legacy_key", "value")
    assert db.db == {"legacy_key": "value"}


def test_put_existing_hierarchical_key():
    db = DB()
    db.db = {"provider": {"item": "old_value"}}
    db.put("provider::item", "new_value")
    assert db.db == {"provider": {"item": "new_value"}}


def test_put_existing_legacy_key():
    db = DB()
    db.db = {"legacy_key": "old_value"}
    db.put("legacy_key", "new_value")
    assert db.db == {"legacy_key": "new_value"}
