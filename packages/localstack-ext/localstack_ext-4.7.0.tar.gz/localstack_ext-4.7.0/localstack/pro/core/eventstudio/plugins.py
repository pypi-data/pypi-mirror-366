...
from localstack.pro.core.tracing.opentelemetry.tracing import get_tracer
class EventStudioPlugin(ProPlatformPlugin):
	...
	def on_platform_ready(B):get_eventstudio_db_manager().initialize_db();A=OpenTelemetryInstrumentationPluginManager.get();A.enable_instrumentation();register_sql_span_exporter();get_tracer()
