...
class S3MWRGeometry(AbstractRadarGeometry):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not valid_navatt:
            raise NotImplementedError("Navatt are required in MWR product, until the Yaw Steering mode is implemented")
        ...
        rotation = (
            np.radians(np.array([-180.0, 0.0, 90.0])) if valid_navatt else np.radians(np.array([0.0, 0.0, 180.0]))
        )
