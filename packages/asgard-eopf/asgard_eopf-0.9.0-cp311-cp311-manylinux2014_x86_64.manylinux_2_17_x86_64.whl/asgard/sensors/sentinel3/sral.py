...
class S3SRALGeometry(AbstractRadarGeometry):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for orbit in orb_list:
            ...
            if orb_frame != FrameId.EF.name:
                logger.warning("Orbit frame used is %r, SRAL constructor converts it to Earth-Fixed frame", orb_frame)
    ...
    def compute_altitude_rate(self, coordinates, eps_value: float = 1e-5):
        ...
        ground = self.body_model.cartesian_to_geodetic(dataset, field_out="geod_pos")

        eps = np.full(ground["position"].shape, eps_value)
        pos_next["position"] = ground["position"] + ground["velocity"] * eps

        # Change coordinate system to compute altitude rate
        ground_next = self.body_model.cartesian_to_geodetic(pos_next, field_out="geod_pos")
    ...
    def direct_loc(
        ...
        propgation model and DEM as it is using only orbit state vector of the satellite
        ...
        assert dataset["position"].shape[0] == len(times)

        self.body_model.cartesian_to_geodetic(dataset)

        if altitude is not None:
            dataset["position"][:, 2] = altitude
