...
class EarthBody(AbstractBody):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._time_ref = kwargs.get("time_reference", TimeReference())
    ...
    def cartesian_to_geodetic(
        ...
        geodetic_positions = np.array(geodetic_positions)
    ...
    def geodetic_to_cartesian(
        ...
        """
        attach_thread()
        # the dataset is an array of coordinates in degrees
        assert dataset.get(field_in) is not None  # check dataset is here
        assert isinstance(dataset[field_in], np.ndarray)
        assert dataset[field_in].dtype == "float64"
        # change shape, leave 3 components at the end
        ...
        for longitude, latitude, altitude in flat_geodetic:
            ...
            cartesian_positions.append([cartesian_point.getX(), cartesian_point.getY(), cartesian_point.getZ()])
    ...
    def curvature_radius(self, sin_lat, azimuth) -> float:
        """
    ...
    def change_reference_frame(
        ...
        # get positions table
        inputs = {"pos": dataset[fields_in[1]]}
        ...
        if len(fields_in) >= 3:
            inputs["vel"] = dataset.get(fields_in[2])
        ...
        if len(fields_in) >= 4:
            inputs["acc"] = dataset.get(fields_in[3])
    ...
    def transform_orbit(self, orbit: dict, dest_frame: FrameId) -> dict:
        ...
        if crt_frame != dest_frame.name:
            fields = ["times"]
            ds_chg_frame = {"times": orbit["times"][orbit["time_ref"]]}
            pva_fields = ["velocities", "positions", "accelerations"]
            intersection = [key for key in list(orbit.keys()) if key in pva_fields]
            for field in intersection:
                ds_chg_frame[field] = orbit[field]  # noqa: B909
                fields.append(field)
            ...
            for field in intersection:
                orbit[field] = ds_chg_frame[field]  # noqa: B909
    ...
    def ground_range(self, point: np.ndarray, distance, normal: np.ndarray) -> np.array:
        """
