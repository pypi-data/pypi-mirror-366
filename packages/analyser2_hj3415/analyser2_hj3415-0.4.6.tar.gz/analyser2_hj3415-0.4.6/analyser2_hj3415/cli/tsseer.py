import argparse
import asyncio
from analyser2_hj3415.tsseer.mydarts import nbeats_forecast, filter_nbeats_by_trend_and_anomaly
...
def main():
    ...
    cache_corp_parser.add_argument('engine', choices=['prophet', 'nbeats'])
    ...
    cache_me_parser.add_argument('engine', choices=['prophet', 'nbeats'])
    ...
    if args.cache_type == 'corp':
        if len(args.targets) == 1 and args.targets[0].lower() == 'all':
            ...
            async def main():
                if engine == 'nbeats':
                    await filter_nbeats_by_trend_and_anomaly(all_codes, "상승", refresh=True)
                    await filter_nbeats_by_trend_and_anomaly(all_codes, "하락", refresh=True)
                elif engine == 'prophet':
        ...
        if args.targets[0].lower() == 'all':
            ...
            async def main():
                if engine == 'nbeats':
                    await filter_nbeats_by_trend_and_anomaly(MI_TICKERS, "상승", refresh=True)
                    await filter_nbeats_by_trend_and_anomaly(MI_TICKERS, "하락", refresh=True)
                elif engine == 'prophet':
