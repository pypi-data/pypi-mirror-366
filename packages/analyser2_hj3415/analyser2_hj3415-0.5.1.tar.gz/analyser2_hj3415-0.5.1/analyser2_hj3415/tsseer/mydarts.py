...
@redis_cached(prefix='nbeats', key_factory=single_ticker_key_factory)
...
@redis_cached(prefix='nhits', key_factory=single_ticker_key_factory)
...
@redis_async_cached(prefix="mydarts_trend", key_factory=trend_key_factory)
async def filter_mydarts_by_trend_and_anomaly(tickers: list[str], trend: Literal["상승", "하락"], *,
    ...
    keys   = [f"mydarts:{t.lower()}" for t in tickers]
