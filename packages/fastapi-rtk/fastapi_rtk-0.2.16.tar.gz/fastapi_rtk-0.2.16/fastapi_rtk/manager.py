...
class UserManager(IDParser, BaseUserManager[User, int]):
    ...
    async def create(
        ...
        if role_names:
            user_dict["roles"] = await self._get_roles_by_names(role_names)
    ...
    async def update(
        ...
        if roles is not None:
            updated_user_data["roles"] = await self._get_roles_by_names(roles)
    ...
    async def _get_roles_by_names(self, role_names: list[str]):
        """
        Get roles by their names.

        Args:
            role_names (list[str]): List of role names to retrieve.

        Raises:
            Exception: If one or more roles do not exist.
