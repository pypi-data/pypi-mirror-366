...
class PreprocessingManager(CustomCodeManager):
    def __init__(
        ...
        super().__init__(preprocessor_class, "preprocessing", "class", download_dir, env, verbose, console, status)

    def modify_state(self, state: np.ndarray) -> np.ndarray:
        if self._code is None:
            raise CustomCodeError("Preprocessor is not loaded")

        if self.has_method("modify_state") == False:
            raise CustomCodeError("Preprocessor class does not contain 'modify_state' method")

        return self._code.modify_state(state)  # type: ignore