...
class YaappCore:
    ...
    def _load_config(self) -> YaappConfig:
        ...
        if not config_file:
            ...
            for search_path in search_paths:
                config_path = search_path / "yaapp.yaml"
                if config_path.exists():
                    config_file = str(config_path)
                    break
        ...
        search_paths = self._get_config_search_paths()
        for search_path in search_paths:
            secrets_path = search_path / "yaapp.secrets.yaml"
    ...
    def _get_config_search_paths(self) -> List[Path]:
        ...
        4. User home config directory (~/.config/yaapp/)
        """
        import sys
        import os
        ...
        # 4. User home config directory (XDG Base Directory compliance)
        xdg_config_home = os.getenv('XDG_CONFIG_HOME')
        if xdg_config_home:
            user_config_dir = Path(xdg_config_home) / 'yaapp'
        else:
            home_dir = os.path.expanduser('~')
            if home_dir and home_dir != '~':  # Make sure expansion worked
                user_config_dir = Path(home_dir) / '.config' / 'yaapp'
            else:
                user_config_dir = None
        
        if user_config_dir and user_config_dir not in search_paths:
